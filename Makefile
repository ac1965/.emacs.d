...
# org-babel-tangle は既定でソースブロックの「共通の先頭空白」を列（column）単位で
# 計算して取り除くが、この計算はタブをいったんスペースに展開してから行うため、
# recipe 行の実体である「タブ 1 文字」が tab-width 分のスペース列に化けてしまい、
# tangle のたびに Make が "missing separator" で落ちる（recipe 行の先頭は
# 文字どおりタブでなければならないため）。この事故を根本から避けるため、
# recipe プレフィックス文字をタブから ">" へ変更する。">" は通常の文字なので
# org 側の空白除去処理に巻き込まれても壊れない。
.RECIPEPREFIX := >
...
# design_spec.org — 図版（*.dot）の tangle 元のひとつ。ORG（README.org）とは別文書だが、
# 同じ svg/ ディレクトリへ tangle するため、dot/mermaid 系ターゲットは両方を対象にする。
...
# sandboxed / headless 環境で Chrome の自動検出に失敗する場合は、PUPPETEER_CONFIG に
# executablePath を書いた JSON ファイルのパスを渡すこと
 ...
 for f in $$(find "$(SVGDIR)" -name '*.mmd' | sort); do \
   ...
   if [ ! -f "$$svg" ] || [ "$$f" -nt "$$svg" ]; then \
     ...
     if [ -n "$(PUPPETEER_CONFIG)" ]; then \
       "$(MMDC)" -i "$$f" -o "$$svg" -b transparent -p "$(PUPPETEER_CONFIG)" || exit 1; \
     else \
       "$(MMDC)" -i "$$f" -o "$$svg" -b transparent || exit 1; \
     fi; \
     count=$$((count + 1)); \
   fi; \
 done; \
