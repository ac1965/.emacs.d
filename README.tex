...
\date{\today}
\title{Modern Emacs Configuration}
\hypersetup{
 pdfauthor={YAMASHITA, Takao},
 pdftitle={Modern Emacs Configuration},
 pdfkeywords={},
 pdfsubject={},
 pdfcreator={},
 pdflang={English}}
...
\subsection{Design Tenets (TL;DR)}
\label{sec:org8a67362}

\begin{itemize}
\item \textbf{Deterministic startup}: no implicit installs, no hidden side effects.
\item \textbf{Layered ownership}: each layer owns policy; dependencies flow one way.
\item \textbf{Opt-in power}: advanced tooling must never be required for baseline use.
\item \textbf{Observable over magical}: favor inspectable state over automation.
\item \textbf{Deletion-friendly}: removing a layer should degrade convenience, not correctness.
\end{itemize}
\subsection{Features}
\label{sec:orga480a43}
\begin{itemize}
\item 🚀 \textbf{Performance \& Native Compilation} — Early-init moves ELN under `.cache/`, silences async warnings, widens GC at startup and restores sane values later, and uses GCMH for idle GC.
\item 🧩 \textbf{Language Server Protocol} — Backend-agnostic helpers in `core/general.el`; choose \textbf{Eglot} or \textbf{lsp-mode} via `core/switches.el` with presence checks and auto-enable logic.
\item 🤖 \textbf{AI Integration} — Aidermacs (vterm backend). Prefers OpenRouter when `OPENROUTER\textsubscript{API}\textsubscript{KEY}` is set; otherwise uses OpenAI with `OPENAI\textsubscript{API}\textsubscript{KEY}`.
\item 🎨 \textbf{Modern UI \& Editing} — Tree-sitter remaps (`*-ts-mode`), ef-themes + spacious-padding, Nerd Icons, Vertico/Orderless/Corfu/CAPE/Embark/Consult, Doom/Nano modeline switchers.
\item 🗂️ \textbf{Productivity Tools} — Opinionated Org stack (agenda, capture, journal, roam, download, TOC), Magit + diff-hl/Forge, REST client, Docker/dev helpers, tidy backups/autosave-visited.
\end{itemize}
...
\label{sec:orgb5245d7}
\begin{itemize}
\item ✅ `lexical-binding: t` is \textbf{mandatory} for all files.
\item ✅ The `(provide 'FEATURE)` symbol \textbf{must match the file’s logical feature name}
(usually derived from the file path).
\item 📦 Built-in packages MUST explicitly declare `:straight nil`.
\item 🌿 Each `leaf` form follows a stable, readable structure:
`:straight` → `:bind` → `:hook` → `:custom` → `:config`
\item 📚 Only documented, public APIs are used.
\begin{itemize}
\item Private, internal, or speculative APIs are intentionally avoided.
\end{itemize}
\item 🧠 \textbf{Compatibility \& Forward-Safety Policy}
\begin{itemize}
\item This configuration targets \textbf{\textbf{Emacs 30+}}.
\item Code is written with \textbf{\textbf{Emacs 31 and later}} in mind.
\item Obsolete APIs are avoided even if still functional.
\begin{itemize}
\item Prefer `if-let*`, `when-let*`, `and-let*` over deprecated forms.
\end{itemize}
\item New compiler or runtime warnings are treated as \textbf{actionable signals}.
\item The codebase aims to remain warning-free under the latest stable Emacs
with default `byte-compile-warnings`.
\end{itemize}
\end{itemize}

See also: \hyperref[sec:org6b7dc99]{Modular Loader \& Core/Utils design}
for how these rules are enforced structurally.
\subsection{Installation}
\label{sec:orga4f1432}
\subsubsection{Prerequisites}
\label{sec:org3a51370}
\begin{itemize}
\item \textbf{Required}
\begin{itemize}
\item Emacs \textbf{30.0+} with native compilation (`--with-native-compilation`)
\item Git
\item GNU Make
\item GCC \textbf{10+} with `libgccjit`
\end{itemize}

\item \textbf{Optional but Recommended}
\begin{itemize}
\item ripgrep (`rg`) → faster project-wide search
\item aspell or hunspell → spell checking
\item pass + GnuPG → password and auth-source integration
\item Homebrew (macOS only) → for consistent toolchain installation
\end{itemize}
\end{itemize}
\subsubsection{Building Emacs}
\label{sec:orgb00f770}
...
\label{sec:org127fae1}

\begin{enumerate}
\item Apple Silicon (Primary)
\label{sec:orge2f57b4}
\begin{itemize}
\item GNU Emacs \textbf{31.0.50}
\end{itemize}
...
Features & ACL DBUS GLIB GNUTLS IMAGEMAGICK LCMS2 LIBXML2 MODULES NATIVE\textsubscript{COMP} NOTIFY KQUEUE NS PDUMPER PNG RSVG SQLITE3 THREADS TOOLKIT\textsubscript{SCROLL}\textsubscript{BARS} TREE\textsubscript{SITTER} WEBP XIM XWIDGETS ZLIB\\
...
\end{tabular}
\end{center}
\item Intel (Secondary)
\label{sec:orgb9680a9}
\begin{itemize}
\item GNU Emacs \textbf{31.0.50}
\end{itemize}
...
Branch & master\\
System & x86\textsubscript{64}-apple-darwin25.1.0\\
Date & 2025-11-01 12:05:25 (JST)\\
Patch & N/A ns-inline.patch\\
Features & ACL DBUS GIF GLIB GMP GNUTLS JPEG LCMS2 LIBXML2 MODULES NATIVE\textsubscript{COMP} NOTIFY KQUEUE NS PDUMPER PNG RSVG SQLITE3 THREADS TIFF TOOLKIT\textsubscript{SCROLL}\textsubscript{BARS} TREE\textsubscript{SITTER} WEBP XIM ZLIB\\
...
\end{enumerate}
\subsection{Tools}
\label{sec:org4633a5d}
\subsubsection{Graph Capture (Require Dependency Visualization)}
\label{sec:org7fdb089}

\begin{enumerate}
\item Purpose
\label{sec:org91ff8fc}
...
\item What it does
\label{sec:org12ded62}
\begin{itemize}
\item Advises `require` to record \textbf{from → to} edges between features
\item Stores edges in a hash table (no duplicates)
\item Exports the dependency graph as:
\begin{itemize}
\item Graphviz DOT
\item Mermaid (for Org / Markdown)
\end{itemize}
\item Provides interactive commands to enable/disable capture at runtime
\end{itemize}
\item Notes
\label{sec:org0eba96a}
\begin{itemize}
\item Intended for \textbf{\textbf{diagnostics only}}, not for normal startup
\item Enable capture \textbf{before} loading modules
\item Disable capture after use to avoid overhead
\item Feature names are derived from file names or `require` symbols
\end{itemize}
\item Implementation
\label{sec:org9eaaa91}
\begin{enumerate}
\item tools/tools-graph.el
\label{sec:org4005f52}
\begin{verbatim}
...
\end{verbatim}
\item tools/tools-graph-unused.el
\label{sec:orgebe4c67}
\begin{verbatim}
...
\end{verbatim}
\end{enumerate}
\item Module Load Summary Helper
\label{sec:org89f6f84}
...
\begin{enumerate}
\item Restart Emacs
\item Enable capture (evaluate in order):
\begin{itemize}
\item `M-: (require 'tools-graph)`
\item `M-: (tools-graph/enable-require-capture)`
\end{itemize}
\item Load modules (e.g. `(require 'modules)` or normal startup)
\item Export graph:
\begin{itemize}
\item `M-: (tools-graph/export-mermaid)`
\item or `M-: (tools-graph/export-dot)`
\item or `M-: (tools-graph/export-org)`
\end{itemize}
\item Paste the result into Org / Markdown
\item (Optional) `M-: (my/modules-summary-line)`
\item (Optional) Disable capture:
\begin{itemize}
\item `M-: (tools-graph/disable-require-capture)`
\end{itemize}
\end{enumerate}
...
\end{verbatim}
\item Mermaid Example
\label{sec:org716a38c}
...
\label{sec:orgcbb21f2}
This Emacs configuration is \textbf{modular by design} and targets \textbf{\textbf{Emacs 30+}}.
...
\begin{itemize}
\item \texttt{early-init.el} → earliest bootstrap (performance, paths, UI defaults)
\item \texttt{init.el}       → package bootstrap, global defaults, module entrypoint
\item \texttt{lisp/}         → shared, versioned modules (core, ui, completion, orgx, dev, vcs, utils)
\item \texttt{personal/}     → user- and device-specific overlays (not shared policy)
\end{itemize}
\subsection{Core Bootstrap — early-init.el \& init.el}
\label{sec:org745dccb}
\subsubsection{Overview}
\label{sec:org24a1731}

\begin{enumerate}
\item Purpose
\label{sec:orgc5aa48e}
Provide a \textbf{clean, fast, and conservative} bootstrap sequence that prepares
Emacs before regular initialization.

The bootstrap is split into two explicit stages:

\begin{itemize}
\item \texttt{early-init.el} runs \textbf{\textbf{before package initialization}} and establishes
directories, performance guards, and flicker-free UI defaults.
\item \texttt{init.el} completes package bootstrapping (\textbf{straight.el + leaf}), imports
...
\item What this configuration does
\label{sec:org0262e85}

\begin{itemize}
\item Disables \texttt{package.el} early; \textbf{straight.el} and \textbf{leaf} are the only package managers.
\item Speeds up startup by temporarily widening GC limits and clearing
\texttt{file-name-handler-alist}, then restoring sane runtime values.
\item Normalizes all state under predictable directories:
\texttt{.cache/}, \texttt{.etc/}, and \texttt{.var/} (including native-comp artifacts).
\item On macOS, prefers the Homebrew toolchain by preparing PATH-related variables
(e.g. \texttt{PATH}, \texttt{LIBRARY\_PATH}, \texttt{CC}) \textbf{before} native compilation is triggered.
\item Disables classic backups and auto-save early; higher-level modules may enable
\texttt{auto-save-visited-mode} later in a controlled way.
\item Applies early UI defaults (no menu/tool/scroll bars, stable frame parameters)
to avoid startup flicker.
\item Bootstraps \textbf{straight.el} robustly, with guarded network access and explicit
error reporting.
\item Initializes \textbf{leaf} and its keywords, and enables conservative performance
helpers (e.g. GCMH, \texttt{read-process-output-max}).
\item Sets URL-related state paths \textbf{before} \texttt{url.el} loads, so downstream consumers
(including \textbf{straight}) inherit them.
\item Provides two stable entrypoints:
\begin{itemize}
\item a per-user personal override file (\texttt{personal/<login-name>.el})
\item a shared module loader (\texttt{lisp/modules.el})
\end{itemize}
\end{itemize}
\item Reproducibility Note (Personal Tangling)
\label{sec:org3a5c231}

This configuration prioritizes reproducibility for all \textbf{shared} layers:

\begin{itemize}
\item \texttt{early-init.el}
\item \texttt{init.el}
\item \texttt{lisp/}
\end{itemize}
...
\begin{itemize}
\item Per-user isolation
\item Safe multi-user sharing of the same repository
\item Zero-conflict personal overrides
\end{itemize}

This behavior is intentional and by design.
\item Module map (where things live)
\label{sec:org5061326}

\begin{center}
\begin{tabular}{ll}
File & Role\\
\hline
\texttt{early-init.el} & Pre-init bootstrap (dirs, performance guards, package.el off, macOS toolchain, early UI)\\
\texttt{init.el} & Main init (URL state, straight bootstrap, env import, runtime knobs, module loader)\\
\end{tabular}
\end{center}
\item How it works (boot flow)
\label{sec:orgc57debf}

\begin{enumerate}
\item Emacs loads \texttt{early-init.el}:
\begin{itemize}
\item Directory paths are established.
\item \texttt{package.el} is disabled.
\item GC and file-handler pressure is relaxed.
\item Early UI defaults are applied.
\item macOS toolchain variables are prepared when applicable.
\end{itemize}

\item Emacs loads \texttt{init.el}:
\begin{itemize}
\item URL state directories are set \textbf{before} \texttt{url.el}.
\item \textbf{straight.el} is bootstrapped.
\item On macOS GUI/daemon sessions, the login environment is imported.
\item \textbf{leaf} is initialized and a minimal base of packages is ensured.
\item Runtime performance knobs (GCMH, IO buffers) are applied.
\item A per-user personal file is loaded safely.
\item \texttt{modules.el} is required as the canonical feature entrypoint.
\end{itemize}

\item After initialization completes, a concise startup summary
(elapsed time and GC count) is printed.
\end{enumerate}
\item Key settings (reference)
\label{sec:org4979c91}

\begin{description}
\item[{\texttt{package-enable-at-startup}}] \texttt{nil} — rely exclusively on \textbf{straight.el}.
\item[{\texttt{straight-base-dir}}] Located under \texttt{.cache/} to keep the config root clean.
\item[{\texttt{native-comp-eln-load-path}}] Centralized under \texttt{.cache/eln-cache}.
\item[{\texttt{read-process-output-max}}] Temporarily raised (4 MiB) for better LSP/IO throughput.
\item[{\texttt{gcmh-high-cons-threshold}}] 16 MiB; \texttt{gcmh-mode} enabled for smoother long sessions.
\end{description}
\item Usage tips
\label{sec:orgf7d1739}

\begin{itemize}
\item Treat \texttt{early-init.el} as infrastructure only; avoid user behavior or feature logic.
\item Put shared behavior in modules loaded via \texttt{modules.el}.
\item Put identity, device-specific glue, and workflow integrations in
\texttt{personal/<login-name>.el} or related personal modules.
\item After installing or upgrading Homebrew toolchains, restart Emacs so native
compilation sees updated paths.
\item To relocate the entire setup, move the config directory; Emacs will regenerate
\texttt{.cache/}, \texttt{.etc/}, and \texttt{.var/} automatically.
\end{itemize}
\item Troubleshooting
\label{sec:orgcc11666}

\begin{itemize}
\item \textbf{“Native compilation can’t find libgccjit on macOS”} →
Ensure Homebrew’s \texttt{libgccjit} is installed and visible. The early bootstrap
prepares \texttt{LIBRARY\_PATH} when possible.
\item \textbf{“Straight bootstrap failed”} →
A transient network issue during \texttt{url-retrieve-synchronously}.
Re-run; failures are reported with a clear \texttt{[straight] bootstrap failed} message.
\item \textbf{“Inhibit startup echo warning”} →
\texttt{inhibit-startup-echo-area-message} is set to the actual user name string
to satisfy Emacs’ type requirements.
\end{itemize}
\end{enumerate}
\subsubsection{early-init.el}
\label{sec:org0b75855}
\begin{verbatim}
...
\end{verbatim}
\subsubsection{init.el}
\label{sec:org7dc0413}
\begin{verbatim}
...
;;; init.el ends here
\end{verbatim}
\subsection{Modular Loader \& Core Module Suite}
\label{sec:org6b7dc99}
\subsubsection{Overview}
\label{sec:orgff00780}

\begin{enumerate}
\item Purpose
\label{sec:orgd7276e0}
Provide a \textbf{deterministic, explicit, and auditable module loading architecture}
for this Emacs configuration.

This layer defines \emph{where feature activation begins} after bootstrap,
...
\item What this layer defines
\label{sec:org4be8881}

This layer establishes:

\begin{itemize}
\item \texttt{modules.el} as the \textbf{only authoritative loader} for shared modules
\item A directory-based responsibility split with clear semantic boundaries
\item An explicit, ordered loading sequence handed off from \texttt{init.el}
\end{itemize}

Modules are grouped by responsibility:

\begin{description}
\item[{\texttt{core/}       }] fundamental runtime, infrastructure, and global policy
\item[{\texttt{ui/}         }] visual presentation and interaction layer
\item[{\texttt{completion/} }] completion frameworks and CAPF orchestration
\item[{\texttt{orgx/}       }] Org-mode extensions beyond upstream defaults
\item[{\texttt{dev/}        }] development, build, and language tooling
\item[{\texttt{vcs/}        }] version control systems and repository interaction
\item[{\texttt{utils/}      }] small, domain-specific helpers not promoted upward
\end{description}
\item What this layer does \textbf{not} do
\label{sec:org51fcc04}

This layer intentionally does \textbf{not}:

\begin{itemize}
\item Perform bootstrap or environment preparation
(handled exclusively by \texttt{early-init.el})
\item Establish runtime foundations
(handled by \texttt{init.el})
\item Define configuration logic, advice, hooks, or feature behavior
\item Discover modules via directory scanning or wildcard expansion
\item Encode user-, device-, or host-specific policy
\end{itemize}

Its sole responsibility is \textbf{orchestration}, not implementation.
\item Design principles
\label{sec:orgb43567d}

\begin{itemize}
\item \texttt{modules.el} contains \textbf{only ordered \texttt{require} forms}
\item Each module file:
\begin{itemize}
\item provides exactly one feature
\item documents its own assumptions and non-goals
\end{itemize}
\item Load order is fixed and intentional:
...
\begin{itemize}
\item Idempotent (safe to load multiple times)
\item Side-effect conscious
\item Safe under batch, byte-compilation, and native-comp contexts
\end{itemize}
\item Benefits
\label{sec:orgf11776a}

This architecture enables:

\begin{itemize}
\item Fully inspectable and reproducible startup behavior
\item Predictable dependency relationships
\item Selective enable/disable during diagnostics
\item Long-term maintainability across Emacs versions
\item Dependency visualization and auditing from a single file
\end{itemize}
\item Relationship to bootstrap
\label{sec:orgadd7fd3}

\begin{itemize}
\item \texttt{early-init.el} prepares infrastructure only
\item \texttt{init.el} establishes runtime foundations and environment state
\item \texttt{modules.el} is the \textbf{first and only layer} where shared features are activated
\end{itemize}

This separation ensures that:

\begin{itemize}
\item bootstrap remains minimal, conservative, and side-effect free
\item feature activation is centralized and reviewable
\item personal overlays remain isolated from global policy
\end{itemize}
\end{enumerate}
\subsubsection{modules.el}
\label{sec:org6ed14e6}
\begin{itemize}
\item Uses only \texttt{require} and comments
\item No conditional loading based on environment or features
\item Load order reflects semantic dependency layers:
\begin{enumerate}
\item core
\item ui
\item completion
...
\item utils
\end{enumerate}
\item Every required feature must:
\begin{itemize}
\item be provided by exactly one file
\item correspond to a clearly named module
\end{itemize}
\end{itemize}
...
\end{verbatim}
\subsubsection{core/}
\label{sec:org74fbf22}
\begin{enumerate}
\item Overview
\label{sec:org4db0b6f}

\begin{itemize}
\item \texttt{early-init.el} prepares the physical environment and startup invariants
\item \texttt{init.el} establishes runtime foundations and hands control to \texttt{modules.el}
\item \texttt{core/} is the \textbf{\textbf{first feature layer}} activated after bootstrap
\end{itemize}
...
\begin{enumerate}
\item Purpose
\label{sec:org73dbc14}
Provide the \textbf{\textbf{foundational runtime, policy, and infrastructure layer}}
required by all other shared modules.

This layer establishes \textbf{hard guarantees} about execution context,
availability of primitives, and global defaults that higher layers
may rely on \textbf{\textbf{without defensive checks or conditional logic}}.

Core exists to make the rest of the configuration simpler, safer,
and more predictable.
\item What this layer does
\label{sec:org79c1395}
Core modules define the \textbf{\textbf{non-negotiable baseline}} of the entire configuration.

This layer is responsible for:

\begin{itemize}
\item Establishing low-level behavior shared globally
\item Defining configuration-wide policy and invariants
\item Initializing essential subsystems with long-lived impact
\item Providing stable primitives consumed by all higher layers
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Compatibility guards and forward-looking fixes across Emacs versions
\item Session lifecycle orchestration and global health thresholds
\item Global keybinding infrastructure and command scaffolding
\item Editing, history, persistence, and filesystem interaction policy
\item Tree-sitter and parsing infrastructure with explicit grammar control
\item Feature switches that gate downstream activation (UI, LSP, etc.)
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:org198fbd7}
Core modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Implement visual presentation, theming, or UI styling
\item Define completion UX or minibuffer interaction patterns
\item Extend Org semantics beyond upstream defaults
\item Configure language servers, build tools, or VCS workflows
\item Encode user-, device-, or host-specific preferences
\end{itemize}

Those responsibilities are explicitly delegated to higher layers
or to \texttt{personal/} overlays.
\item Design constraints
\label{sec:org48302cf}
\begin{itemize}
\item Core modules must not depend on:
\begin{itemize}
\item \texttt{ui}
\item \texttt{completion}
\item \texttt{orgx}
...
\item All side effects must be:
\begin{itemize}
\item explicit
\item documented
\item global in intent
\end{itemize}

\item Failure in this layer is \textbf{\textbf{configuration-critical}} and must:
\begin{itemize}
\item surface early
\item fail loudly
\item never degrade silently
\end{itemize}
\end{itemize}
\item Implementation principles
\label{sec:org49c456b}
\begin{itemize}
\item Loaded \textbf{\textbf{first}} by \texttt{modules.el}
\item Each file:
\begin{itemize}
\item provides exactly one feature
\item documents its scope, assumptions, and non-goals
\end{itemize}
\item Optional behavior is gated behind explicit customization variables
\item No module assumes:
\begin{itemize}
...
\item Benefits
\label{sec:orgd913db5}
This separation ensures that:

\begin{itemize}
\item Higher layers can rely on stable, unconditional primitives
\item Debugging starts from a known, deterministic baseline
\item Policy decisions are centralized and reviewable
\item Long-running sessions remain predictable and observable
\end{itemize}
\item Module map
\label{sec:orgc3cac08}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{core/fixes.el} & Compatibility guards and minimal hotfixes scoped by Emacs version\\
\texttt{core/core-session.el} & Long-running session orchestration and global health policy\\
\texttt{core/general.el} & Global keybinding infrastructure and non-modal leader layout\\
\texttt{core/tools.el} & Cross-cutting helper commands for navigation, inspection, and tooling\\
\texttt{core/utils.el} & Core-level utility helpers and global hooks shared across layers\\
\texttt{core/core-treesit.el} & Centralized Tree-sitter infrastructure and grammar policy\\
\texttt{core/core-history.el} & Session persistence (history, places, recent files)\\
\texttt{core/editing.el} & Editing behavior, UX aids, and filesystem interaction policy\\
\texttt{core/switches.el} & Unified switches controlling UI and LSP activation\\
\texttt{core/custom.el} & Routing and management of Customize output (\texttt{custom-file})\\
\texttt{core/custom-ui-extras.el} & Optional UI-related module extensions appended at runtime\\
\end{tabular}
\end{center}
\item Notes
\label{sec:orge7e5c4d}
\begin{itemize}
\item Core is the \textbf{only} layer allowed to define global invariants.
\item Any logic that depends on UI, completion, or Org semantics is misplaced here.
\item If a change in core feels “convenient” rather than “necessary”,
it likely belongs elsewhere.
\end{itemize}

This layer exists to make everything above it \textbf{\textbf{boring, stable, and trustworthy}}.
\end{enumerate}
\item core/core-fixes.el
\label{sec:org2b28794}
\begin{verbatim}
...
\end{verbatim}
\item core/core-policy.el
\label{sec:org6e1cfb5}
\begin{verbatim}
...
\end{verbatim}
\item core/core-session.el
\label{sec:orge6132ed}
\begin{verbatim}
...
\end{verbatim}
\item core/core-gc.el
\label{sec:org16e6b93}
\begin{verbatim}
...
\end{verbatim}
\item core/core-buffers.el
\label{sec:orga7d5213}
\begin{verbatim}
...
\end{verbatim}
\item core/core-persistence.el
\label{sec:orgf98ddfe}
\begin{verbatim}
...
\end{verbatim}
\item core/core-general.el
\label{sec:org7cd9d07}
\begin{verbatim}
...
\end{verbatim}
\item core/core-tools.el
\label{sec:org1e0cec1}
\begin{verbatim}
...
\end{verbatim}
\item core/core-treesit.el
\label{sec:org9a8b998}
\begin{verbatim}
...
\end{verbatim}
\item core/core-history.el
\label{sec:org7ddec4a}
\begin{verbatim}
...
\end{verbatim}
\item core/core-editing.el
\label{sec:org176fe5e}
\begin{verbatim}
...
\end{verbatim}
\item core/core-switches.el
\label{sec:org6799c52}
\begin{verbatim}
...
\end{verbatim}
\item core/core-custom.el
\label{sec:orgec95363}
\begin{verbatim}
...
\end{verbatim}
\item core/core-custom-ui-extras.el
\label{sec:org9fd4641}
\begin{verbatim}
...
\label{sec:org361b9b8}
\begin{enumerate}
\item Overview
\label{sec:org0aa4eaf}

\begin{itemize}
\item Loaded after \texttt{core} and \texttt{ui}
\item Each file:
\begin{itemize}
\item provides exactly one completion feature
\item documents its assumptions and non-goals
\end{itemize}
\item Disabling this entire layer must leave Emacs functional,
with only basic `completion-at-point` behavior
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:orga0a070e}
Provide a \textbf{\textbf{coherent, explicit, and debuggable completion architecture}}
for this Emacs configuration.

This layer centralizes completion behavior to avoid fragmentation across
modes and subsystems, and to keep completion flow \textbf{fully inspectable} via
standard Emacs mechanisms (CAPFs, completion categories, minibuffer UI).
\item What this layer does
\label{sec:orgea58095}
...
\begin{itemize}
\item Defines the minibuffer completion UI and navigation/search commands
\item Defines in-buffer completion UI and its default behavior
\item Establishes matching policy and completion category overrides
\item Orchestrates `completion-at-point-functions` (CAPFs) per major mode
\item Provides targeted, buffer-local tweaks for Org SRC edit buffers
\item Provides optional session hygiene around project switches (Eglot lifecycle)
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Minibuffer completion (selection UI, narrowing, annotations)
\item In-buffer completion (popup UI, auto completion defaults)
\item Matching styles (orderless vs basic, file partial completion)
\item CAPF ordering and safe fallbacks across editing contexts
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:org96c981d}
Completion modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Define global keybinding schemes or leader layouts
\item Implement UI themes or palette policy (owned by \texttt{ui})
\item Introduce language- or project-specific semantics
\item Own LSP configuration or server selection policy (owned by \texttt{dev})
\item Extend Org workflows beyond completion mechanics (owned by \texttt{orgx})
\item Provide VCS features (owned by \texttt{vcs}) or general utilities (owned by \texttt{utils})
\end{itemize}
\item Design constraints
\label{sec:orga362fed}
\begin{itemize}
\item Completion modules may depend on:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\end{itemize}

\item Completion modules must not depend on:
\begin{itemize}
\item \texttt{orgx}
\item \texttt{dev}
\item \texttt{vcs}
...
\item Completion behavior must be:
\begin{itemize}
\item deterministic
\item composable
\item debuggable using standard Emacs facilities
(e.g. inspecting CAPFs, `completion-category-overrides`,
 and the active minibuffer frontend)
\end{itemize}
\end{itemize}
\item Design principles
\label{sec:org8650e93}
\begin{itemize}
\item There is a \textbf{\textbf{single completion flow}} per context:
\begin{itemize}
\item minibuffer completion (Vertico + Marginalia + Consult)
\item in-buffer completion (CAPF + Corfu)
\end{itemize}

\item CAPF order and fallback rules are explicit and reviewable
\item Frontend (UI) and backend (sources/CAPFs) are clearly separated
\item Category-based policy is preferred over mode-specific special cases
\item Org SRC edit buffers are treated as a controlled exception:
buffer-local overrides apply, without changing global policy
\end{itemize}
\item Module map
\label{sec:orgc6bd7d0}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{completion/completion-core.el} & Global completion policy: `completion-styles` and `completion-category-overrides`\\
\texttt{completion/completion-vertico.el} & Minibuffer completion UI (Vertico) and annotations (Marginalia)\\
\texttt{completion/completion-consult.el} & Search/navigation integration, including xref display via Consult\\
\texttt{completion/completion-embark.el} & Contextual actions and prefix help integration (Embark + Embark-Consult)\\
\texttt{completion/completion-corfu.el} & In-buffer completion UI (Corfu) + candidate visuals (kind-icon) + CAPF providers (Cape)\\
\texttt{completion/completion-icons.el} & Nerd icons integration for minibuffer annotations (Marginalia)\\
\texttt{completion/completion-capf.el} & Mode-specific CAPF presets and shared category overrides for CAPF-based completion\\
\texttt{completion/completion-capf-org-src.el} & CAPF switching when entering Org SRC edit buffers (`org-src-mode`)\\
\texttt{completion/completion-capf-org-src-lang.el} & Language-specific CAPF presets inside Org SRC edit buffers (sh/sql/python)\\
\texttt{completion/completion-corfu-org-src.el} & Buffer-local Corfu behavior tuned for Org SRC edit buffers\\
\texttt{completion/completion-orderless-org-src.el} & Buffer-local category overrides for permissive Orderless matching in Org SRC\\
\texttt{completion/completion-lsp.el} & Optional cleanup helper: shut down obsolete Eglot servers on project switch\\
\end{tabular}
\end{center}
\item Notes
\label{sec:org5abd42c}
\begin{itemize}
\item Matching policy is defined centrally via `completion-styles` and
`completion-category-overrides`; Org SRC overrides are buffer-local only.
\item CAPF presets are applied via major-mode hooks; exceptions are limited to
Org SRC edit buffers where `org-src-mode-hook` controls local overrides.
\item LSP server lifecycle policy is not implemented here; only safe cleanup
is provided, and only when Eglot is present and enabled.
\end{itemize}
\end{enumerate}
\item completion/completion-core.el
\label{sec:org3964840}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-vertico.el
\label{sec:org7a2fc48}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-consult.el
\label{sec:orgb48907b}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-embark.el
\label{sec:orga4bcc15}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-corfu.el
\label{sec:org946bc82}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-icons.el
\label{sec:org53a77da}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-capf.el
\label{sec:org1ded69a}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-capf-org-src.el
\label{sec:orgb670466}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-capf-org-src-lang.el
\label{sec:org4566e1e}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-corfu-org-src.el
\label{sec:org37ff730}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-orderless-org-src.el
\label{sec:org6bd5176}
\begin{verbatim}
...
\end{verbatim}
\item completion/completion-lsp.el
\label{sec:org5c135ed}
\begin{verbatim}
...
\end{enumerate}
\subsubsection{ui/}
\label{sec:org109a236}
\begin{enumerate}
\item Overview
\label{sec:org65af9ce}

\begin{itemize}
\item ui modules are loaded \textbf{\textbf{after \texttt{core} and before functional layers}}
\item Each file:
\begin{itemize}
\item provides exactly one feature
\item contains no non-visual side effects
\end{itemize}
\item UI bundle modules:
\begin{itemize}
\item are never loaded implicitly
\item are activated explicitly through \texttt{core/switches.el}
\end{itemize}
\item Removing or disabling all ui modules must leave Emacs fully functional,
albeit visually minimal
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:org5e590b2}
Define the \textbf{\textbf{visual presentation and interaction layer}} of this Emacs configuration.
...
UI modules exist to improve \textbf{clarity, comfort, and legibility}, never correctness.
\item What this layer does
\label{sec:org6cf480b}
...
\begin{itemize}
\item Configures visual appearance:
\begin{itemize}
\item theme selection and palette coordination
\item face definitions and typography (fixed / variable pitch)
\item line spacing, margins, and window layout hints
\end{itemize}
\item Adjusts interaction-related presentation:
\begin{itemize}
\item cursor style and visibility
\item modeline content, density, and signaling
\item subtle visual feedback for focus, state, and context
\end{itemize}
\item Applies UI customizations that are:
\begin{itemize}
\item orthogonal to functionality
\item safe to toggle or replace independently
\item reversible at runtime
\end{itemize}
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:orga336412}
UI modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Define global keybinding schemes or leader layouts
(handled by \texttt{core/general.el})
\item Select, configure, or orchestrate functional backends
(completion, LSP, VCS, Org logic, etc.)
\item Introduce workflow logic or mode-specific behavior
\item Encode user-, device-, or host-specific policy
(handled by \texttt{personal/} overlays)
\end{itemize}

Those responsibilities belong to other layers.
\item Design constraints
\label{sec:orgd13affd}
\begin{itemize}
\item UI modules may depend on:
\begin{itemize}
\item \texttt{core}
\end{itemize}

\item UI modules must not depend on:
\begin{itemize}
\item \texttt{completion}
\item \texttt{orgx}
\item \texttt{dev}
...
\item All UI changes must be:
\begin{itemize}
\item presentation-only (no semantic side effects)
\item reversible at runtime
\item safe in GUI, terminal, daemon, and batch contexts
\item tolerant of partial availability (fonts, icons, faces)
\end{itemize}
\end{itemize}
\item Design principles
\label{sec:org77e5541}
\begin{itemize}
\item Presentation and behavior are strictly separated
\item UI modules must be safe to disable wholesale for debugging
\item Defaults favor readability and low visual noise
\item Visual signaling should be subtle, not distracting
\item Optional UI bundles are treated as \textbf{overlays}, not foundations
\end{itemize}
\item Optional UI bundles
\label{sec:org2a28296}
Theme and modeline bundles (e.g. Doom-style or Nano-style) are treated as
\textbf{\textbf{optional overlays}}.

Selection and activation are controlled exclusively via
\texttt{core/switches.el}, ensuring that:

\begin{itemize}
\item the base UI remains minimal and stable
\item bundles can be compared or disabled safely
\item UI experimentation never destabilizes core behavior
\end{itemize}
\item Benefits
\label{sec:org1aa480d}
This separation ensures that:

\begin{itemize}
\item visual experimentation does not affect correctness
\item alternative UI styles can be swapped without refactoring logic
\item debugging can be performed with a minimal, presentation-free setup
\item long-running sessions remain visually predictable
\end{itemize}
\item Module map
\label{sec:orgd9f8cdf}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{ui/ui-font.el} & Font family, size, and typography defaults\\
\texttt{ui/ui-theme.el} & Theme selection and color palette coordination\\
\texttt{ui/ui-window.el} & Frame, window, margin, and spacing presentation rules\\
\texttt{ui/ui-utils.el} & Small visual helpers and UI-safe conveniences\\
\texttt{ui/ui-health-modeline.el} & Session health indicators and visual status signaling\\
\texttt{ui/ui-icons.el} & Icon and glyph integration (GUI-safe, optional)\\
\texttt{ui/ui-macos.el} & macOS-specific visual tuning (fonts, frames, rendering)\\
\texttt{ui/ui-doom-modeline.el} & Doom-style modeline bundle (optional overlay)\\
\texttt{ui/ui-nano-modeline.el} & Nano-style modeline bundle (optional overlay)\\
\end{tabular}
\end{center}
\item Notes
\label{sec:orga0d5bdf}
\begin{itemize}
\item UI modules must never introduce functional dependencies.
\item Any visual adjustment that affects behavior belongs elsewhere.
\item If a UI change feels “clever” or “opinionated”, it is likely misplaced.
\end{itemize}

This layer should remain \textbf{boring, predictable, and comfortable}.
\end{enumerate}
\item ui/ui-font.el
\label{sec:org12966e4}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-nano-palette.el
\label{sec:org263ef87}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-doom-modeline.el
\label{sec:org5103779}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-nano-modeline.el
\label{sec:org7767c2f}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-health-modeline.el
\label{sec:orgbd3784d}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-window.el
\label{sec:org2289a8f}
\begin{verbatim}
...
\end{verbatim}
\item ui/ui-visual-aids.el
\label{sec:org35d1f7f}
\begin{verbatim}
...
\end{enumerate}
\subsubsection{orgx/}
\label{sec:orgcff28e6}
\begin{enumerate}
\item Overview
\label{sec:org7ac631a}

\begin{itemize}
\item Loaded after \texttt{completion} and before \texttt{dev}
\item Each file:
\begin{itemize}
\item provides exactly one Org-related feature
\item documents its scope and assumptions clearly
\end{itemize}
\item Disabling this layer must leave:
\begin{itemize}
\item basic Org editing functional
\item Org files readable and editable without loss
\end{itemize}
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:orgf5ea5c9}
Extend Org-mode \textbf{\textbf{beyond upstream defaults}} while preserving its
...
Orgx exists to make Org usage \textbf{explicit, auditable, and modular},
without turning Org into a bespoke or opaque system.
\item What this layer does
\label{sec:org44b0c5c}
Orgx modules are responsible for \textbf{\textbf{Org-mode–scoped behavior only}}.

This layer:

\begin{itemize}
\item Extends Org-mode in a structured, non-invasive manner
\item Defines Org-specific workflows, commands, and helper utilities
\item Integrates Org with other subsystems \textbf{only at explicit boundaries}
\item Centralizes agenda, capture, and navigation policy
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Agenda file discovery, scoping rules, and caching
\item Capture templates, inbox flows, and refiling strategy
\item Org-specific navigation, folding, and structural helpers
\item Visual and UX enhancements limited strictly to Org buffers
\item Optional integration with roam-style or graph-based note systems
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:orgecf152d}
Orgx modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Reimplement, fork, or override core Org internals
\item Affect behavior in non-Org buffers
\item Define global editing, completion, or UI policy
\item Introduce development, VCS, or infrastructure concerns
\item Encode user-, device-, or host-specific paths directly
\end{itemize}

Those responsibilities belong to other layers or to
\texttt{personal/} overlays.
\item Design constraints
\label{sec:org4c47d48}
\begin{itemize}
\item Orgx modules may depend on:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\item \texttt{completion}
...
\item Orgx modules must not depend on:
\begin{itemize}
\item \texttt{dev}
\item \texttt{vcs}
\item \texttt{utils}
...
\item All Org-specific assumptions must be:
\begin{itemize}
\item confined to Org buffers
\item discoverable through module boundaries
\item safe to disable as a group without data loss
\end{itemize}
\end{itemize}
\item Design principles
\label{sec:orgcaa1ed3}
\begin{itemize}
\item Org remains \textbf{\textbf{plain Org}} on disk
(no proprietary formats, sidecar metadata, or irreversible transforms)
\item Extensions are additive, reversible, and optional
\item Agenda and capture behavior is explicit and reviewable
\item Org-specific UX must degrade gracefully when extensions are disabled
\item Configuration favors transparency over clever automation
\end{itemize}
\item Benefits
\label{sec:org8db4445}
This separation ensures that:

\begin{itemize}
\item Org files remain portable and future-proof
\item Non-Org workflows are unaffected by Org customization
\item Debugging Org behavior is localized and predictable
\item Org extensions can evolve without destabilizing the rest of Emacs
\end{itemize}
\item Module map
\label{sec:orgc944dcb}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{orgx/org-core.el} & Core Org defaults and shared Org policy\\
\texttt{orgx/org-agenda.el} & Agenda discovery, scoping, and views\\
\texttt{orgx/org-capture.el} & Capture templates and inbox workflows\\
\texttt{orgx/org-navigation.el} & Navigation, folding, and structural helpers\\
\texttt{orgx/org-roam.el} & Roam-style linking and graph-based notes\\
\texttt{orgx/org-export.el} & Export configuration and backends\\
\texttt{orgx/org-ui.el} & Org-specific visual and UX enhancements\\
\end{tabular}
\end{center}
\item Notes
\label{sec:org8f4a756}
\begin{itemize}
\item Orgx is the \textbf{only} layer allowed to assume Org semantics.
\item Any logic that could apply outside Org likely belongs elsewhere.
\item If an Org change affects global behavior, it is misplaced.
\end{itemize}

This layer exists to make Org powerful \textbf{without making it fragile}.
\end{enumerate}
\item orgx/orgx-core.el
\label{sec:org7f43e87}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-visual.el
\label{sec:org81ecbcc}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-extensions.el
\label{sec:orgcb4aae1}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-fold.el
\label{sec:orgf326bd6}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-export.el
\label{sec:org3ec9b07}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-notes-markdown.el
\label{sec:org1cf0ea0}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-auto-tangle.el
\label{sec:orga5ef7c3}
\begin{verbatim}
...
\end{verbatim}
\item orgx/orgx-typography.el
\label{sec:orgbcd6d64}
\begin{verbatim}
...
\end{enumerate}
\subsubsection{dev/}
\label{sec:org5332030}
\begin{enumerate}
\item Overview
\label{sec:orga416442}

\begin{itemize}
\item Loaded after \texttt{orgx}
\item Activation is controlled via:
\begin{itemize}
\item \texttt{core/switches.el}
\item explicit user or maintainer intent
\end{itemize}
\item Each file:
\begin{itemize}
\item provides exactly one dev-oriented feature
\item documents activation conditions and non-goals
\end{itemize}
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:org803572b}
Support \textbf{\textbf{development, diagnostics, and build-oriented workflows}}
for this Emacs configuration.
...
Dev modules are strictly \textbf{auxiliary}: useful when needed, invisible
when not.
\item What this layer does
\label{sec:orgcb8b90e}
Dev modules provide tooling that helps \textbf{understand, debug, and extend}
the running Emacs environment and its configuration.

This layer is responsible for:

\begin{itemize}
\item Tooling for configuration development and internal inspection
\item Support for build, compile, and debug workflows
\item Diagnostics and introspection of the live Emacs instance
\item Maintenance-oriented helpers for evolving the configuration safely
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Language Server Protocol (LSP) client wiring and toggles
\item Debugging, profiling, tracing, and performance analysis helpers
\item Compilation, test execution, and task orchestration
\item Developer-facing inspection commands and reports
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:org3cc0c73}
Dev modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Define global editing behavior or keybinding policy
\item Establish startup-critical behavior or core invariants
\item Introduce visual presentation or theming concerns
\item Encode user-, device-, or host-specific preferences
\item Affect Org workflows or semantic editing behavior
\end{itemize}

Those responsibilities belong to other layers.
\item Design constraints
\label{sec:org5c2d083}
\begin{itemize}
\item Dev modules may depend on:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\item \texttt{completion}
...
\item Dev modules must not depend on:
\begin{itemize}
\item \texttt{orgx}
\item \texttt{vcs}
\item \texttt{utils}
...
\begin{itemize}
\item explicit
\item opt-in via customization variables or switches
\item safe to disable as a group
\end{itemize}
\end{itemize}
\item Design principles
\label{sec:org6723da4}
\begin{itemize}
\item Dev tooling is \textbf{\textbf{additive}}, never required for normal operation
\item Failure in this layer must degrade gracefully
\item Disabling this layer must not affect:
\begin{itemize}
\item startup success
\item baseline editing behavior
\item Org workflows or persistence
\end{itemize}
\item Debugging aids should favor observability over automation
\end{itemize}
\item Benefits
\label{sec:orgc66ee32}
This separation ensures that:

\begin{itemize}
\item Development aids never compromise configuration stability
\item Power-user tooling can evolve rapidly without risk
\item Production usage remains lightweight and predictable
\item Debugging can be enabled surgically when needed
\end{itemize}
\item Module map
\label{sec:org84d9fcf}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{dev/dev-lsp-eglot.el} & Eglot-based LSP configuration and integration\\
\texttt{dev/dev-lsp-mode.el} & lsp-mode–based LSP configuration (alternative backend)\\
\texttt{dev/dev-dap.el} & Debug Adapter Protocol integration\\
\texttt{dev/dev-compile.el} & Compile, build, and task helpers\\
\texttt{dev/dev-profile.el} & Profiling and performance diagnostics\\
\texttt{dev/dev-inspect.el} & Runtime inspection and developer-facing debug utilities\\
\end{tabular}
\end{center}
\item Notes
\label{sec:org5e26b4b}
\begin{itemize}
\item Dev is the \textbf{only} layer allowed to assume a maintainer mindset.
\item Any tool required for basic usage does not belong here.
\item If disabling this layer breaks normal editing, it is misplaced.
\end{itemize}

This layer exists to make the configuration \textbf{easier to evolve},
not harder to use.
\end{enumerate}
\item dev/dev-lsp-eglot.el
\label{sec:orga566dde}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-lsp-mode.el
\label{sec:orgf0e1cb6}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-term.el
\label{sec:org4262ee5}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-web-core.el
\label{sec:orgcced69a}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-build.el
\label{sec:org431138d}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-format.el
\label{sec:org9dcdede}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-infra-modes.el
\label{sec:orgfec5619}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-docker.el
\label{sec:org8fd6301}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-rest.el
\label{sec:orgaa8819c}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-navigation.el
\label{sec:orgf7caf48}
\begin{verbatim}
...
\end{verbatim}
\item dev/dev-tools.el
\label{sec:orgeaf1ab9}
\begin{verbatim}
...
\end{enumerate}
\subsubsection{vcs/}
\label{sec:org84cc1bc}
\begin{enumerate}
\item Overview
\label{sec:orge9b9df0}

\begin{itemize}
\item Loaded after \texttt{dev}
\item Each file:
\begin{itemize}
\item provides exactly one VCS-related feature
\item documents backend assumptions and failure modes
\end{itemize}
\item Disabling this layer must leave Emacs fully usable for
non-version-controlled files
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:org173dafb}
Provide a \textbf{\textbf{consistent, explicit, and inspectable version control interaction
layer}} within Emacs.
...
Emacs is used as a \textbf{control surface}, not as a replacement for VCS tooling.
\item What this layer does
\label{sec:org9025905}
...
\begin{itemize}
\item Bridging Emacs with VCS backends (primarily Git)
\item Exposing repository state, history, and diffs inside Emacs buffers
\item Providing interactive commands for common VCS workflows
\item Surfacing VCS information in buffers without mutating editing semantics
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Repository status inspection and change overview
\item Commit, diff, hunk, and blame navigation
\item File- and project-scoped VCS operations
\item Lightweight review, history browsing, and annotation
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:orge83b2f9}
VCS modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Define global editing behavior or keybinding policy
\item Enforce a specific branching, commit, or review workflow
\item Replace, fork, or reimplement external VCS tools
\item Assume the presence of a repository or remote
\end{itemize}

Those responsibilities belong to user workflows or external tooling.
\item Design constraints
\label{sec:org2191a3b}
\begin{itemize}
\item VCS modules may depend on:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\item \texttt{completion}
...
\item VCS modules must not depend on:
\begin{itemize}
\item \texttt{orgx}
\item \texttt{dev}
\item \texttt{utils}
...
\item Interaction with external tools must be:
\begin{itemize}
\item explicit
\item inspectable
\item resilient to tool absence or failure
\end{itemize}
\end{itemize}
\item Design principles
\label{sec:org95b6a40}
\begin{itemize}
\item External VCS tools are the \textbf{\textbf{source of truth}}
\item Emacs acts as an interface layer, not an abstraction leak
\item All commands must degrade gracefully when:
\begin{itemize}
\item no repository is present
\item required binaries are missing
\item network access is unavailable
\end{itemize}
\item User workflow choices remain:
\begin{itemize}
\item opt-in
\item reversible
\item discoverable
\end{itemize}
\end{itemize}
\item Observed implementation characteristics
\label{sec:org5282081}
Based on the current configuration:

\begin{itemize}
\item Git integration is centered on \textbf{Magit} for interactive workflows
\item Change visualization relies on lightweight gutter-style indicators
\item No assumptions are made about:
\begin{itemize}
\item hosting provider
\item branching model
\item collaboration workflow
\end{itemize}
\item VCS features can be enabled or disabled independently
\end{itemize}
\item Module map
\label{sec:org76b8633}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{vcs/vcs-magit.el} & Magit-based interactive Git workflows\\
\texttt{vcs/vcs-gutter.el} & In-buffer change indicators (diff-hl)\\
\texttt{vcs/vcs-forge.el} & Optional GitHub / GitLab integration via Forge\\
\end{tabular}
\end{center}
\item Notes
\label{sec:org747a6fa}
\begin{itemize}
\item This layer is intentionally thin and compositional.
\item Any logic that \textbf{depends} on VCS state belongs elsewhere.
\item If disabling this layer breaks core editing, the dependency is misplaced.
\end{itemize}

This layer exists to make version control \textbf{visible and accessible},
not opinionated or mandatory.
\end{enumerate}
\item vcs/vcs-magit.el
\label{sec:orgbd874dd}
\begin{verbatim}
...
\end{verbatim}
\item vcs/vcs-gutter.el
\label{sec:orgaf853ac}
\begin{verbatim}
...
\end{verbatim}
\item vcs/vcs-forge.el
\label{sec:org154bb1e}
\begin{verbatim}
...
\end{enumerate}
\subsubsection{utils/}
\label{sec:orgd9b4a11}
\begin{enumerate}
\item Overview
\label{sec:org624904b}

\begin{itemize}
\item Loaded last by \texttt{modules.el}
\item Each file:
\begin{itemize}
\item provides exactly one utility feature
\item remains safe to load in isolation
\end{itemize}
\item Removing this layer must not break
any higher-level architectural guarantees
\end{itemize}
\begin{enumerate}
\item Purpose
\label{sec:orgdd717a7}
Host \textbf{\textbf{small, domain-specific utilities}} that do not justify
a dedicated architectural layer.

This layer exists as a \textbf{pragmatic utility shelf}: narrowly scoped helpers
live here to avoid contaminating core abstractions, while remaining easy
to audit, extract, or delete.
\item What this layer does
\label{sec:orga883b4a}
Utils modules provide \textbf{localized assistance} to other layers without
owning workflows, policy, or long-lived behavior.

They are responsible for:

\begin{itemize}
\item Small, focused helper functions and interactive commands
\item Solving localized or cross-cutting problems that span layers
\item Supplying glue code that would be inappropriate in higher layers
\end{itemize}

Typical responsibilities include:

\begin{itemize}
\item Buffer, window, and process housekeeping helpers
\item Lightweight automation and defensive cleanup logic
\item One-off integrations with external tools or Emacs internals
\item Quality-of-life helpers that do not define behavior on their own
\end{itemize}
\item What this layer does \textbf{not} do
\label{sec:orga409fb1}
Utils modules intentionally do \textbf{\textbf{not}}:

\begin{itemize}
\item Define global policy, defaults, or invariants
\item Perform orchestration or lifecycle management
\item Introduce new architectural concepts or abstractions
\item Replace, override, or shadow behavior from higher layers
\item Encode user-, device-, or environment-specific configuration
\end{itemize}

Those responsibilities are explicitly owned by other layers.
\item Design constraints
\label{sec:orgd9c52c1}
\begin{itemize}
\item Utils modules may depend on:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\item \texttt{completion}
...
\item Utils modules must \textbf{\textbf{not}} be depended on by:
\begin{itemize}
\item \texttt{core}
\item \texttt{ui}
\item \texttt{completion}
...
This makes utils a \textbf{leaf layer} in the dependency graph.
\item Design principles
\label{sec:org7d5ff5d}
\begin{itemize}
\item Keep modules \textbf{\textbf{small, single-purpose, and disposable}}
\item Avoid accumulation of loosely related helpers in one file
\item Prefer promotion to a dedicated layer if scope or importance grows
\item Deletion must always remain a valid and low-risk refactoring option
\item Utilities should be easy to reason about in isolation
\end{itemize}
\item Observed implementation characteristics
\label{sec:org5df3eca}
Based on the current configuration:

\begin{itemize}
\item Utilities are defensive and side-effect conscious
\item Many helpers are interactive diagnostics or maintenance tools
\item No utils module is required for startup correctness
\item Disabling the entire utils layer should degrade convenience,
not correctness
\end{itemize}
\item Module map
\label{sec:orgf31e245}
\begin{center}
\begin{tabular}{ll}
File & Responsibility\\
\hline
\texttt{utils/utils-path.el} & Safe directory and path helpers\\
\texttt{utils/utils-async.el} & Minimal asynchronous task helpers\\
\texttt{utils/utils-buffers.el} & Buffer lifecycle and cleanup helpers\\
\texttt{utils/utils-edit.el} & Editing and save-time helper utilities\\
\texttt{utils/utils-dired.el} & Dired convenience helpers\\
\texttt{utils/utils-org-agenda.el} & Cached org-agenda file discovery\\
\texttt{utils/utils-lint.el} & Static lint helpers for Emacs Lisp\\
\texttt{utils/utils-diagnostics.el} & Configuration and runtime diagnostics\\
\texttt{utils/utils-functions.el} & Small general-purpose helper functions\\
\end{tabular}
\end{center}
\item Notes
\label{sec:org5a06310}
\begin{itemize}
\item This layer is intentionally heterogeneous and low-ceremony.
\item Utilities should \textbf{never} become implicit dependencies.
\item If a utils module feels “important”, it likely belongs elsewhere.
\end{itemize}

This layer exists to make the rest of the system \textbf{simpler}, not smarter.
\end{enumerate}
\item utils/utils-functions.el
\label{sec:orgf51f1da}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-path.el
\label{sec:org864996c}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-async.el
\label{sec:orga3885df}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-dired.el
\label{sec:org01b633b}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-buffers.el
\label{sec:org7dc418d}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-edit.el
\label{sec:org001fad1}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-org-agenda.el
\label{sec:org8f95a54}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-lint.el
\label{sec:orgac7bff4}
\begin{verbatim}
...
\end{verbatim}
\item utils/utils-diagnostics.el
\label{sec:orge8aa0e3}
\begin{verbatim}
...
\end{verbatim}
\end{enumerate}
\subsection{Personal Profile \& Device Integrati}
\label{sec:orgeb991ef}
\subsubsection{Overview}
\label{sec:orgd78ae44}

\begin{enumerate}
\item Personal Layer Philosophy
\label{sec:orgfccd95f}
The \texttt{personal/} layer provides \textbf{\textbf{user- and device-specific overlays}}
on top of the shared, version-controlled configuration.

Its role is to express \textbf{identity, environment, and personal workflow glue}
...
\item reproducible
\item auditable
\item portable across machines and users
\end{itemize}

while still allowing the configuration to feel \textbf{native} on a specific device.
\item Scope definition
\label{sec:orgf6dbc14}

This layer MAY contain:

\begin{itemize}
\item Identity information (user name, email address)
\item Personal feature flags and thresholds
(UI bundle selection, LSP preference, timing knobs)
\item Device- and OS-specific glue
(IME behavior, mouse/scroll tuning, platform quirks)
\item Personal keybindings and workflow integrations
\item Optional integrations with external, user-owned services
\end{itemize}

This layer MUST NOT contain:

\begin{itemize}
\item Core architectural or cross-user decisions
\item Shared defaults or global policy
\item Modules that other layers depend on
\item Assumptions that affect startup correctness
\end{itemize}

Hooks and timers are permitted \textbf{\textbf{only}} when they are:

\begin{itemize}
\item strictly local to the user’s device or workflow
\item defensive (safe to no-op when unavailable)
\item non-invasive to global behavior
\end{itemize}
\item Purpose
\label{sec:orgb69aef0}
Provide \textbf{personal overlays} that adapt the configuration to a specific
...
\begin{itemize}
\item encode identity and safety-related editor defaults
\item select between alternative shared stacks (UI / LSP)
\item bind shared infrastructure to local filesystem layout
\item integrate OS- and device-specific affordances
\item host convenience glue that would be inappropriate elsewhere
\end{itemize}
\item What this configuration does
\label{sec:orga106c40}

\begin{enumerate}
\item Identity \& safety
\label{sec:org026f304}
\begin{itemize}
\item Sets \texttt{user-full-name} and \texttt{user-mail-address}
\item Applies safety-related editor preferences:
\begin{itemize}
\item disables font cache compaction on macOS
\item enables in-memory passphrase caching for \texttt{plstore}
\end{itemize}
\end{itemize}
\item Look \& feel switches
\label{sec:org69a6804}
\begin{itemize}
\item Declares personal font preferences and default size
\item Selects UI and LSP stacks explicitly:
\begin{itemize}
\item \texttt{my:use-ui} (e.g. \texttt{nano})
\item \texttt{my:use-lsp} (e.g. \texttt{eglot})
\end{itemize}
\item Delegates actual behavior to shared UI and dev layers
\end{itemize}
\item Directories \& Org wiring
\label{sec:org7b3524c}
\begin{itemize}
\item Defines a cloud-rooted workspace (default: \texttt{\textasciitilde{}/Documents})
\item Derives:
\begin{itemize}
\item \texttt{my:d:org}
\item \texttt{my:d:blog}
\end{itemize}
\item Ensures required directories exist
\item Sets \texttt{org-directory}
\item Computes \texttt{org-agenda-files} dynamically by scanning
non-archive \texttt{.org} files
\item Removes sensitive or excluded paths from \texttt{load-path}
\end{itemize}
\item macOS input method integration (\texttt{sis})
\label{sec:org16a2ec5}
\begin{itemize}
\item Configures ABC ⇄ Kotoeri (Romaji) switching via \texttt{macism}
\item Enables cursor-color, respect, and inline modes when available
\item All functionality is guarded with \texttt{fboundp} checks
\end{itemize}
\item Cursor color keep-alive
\label{sec:org6f2e100}
\begin{itemize}
\item Reapplies the frame’s cursor color to the \texttt{cursor} face
after theme reloads
\item Prevents cursor desynchronization across theme switches
\end{itemize}
\item Device profile (MX Ergo S)
\label{sec:orgaa3dfb1}
\begin{itemize}
\item Applies conservative, smooth scrolling defaults
\item Preserves screen position and margins
\item Enables tilt scrolling
\item Mouse bindings:
\begin{itemize}
\item \texttt{mouse-2} → yank
\item \texttt{mouse-4/5} → previous / next buffer
\end{itemize}
\end{itemize}
\item Apple Music integration (macOS)
\label{sec:orgd00d6cb}
\begin{itemize}
\item Defines asynchronous and synchronous AppleScript helpers
\item Exposes interactive commands:
\begin{itemize}
\item play / pause
\item next / previous track
\item playlist selection
\item current track info
\end{itemize}
\item Provides a Hydra bound to \texttt{C-c M}
\item Optionally integrates with Meow leader keys when available
\end{itemize}
\end{enumerate}
\item Module map
\label{sec:org761f175}
\begin{center}
\begin{tabular}{ll}
Module file & Role\\
\hline
\texttt{personal/user.el} & Identity, fonts, UI/LSP switches, Org paths\\
\texttt{personal/device-darwin.el} & macOS-specific device and IME glue\\
\texttt{personal/apple-music.el} & Apple Music control (AppleScript + Hydra)\\
\end{tabular}
\end{center}
\item Execution flow
\label{sec:org07b34ce}

\begin{enumerate}
\item \textbf{Personal bootstrap}
\begin{itemize}
\item Identity, fonts, and UI/LSP switches are established
\item Cloud, Org, and blog directories are defined and ensured
\end{itemize}

\item \textbf{Org wiring}
\begin{itemize}
\item \texttt{org-directory} is set
\item \texttt{org-agenda-files} is derived by filtering non-archive Org files
\end{itemize}

\item \textbf{Hygiene}
\begin{itemize}
\item Sensitive or excluded directories are removed from \texttt{load-path}
\end{itemize}

\item \textbf{macOS-only glue}
\begin{itemize}
\item \texttt{sis} is configured defensively
\item Cursor color synchronization hook is installed
\end{itemize}

\item \textbf{Device profile}
\begin{itemize}
\item Mouse and scroll tuning for MX Ergo S is applied
\end{itemize}

\item \textbf{Apple Music integration}
\begin{itemize}
\item AppleScript runners are defined
\item Interactive commands and Hydra are exposed
\end{itemize}
\end{enumerate}
\item Key settings (reference)
\label{sec:org50fe9a3}

\begin{description}
\item[{\texttt{my:font-default}}] "JetBrains Mono"
\item[{\texttt{my:font-variable-pitch}}] "Noto Sans JP"
\item[{\texttt{my:font-size}}] 18
\item[{\texttt{my:use-ui}}] 'nano
\item[{\texttt{my:use-lsp}}] 'eglot
\item[{\texttt{org-directory}}] \texttt{\textasciitilde{}/Documents/org}
\item[{\texttt{org-agenda-files}}] all \texttt{*.org} under \texttt{org-directory}, excluding \texttt{archives}
\item[{MX Ergo scroll profile}] \begin{itemize}
\item \texttt{mouse-wheel-scroll-amount}'(1 ((shift) . 5) ((control) . 10))=
\item \texttt{scroll-conservatively=10000}
\item \texttt{scroll-margin=2}
\item \texttt{scroll-preserve-screen-position=t}
\end{itemize}
\item[{Cursor color keep-alive}] reapply \texttt{(set-face-background 'cursor (frame-parameter nil 'cursor-color))}
on \texttt{after-load-theme}
\end{description}
\item Usage tips
\label{sec:org27ace8c}

\begin{itemize}
\item \textbf{Switch UI or LSP stacks}
\begin{itemize}
\item Change \texttt{my:use-ui} to \texttt{'nano}, \texttt{'doom}, or \texttt{'none}
\item Change \texttt{my:use-lsp} to \texttt{'eglot} or \texttt{'lsp}
\end{itemize}

\item \textbf{Change fonts}
\begin{itemize}
\item Adjust font variables here
\item Shared UI modules consume them automatically
\end{itemize}

\item \textbf{Apple Music control}
\begin{itemize}
\item \texttt{C-c M} opens the Hydra
\item Keys:
\begin{itemize}
\item \texttt{p} play/pause
\item \texttt{n} next
\item \texttt{b} back
\item \texttt{l} playlist
\item \texttt{i} track info
\end{itemize}
\end{itemize}

\item \textbf{Agenda scope control}
\begin{itemize}
\item Place files under an \texttt{archives/} directory
\item Or include “archives” in filenames to exclude them
\end{itemize}
\end{itemize}
\item Troubleshooting
\label{sec:org145a644}

\begin{itemize}
\item \textbf{sis does not switch input methods}
\begin{itemize}
\item Verify input source IDs:
\begin{itemize}
\item "com.apple.keylayout.ABC"
\item "com.apple.inputmethod.Kotoeri.RomajiTyping.Japanese"
\end{itemize}
\item All calls are guarded; missing functions will no-op
\end{itemize}

\item \textbf{Cursor color incorrect after theme change}
\begin{itemize}
\item Ensure the theme sets the frame’s \texttt{cursor-color} parameter
\end{itemize}

\item \textbf{Hydra key missing}
\begin{itemize}
\item Confirm \texttt{hydra} is installed and loaded
\item Binding is added via \texttt{with-eval-after-load}
\end{itemize}

\item \textbf{Meow leader binding missing}
\begin{itemize}
\item Requires both \texttt{meow} and \texttt{hydra}
\item Binding is added defensively
\end{itemize}
\end{itemize}
\item Related source blocks
\label{sec:org7507cc4}
\begin{verbatim}
;; See:
;; - personal/user.el
...
\end{verbatim}
\end{enumerate}
\subsubsection{user.el}
\label{sec:orgecc9028}
\begin{verbatim}
...
\end{verbatim}
\subsubsection{device-darwin.el}
\label{sec:org9e27c53}
\begin{verbatim}
...
\end{verbatim}
\subsubsection{apple-music.el}
\label{sec:org1703599}
Purpose:
Control Apple Music from Emacs on macOS.

What it does:
\begin{itemize}
\item Provides async/sync AppleScript helpers
\item Defines interactive playback commands
\item Exposes a hydra and optional meow leader binding
\end{itemize}

Notes:
\begin{itemize}
\item This module is strictly optional and macOS-only.
\item No core/session or utils logic is used here.
\end{itemize}
