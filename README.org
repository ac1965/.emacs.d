...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  (when (eq system-type 'darwin)
    ...
    (defun my-prepend-env-path (env-var dir)
      ...
      (when (file-directory-p dir)
        (let* ((sep     (char-to-string path-separator))   ;; safer than hardcoding ":"
               (current (split-string (or (getenv env-var) "") sep t))
               (new     (delete-dups (cons dir current))))
          (setenv env-var (mapconcat #'identity new sep))
  ...
  ;; Redirect eln-cache before packages load
  (when (fboundp 'startup-redirect-eln-cache)
    (startup-redirect-eln-cache my:d:eln))

  ;; Fallback for older comp vars
  (when (boundp 'native-comp-eln-load-path)
    (setq native-comp-eln-load-path (list my:d:eln)))
  (when (boundp 'comp-eln-load-path)
    (setq comp-eln-load-path (list my:d:eln)))
...
#+begin_src emacs-lisp :tangle lisp/README.el
  ...
  ;; $Lastupdate: 2025/09/16 23:40:42 $
