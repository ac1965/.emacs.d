...
- =orgx-roam-ui= — =web-server= 経由で提供される =org-roam-ui= グラフビジュアライザ
  （MELPA 上のパッケージ名は =simple-httpd=、feature シンボルは =web-server=）。
  load-path を確実にするためパッケージはトップレベル（=with-eval-after-load= の外）で
  インストールする。=provide 'simple-httpd= のシムが =org-roam-ui.el= 内の
  =(require 'simple-httpd)= を満たす。UI は =org-roam= の autosync（5 秒タイマー）が
  落ち着いた後、8 秒のアイドルタイマーで開く
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; 本ファイルの責務:
  ;;   1. straight.el、leaf、Org のブートストラップ
...
#+begin_src emacs-lisp
  ...
  (leaf org
    ...
    :config
    ;; フェーズ 1: utils-org-agenda が未ロードでも org-agenda-files が nil に
    ;; ならないよう、最小限のフォールバックを即座に設定する。
...
#+begin_src emacs-lisp
  ...
  ;; :straight と :require は別のキーワードである。
  ;; (leaf ox :straight :require t) は :require を :straight の値として渡してしまい、
  ;; 構文エラーだった。ox は Org に同梱される組み込みなので :straight nil とする。
  ...
  ;;
  ;; 設計上の注記:
  ;;   :require t  — ob-mermaid（およびその defcustom）が :config の実行前に
  ;;                 ロードされることを保証するため。これにより setopt が安全になる。
...
#+begin_src emacs-lisp
  ...
  ;; - ox-hugo のロードに依存する（orgx 層で orgx-export が先にロードするため、
  ;;   依存は正しく下方向に流れる）
  ;;
  ;; 設計の根拠:
  ;;
...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; org-roam ノードの対話的グラフビジュアライザ。ローカルの WebSocket サーバ経由で
  ;; 提供され、ブラウザで閲覧する。
  ;;
  ;; 本ファイルの責務:
  ;; - simple-httpd（レシピ名。feature は web-server）、websocket、org-roam-ui を
  ;;   トップレベル（with-eval-after-load の外側）で straight-use-package により導入し、
  ;;   org-roam を require する前に 3 つとも load-path 上に載せる
  ;;
  ;; - web-server（simple-httpd ではない — 下記の注記を参照）、websocket、org-roam-ui を
  ;;   トップレベルで無条件に require する。各組は、ロードを隔離するため condition-case で
  ;;   包む
  ...
  ;;
  ;; leaf :after を使わない理由:
  ;;   leaf :after は :straight の導入と :config の両方を with-eval-after-load 'org-roam の
  ;;   内側に包んでしまう。orgx-roam-ui.el が require される時点で org-roam は既にロード済み
  ;;   の feature である（my:modules-extra 内で orgx-extensions が先にロードするため）ので、
  ;;   with-eval-after-load は即座に同期実行される。その瞬間、straight はまだ simple-httpd も
  ;;   websocket もビルドしていないため、包まれた中の (require 'org-roam-ui) は
  ;;   "Cannot open load file: simple-httpd" で失敗する。
  ;;
  ;;
  ;;   3 つのパッケージをすべてトップレベルで導入すれば、遅延評価が走るより前に、
  ;;   セッション全体を通じてビルド済みかつ load-path 上にあることが保証される。
  ;;
  ...
  ;; 依存パッケージをトップレベルで同期的に導入・ロードする。
  ;;
  ;; skeeto/emacs-web-server が provide する feature シンボルは 'web-server であり、
  ;; 'simple-httpd ではない。'simple-httpd は MELPA 上のパッケージ名にすぎず、実際に同梱
  ;; されるファイルは web-server.el で、その末尾に (provide 'web-server) がある。
  ;; (require 'simple-httpd) が過去に失敗し続けたのは、ビルドディレクトリに simple-httpd.el
  ;; が存在しないためである:
  ;;
  ;;   build-31.0/simple-httpd/
  ;;     web-server.el      <- 実体のファイル
  ;;     web-server.elc
  ;;     simple-httpd-autoloads.el   <- autoload のみ。(provide 'simple-httpd) は無い
  ;;
  ;; 対処: (require 'simple-httpd) を (require 'web-server) に置き換える。
  ;; straight のレシピ名は 'simple-httpd のまま（MELPA のパッケージ名）である。
  (condition-case err
      (progn
        (straight-use-package 'simple-httpd)
        (require 'web-server)
        ;; 上流の org-roam-ui.el には (require 'simple-httpd) が含まれる。
        ;; skeeto/emacs-web-server が provide するのは 'web-server であり 'simple-httpd ではない。
        ;; このシムが、上流の require を何もしない形で満たす。
...
本 Changelog は 2026-07 のリファクタリングでリセットした。
それ以前の記述（評価パス 25 回分、約 3,400 行）は本ファイルから削除している。

*記録の方針*

- 過去の個別修正はすべて現行コードに反映済みであり、記述と実装の差分は存在しない。
  削除した記述は Git の履歴に残っており、必要なら =git log -p README.org= で参照できる。
- Fix ID による追跡方式は廃止した。コード中のコメントから Changelog のエントリを参照しない。
  変更の *理由* は Fix ID を外したうえで、該当箇所の Commentary に残してある。
- =README.org= が唯一の真実の源であり、Changelog はその要約にすぎない。既に解消済みの
  過去の不整合を保持し続けることは、その原則に照らして冗長である。
