...
;;; Commentary:
;; Main initialization file for Emacs 30+.
;; Provides package management (straight.el + leaf),
;; user customization groups, base UI/behavior,
;; and modular loading of configuration from lisp/.
...
(let* ((base (or (bound-and-true-p straight-base-dir) user-emacs-directory))
  ...
  (unless (file-exists-p bootstrap-file)
    (condition-case err
      ...
      (error
       (user-error "[straight] bootstrap failed: %s" (error-message-string err)))))
...
;; Increase process buffer size temporarily
...
(leaf exec-path-from-shell
  ...
  (condition-case err
    ...
    (error (message "[exec-path-from-shell] skipped: %s" (error-message-string err)))))
...
(when (file-readable-p custom-file)
  (condition-case err
    ...
    (error (message "[custom-file] failed: %s" (error-message-string err)))))
    ...
    (condition-case err
      ...
      (error
       (funcall (if noerror #'message #'user-error)
                "[personal] failed to load %s: %s"
                file (error-message-string err))))))

;; User-specific config
(let ((user-specific-config (concat my:d "personal/" user-login-name ".el")))
  (my/safe-load-file user-specific-config t))
...
(let* ((root (if (and (boundp 'my:d) (stringp my:d) (file-directory-p my:d))
  ...
  ;; Avoid load-path pollution with user-emacs-directory
