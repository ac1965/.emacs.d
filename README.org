...
- *Opt-in power* — advanced tooling is never required for baseline use.
- *Observable over magical* — prefer inspectable state to opaque automation.
...
- 🚀 *Performance & Native Compilation* — ELN files under `.cache/`, optimized startup GC, restored runtime thresholds, and GCMH for idle collection.
- 🧩 *Language Server Protocol* — Backend-agnostic helpers; switch between *Eglot* and *lsp-mode* via `core/switches.el`.
- 🤖 *AI Integration* — Aidermacs (vterm). Uses OpenRouter when `OPENROUTER_API_KEY` is set, otherwise OpenAI via `OPENAI_API_KEY`.
- 🎨 *Modern UI & Editing* — Tree-sitter remaps, ef-themes + spacious-padding, Nerd Icons, Vertico/Orderless/Corfu/CAPE/Embark/Consult, Doom/Nano modelines.
- 🗂️ *Productivity* — Curated Org stack (agenda, capture, journal, roam, download, TOC), Magit + diff-hl/Forge, REST client, Docker/dev helpers, controlled backups.
...
- `lexical-binding: t` is mandatory in all files.
- `(provide 'FEATURE)` must match the logical feature name.
- Built-in packages explicitly declare `:straight nil`.
- `leaf` forms follow a consistent order:
...
- Obsolete APIs are avoided.
  - Prefer `if-let*`, `when-let*`, `and-let*`.
- New compiler/runtime warnings are treated as actionable.
- The codebase aims to remain warning-free under default
  `byte-compile-warnings` on current stable Emacs.

See [[#modular-loader-and-core-suite][Modular Loader & Core/Utils design]]
for structural enforcement of these rules.
...
This configuration uses a strictly layered, explicitly orchestrated
module loading architecture.

Feature activation is:

- deterministic
- reproducible
- inspectable
- independent of filesystem side effects

All shared behavior is activated from a centralized entry point.
Modules are never discovered implicitly or loaded via directory scanning.

This section describes the design intent and constraints of the loader.

**** Architectural Objective

The modular loader establishes a clear boundary between:

1. Infrastructure bootstrap
2. Runtime foundation
3. Feature activation

Feature activation begins in =modules.el=.
Earlier stages prepare the environment but do not enable shared behavior.

This separation ensures that:

- Bootstrap remains minimal
- Runtime preparation remains infrastructure-focused
- Feature activation remains centralized and reviewable

**** Responsibility Model

The loader defines a directory-based partition of responsibilities:
...
| =orgx/=       | Org extensions beyond upstream defaults |
| =dev/=        | Development and language tooling |
| =vcs/=        | Version control integration |
| =utils/=      | Narrow, domain-specific helpers |

Each directory represents a bounded responsibility domain.
Dependencies flow strictly downward.

**** Authoritative Entry Point

=modules.el= is the authoritative activation point
for shared configuration modules.

It:

- Defines the ordered module list
- Requires modules with error isolation
- Supports skip and extension mechanisms
- Reports load timing and failures

It may contain minimal loader logic, but no feature behavior.
All feature logic resides inside module files.
...
This sequence encodes dependency direction.
...
- Modules are not discovered via directory scanning
- Wildcard expansion is not used
- Environment-based conditionals are excluded from the loader
- Personal or host-specific logic is excluded from shared layers
...
The loader contains no business logic.
...
- Inspectable startup behavior
- Predictable dependency relationships
- Controlled enable/disable during diagnostics
- Stable compilation flows
- Long-term maintainability
- Safe layer removal without cascading failure

Removing a module reduces convenience, not correctness.

**** Relationship to Bootstrap

Initialization is segmented:

1. =early-init.el= — infrastructure preparation
2. =init.el=       — runtime and environment setup
3. =modules.el=    — shared feature activation

This segmentation keeps infrastructure and behavior separate,
centralizes activation, and isolates personal overlays from global policy.

The modular loader forms the structural backbone of the configuration.
...
- =early-init.el= prepares startup invariants and environment state
- =init.el= establishes runtime foundations and transfers control to =modules.el=
- =core/= is the first feature layer activated after bootstrap

This ordering ensures that all higher layers operate on a stable,
explicitly defined foundation.

***** Purpose

Provide the foundational runtime, policy, and infrastructure layer
required by all shared modules.

Core establishes execution guarantees, primitive availability,
and global defaults that higher layers may rely on without
defensive checks.

Its role is to reduce complexity elsewhere by centralizing invariants.

***** What this layer does

Core defines the baseline behavior of the configuration.

Responsibilities include:

- Establishing globally shared low-level behavior
- Defining configuration-wide policy and invariants
- Initializing subsystems with long-lived impact
- Providing primitives consumed by higher layers

Typical scope:

- Compatibility guards and version-scoped fixes
- Session lifecycle orchestration and global thresholds
- Global keybinding infrastructure
- Editing, persistence, and filesystem policy
- Tree-sitter infrastructure with explicit grammar control
- Feature switches gating downstream activation (UI, LSP, etc.)

***** What this layer does *not* do

Core does not:

- Implement visual presentation or theming
- Define completion UX or minibuffer behavior
- Extend Org semantics
- Configure language servers, build tools, or VCS workflows
- Encode user-, device-, or host-specific preferences

These concerns belong to higher layers or =personal/= overlays.
...
- Side effects must be explicit, documented, and global in intent.

- Failure in this layer is configuration-critical and must surface early.
...
- Each file:
  ...
  - Documents scope and non-goals
- Optional behavior is gated by explicit customization variables
- No module assumes interactive usage, GUI presence, or user input
...
- Higher layers to rely on unconditional primitives
- Debugging from a deterministic baseline
- Centralized, reviewable policy
- Predictable long-running sessions

***** Module map

| File | Responsibility |
|------+----------------|
| =core/fixes.el= | Version-scoped compatibility guards |
| =core/core-session.el= | Session orchestration and global health policy |
| =core/general.el= | Global keybinding infrastructure |
| =core/tools.el= | Cross-cutting navigation and inspection helpers |
| =core/utils.el= | Core-level shared utilities |
| =core/core-treesit.el= | Tree-sitter infrastructure and grammar policy |
| =core/core-history.el= | Session persistence (history, places, recent files) |
| =core/editing.el= | Editing and filesystem interaction policy |
| =core/switches.el= | Switches controlling UI and LSP activation |
| =core/custom.el= | Customize routing and =custom-file= management |
| =core/custom-ui-extras.el= | Optional UI extensions appended at runtime |

***** Notes

- Core is the only layer allowed to define global invariants.
- Logic dependent on UI, completion, or Org semantics does not belong here.
- If a change feels convenient rather than necessary, it likely belongs elsewhere.

This layer exists to make everything above it stable and predictable.
**** core/core-fixes.el
:PROPERTIES:
