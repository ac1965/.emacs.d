...
#+begin_src emacs-lisp
  ...
  ;; This file orchestrates visualization, logging, alerts, and manual recovery
  ;; for long-running Emacs sessions.
  ;;
  ;; Operational execution (GC / buffer cleanup / LSP lifecycle) lives in utils/.
  ;;
  ;;; Code:
  ...
  (leaf *personals
    ...
    (setq org-roam-db-node-include-function
        (lambda ()
          (not (string-match-p "/chatgpt/" (buffer-file-name)))))
    (setq org-agenda-files
          (condition-case nil
              (seq-filter
               (lambda (file)
  ...
  ;;; ---------------------------------------------------------------------------
  ;;; Long-running session: visualization / judgment / orchestration
  ;;; ---------------------------------------------------------------------------
  ...
  (defvar my/idle-refresh-delay (* 60 30)
    "Seconds before running a lightweight refresh during idle.")

  (defun my/idle-light-refresh ()
    "Lightweight refresh during long idle.
  Delegates cleanup to utils layer."
    (garbage-collect)
    (clear-image-cache)
  ...
  (defun my/eglot-workspace-count ()
    "Return the number of active eglot workspaces."
  ...
  (defun my/log-memory-usage ()
    "Append a simple memory usage snapshot to log file."
    (with-temp-buffer
      (insert
  ...
  (defun my/memory-health-alert ()
    "Notify when Emacs enters a risky memory state."
  ...
  Delegates operational cleanup to utils layer."
    (interactive)
    (when (and (featurep 'eglot)
               (boundp 'eglot--servers))
      (dolist (server eglot--servers)
