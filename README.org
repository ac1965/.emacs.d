...
- ✅ `lexical-binding: t` is *mandatory*. The `(provide 'FEATURE)` *must match the file name* (e.g., `lisp/core.el` → `(provide 'core)`).
- 📚 Rely *only on official documentation*. Do *not* use private/undocumented APIs or speculative future behavior.
- 🌿 In every `leaf` form, group settings under *`:straight` / `:bind` / `:hook` / `:custom`* (in this order).
- 📦 For built-in packages, *always* specify *`:straight nil`*.
- 🧠 *Emacs 31+ Compatibility Policy*
  - This configuration targets *Emacs 30+* and proactively follows
    compatibility guidance for *Emacs 31 and later*.
  - Obsolete macros and APIs are avoided even if they still work.
    In particular:
    - Use `if-let*`, `when-let*`, or `and-let*` instead of deprecated `if-let` / `when-let`.
  - Warnings emitted by newer Emacs versions are treated as *actionable signals*, not ignored noise.
  - Code is written to remain warning-free under `byte-compile-warnings` on the latest stable Emacs.
  - See also: [[#modular-loader-and-core-suite][Modular Loader & Core/Utils design]] for how this policy is applied in practice.

*** ✅ Per-module Checklist
- [ ] File header includes `-*- lexical-binding: t; -*-`
- [ ] `(provide '…)` matches the file’s feature name
- [ ] `leaf` keywords ordered: `:straight` → `:bind` → `:hook` → `:custom`
...
|Property|Value|
|--------|-----|
|Commit|4473b117973f561b8b7a33d6adeabaafe0bac7b3|
|Branch|master|
|System|aarch64-apple-darwin25.1.0|
|Date|2025-12-31 12:39:46 (JST)|
...
|Options|--with-ns --enable-mac-app=yes --with-native-compilation --with-json --with-tree-sitter --with-imagemagick --with-gnutls --prefix=/Users/ac1965/.local CPPFLAGS=-I/opt/homebrew/opt/llvm/include 'LDFLAGS=-L/opt/homebrew/opt/llvm/lib -L/opt/homebrew/opt/llvm/lib/c++ -Wl,-rpath,/opt/homebrew/opt/llvm/lib/c++'|
...
- *Forward-compatible*: proactively follows *Emacs 31+ compatibility guidance*
  (see [[#conventions][Coding Rules]]), avoiding obsolete macros/APIs even when
...
- *Switches UI/LSP stack* centrally (=core/switches.el=):
  - UI: =none | doom | nano=
  - LSP: =eglot | lsp=
- *Hardens daily UX* with focused cores and utilities:
  ...
  - **Operational utilities** for long-running sessions:
    GC strategy, buffer/process housekeeping, and LSP lifecycle cleanup.
- *UI surface*:
...
- *Operational helpers*: the =utils/= layer provides cross-cutting helpers for
  long-running sessions (GC strategy, buffer/process cleanup, LSP lifecycle).
- *Opt-in and non-intrusive*: utilities must be safe to disable and never impose
  UI, logging, or personal preferences.
- *Future-proof*: implementations proactively follow *Emacs 31+ compatibility
  guidance* and avoid obsolete macros/APIs.
...
| =core/utils.el=                  | Auto-tangle updated Org, insert lexical-binding headers, view-mode on read-only buffers.          |
...
| =utils/utils-async.el=           | Safe async runner with error trapping.                                                            |
| =utils/utils-scratch.el=         | Scratch buffer auto-recreation.                                                                   |
| =utils/search-nav.el=            | ripgrep via Consult; dumb-jump Xref backend; EWW helpers + image toggle.                          |
| =utils/utils-gc.el=              | **Long-running GC strategy** (focus-out/minibuffer GC; no UI/logging).                            |
| =utils/utils-buffers.el=         | **Buffer & process housekeeping** for long-lived sessions.                                       |
| =utils/utils-lsp.el=             | **Eglot lifecycle helpers** (project switch cleanup).                                             |
| =utils/utils-notes-markdown.el=  | Prose-oriented Markdown notes; consult-notes, rg, org-download glue.                              |
...
#+begin_src emacs-lisp
  ...
  (leaf utils-gc
    ...
    (defun utils-gc--safe-collect ()
      "Run garbage collection safely."
      (when utils-gc-enable-p
        (garbage-collect)))

    ;; Run GC at safe moments
    (add-hook 'focus-out-hook #'utils-gc--safe-collect)
    (add-hook 'minibuffer-exit-hook #'utils-gc--safe-collect))
...
#+begin_src emacs-lisp
  ...
  ;; Includes long-running session guards (GC / memory / buffers / LSP).
  ;;
  ;;; Code:
  ...
  ;;; ---------------------------------------------------------------------------
  ;;; Long-running session guard (GC / Memory / Buffers / LSP)
  ;;; ---------------------------------------------------------------------------

  ;;;; GC / Memory visualization ----------------------------------------------

  (defun my/gc-memory-status ()
    "Display current GC and memory-related status."
    (interactive)
    (message
     "GC:%d | threshold:%.1fMB | heap:%.1fMB | buffers:%d | processes:%d"
     gcs-done
     (/ gc-cons-threshold 1048576.0)
     (/ (memory-limit) 1048576.0)
     (length (buffer-list))
     (length (process-list))))

  (defun my/gc-diff ()
    "Run GC and report freed memory."
    (interactive)
    (let ((before (memory-limit)))
      (garbage-collect)
      (message "GC freed %.1fMB"
               (/ (- before (memory-limit)) 1048576.0))))

  (defun my/system-health-check ()
    "Run a simple Emacs health check."
    (interactive)
    (my/gc-memory-status)
    (my/gc-diff))
  ...
  ;;;; Buffer / process housekeeping ------------------------------------------

  (defvar my/housekeeping-buffer-regexp
    (rx string-start "*"
        (or "Help" "Warnings" "Compile-Log" "Backtrace"
            "Async-native-compile-log" "eglot-events")
        (* any) string-end))

  (defun my/temporary-buffer-p (buffer)
    (with-current-buffer buffer
      (and (string-match-p my/housekeeping-buffer-regexp (buffer-name))
           (not (buffer-file-name))
           (not (buffer-modified-p))
           (not (get-buffer-window buffer 'visible)))))

  (defun my/dead-process-buffer-p (buffer)
    (let ((proc (get-buffer-process buffer)))
      (and proc (memq (process-status proc) '(exit signal)))))

  (defun my/cleanup-buffers ()
    "Clean up unused temporary and dead process buffers."
    (interactive)
    (let ((killed 0))
      (dolist (buf (buffer-list))
        (when (or (my/temporary-buffer-p buf)
                  (my/dead-process-buffer-p buf))
          (kill-buffer buf)
          (setq killed (1+ killed))))
      (when (> killed 0)
        (message "Housekeeping: killed %d buffers" killed))))

  (run-with-timer 900 900 #'my/cleanup-buffers)

  ;;;; Idle lightweight refresh -----------------------------------------------

  (defvar my/idle-refresh-delay (* 60 30))

  (defun my/idle-light-refresh ()
    "Lightweight refresh during long idle."
    (garbage-collect)
    (clear-image-cache)
    (my/cleanup-buffers))

  (run-with-idle-timer my/idle-refresh-delay t #'my/idle-light-refresh)

  ;;;; Mode-line indicators ---------------------------------------------------

  (defvar my/mode-line-health-string "")
  (defvar my/mode-line-eglot-string "")

  (defun my/update-mode-line-health ()
    (setq my/mode-line-health-string
          (format " B:%d P:%d"
                  (length (buffer-list))
                  (length (process-list))))
    (force-mode-line-update))

  (add-hook 'buffer-list-update-hook #'my/update-mode-line-health)
  (add-hook 'process-status-change-hook #'my/update-mode-line-health)

  (setq global-mode-string
        (append global-mode-string '(my/mode-line-health-string)))

  ;;;; Eglot workspace count --------------------------------------------------

  (defun my/eglot-workspace-count ()
    (when (and (featurep 'eglot) (boundp 'eglot--servers))
      (length eglot--servers)))

  (defun my/update-eglot-workspace ()
    (setq my/mode-line-eglot-string
          (if-let* ((n (my/eglot-workspace-count)))
              (format " LSP:%d" n)
            ""))
    (force-mode-line-update))

  (with-eval-after-load 'eglot
    (add-hook 'eglot-managed-mode-hook #'my/update-eglot-workspace)
    (add-hook 'eglot-server-stopped-hook #'my/update-eglot-workspace)
    (my/update-eglot-workspace)
    (setq global-mode-string
          (append global-mode-string '(my/mode-line-eglot-string))))
  ...
  (defun my/project-switch-cleanup ()
    (let ((new-root (my/project-root)))
      (when (and my/current-project-root new-root
                 (not (string-equal my/current-project-root new-root)))
        ;; Shutdown eglot
        (when (featurep 'eglot)
          (dolist (server eglot--servers)
            (when (string-prefix-p my/current-project-root
                                   (eglot--project-root (cdr server)))
              (ignore-errors (eglot-shutdown (cdr server))))))
        ;; Kill non-file buffers
        (dolist (buf (buffer-list))
          (with-current-buffer buf
            (when (and default-directory
                       (string-prefix-p my/current-project-root default-directory)
                       (not (buffer-file-name))
                       (not (buffer-modified-p)))
              (kill-buffer buf)))))
      (setq my/current-project-root new-root)))

  (add-hook 'find-file-hook #'my/project-switch-cleanup)
  (add-hook 'project-switch-project-hook #'my/project-switch-cleanup)

  ;;;; Memory logging / alert / lightweight restart ----------------------------

  (defvar my/memory-log-file
    (expand-file-name "memory.log" user-emacs-directory))

  (defun my/log-memory-usage ()
    (with-temp-buffer
      (insert
       (format "%s GC:%d Buffers:%d Processes:%d\n"
               (format-time-string "%Y-%m-%d %H:%M:%S")
               gcs-done
               (length (buffer-list))
               (length (process-list))))
      (append-to-file (point-min) (point-max) my/memory-log-file)))

  (run-with-timer 600 600 #'my/log-memory-usage)

  (defvar my/memory-alert-buffer-threshold 350)
  (defvar my/memory-alert-process-threshold 8)
  (defvar my/memory-alert-last-gc-freed 0)

  (defun my/update-last-gc-freed ()
    (let ((before (memory-limit)))
      (garbage-collect)
      (setq my/memory-alert-last-gc-freed
            (- before (memory-limit)))))

  (defun my/memory-health-alert ()
    (when (or (> (length (buffer-list)) my/memory-alert-buffer-threshold)
              (> (length (process-list)) my/memory-alert-process-threshold)
              (< my/memory-alert-last-gc-freed (* 5 1024 1024)))
      (message "⚠ Emacs memory risk detected")))

  (defun my/lightweight-restart ()
    "Safe lightweight refresh."
    (interactive)
    (when (featurep 'eglot)
      (dolist (server eglot--servers)
        (ignore-errors (eglot-shutdown (cdr server)))))
    ...
    (garbage-collect)
    (clear-image-cache)
    (message "Lightweight restart completed"))

  (run-with-timer 600 600
                  (lambda ()
                    (my/update-last-gc-freed)
                    (my/memory-health-alert)))

  ;;; ---------------------------------------------------------------------------
  ;;; Original macOS / device / Apple Music / meow sections (unchanged)
  ;;; ---------------------------------------------------------------------------
  ...
  If CALLBACK is non-nil, call it with the trimmed output."
    ...
    (defun my/apple-music-current-track-info ()
      "Show current track info as: Title — Artist [Album]."
      (interactive)
      (let ((info
