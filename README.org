...
#+begin_src emacs-lisp :tangle lisp/dev/dev-ai.el
  ...
  (with-eval-after-load 'aidermacs
    (let* ((bin (or (executable-find "aider-ce")
                    (executable-find "aider")
                    (executable-find "aider-chat"))))
      (unless bin
        (user-error "[aidermacs] No aider CLI found. Install via Homebrew/pipx, or add it to PATH."))
      (dolist (sym '(aidermacs-executable aidermacs-command aidermacs-program))
        (when (boundp sym)
          (set sym bin)))
      (setenv "AIDER_BIN" bin)))
