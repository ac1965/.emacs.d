...
#+begin_src emacs-lisp :tangle lisp/ui/ui-font.el
  ...
  (defun my/system-emoji-font ()
    "Set a color emoji font for unicode emoji ranges."
    (interactive)
    (let* ((emoji-font
            (cond
             ((eq system-type 'darwin) "Apple Color Emoji")
             ((eq system-type 'gnu/linux)
              (or (car (seq-filter (lambda (f) (member f (font-family-list)))
                                   '("Noto Color Emoji" "JoyPixels" "Twitter Color Emoji")))
                  "Noto Color Emoji"))
             ((eq system-type 'windows-nt) "Segoe UI Emoji")
             (t nil))))
      (when (and emoji-font (member emoji-font (font-family-list)))
