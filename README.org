...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; $Lastupdate: 2025/09/27 07:44:53 $
...
#+begin_src emacs-lisp :tangle lisp/lsp.el
  ...
  (leaf eglot
    ...
    :commands (eglot eglot-ensure) ; autoload only these entry points
    :hook ((prog-mode . (lambda ()
                          ;; Enable only when server contact can be guessed
                          ;; -> prevents useless eglot startup in unsupported modes
                          (when (eglot--guess-contact) (eglot-ensure)))))
    :custom
    ((eglot-autoreconnect . t))) ; reconnect if server dies
  ...
  (leaf lsp-mode
    ...
    :custom
    ((lsp-keymap-prefix . "C-c l"))) ; consistent keybinding prefix
