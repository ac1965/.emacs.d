...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This file:
  ;; - loads org-roam-ui after org-roam is available
  ;; - sets sync-theme, follow, and update-on-save via setopt in :config
  ;; - opens the UI automatically via a one-shot idle timer (8 s) so that
  ;;   org-roam autosync (orgx-extensions, 5 s timer) has already fired
  ;;
  ;; Load path: appended to `my:modules-extra' by personal/user.el.
  ;; This module must not appear in the canonical `my:modules' list; it is
  ;; a personal presentation choice.
  ;;
  ;;; Code:
