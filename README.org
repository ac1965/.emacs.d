...
#+begin_src emacs-lisp
  ...
  ;; - sets font: JetBrains Mono (mono), Noto Sans JP (variable),
  ;;   Apple Color Emoji (emoji), size 18
  ...
  ;; ui-font-emoji: explicit declaration overrides the auto-detect fallback
  ;; in ui-font--system-emoji so that the emoji fontset is always set to
  ;; "Apple Color Emoji" on this machine, regardless of system-type detection.
  ;; ui-font-apply calls (set-fontset-font t 'emoji (font-spec :family ...)),
  ;; which registers the family for the 'emoji Unicode script range.
