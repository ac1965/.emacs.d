...
This pattern applies when the module must not be loaded unconditionally at startup
(LSP backend selection, optional UI bundles).

| Module             | Declared in     | Trigger                                 |
|--------------------+-----------------+-----------------------------------------|
| =dev-lsp-eglot=    | =core-switches= | =(my:use-lsp 'eglot)= after-init        |
| =dev-lsp-mode=     | =core-switches= | =(my:use-lsp 'lsp)= after-init          |
| =ui-doom-modeline= | =core-switches= | =(my:use-ui 'doom)= after-init          |
| =ui-nano-modeline= | =core-switches= | =(my:use-ui 'nano)= after-init          |
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; [Fix BW] my:modules-extra is declared `defcustom' in modules.el.
  ;; Coding Rule 6 requires `setopt' for defcustom variables to respect the
  ;; :set hook and type validator.  The previous `setq' bypassed both.
  ;; `setopt' evaluates its value argument, so the (delete-dups (append ...))
  ;; expression is valid here — it is not quoted as it would be in :custom.
  (setopt my:modules-extra
          (delete-dups
...
*** Fix BI: README.org — :comments header-arg corrected to :comments link

🟠 Medium — Coding Rule violation (global header-args)

- *[Fix BI] README.org (L9, #+PROPERTY: header-args:emacs-lisp)*: The global
  property line specified =:comments no=.  The system prompt Coding Rules and
  the header-args specification both require =:comments link=.

  With =:comments no=, =org-babel-tangle= omits the Org heading breadcrumb
  lines (=;; [[file:README.org::*<heading>]]=) from every generated =.el= file.
  Those breadcrumbs enable =M-x find-function= and =M-x xref-find-definitions=
  to navigate from a symbol back to its source block in =README.org=.  Without
  them, the round-trip from compiled symbol to literate source is broken.

  With =:comments link=, each tangled code section is preceded by a comment
  of the form:

  #+begin_example
  ;; [[file:../../README.org::*core/core-policy.el][core/core-policy.el:1]]
  #+end_example

  This satisfies the system prompt requirement and restores full navigability.
  Changed =:comments no= → =:comments link= on line 9.

*** Fix BJ: Module Structure — document autoload-only module pattern

🟡 Minor — documentation gap

- *[Fix BJ] * Overview > ** Module Structure (new *** Autoload-Only Modules subsection)*:
  Six tangle targets (=dev-lsp-eglot=, =dev-lsp-mode=, =ui-doom-modeline=,
  =ui-nano-modeline=, =ui-nano-palette=, =core-custom-ui-extras=) are absent
  from =my:modules= by design: they are loaded on demand via =autoload=
  declarations in =core-switches.el= and =ui-theme.el=.

  The architecture documentation did not acknowledge this pattern, creating an
  apparent contradiction with the rule that all modules appear in =my:modules=.
  A new =*** Autoload-Only Modules= subsection was added to =** Module Structure=
  with a table mapping each autoload-only module to its declaring file and
  trigger condition.  No code was changed; this is a documentation-only addition.

*** Fix BK: personal/user.el — setq → setopt for defcustom my:modules-extra

🟠 Medium — Coding Rule 6 violation

- *[Fix BK] personal/user.el (my:modules-extra assignment)*: =my:modules-extra=
  is declared =defcustom= in =modules.el= (see =defcustom my:modules-extra nil=,
  type ='(repeat symbol)=, group ='my:modules=).  Coding Rule 6 mandates =setopt=
  for all =defcustom= variables to invoke the =:set= hook and type validator.

  The previous code used =setq=:

  #+begin_example
  (setq my:modules-extra
        (delete-dups (append my:modules-extra '(ui-visual-aids ...))))
  #+end_example

  This bypassed the type validator (=:type '(repeat symbol)=), meaning a typo
  producing a non-symbol element would be silently accepted and later cause a
  confusing =wrong-type-argument= in =my/modules-load=.

  Replaced with =setopt=:
  ...
  =setopt= evaluates its value argument before assignment, so the
  =(delete-dups (append ...))= expression is evaluated correctly — it is not
  quoted as it would be in =leaf :custom=.  A clarifying comment citing Rule 6
  was added above the form.

*** Fix BY: * Changelog — renumber duplicate Fix IDs BO and BR

🟡 Minor — changelog integrity

- *[Fix BY] * Changelog (Fix ID sequence)*: Two Fix IDs each appeared twice,
  breaking the monotonic sequence required for unambiguous cross-reference.

  - *Fix BO* was assigned to two separate entries:
    1. =2026-04 (thirty-fourth pass)= — tangle-workflow cookie warnings
    2. =2026-05 (thirty-eighth pass)= — ob-mermaid Babel integration

    The second entry is renumbered *Fix BO2*.  The heading and the
    =*[Fix BO]...= body bullet were updated.

  - *Fix BR* was assigned to two separate entries:
    1. =2026-05 (thirty-seventh pass)= — keybinding overhaul
    2. =2026-05 (forty-first pass)= — Japanese → English prose translation

    The second entry is renumbered *Fix BS*.  The heading and the
    =*[Fix BR]...= body bullet were updated.

  No code was changed.  All =:CUSTOM_ID:= and cross-reference links were
  unaffected because neither BO nor BR appeared as anchor targets elsewhere.
