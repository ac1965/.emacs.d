...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; Theme configuration for the UI layer.
  ;;
  ;; Responsibilities:
  ;;
  ;; - load the Nano theme
  ...
  ;; - configure subtree and pairing highlights
  ;; - apply spacing adjustments for graphical displays
  ;;
  ;; The initialization is idempotent and safe to run multiple times.
  ;; It also supports daemon mode and new frame creation.
  ...
  (defun ui-theme--reapply-after-theme (&rest _)
    "Reapply Nano palette adjustments after a theme change."
    (when (fboundp 'my/nano-apply-faces)
      (my/nano-apply-faces)))

  (defun ui-theme--apply-frame (frame)
    "Apply graphical UI adjustments to FRAME."
  ...
  This function ensures initialization runs only once and
  installs hooks that keep theme customizations stable
  across theme reloads and frame creation."
    (unless ui-theme--initialized
      (setq ui-theme--initialized t)

      ;; Reapply Nano palette when themes change
      (with-eval-after-load 'cus-theme
        (if (boundp 'enable-theme-functions)
            (add-hook 'enable-theme-functions #'ui-theme--reapply-after-theme)
          (advice-add 'enable-theme :after #'ui-theme--reapply-after-theme)))
      ...
      ;; Ensure standard emphasis behaviour
      (set-face-attribute 'bold nil :weight 'bold)
      (set-face-attribute 'italic nil :slant 'italic)

      ;; Configure pairing highlight
      (with-eval-after-load 'elec-pair
        (custom-set-faces
         '(electric-pair-overlay-face
           ((t (:background "#e5e7eb"))))
         '(show-paren-match
           ((t (:background "#e5e7eb" :foreground "#111827" :weight bold))))
         '(show-paren-mismatch
           ((t (:background "#dc2626" :foreground "white" :weight bold))))))

      ;; Apply graphical settings for existing frame
      (ui-theme--apply-frame (selected-frame))

      ;; Ensure daemon / new frame support
