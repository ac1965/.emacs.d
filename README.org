...
#+begin_src emacs-lisp
  ...
  ;; $Lastupdate: 2025/04/27 14:18:28 $
...
#+begin_src emacs-lisp
  (leaf emigo
    ...
    :commands (emigo-enable emigo-chat)
    :custom
    ((emigo-model . "openrouter/qwen/qwen2.5-vl-3b-instruct:free")
     (emigo-base-url . "https://openrouter.ai/api/v1")
     (emigo-api-key . (emigo-read-file-content "~/.config/openrouter/key.txt")))
    :init
    (unless (and (boundp 'emigo-api-key) emigo-api-key (not (string= emigo-api-key "")))
      (message "[Emigo Warning] API key not found or empty!"))
    :config
    (emigo-enable))

  (defun my/emigo-chat-popup ()
    "Launch Emigo chat in a popup buffer."
    (interactive)
    (let ((buf (get-buffer-create "*Emigo Chat*")))
      (pop-to-buffer buf)
      (unless (derived-mode-p 'emigo-chat-mode)
        (emigo-chat-mode)
        (emigo-enable))))

  (defun my/emigo-save-chat-to-org ()
    "Save Emigo Chat buffer content to an Org file with tags."
    (interactive)
    (let* ((chat-content (buffer-string))
           (default-tags ":chat:emigo:")
           (default-category "Emigo-Chat")
           (org-file (expand-file-name (format-time-string "emigo-chat-%Y%m%d.org") "~/org/emigo-chats/")))
      (unless (file-exists-p (file-name-directory org-file))
      ...
      (with-current-buffer (find-file-noselect org-file)
        ...
        (insert (format "* %s (%s) %s\n\n%s\n\n"
                        ...
                        chat-content))
        (save-buffer))
      (message "Chat saved with tags to %s" org-file)))
