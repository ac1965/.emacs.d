...
|Property|Value|
|--------|-----|
|Commit|baa52744457222ebe621b11eb997a5fc4bd67293|
|Branch|master|
|System|aarch64-apple-darwin25.4.0|
|Date|2026-04-23 22:14:54 (JST)|
...
#+begin_src emacs-lisp
  ...
  ;; exec-path-from-shell: import login-shell environment into Emacs.
  ;; Active on macOS (GUI/daemon) and on Linux GUI/daemon sessions where
  ;; Emacs is launched outside a login shell (e.g. from a desktop launcher
  ;; or via `systemctl --user start emacs`).
  ;; On Linux the arguments list is left empty ('()) because bash/zsh sourced
  ;; as non-interactive non-login shells still inherit the correct PATH;
  ;; passing "-l" on Linux triggers .bash_profile which is both slow and
  ;; unreliable across distributions.
  (leaf exec-path-from-shell
    :straight t
    :when (or (and (eq system-type 'darwin)
...
#+begin_src emacs-lisp
  ...
  (leaf dirvish
    ...
    (dirvish-peek-mode 1)
    ;; ls switch configuration — portable across macOS and Linux.
    ;; Priority: gls (GNU coreutils) > system ls with GNU flags > BSD ls fallback.
    ;; On macOS, GNU coreutils `gls` is installed via Homebrew; on Linux the
    ;; system ls is already GNU ls and supports --group-directories-first.
    ...
    (let ((gls (or (executable-find "gls")
                   (and (eq system-type 'gnu/linux) (executable-find "ls")))))
      (if (and gls (eq 0 (call-process gls nil nil nil "--version")))
          (progn
...
On Linux, prefers JetBrains Mono → Hack → DejaVu Sans Mono → Monospace
so the fallback chain works on distributions where only generic aliases
are guaranteed.  On macOS Menlo is always present."
    (cond ((eq system-type 'darwin)     "Menlo")
          ((eq system-type 'windows-nt) "Consolas")
          (t
           ;; Linux / other: probe available fonts in preference order.
           (let ((candidates '("JetBrains Mono" "Hack" "DejaVu Sans Mono"
                               "Source Code Pro" "Noto Mono" "Monospace")))
             (or (seq-find (lambda (f)
                             (ignore-errors
                               (x-list-fonts (concat f ":size=12") nil nil 1)))
                           candidates)
                 "Monospace")))))

  (defun ui-font--system-emoji ()
    "Return the default emoji font family for the current OS.
On Linux, prefers Noto Color Emoji → Twemoji → Segoe UI Emoji (Wine/Crossover)."
    (cond ((eq system-type 'darwin)     "Apple Color Emoji")
          ((eq system-type 'windows-nt) "Segoe UI Emoji")
          (t
           (let ((candidates '("Noto Color Emoji" "Twemoji" "Segoe UI Emoji"
                               "EmojiOne Color" "Noto Emoji")))
             (or (seq-find (lambda (f)
                             (ignore-errors
                               (x-list-fonts (concat f ":size=12") nil nil 1)))
                           candidates)
...
#+begin_src emacs-lisp
  ...
  (leaf apheleia
    ...
    :require t
    :config
    ;; Register formatters only when the external executable is available.
    ;; `prettierd' (daemon mode) is preferred over `npx prettier' for speed;
    ;; fall back to npx when prettierd is absent.  Both are registered so that
    ;; mode-alist entries remain valid regardless of which is installed.
    ...
    (let ((fmt (if (executable-find "prettierd") 'prettierd 'prettier)))
      (dolist (pair `((typescript-ts-mode . ,fmt) (tsx-ts-mode . ,fmt)
                      (json-ts-mode . ,fmt) (css-ts-mode . ,fmt)
                      (markdown-mode . ,fmt)))
...
** 2026-05 (thirty-sixth evaluation pass)
*** Fix BQ: Six portability and mobility refactors (cross-platform / environment-agnostic)

🟠 Medium — portability / platform-independence

- *[Fix BQ-1] =init.el= (=leaf exec-path-from-shell=)*: Extended the =:when=
  condition to activate on Linux GUI and daemon sessions in addition to
  macOS.  Previously, only macOS sessions imported the login-shell
  environment.  On Linux, Emacs launched from a desktop session or
  =systemctl --user= does not inherit =PATH= from the login shell;
  =exec-path-from-shell= corrects this.  The =:arguments= list is
  conditionally set to ='("-l")= on macOS and ='()= on Linux: sourcing a
  login shell on Linux is slower and unreliable across distributions,
  whereas the shell inherits the correct environment as a non-login
  interactive shell.

- *[Fix BQ-2] =core/core-editing.el= (=leaf dirvish :config= — ls setup)*:
  Replaced the macOS-specific =gls= probe with a portable GNU ls detection
  strategy.  The new logic uses =when-let*= to find =gls= first, then falls
  back to the system =ls= on Linux (which is already GNU ls and supports
  =--group-directories-first=).  Availability of the GNU flag is confirmed by
  =call-process ls --version= so the flag is never applied to BSD ls.  This
  makes =--group-directories-first= available on Linux without extra packages,
  and preserves the BSD ls fallback on macOS without Homebrew coreutils.

- *[Fix BQ-3] =orgx/orgx-export.el= (=leaf ob-mermaid :config=)*: Replaced
  the two sequential =(executable-find "mmdc")= calls with a single
  =when-let*= binding.  =executable-find= hits the filesystem each call;
  caching the result eliminates the redundant lookup.

- *[Fix BQ-4] =dev/dev-format.el= (=leaf apheleia :config=)*: Made
  =prettierd= registration conditional on its availability via
  =executable-find=.  Previously the formatter was registered
  unconditionally; on systems without =prettierd= the mode-alist entries
  silently pointed to a non-existent formatter.  The fix selects =prettierd=
  when available, otherwise falls back to =prettier= (via =npx=), and
  registers mode-alist entries for whichever is present.  Both formatters
  remain registered so that personal-overlay overrides can reference either.

- *[Fix BQ-5] =ui/ui-font.el= (=ui-font--system-default=)*: Replaced the
  single =gnu/linux= → ="Monospace"= arm with a runtime font-probe loop that
  tries candidates in preference order: JetBrains Mono → Hack → DejaVu Sans
  Mono → Source Code Pro → Noto Mono → Monospace.  =x-list-fonts= is called
  with a result limit of 1 to keep the probe cheap.  The =t= catch-all covers
  Linux and any other non-macOS, non-Windows platform.

- *[Fix BQ-6] =ui/ui-font.el= (=ui-font--system-emoji=)*: Applied the same
  runtime probe pattern to the emoji font fallback: Noto Color Emoji →
  Twemoji → Segoe UI Emoji → EmojiOne Color → Noto Emoji.  On Ubuntu the
  default is =fonts-noto-color-emoji=; on Fedora/Arch it varies.  The probe
  finds whichever is installed rather than hardcoding a single name.
