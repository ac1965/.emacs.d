...
- *Language Server Protocol* - Switchable LSP backend (Eglot/LSP-Mode via ~my:use-lsp~) with automatic server management
...
| ui/           | ui-modeline.el        | Modeline, minions, time, battery                            |
...
| orgx/         | org-extensions.el     | Journal, Roam, download, toc-org, cliplink                  |
...
| utils/        | search-nav.el         | Dumb-jump fallback, multiple-cursors                        |
...
- **Optional but Recommended**
  ...
  - (*Note*) Homebrew libgccjit / `LIBRARY_PATH` is auto-assisted in *early-init* on macOS.
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; License: GNU GPL v3 or later
  ;;
  ;; $Lastupdate: 2025/10/10 00:40:02 $
  ;;
  ;;; Commentary:
  ...
  When called interactively, print a compact summary."
    ...
    (let ((result (my/ensure-straight-packages my:bootstrap-packages quiet)))
      ...
      (when noninteractive (ignore result))
      result))

  ;; Root dir for this config (prefer the file's directory)
  ...
  ;; straight.el: keep everything inside .var/
  (setopt straight-base-dir my:d:var
          straight-use-package-by-default t
          straight-profiles '((nil . "default.el")))
...
**What it does**
- Keeps your original theme setup (incl. `(ef-themes-to-toggle . '(ef-frost ef-spring))`).
- Adds `ui--normalize-faces` to set problematic face attributes to `'unspecified` instead of `nil`.
- Hooks the normalizer to theme enabling and runs it once at init.
...
#+begin_src emacs-lisp :tangle lisp/ui/ui-theme.el
  ...
  (leaf ef-themes :straight t
    ;; ef-themes toggle pair
    :custom
    ((modus-themes-to-toggle . '(ef-frost ef-spring)))
    :config
  ...
  (defun ui--normalize-faces (&rest _)
  ...
  ;; Run after any theme is enabled (29+)
  (add-hook 'enable-theme-functions #'ui--normalize-faces)
...
#+begin_src emacs-lisp :tangle lisp/ui/ui-modeline.el
  ...
  ;; ---- Try nano-modeline; fallback to doom-modeline if unavailable ----
  (when (require 'nano-modeline nil t)
    (setopt nano-modeline-padding '(0.20 . 0.25))
    (when (fboundp 'nano-modeline-mode) (nano-modeline-mode 1))
