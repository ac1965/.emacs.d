...
|---------------+----------------+------------------|
| early-init.el | Earliest startup tuning (GC, handlers, frame defaults) | Package setup, hooks, timers |
| init.el | Bootstrap (straight.el, leaf) and load orchestration | Feature logic, UI, personal settings |
...
| lisp/core/startup.el | Startup/restore GC & handlers | Package-specific behavior |
