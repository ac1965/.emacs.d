...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This file:
  ;; - owns the job registry hash-table and idle/periodic timer handles
  ;; - normalises job metadata (name, fn, lane, enabled flag)
  ;; - runs jobs dispatched via `core-session--start` on startup
  ;;
  ;; NOTE: This module is an internal detail of core-session.el.
  ;; Do not require or call its symbols from outside the core layer.
  ;;; Code:
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This file:
  ;; - `my/dired-get-file-or-dir`: visits a directory with `dired-find-file`
  ;;   or a file with `find-file`, bound to RET in `dired-mode-map`
  ;;; Code:
