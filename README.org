...
#+begin_src emacs-lisp
  ...
  ;; - extends Dired with dirvish (replaces dired-filter and dired-subtree)
...
:END:
#+begin_src emacs-lisp
  ;;; utils/utils-dired.el --- Dired/Dirvish utilities -*- lexical-binding: t; -*-
  ;;
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ...
  ;; Commentary:
  ;;
  ;; Dired/Dirvish utility: smart RET for file-or-directory navigation.
  ;;
  ;; This file:
  ;; - `my/dired-get-file-or-dir`: visits a directory with `dirvish` (if active)
  ;;   or `dired-find-file`, opens a file with `find-file`, bound to RET
  ;;   in `dired-mode-map`
  ;;; Code:
...
*** Refactoring: dired-filter → dirvish (今回)

*🟠 Medium — パッケージ置換*

- *[Fix D] core-editing.el*: =dired-filter= / =dired-subtree= の2パッケージを
  =dirvish= 1パッケージに統合。=leaf dirvish= に =:global-minor-mode dirvish-override-dired-mode=
  を追加し、=dirvish-attributes= で =subtree-state= / =vc-state= / =git-msg= 等の
  アトリビュートを有効化。=dirvish-peek-mode= を有効化し、プレビュー機能を付与。
  =ls= スイッチ設定は =leaf dirvish= の =:config= ブロック内に集約。

- *[Fix D] ui-keymap.el*: =dired-mode-map= の =dired-subtree-toggle= /
  =dired-subtree-insert= キーバインドを削除。=dirvish-mode-map= に対して
  =dirvish-subtree-toggle= (TAB / i) と =dirvish-quit= (q) を設定。

- *[Fix D] ui-nano-palette.el*: =dired-filter-group-header= 等の
  =dired-filter= 専用 face 定義 4個を =dirvish= の対応 face
  (=dirvish-hl-line= / =dirvish-git-commit-message-face= /
  =dirvish-header-line-path-face= / =dirvish-subtree-state-face=) に置換。

- *[Fix D] utils-dired.el*: =my/dired-get-file-or-dir= のディレクトリ訪問を
  =(fboundp 'dirvish)= でガードした上で =dirvish= 呼び出しに変更。
  =dirvish= が未ロードの場合は従来の =dired-find-file= へフォールバック。
