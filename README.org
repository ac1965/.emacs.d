...
#+begin_src emacs-lisp
  ...
  ;; $Lastupdate: 2025/01/11 13:48:42 $
...
#+begin_src emacs-lisp
  ;; Functions to save and restore window layout configurations.
  (defvar my/saved-window-config nil
    "Stores the current window configuration for restoration.")
  ...
  (defun my/restore-window-layout ()
    "Restore the saved window configuration from `my/saved-window-config`.
  ...
  ;; Function to reuse the currently selected window when displaying a buffer.
  ...
  ;; Function to split the window below from the parent or root window.
  ...
  ;; Function to toggle the dedicated status of the selected window.
  ...
  (defun my/restore-desktop-session ()
    "Restore a previously saved desktop session using `desktop-read`."
    (interactive)
    (desktop-read)
  ...
  (defun my/delete-old-backups ()
    "Delete backup files from `no-littering-var-directory/backup/` older than 7 days."
        ...
        (dolist (file (directory-files backup-dir t))
          (when (and (file-regular-p file)
                     (> (- (float-time (current-time))
                        ...
                        (* 7 24 60 60))) ;; Check if older than 7 days
            (delete-file file))))))
  (add-hook 'emacs-startup-hook #'my/delete-old-backups)

  ;; Automatically enable or disable `view-mode` based on buffer's read-only status.
  (defun my/enable-view-mode-on-read-only ()
    "Enable `view-mode` when the buffer is read-only, disable otherwise."
    (if buffer-read-only
        (view-mode 1)
      (view-mode -1)))
  ...
  ;; Function to open the current file in Visual Studio Code at the current line and column.
  ...
  ;; Function to display detailed information about the current Emacs build.
  (defun my/print-build-info ()
    "Print detailed information about the current Emacs build in a new buffer."
    (interactive)
  ...
  ;; Generate a tabulated list of keybindings, sorted by key sequence and command name.
  (defun my/generate-keybinding-table ()
    "Generate a table of all keybindings sorted by key sequence and command name."
    (interactive)
    (let ((bindings '()))
      ;; Collect keybindings from all keymaps
      (mapatoms
       (lambda (sym)
         (when (commandp sym)
      ...
      ;; Display keybindings in a tabulated list
  ...
  ;; Remove duplicates from the kill ring before adding a new entry.
  ...
  ;; Hook to clean up trailing whitespace before saving a file.
