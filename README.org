...
#+begin_src emacs-lisp
  ...
  ;; the helper functions they depend on are not defined anywhere in this
  ;; configuration.  The dead code was causing confusion without providing
  ;; any benefit.  If these helpers are added in a future layer, restore
  ;; the advice block here.
  ;;
  ;; Deduplication note (see also core-tools.el):
  ;; `core-fixes--emacs>=` is now a thin alias pointing to `core-tools-emacs>=`.
  ;; The implementation lives in core-tools.el, which is loaded first in the
  ;; module sequence.  This eliminates the duplicate definition that previously
  ;; existed between these two modules.
  ;;
  ;;; Code:

  ;; [R9 FIX] core-fixes--emacs>= was identical to core-tools-emacs>=.
  ;; Provide it as an alias so any internal use sites continue to compile.
  ;; core-tools must be loaded before core-fixes (guaranteed by my:modules order).
  (defalias 'core-fixes--emacs>= #'core-tools-emacs>=)
