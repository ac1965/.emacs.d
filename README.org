...
#+begin_src emacs-lisp :tangle lisp/utils/utils-scratch.el
  ...
  (defun my/kill-scratch-buffer-advice (buf)
    "Recreate *scratch* if BUF is *scratch* and `my:scratch-auto-recreate` is non-nil."
...
#+begin_src emacs-lisp :tangle lisp/utils/utils-meow-readonly.el
  ...
  (defun my/meow-apply-state-for-readonly ()
    ...
    (when (featurep 'meow)
      (if (my/meow--readonly-context-p)
          ;; In viewing contexts, use motion state (SPC should scroll).
          (meow-motion-mode 1)
        ;; Leave motion state if it is on; go back to normal.
  ...
  (with-eval-after-load 'meow
    ...
    (meow-motion-overwrite-define-key
     ...
     ;; Give alternative access to leader/keypad while viewing:
     '(","          . meow-keypad)
     '("M-SPC"      . meow-keypad)
     ;; Quality-of-life: q to quit-window in viewing contexts.
     '("q"          . quit-window)))
