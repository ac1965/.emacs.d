...
#+begin_src emacs-lisp
  ...
  ;; $Lastupdate: 2025/01/04 16:32:25 $
...
#+begin_src emacs-lisp
  ...
  (leaf UI
    :preface
    ;; These two functions save and restore the window layout configuration.
  ...
  If no configuration is saved, notify the user."
    ...
    ;; Custom function to display a buffer in the same window when possible.
    ...
    ;; Custom function to split the window below, from the parent or root window.
    ...
    :config
    ;; Set fullscreen mode if the display is graphical.
    (when (display-graphic-p)
      (set-frame-parameter nil 'fullscreen 'fullboth))

    ;; Enable Golden Ratio mode to dynamically adjust window sizes for optimal viewing.
    (leaf golden-ratio
      :ensure t
      :global-minor-mode t)

    ;; Set a visually pleasant theme (ef-frost).
    (leaf ef-themes
      :ensure t
      :config
      (load-theme 'ef-frost t))

    ;; Configure the modeline for better visibility and information display.
    (leaf modeline
      :config
      (leaf minions
        :ensure t
      ...
      ;; Enable time and battery status in the modeline.
      (setq display-time-interval 30
            display-time-day-and-date t
            display-time-24hr-format t)
      (display-time-mode 1))

    ;; Add padding around the mode-line for readability, toggled with F7 key.
    ...
    ;; Enable global tab line mode for easier tab navigation.
    (leaf tabmode
      :config
      (global-tab-line-mode))

    ;; Configure Treemacs for file navigation.
    (leaf treemacs
      :ensure t
      :bind
      (:treemacs-mode-map
...
#+begin_src emacs-lisp
  ...
  (leaf Fonts
    :preface
    ;; Function to check if a font exists in the system.
    (defun font-exists-p (font)
      "Check if FONT exists in the system."
      (member font (font-family-list)))

    ;; Function to set up default font and icon fonts for a specific frame.
    ...
    :config
    ;; Set default font family and size if not already configured.
    ...
    ;; Icons settings using Nerd Fonts
    (leaf nerd-icons
    ...
    ;; Ligature settings for programming modes
    (leaf ligature
      :ensure t
      :config
...
#+begin_src emacs-lisp
  ...
  ;; Keep Emacs configuration and cache files organized with no-littering
  (leaf no-littering
    :ensure t
    :require t)
...
#+begin_src emacs-lisp
    ...
    (leaf Org-mode
      ...
      (leaf ox-hugo :ensure t :after ox)
      (setq org-hugo-base-dir (expand-file-name "~/Hugo/")))

    ;; Initialize Leaf to load the configuration efficiently
    (leaf use-package :ensure t)

    ;; ;;; LSP and Tree-Sitter configuration for programming modes
    ...
    ;;              (< (buffer-size) 1048576))  ;; 1MB size limit for performance
    ;;     (lsp)
    ;;     (tree-sitter-mode)))
