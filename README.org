...
- =macos-keychain-internet= — =:host= はそのまま =security find-internet-password -s HOST= の
  =-s= （サービス名）に渡る。 =auth-gpg--lookup= や =dev-ai--setup-api-key= など、他の
...
#+begin_src emacs-lisp
  ...
  (leaf aidermacs
    ...
    :config
    ;; API キーの参照は after-init-hook で行う。
    ;;
    ;; 落とし穴（実機で踏んだ）: `add-hook' は APPEND 引数を渡さない限り新しい関数を
    ;; フックリストの *先頭* に追加する（同じ DEPTH 内では後から登録した関数ほど先に
    ;; 走る）。auth-core（epa-file-enable）と auth-gpg（~/.authinfo.gpg を auth-sources
    ;; へ追加）はどちらも my:modules 上では dev-ai より前にロードされるが、3 つとも
    ;; DEPTH を指定せず既定の 0 で `add-hook' していたため、実際の実行順は
    ;; 「ロード順」ではなく「登録順の逆」になり、dev-ai--setup-api-key が
    ;; auth-core--startup-init／auth-gpg--configure-sources より *先に* 走っていた。
    ;; epa-file が未初期化のまま auth-source-search が ~/.authinfo.gpg を読むため
    ;; 復号に失敗し、"No API key found" の警告が出る一方でファイル自体は開かれる、
    ;; という一見矛盾した挙動になっていた。
    ;;
    ;; 対処として dev-ai--setup-api-key の登録に DEPTH 10 を明示した。DEPTH は
    ;; personal/ac1965.el の ac1965--bind-leader-keys と同じ仕組みで、既定 DEPTH（0）の
    ;; フックがすべて走り終えたあとに実行されることを、モジュールのロード順に関係なく
    ;; 保証する。 dev-ai--ensure-authinfo-in-sources は auth-gpg--configure-sources の
    ;; 実行結果に頼らない自前の安全策として残してあるが、epa-file-enable 自体は
    ;; auth-core--startup-init 側でしか行わないため、DEPTH による実行順の保証が
    ;; 本質的な修正である。
    ;;
...
Registered on `after-init-hook' at DEPTH 10 (see the :config comment above)
so it runs after `auth-core--startup-init' has enabled epa-file decryption."
    ...
    (add-hook 'after-init-hook #'dev-ai--setup-api-key 10)
