...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  ;; We use string-trim / when-let; ensure availability at compile time.
  ...
  ;; Suppress file-name-handler overhead during init, then restore
  ...
  ;; Native compilation (defensive)
  (when (featurep 'native-compile)
    (setopt native-comp-async-report-warnings-errors 'silent))

  ;; macOS + Homebrew libgccjit support (prepend safely to LIBRARY_PATH)
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; License: GNU GPL v3 or later
  ;;
  ;; $Lastupdate: 2025/10/06 22:14:26 $
  ;;
  ;;; Commentary:
  ...
  ;;   M-x my/ensure-bootstrap
  ;;
  ;; Ensure a custom list silently:
  ;; (my/ensure-straight-packages
  ;;  '(vertico orderless marginalia nerd-icons) t)
  ;;
  ;; Run quietly after startup:
  ;; (add-hook 'emacs-startup-hook (lambda () (my/ensure-bootstrap t)))
...
#+begin_src emacs-lisp :tangle lisp/ui/ui-theme.el
  ...
  ;; ---------------------------------------------------------------------------
  ;; Face normalization to avoid `nil` attribute warnings on Emacs 30+
  ;;
  ;; Emacs warns if a face attribute is set to NIL explicitly. Use 'unspecified.
  ;; We fix known offenders conservatively (extend list if needed).

  (defun ui--normalize-face-attr (face &rest plist)
    "Set FACE attributes from PLIST, rewriting any nil values to 'unspecified."
    (when (facep face)
      (let ((fixed '()))
        (while plist
          (let* ((attr (pop plist))
                 (val  (pop plist)))
            (push (if (null val) 'unspecified val) fixed)
            (push attr fixed)))
        (apply #'set-face-attribute face nil (nreverse fixed)))))

  (defun ui--normalize-faces-after-theme ()
    "Normalize face attributes that may have been set to nil by themes/custom."
    ;; Your reported warning: font-lock-doc-markup-face :foreground nil
    (ui--normalize-face-attr 'font-lock-doc-markup-face :foreground (face-attribute 'font-lock-doc-markup-face :foreground))
    ;; Add other normalizations here if you encounter similar warnings, e.g.:
    ;; (ui--normalize-face-attr 'font-lock-doc-face :foreground (face-attribute 'font-lock-doc-face :foreground))
    )

  (add-hook 'after-load-theme-hook #'ui--normalize-faces-after-theme)

  ;; Run once at startup to cover faces set by custom-file before theme changes.
  (add-hook 'emacs-startup-hook #'ui--normalize-faces-after-theme)
