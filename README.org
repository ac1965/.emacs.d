...
#+begin_src emacs-lisp
  ...
  ;; ob-dot ships with org-contrib (not Org core).  In a straight.el environment
  ;; org-contrib must be declared separately; :straight t ensures straight.el
  ;; manages the package rather than assuming it is a built-in.
  (leaf ob-dot
    :straight t
    :after org
    :config
