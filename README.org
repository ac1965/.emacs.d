...
**** What this configuration does
- **Bootstraps modules** via `modules.el`, respecting:
  - `my:modules-verbose` for per-module timing + summary logs,
  - `my:modules-skip` to omit features,
  - `my:modules-extra` to append features at the end.
- **Switches UI/LSP stacks** centrally (`core/switches.el`):
  - UI: `none | doom | nano`
  - LSP: `eglot | lsp`
- **Hardens day-to-day UX** with focused cores:
...
- **UI surface**:
  ...
  - Optional **nano-modeline** or **doom-modeline** entry bundles.
- **Development surface**:
  - Treesit grammars and language remaps, project/editorconfig,
  - Eglot or lsp-mode (+ lsp-ui), Apheleia formatting, Docker/REST/SQL helpers,
  - Aidermacs integration with `~/.var/aideremacs/`.
...
| =core/tools.el=                           | Win split toggle, keybinding conflict scanner, Treesit install/ensure, Org fold helpers.     |
| =core/utils.el=                           | Auto-tangle updated Org, insert lexical-binding headers under =.var/=, view-mode on RO.      |
| =ui/ui-font.el=                           | Default/emoji font selection, daemon-safe apply, optional ligature setup.                    |
...
| =dev/web-core.el=                         | Treesit remaps, project/files/editorconfig, mode associations for TS/TSX/YAML/JSON.          |
...
3. **State & Customize**
   `core/custom.el` sets `custom-file` to `.etc/custom.el`, ensuring the file/dir exists, and provides commands to open/dump curated settings and faces.
4. **Daily UX**
   Editing/Dired/window/session modules apply defaults; UI modules configure fonts/themes/modeline; completion stack is layered; dev modules wire LSP/treesit/formatting.
5. **Org & export**
...
- **Treesit**
  ...
  - =my/treesit-ensure= :: install missing grammars safely (Emacs 30 treesit API)
- **Customize & State**
...
**** Usage tips
- **Skip slow/unused** modules with =my:modules-skip= (e.g., temporarily exclude =dev/dev-docker=).
- **Append experimentals** using =my:modules-extra= so they load after the stable base.
- **Flip UI/LSP** quickly: set =my:use-ui= / =my:use-lsp= before the loader runs (or re-eval switches).
- **Install Treesit grammars** once with =M-x my/treesit-ensure=; re-run after adding new languages.
...
- *“LSP not engaging in some buffers (Eglot)”* → Eglot is only auto-enabled when a server contact is guessable; run =M-x eglot= manually or add a project config.
- *“Treesit install error”* → Requires Emacs built with treesit; verify =M-: (featurep 'treesit)= and Xcode/Build deps on macOS.
...
;; --- Treesit bootstrapping (run once; re-run when adding languages) ----------
...
#+begin_src emacs-lisp :tangle lisp/ui/ui-theme.el
  ...
  (leaf nano-emacs
    ...
    (leaf nano-theme
      ...
      :config
      (let ((apply-nano-theme
             (cond ((fboundp 'nano-theme-light) (lambda () (nano-theme-light)))
  		 ((fboundp 'nano-theme-dark)  (lambda () (nano-theme-dark)))
  		 ((fboundp 'nano-theme)       (lambda () (nano-theme)))
  		 ((fboundp 'nano-light)       (lambda () (nano-light)))
  		 ((fboundp 'nano-dark)        (lambda () (nano-dark)))
  		 (t nil))))
        (if apply-nano-theme
            (funcall apply-nano-theme)
  	(message "[nano-theme] No applicable theme function was found.")))
