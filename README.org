...
#+begin_src emacs-lisp
  ...
  ;; そのまま (set-frame-parameter nil ...) すると存在しない/幽霊フレームに対して
  ;; 適用され、かつ core--did-fullscreen が立ってしまうため、後から
  ;; emacsclient -c で作られる実際のフレームには二度と適用されなくなる。
  ;; ui-font.el のフォント適用・ligature 有効化と同じ daemon 分岐パターンに揃え、
  ;; FRAME がグラフィカルであることを確認してから初回に限り適用する。
  (defun core--fullscreen-once (&optional frame)
    "初回のグラフィカルフレームをフルスクリーンにする。以降のフレーム
（daemon での 2 枚目以降の `emacsclient -c` を含む）には適用しない。"
  ...
  (if (daemonp)
      (add-hook 'after-make-frame-functions #'core--fullscreen-once)
