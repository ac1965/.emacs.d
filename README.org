...
;;; Commentary:
;; Minimal, side-effect free startup tweaks for Emacs 30+.
;; - No package loading here; package.el is initialized once in init.el.
;; - Speed up startup (GC, file-name handlers); restore after init.
;; - Set native-comp & frame params early.
;;; Code:
...
;;;; Directory layout (align with init.el / no-littering)
...
(defvar my:d:elpa (expand-file-name "elpa/" my:d:var))
(defvar my:d:eln  (expand-file-name "eln-cache/" my:d:var))
(defvar my:f:custom (expand-file-name "custom.el" my:d:etc))

(mapc #'my:ensure-directory-exists (list my:d:etc my:d:var my:d:elpa my:d:eln))

;;;; Package system (defer activation to init.el, but decide dirs early)
...
;; Larger pipe buffer helps LSP/VCS subprocess throughput.
(setq read-process-output-max (* 8 1024 1024)) ; 8MiB

;;;; Native compilation (configure, do not load packages)
(when (native-comp-available-p)
  (setq native-comp-async-report-warnings-errors 'silent
        native-comp-async-jobs-number
        (let ((e (getenv "EMACS_NATIVE_COMP_JOBS"))) (if e (string-to-number e) 4))
        native-comp-speed 2))
(when (boundp 'native-comp-eln-load-path)
  (startup-redirect-eln-cache my:d:eln))
...
;;; Commentary:
;; Packages and user config live here. Early boot tweaks are in early-init.el.
;;; Code:

;;;; Fallbacks for directory variables (in case early-init didn't define them)
(defvar my:d user-emacs-directory "Root of this Emacs config.")
(defvar my:d:etc (expand-file-name ".etc/" my:d))
(defvar my:d:var (expand-file-name ".var/" my:d))
(defvar my:f:custom (expand-file-name "custom.el" my:d))

;;;; package.el bootstrap (initialize exactly once)
(require 'package)
...
;;;; Install leaf (+ keywords) if missing
...
;;;; Keep files tidy (no-littering)
(leaf no-littering
  :ensure t
  :require t
  :config
  ;; Use our dirs if present; otherwise no-littering's defaults still work.
  (setq no-littering-etc-directory my:d:etc
        no-littering-var-directory my:d:var)
  ;; Put Custom into etc/, load it if it exists.
...
;;;; GC management (runtime; early boot tweaks are in early-init.el)
(leaf gcmh
  :ensure t
  :hook (emacs-startup-hook . gcmh-mode)
  ;; Avoid list-mutation error: use :setq with immediates (not expressions).
  :setq ((gcmh-high-cons-threshold . 67108864) ; 64 MiB
         (gcmh-idle-delay . 1.0)))

;;;; Hydra on-demand
(leaf hydra :ensure t :commands (defhydra))

;;;; Global key unsets (were in early-init; do here to keep early side-effect free)
(dolist (key '("C-z" "M-z" "M-m" "C-x C-z" "M-/"))
  (keymap-global-unset key))

;;;; Light UI chrome (defer to avoid redraws during startup)
(add-hook 'emacs-startup-hook
          (lambda ()
...
;;;; Load literate config safely
(require 'org) ;; load org first to avoid dynamic/lexical conflict on its vars
(let ((init-org (expand-file-name "README.org" my:d)))
  (when (file-exists-p init-org)
    ;; Bind dynamic var *after* org is loaded; keep scope local to this call.
    (let ((org-confirm-babel-evaluate nil))
      (condition-case err
          (org-babel-load-file init-org) ; tangle + load
        (error
         (display-warning
          'init
          (format "Failed to load %s: %s"
                  init-org (error-message-string err))
          :error))))))
...
#+begin_src emacs-lisp :tangle README.el
  ...
  ;; $Lastupdate: 2025/08/31 23:24:30 $
...
#+begin_src emacs-lisp :tangle README.el
  ...
  (leaf treemacs
    ...
    :config
    ;; Run after Treemacs is loaded to avoid void-function errors.
    (with-eval-after-load 'treemacs
      ;; keymap (defined only after treemacs loads)
      (when (boundp 'treemacs-mode-map)
        (keymap-set treemacs-mode-map "<mouse-1>"
                    #'treemacs-single-click-expand-action))
      ;; minor modes (defined by treemacs and its modules)
  ...
  (leaf desktop
    ...
    :hook ((kill-emacs-hook . desktop-save-in-desktop-dir)
           (after-init-hook . (lambda ()
                                (make-directory (no-littering-expand-var-file-name "desktop/") t)
                                (desktop-read)
                                (desktop-save-mode 1)))))
  ...
  ;; ---------------------------------------------------------------------------
  ;;; Custom Window Layout Utilities
  ;; Lightweight utilities for window layout management.

  (defvar my:saved-window-config nil
    "Cached window configuration for restoration.")

  ;;;###autoload
  (defun my:save-window-layout ()
    "Save current window configuration (frame-local)."
    (interactive)
  ...
  (defun my:restore-window-layout ()
    "Restore previously saved window configuration."
    (interactive)
  ...
  (defun my:toggle-window-dedication ()
    ...
    (let ((win (selected-window)))
      ...
      (message "Window dedication %s"
               (if (window-dedicated-p win) "enabled" "disabled"))))
#+end_src
...
- *early-init.el* :: Startup-time tuning only (GC, file-name-handler, native-comp, frame defaults).
- *init.el*       :: Packages & leaf-based modules; UI toggles are deferred via hooks.
- *user.el*       :: Personal or machine-specific overrides (optional; loaded from init).
- *README.el*     :: Unified, auto-generated; can be loaded quickly in production.
- *Makefile*      :: Reproducible tangle/clean; optional byte-compile target.

** Boot Sequence (Emacs 30+)
1. Emacs loads `early-init.el` before frame creation.
2. Emacs loads `init.el`.
3. `init.el` may load `README.el` or fall back to `(org-babel-load-file "README.org")`.

** Loading Strategy
:PROPERTIES:
:CUSTOM_ID: loading-strategy
:END:

During development, you may load `README.org` directly via `org-babel-load-file`
for convenience (auto-tangle + load). For production or fast startup, prefer
loading pre-tangled files (`early-init.el`, `init.el`, `user.el`, and/or `README.el`)
that are generated by the Makefile.
...
The Makefile provides reproducible tangling. By default it tangles `README.org`
into all declared :tangle targets (e.g., `early-init.el`, `init.el`, `user.el`, `README.el`).

- `make` / `make tangle` :: Tangle all outputs from `README.org`.
- `make clean`           :: Remove generated `.el` files to force a fresh tangle.
- `make compile`         :: Byte-compile the pre-tangled files (optional).

** UI & OS Integration Notes
- UI toggles (menu/tool/scroll bars) are deferred to `emacs-startup-hook` to avoid early redraw overhead.
- On macOS, environment synchronization (`exec-path-from-shell`) is limited to GUI sessions to avoid side effects in batch mode.
- Global key unsets live in `init.el` (not early-init) to keep the early phase side-effect free.
