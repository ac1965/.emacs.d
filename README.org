...
- =utils-ime= — macism による入力ソース制御（リーダー／dired 時に英字へ）
...
（ =b l= など）や =dired= のナビゲーションキー（ =RET= / =g= / =d= ）が
IME に composed されて誤動作する。本モジュールは =macism= を同期呼び出しして、
次の 2 つの契機で OS の入力ソースを ASCII（英字）へ切り替える。

- *リーダーキー押下時* — =ui-leader-key= を =menu-item= の =:filter= で包み、
  キー押下の瞬間に ASCII 化してから =ui-leader-map= を返す。プレフィックスキーの
  性質は保たれるため、 which-key と =describe-key= は従来どおり機能する。
- ~dired-mode~ 進入時 — =dired-mode-hook= で ASCII 化する。dirvish は
  =dired-mode= を土台にするため同じフックで捕捉できる。
...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; macism を用いて OS の入力ソースを ASCII（英字）へ切り替えるユーティリティ。
  ;;
  ;; 目的:
  ;; - リーダーキー（`ui-leader-key'）を押した瞬間に ASCII へ切り替え、後続の
  ;;   1 打鍵（`b l' など）が日本語 IME に composed されず、確実に届くようにする。
  ;; - `dired-mode' に入ったら ASCII へ切り替え、`RET'/`g'/`d' などのナビゲーション
  ;;   キーが誤爆しないようにする。
  ;;
  ;; 本ファイルの責務:
  ...
  ;; - `dired-mode-hook' への ASCII 化フック登録。
  ;;
  ;; 設計上の注意:
  ...
  (defcustom utils-ime-enable-dired-p t
    "If non-nil, switch to ASCII when entering `dired-mode'."
    :type 'boolean
    :group 'utils-ime)
  ...
  blocks command dispatch.  Synchronous by design (see Commentary)."
    ...
    (when (utils-ime-available-p)
      (ignore-errors
        (call-process utils-ime-macism-executable nil nil nil
                      utils-ime-ascii-source))))
  ...
  (defun utils-ime--dired-hook ()
    "Switch to ASCII when entering `dired-mode' (incl. dirvish)."
    (when utils-ime-enable-dired-p
      (utils-ime-to-ascii)))
  ...
  ;; dirvish は dired-mode を土台にするため dired-mode-hook で捕捉できる。
  (add-hook 'dired-mode-hook #'utils-ime--dired-hook)
