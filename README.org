...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; License: GNU GPL v3 or later
  ;;
  ;; $Lastupdate: 2025/10/08 17:25:00 $
  ;;
  ;;; Commentary:
...
#+begin_src emacs-lisp :tangle lisp/core/history.el
  ...
  ;; Transient (used by Magit, Forge, etc.)
  (leaf transient
    :straight t
    :config
...
**What it does**
- Keeps your original theme setup (incl. toggle pair).
- Adds a guard for `set-face-attribute` to coerce `nil` → `'unspecified`.
- Hooks normalization to theme enable & startup.
...
*Notes*
`my:modeline` can switch between doom / nano.
...
#+begin_src emacs-lisp :tangle lisp/orgx/org-core.el
  ...
  ;; Add “j: Journal” template only after org-journal is available (robust)
  ...
  (defun my/delete-old-backups ()
    ...
    (let ((backup-dir (concat no-littering-var-directory "backup/"))
      ...
      (when (file-directory-p backup-dir)
        (dolist (file in (directory-files backup-dir t))
