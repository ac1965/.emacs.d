...
#+begin_src makefile :tangle Makefile :mkdirp no :padline no
  ...
  #
  # 検査本体は scripts/check-emphasis.el（Appendix を参照）。シェル引用符の入れ子を
  # 避けるため、check-tangle のような --eval 一行ではなく --load で読み込む。
  .PHONY: check-emphasis
  check-emphasis:
  	@$(EMACS) -Q --batch --load scripts/check-emphasis.el -- "$(ORG)"
...
* Appendix: check-emphasis.el — 効かない Org 強調記法の検出スクリプト
:PROPERTIES:
:CUSTOM_ID: appendix-check-emphasis-el
:header-args:emacs-lisp: :tangle scripts/check-emphasis.el :mkdirp yes :comments link
:END:

=make check-emphasis=（=make lint= にも含まれる）が呼び出す検査本体です。実装を
=README.org= 本文の Makefile ブロックへ =--eval= で直書きすると、シェルの引用符と
Elisp の文字リテラル・正規表現のエスケープが何重にも重なり、書いた本人ですら
検証できなくなります。通常の Elisp ファイルとして独立させ、=--load= で読み込む
構成にしました。

*検出方法*

=org-emphasis-regexp-components= が定める「直前・直後に許される文字」の集合を
そのまま Elisp の文字リストとして持ち、 =/= verbatim と ~code~ を先に確定させて
...
コードブロック本文・見出し行・drawer/keyword/table 行（先頭が =:= =#= =\vert=）は
検査対象から除外します。
...
[check-emphasis] scanning README.org for ineffective emphasis markers ...
  L249: =my:modules= に含まれる
[check-emphasis] FAILED: 1 ineffective marker(s)
#+end_example

#+begin_src emacs-lisp
  ;;; scripts/check-emphasis.el --- 効かない Org 強調記法の検出 -*- lexical-binding: t; -*-
  ...
  ;;
  ;; org-emphasis-regexp-components の既定値では、開始マーカーの直前・終了マーカーの
  ;; 直後が特定の文字集合（半角空白・タブ・- – — ( " ' { が直前側、
  ;; . , : ! ? ; ' " ) } [ が直後側、および行頭・行末）でなければ強調は成立しない。
  ;; 全角文字（、。（）「」等）はこの集合に含まれないため、和文に接した =verbatim=、
  ;; ~code~、*bold* は静かに機能しなくなる（マーカーがリテラル文字としてそのまま
  ;; 表示され、シンタックスハイライトも失われる）。tangle には影響しないため、
  ;; 見た目の欠陥として気づかれにくい。
  ;;
  ;; 使い方:
  ;;   emacs -Q --batch --load scripts/check-emphasis.el -- README.org
  ;;
  ;; 終了コード: 検出 0 件で 0、1 件以上で 1。
  ;;
  ;;; Code:

  (require 'cl-lib)

  (defconst check-emphasis--pre-ok
    '(?\s ?\t ?- ?\u2013 ?\u2014 ?\( ?\" ?\' ?\{)
    "Characters allowed immediately before an emphasis marker's opening char.")

  (defconst check-emphasis--post-ok
    '(?\s ?\t ?- ?\u2013 ?\u2014 ?. ?, ?: ?! ?\? ?\; ?\' ?\" ?\) ?\} ?\[)
    "Characters allowed immediately after an emphasis marker's closing char.")

  (defun check-emphasis--marker-regexp (mk)
    "Build a regexp matching MK CONTENT MK for the single-character marker MK.
  CONTENT must not start or end with whitespace or MK itself, and must not
  contain MK internally.  Mirrors the border/body rules implied by
  `org-emphasis-regexp-components'."
    (let* ((lit (char-to-string mk))
           (esc (regexp-quote lit)))
      (concat esc
              "\\(" "[^[:space:]" lit "]"
              "[^" lit "\n]*?"
              "[^[:space:]" lit "]"
  ...
  (defun check-emphasis--collect-spans (line mk)
    "Return all (BEG . END) spans in LINE matching the marker MK, unfiltered."
    (let ((re (check-emphasis--marker-regexp mk))
          (pos 0)
          (spans nil))
      (while (string-match re line pos)
        (push (cons (match-beginning 0) (match-end 0)) spans)
        (setq pos (match-end 0)))
      (nreverse spans)))

  (defun check-emphasis--overlaps-p (a b spans)
    "Return non-nil if the span [A, B) overlaps any span in SPANS."
    (cl-some (lambda (s) (and (< a (cdr s)) (< (car s) b))) spans))

  (defun check-emphasis--check-span (line lineno a b)
    "Report LINE's span [A, B) at LINENO if its border chars are disallowed.
  Returns 1 if a violation was reported, 0 otherwise."
    (let* ((pre  (and (> a 0) (aref line (1- a))))
           (post (and (< b (length line)) (aref line b))))
      (if (or (and pre  (not (memq pre  check-emphasis--pre-ok)))
              (and post (not (memq post check-emphasis--post-ok))))
          (progn
            (message "  L%d: %s" lineno
                     (substring line a (min (length line) (+ a 40))))
            1)
        0)))

  (defun check-emphasis--scan-line (line lineno)
    "Scan LINE (at LINENO) for ineffective emphasis markers.
  Returns the count of violations found.

  =verbatim= and ~code~ spans are resolved first and treated as protected
  regions, since a bare =/~ character is always literal, never a bold/italic
  delimiter.  *bold* candidates are then checked only outside those protected
  regions, and are also skipped when immediately adjacent to = or ~ (e.g. the
  `*' characters inside =*scratch*= are part of the verbatim span, not a
  bold marker, even though the naive regexp would also match them)."
    (let ((bad 0) (protected nil))
      (dolist (mk '(?= ?~))
        (dolist (span (check-emphasis--collect-spans line mk))
          (let ((a (car span)) (b (cdr span)))
            (unless (check-emphasis--overlaps-p a b protected)
              (push span protected)
              (setq bad (+ bad (check-emphasis--check-span line lineno a b)))))))
      (dolist (span (check-emphasis--collect-spans line ?*))
        (let ((a (car span)) (b (cdr span)))
          (unless (or (check-emphasis--overlaps-p a b protected)
                      (and (> a 0) (memq (aref line (1- a)) '(?= ?~)))
                      (and (< b (length line)) (memq (aref line b) '(?= ?~))))
            (setq bad (+ bad (check-emphasis--check-span line lineno a b))))))
      bad))

  (defun check-emphasis--run (file)
    "Scan FILE and return the total count of ineffective emphasis markers.
  Skips source/example block bodies, heading lines, and lines beginning
  with a drawer, keyword, or table marker (: # |)."
    (let ((bad 0) (in-block nil) (lineno 0))
      (with-temp-buffer
          ...
          (let ((line (buffer-substring-no-properties
            ...
            (cond
             ...
             ((or in-block
                  ...
                  (string-match-p "\\`[ \t]*[:#|]" line)))
             (t
              (setq bad (+ bad (check-emphasis--scan-line line lineno))))))
          (forward-line 1)))
      bad))

  (let ((file (car command-line-args-left)))
    (unless (and file (file-exists-p file))
      (message "[check-emphasis] usage: emacs -Q --batch --load check-emphasis.el -- FILE.org")
      (kill-emacs 2))
    (message "[check-emphasis] scanning %s for ineffective emphasis markers ..." file)
    (let ((bad (check-emphasis--run file)))
      (if (> bad 0)
          (progn
            (message "[check-emphasis] FAILED: %d ineffective marker(s)" bad)
            (kill-emacs 1))
        (message "[check-emphasis] ok"))))

  (provide 'check-emphasis)
  ;;; scripts/check-emphasis.el ends here
#+end_src
...
** 効かない Org 強調記法の検出（Makefile: check-emphasis）と回帰の修正
...
以前の一括修正（217 箇所）の *後* に行った複数回の編集（org-roam 関連の変更、
=on-mismatch= の修正など）で、ガード無しの新しい散文が追加され、同じ欠陥が 37 箇所
再混入していた。検査ロジックを実装する過程でこれを検出し、同じ自動修正ルーチンを
再適用して解消した（冪等な操作であるため、既に正しい箇所には影響しない）。

*対処: make check-emphasis*

=scripts/check-emphasis.el=（Appendix を参照）を新設し、=make check-emphasis= および
=make lint= から呼び出せるようにした。=check-tangle= と異なり、検査本体は
=README.org= 内の Makefile ブロックへ =--eval= で直書きしない。シェルの引用符と
Elisp の文字リテラル・正規表現のエスケープが何重にも重なり、検証も保守もできなくなる
ためである。通常の Elisp ファイルとして独立させ、=--load= で読み込む。

検出方法は README.org 側で使ってきた自動修正スクリプトと同じ判定規則を踏襲する。
