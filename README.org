...
#+begin_src emacs-lisp
  ...
  ;;
  ;; sis--current is only updated when sis--get is called (on IME switch
  ;; events or idle timers).  Reading it directly in the :eval block works
  ;; only if sis has already polled macism.  Calling sis--get every frame
  ;; would spawn an osascript process per redisplay — unacceptable cost.
  ;;
  ;; Solution: maintain a buffer-local cache `ac1965--sis-indicator' that is
  ;; updated via `sis-change-hook'.  The hook fires whenever sis--update-state
  ;; detects a change, so the cache stays current without polling.
  ;; The :eval block reads only the cache string — zero process overhead.

  (defvar-local ac1965--sis-indicator ""
