...
#+begin_src emacs-lisp
  ...
  ;;   拡張子の書き換え（.svg → .pdf）は orgx-export--svg-to-pdf-graphics（下記）
  ;;   が担う。文書本体の画像リンクは [[file:foo.svg]] のままでよく、個々の
  ;;   リンクを書き換える必要はないが、対応する *.pdf を事前に生成しておくこと。
  ;;; Code:
...
where npm global installs are not on PATH."
  ...
  ;;
  ;; SVG 埋め込み方式の変更履歴（2026-07）:
  ;;   当初 \usepackage[inkscapelatex=false]{svg} + -shell-escape + \includesvg
  ;;   書き換えで試したが、inkscapelatex=false は Inkscape に LaTeX テキストを
  ;;   処理させるかどうかの切替に過ぎず、svg パッケージは変換手段として常に
  ;;   Inkscape 本体の CLI を要求する。mactex-no-gui 環境には Inkscape が無いため
  ;;   `Inkscape version not detected' で実際に失敗した（design_spec.org のビルド
  ;;   ログで確認）。svg パッケージ／shell-escape を使わない事前変換方式
  ;;   （下記）に変更した。

  (defun orgx-export--svg-to-pdf-graphics (contents backend _info)
    "Rewrite \\includegraphics{FILE.svg} to \\includegraphics{FILE.pdf} for latex.
Org always emits \\includegraphics for any image link matching
`org-latex-inline-image-rules' — svg is included by default — regardless of
whether the LaTeX engine can actually rasterize svg (it cannot, without the
svg package's Inkscape dependency, which this configuration deliberately
avoids; see the change history comment above). Instead, *.svg figures are
expected to be pre-rendered to a sibling *.pdf via rsvg-convert
(brew install librsvg):

  for f in svg/*.svg; do rsvg-convert -f pdf -o \"${f%.svg}.pdf\" \"$f\"; done

This filter only swaps the extension so the ordinary graphicx pipeline
(no shell-escape needed) picks up the pre-rendered PDF. Runs on
`org-export-filter-link-functions', so it only touches the exported form of
image links, never the buffer itself."
