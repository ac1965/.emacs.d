...
- AI-assisted editing (aidermacs via dev-ai; gptel)
- terminal integration (vterm)
- build and compilation helpers
- code formatting (apheleia)

Note: Org Babel + Claude API workflow integration resides in
=personal/personal-ai.el= (personal layer), which depends on
=utils/utils-claude.el= (utils layer) for HTTP transport.
...
- Every file must set lexical binding via the file-local variable line
  =-*- lexical-binding: t -*-= on the first line of the file header.
- =(provide 'feature-name)= must match the file name without the =.el= extension.
- Built-in packages declare =:straight nil= in their =leaf= form.
- =leaf= keyword order:
  =:straight= → =:ensure= → =:after= → =:require= →
  =:pre-setq= → =:custom= → =:bind= → =:hook= → =:init= → =:config=.
- All public =defun= (=my/= or =<module>-= prefix, no double dash)
  must carry a docstring that documents the function contract.
  Cross-module callable functions and interactive commands constitute the
  public API; they must not rely on undocumented internals of other modules.

*Variable assignment*

- Use =setopt= (not =setq=) for all =defcustom= variables.
  =:custom= or =:pre-setq= inside a =leaf= declaration is preferred.
  =setopt= inside =:config= is acceptable when the value must be
  derived at runtime (e.g. from a path variable).
  Bare =setq= is reserved for =defvar= variables and internal mutable
  state (e.g. phase-2 updates, guard flags, temporary startup overrides).
- Use =customize-save-variable= instead of =setopt= when a =defcustom=
  value must persist across Emacs restarts (e.g. timestamps or user state
  written by interactive commands).  =customize-save-variable= both applies
  the setter (like =setopt=) and writes the value to =custom-file=.

*Naming conventions*

- Public interactive commands and cross-module functions use the =my/=
  prefix (e.g. =my/ensure-directory-exists=) or the module prefix
  (e.g. =core-tools-emacs=).
- Module-private helpers use the =<module>--name= double-dash pattern
  (e.g. =core-custom--ensure-file=, =modules--require-safe=).
  The mixed form =my/<topic>--helper= is not permitted; it conflates the
  public-namespace indicator with the private-scope indicator.
- Path and directory variables use the =my:d:*= prefix.
- Each module that owns =defcustom= variables must declare a =defgroup=
  using the module name as the group symbol, and every =:group= slot in
  that module's =defcustom= declarations must reference that group symbol,
  not an external package's group.

*Documentation*

- All public =defun= (=my/= or =<module>-= prefix without double dash)
  must carry a docstring documenting their contract.
- Private helpers (=<module>--= prefix) are exempt from the docstring
  requirement but are encouraged to carry one.
...
#+begin_src emacs-lisp
  ...
  ;; [Fix G] treesit-install-dir and treesit-extra-load-path are defcustom
  ;; variables in Emacs 29+.  Use setopt (not setq) to respect their setter
  ;; functions and type validation.  boundp guards preserve forward
  ;; compatibility on Emacs builds without tree-sitter support.
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Internal helpers use the `bootstrap--` prefix (not `utils--`) because
  ;; they are defined before the utils layer exists.  `bootstrap--safe-load-file`
  ;; is aliased as `my/safe-load-file` for call sites in other layers.
  ;;
  ;;; Code:
  ...
  ;; leaf macros are expanded at byte-compile time; eval-and-compile ensures
  ;; the macro definitions are available both during compilation and at runtime
  ;; (needed for interpreted evaluation when init.el is not byte-compiled).
  (eval-and-compile
    (require 'leaf))
  ...
  ;;
  ;; my:modules-extra is declared here, before personal files load, so that
  ;; personal/user.el can safely append to it via setq / append.
  ;; (The defvar in modules.el is a no-op once this one has run — defvar never
  ;; overwrites an already-bound variable.)
  ;; ---------------------------------------------------------------------------
  ...
  (defun ui-keymap--local-leader-binds ()
    ...
    (with-eval-after-load 'org
      (when (boundp 'org-mode-map)
        (keymap-set org-mode-map "C-c SPC m t" #'org-todo)
        (keymap-set org-mode-map "C-c SPC m s" #'org-schedule)
        (keymap-set org-mode-map "C-c SPC m d" #'org-deadline))))
...
Binds navigation, buffer management, search, and help commands to ergonomic
keys (C-., C-c b/k/o/r/s, s-arrows, <f1>).  Runs via `after-init-hook' so
all command symbols are defined before the bindings are established."
...
#+begin_src emacs-lisp
  ...
  ;; [Fix BJ] Pre-install simple-httpd and websocket here, alongside org-roam,
  ;; so they are built and on load-path before orgx-roam-ui.el is required
  ;; later in my:modules-extra.  orgx-roam-ui.el calls straight-use-package for
  ;; both, but on a non-standard straight-base-dir the build directory is not
  ;; added to load-path reliably on first run.  Installing them unconditionally
  ;; in this earlier module guarantees availability regardless of load order.
  ;; These calls are no-ops on subsequent startups (packages already built).
  ...
  (leaf org-roam
    ...
    ;; [Fix BH] Emacs 31.0.50 changed sqlite-available-p / sqlitep behaviour.
    ;; Pin the connector explicitly to bypass emacsql's auto-detection heuristic.
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org
    ...
    (keymap-set org-mode-map "C-c e" #'my/org-unfold-subtree)
    (keymap-set org-mode-map "C-c t" #'my/org-toggle-fold))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This file:
  ;; - installs simple-httpd, websocket, and org-roam-ui via
  ;;   straight-use-package at top level (outside any with-eval-after-load
  ;;   wrapper) so all three are on load-path before org-roam is required
  ;; - requires all three packages unconditionally at top level so that
  ;;   load-path registration is guaranteed regardless of straight-base-dir
  ;;   configuration
  ...
  ;; simple-httpd and websocket are pre-installed by orgx-extensions.el
  ;; (Fix BJ), which loads earlier in my:modules.  The straight-use-package
  ;; calls here are kept as no-ops for resilience; the require calls are
  ;; wrapped in condition-case so a missing package does not abort module load.
  (straight-use-package 'simple-httpd)
  (condition-case err
      (require 'simple-httpd)
    (error
     (message "[orgx-roam-ui] simple-httpd load failed: %s"
              (error-message-string err))))
...
- *[Fix N] Changelog*: Rewrote the =2026-03 (第2回評価修正)= section entirely in
...
- *[Fix AZ] ui/ui-leader.el (=ui-leader--setup-maps=)*: Replaced all 13
  ...
  =(keymap-set ui-leader-map KEY MAP)=.  The non-string key =(kbd
  ui-leader-local-suffix)= becomes the bare variable reference because
  =keymap-set= accepts a string-valued variable directly without =kbd=.
...
- *[Fix BA] ui/ui-keymap.el*: Replaced 10 occurrences across =ui-keymap--bind=,
  =ui-keymap--leader-bind=, and =ui-keymap--local-leader-binds= (=dirvish-mode-map=
  ×5, =org-mode-map= ×3, generic map helper ×2).  The two
  =(define-key key-translation-map ...)= calls in =ui-keyboard--apply= are
  retained: =key-translation-map= is a pre-Emacs-29 internal structure and
  =keymap-set= compatibility is not documented for it.

*** Fix BB: orgx/orgx-fold.el — replace define-key+kbd with keymap-set (3 occurrences)

🟠 Medium — keymap API violation (Coding Rule 8)

- *[Fix BB] orgx/orgx-fold.el*: Replaced 3 =(define-key org-mode-map (kbd KEY) FN)=
  calls (=C-c f=, =C-c e=, =C-c t=) with =(keymap-set org-mode-map KEY FN)=.
...
- *[Fix BC] orgx/orgx-notes-markdown.el*: Moved =my/markdown-toggle-live-preview=
  (from =leaf markdown-mode :config=), =my/notes-markdown-visual-fill= (from
  =leaf visual-fill-column :init=), =my/notes-consult-ripgrep= (from =leaf
  consult-notes :init=), and =my/notes-markdown-org-download-setup= (from
  =with-eval-after-load 'org-download=) to module top level.  Empty =:init=
  keywords removed.  =define-key= + =kbd= in =leaf markdown-mode :config=
  replaced with =keymap-set=.
...
- *[Fix BE] dev/dev-music.el (=dev-music--setup-dired=)*: Replaced multi-line
  =(define-key dired-mode-map (kbd "E") ...)= with
  =(keymap-set dired-mode-map "E" #'emms-play-dired)=.
...
- *[Fix BF] utils/utils-dired.el*: Replaced
  =(define-key dired-mode-map (kbd "RET") #'my/dired-get-file-or-dir)= with
  =(keymap-set dired-mode-map "RET" #'my/dired-get-file-or-dir)=.
...
- *[Fix BG] README.org (final line)*: Removed isolated character =k= following
  the =utils-scratch= changelog entry.  Caused =org-lint= to report a stray
...
- *[Fix BH] orgx/orgx-extensions.el (=leaf org-roam :config=)*: Added
  =setopt org-roam-db-connector= before the idle timer registration.

  *Root cause.* Emacs 31.0.50 changed the behaviour of =sqlite-available-p= /
  =sqlitep=.  The =emacsql= connector selection heuristic evaluates =(sqlitep
  nil)= during type-dispatch; on Emacs 31 this signals an unhandled condition
  rather than returning =nil=, aborting =org-roam-db-autosync-mode=.
  ...
  *Fix.*
  ...
  The =fboundp= + =(sqlite-available-p)= guard selects ='sqlite-builtin= on
  Emacs 31+ with the built-in SQLite module and falls back to ='sqlite= on
  older builds.  =setopt= is used because =org-roam-db-connector= is a
  =defcustom= (Coding Rule 6).  The conditional expression cannot appear in
  =:custom= (Fix AA precedent), so =setopt= in =:config= is correct.

** 2026-04 (twenty-ninth evaluation pass)
*** Fix BI: orgx/orgx-roam-ui.el — condition-case around require calls

🟠 Medium — load robustness; isolates per-package failures

- *[Fix BI] orgx/orgx-roam-ui.el (dependency bootstrap)*: Wrapped each
  =require= call for =simple-httpd=, =websocket=, and =org-roam-ui= in
  =condition-case= with distinct =[orgx-roam-ui] <pkg> load failed= messages.
  A failure in one package is isolated: the remaining packages are still
  attempted and the module itself loads successfully (=failed=0= in the
  modules report, versus =failed=1= before this fix).

*** Fix BJ: orgx/orgx-extensions.el — pre-install simple-httpd and websocket

🔴 Critical — "Cannot open load file: No such file or directory, simple-httpd"

- *[Fix BJ] orgx/orgx-extensions.el (before =leaf org-roam=)*: Added two
  unconditional =straight-use-package= calls for =simple-httpd= and =websocket=
  before the =leaf org-roam= declaration.

  *Root cause analysis.*  =straight-use-package= builds a package into
  =<straight-base-dir>/straight/<straight-build-dir>/<pkg>/= and registers
  that directory on =load-path=.  On this system:

  | Variable | Value |
  |----------+-------|
  | =straight-base-dir= | =~/.emacs.d/.cache/emacs/= |
  | =straight-build-dir= | =build-31.0= |
  | Expected build path | =~/.emacs.d/.cache/emacs/straight/build-31.0/simple-httpd/= |

  When =orgx-roam-ui.el= calls =straight-use-package 'simple-httpd= on first
  run, =straight= initiates a network clone.  The clone completes, but
  =straight= does not re-add the build directory to =load-path= within the
  same synchronous call frame on the first invocation with this
  =straight-base-dir= layout.  The immediately following =(require
  'simple-httpd)= therefore fails.

  *Fix.*  Installing =simple-httpd= and =websocket= in =orgx-extensions.el=
  — which loads earlier in =my:modules= (not =my:modules-extra=) — guarantees
  that =straight= clones, builds, and registers both packages on =load-path=
  before =orgx-roam-ui.el= is required.  By the time =orgx-roam-ui.el=
  executes, the packages are already built from the previous Emacs session
  (or cloned during this session's earlier module load), so the =require=
  calls in =orgx-roam-ui.el= succeed reliably.

  The two =straight-use-package= calls are placed outside the =leaf org-roam=
  block to make the dependency relationship explicit.  They are no-ops on
  subsequent startups (packages already built).
