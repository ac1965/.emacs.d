...
:END:
#+begin_src emacs-lisp
  ;;; orgx/orgx-auto-hugo.el --- Automatic ox-hugo export on save -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; Automatic ox-hugo all-subtrees export on save for the Hugo blog source file.
  ;;
  ;; This file:
  ;; - targets only the file identified by `my:f:capture-blog-file`
  ;; - uses a buffer-local in-progress flag to prevent recursive save loops
  ;; - calls `org-hugo-export-wim-to-md` with :all-subtrees t on each save
  ;; - registers `orgx-auto-hugo--maybe` in `after-save-hook` buffer-locally
  ;;   via the named hook function `orgx-auto-hugo--install-hook`
  ...
  (defvar-local orgx-auto-hugo--in-progress nil
    "Non-nil while an ox-hugo export triggered by `after-save-hook' is running.
Prevents recursive export loops when ox-hugo writes intermediate files.")

  (defun orgx-auto-hugo--eligible-p ()
    "Return non-nil when the current buffer is the Hugo blog source file."
    (and (derived-mode-p 'org-mode)
         buffer-file-name
  ...
  (defun orgx-auto-hugo--maybe ()
    "Export all Hugo subtrees when the current buffer is the blog source file."
    (when (and (orgx-auto-hugo--eligible-p)
               (not orgx-auto-hugo--in-progress))
      (let ((orgx-auto-hugo--in-progress t))
        (condition-case err
            (progn
              (message "[orgx-auto-hugo] exporting all subtrees...")
              (org-hugo-export-wim-to-md :all-subtrees)
              (message "[orgx-auto-hugo] export complete."))
          (error
           (message "[orgx-auto-hugo] error: %s" (error-message-string err)))))))

  (defun orgx-auto-hugo--install-hook ()
    "Install the auto-hugo after-save hook buffer-locally."
    (add-hook 'after-save-hook #'orgx-auto-hugo--maybe nil 'local))

  (add-hook 'org-mode-hook #'orgx-auto-hugo--install-hook)
...
#+begin_src emacs-lisp
  ...
  (setq my:d:org          "/Users/ac1965/Documents/org/"
        ...
        my:f:capture-blog-file "/Users/ac1965/Documents/devel/repos/mysite/all-posts.org")
