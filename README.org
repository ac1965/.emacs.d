...
**Purpose**
Route all Customize output to `~/.emacs.d/.etc/custom.el`, keep it separate from `init.el`, and provide a one-shot command to dump selected current settings/faces into `custom-file`.

**What it does**
- Ensures `custom-file` is `.etc/custom.el` and loads it if present.
- Provides `M-x my/custom-open` to open `custom-file`.
- Provides `M-x my/custom-dump-current` to *explicitly* persist selected variables/faces to `custom-file`.
  - This does **not** auto-run at startup (no surprise edits).

**Notes**
- Ordinary `M-x customize` → “Save for future sessions” will be written into `.etc/custom.el`.
- `leaf :custom` values are *runtime settings* and are not automatically considered “saved” by Customize. Persist only what you need via `my/custom-dump-current`.
