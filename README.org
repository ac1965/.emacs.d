...
- Purpose: Startup performance optimization and minimal environment setup.
- Features:
  - Enforces Emacs 30+.
  - Disables `package.el` (using `straight.el` instead).
  - GC and `file-name-handler` temporarily relaxed and restored after startup.
  - Enlarges `read-process-output-max` for faster subprocess I/O.
  - Native compilation settings with XDG ELN cache support.
  - macOS-specific libgccjit environment fixes.
  - Prepares config directories (`my:d:cache`, `my:d:etc`, `my:d:var`, etc.).
  - UI tweaks (fullscreen, no menu/tool/scroll bars).
  - Bootstraps `straight.el` with profile lockfiles.
...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  ;; Package.el (disabled; straight.el handles packages)
  ...
  (add-hook 'emacs-startup-hook
            (lambda ()
              "Restore GC thresholds once after startup."
  	    (setq gc-cons-threshold my--orig-gc-cons-threshold
  		  gc-cons-percentage my--orig-gc-cons-percentage)
  	    ;; Print how long Emacs took to start and how many GCs happened
  ...
  ;; Avoid accidental truthy value; ensure scroll bars are disabled on first paint.
  ...
  ;; Base directory inside `my:d:cache` for reproducibility.
  (setq straight-base-dir my:d:cache
        straight-use-package-by-default t)

  ;; Use profile lockfiles for reproducibility
    ...
    (unless (file-exists-p bootstrap-file)
      (let ((url-request-timeout 15))
        (condition-case err
          ...
          (error
           (warn "[straight] bootstrap failed: %s"
...
- Purpose: Main configuration loader and base setup.
- Features:
  - Unbinds disruptive defaults; integrates CAPE completion.
  - Loads `leaf.el` with `straight.el`.
  - Configures basic behaviors (scrolling, auto-save, relative line numbers, etc.).
  - Uses `no-littering` for clean paths.
  - macOS integration: shell env sync (`exec-path-from-shell`) and PATH fallback.
  - Dired enhancements (GNU ls).
  - GC optimization via `gcmh`.
  - Enables `hydra`.
  - Loads `custom.el` and `personal/` user-specific configs.
  - Loads modular configs from `lisp/`.

#+begin_src emacs-lisp :tangle init.el
  ;;; init.el --- Main initialization -*- lexical-binding: t; -*-
  ;;; Commentary:
  ;; Modularized configuration (tangled from README.org into lisp/*.el)
  ;;; Code:
...
#+begin_src emacs-lisp :tangle lisp/lsp.el
  (defcustom my:use-lsp 'eglot
    "LSP backend selection.
  Valid values are:
  - `eglot` (default, lightweight)
  - `lsp`   (full-featured LSP-Mode)."
