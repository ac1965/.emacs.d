...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Commentary:
  ;; Public, stable facade for session orchestration.
  ;;
  ;; This module:
  ;; - exposes the minimal stable public API
  ;; - delegates implementation to core-session-private
  ;; - provides deterministic scheduler lifecycle control
  ;;
  ;; Design principles:
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Commentary:
  ;; Private implementation of core-session.
  ;;
  ;; This module:
  ;; - contains internal state and timers
  ;; - must never be required outside core-session
  ;;
  ;; Design principles:
  ;; - All symbols are core-session--*
  ...
  (defun core-session--normalize-job-meta (name fn plist)
    (unless (symbolp name)
      (error "Job name must be symbol"))
    (unless (functionp fn)
      (error "Job fn must be function"))
    (append (list :name name :fn fn :enabled t)
            plist))
  ...
  (defun core-session--start ()
    ...
    (setq core-session--idle-timer
          (run-with-idle-timer core-session-idle-delay
                               t
                               #'core-session--run-all)))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Commentary:
  ;; Stable API wrappers for cross-layer consumption.
  ;;
  ;; This module:
  ;; - provides forward-safe wrapper functions
  ;; - isolates internal API churn
  ;; - is the only allowed interface for other layers
  ;;
  ;; Design principles:
  ;; - No business logic
  ;; - Thin wrappers only
  ...
  (defun core-api-session-register-job (name fn &rest plist)
    "Stable wrapper to register a session job."
...
#+begin_src emacs-lisp
  ...
  ;; ---------------------------------------------------------------------------
  ;; Keyboard translation (C-h → DEL only)
  ;; Responsibility: physical key translation only
  ;; Does NOT touch help-command or F1
  ;; ---------------------------------------------------------------------------
  ...
  (defvar ui-keyboard--translation-active-p nil
    "Internal state flag indicating whether C-h translation is active.")

  (defun ui-keyboard--apply ()
    "Apply C-h translation according to customization."
    (cond
     ;; Enable
  ...
  ;; ---------------------------------------------------------------------------
  ;; Leader bindings (original code preserved)
  ;; ---------------------------------------------------------------------------
...
#+begin_src emacs-lisp
  ...
  ;; Ensures xref backend preference:
  ;; - Prefer native xref backend from Eglot (when managed)
  ...
  (defun my/dumb-jump-backend ()
    "Fallback dumb-jump backend.
  Only activates when Eglot is NOT managing the current buffer."
...
#+begin_src emacs-lisp
  ...
  ;;; ---------------------------------------------------------------------------
  ;;; Dired view-mode key (opt-in, reversible)
  ;;; ---------------------------------------------------------------------------

  (defgroup utils-dired nil
    "Lightweight Dired helpers."
    :group 'dired)

  (defcustom utils-dired-enable-view-key t
    "If non-nil, bind `z` in Dired to view files in another window.

  The binding opens files using `view-file-other-window`,
  providing a read-only preview workflow.

  This feature is reversible and introduces no global policy."
    :type 'boolean
    :group 'utils-dired)

  (defvar utils-dired--view-key-installed-p nil
    "Non-nil when the Dired view keybinding is installed.")

  (declare-function dired-get-file-for-visit "dired")

  (defun my/dired-view-file-other-window ()
    "View file at point in another window using `view-mode'.

  Directories are opened via `dired-other-window`."
    (interactive)
  ...
  (defun utils-dired--install-view-key ()
    "Install the Dired view keybinding."
    (unless utils-dired--view-key-installed-p
      (define-key dired-mode-map (kbd "z")
                  #'my/dired-view-file-other-window)
      (setq utils-dired--view-key-installed-p t)))

  (defun utils-dired--remove-view-key ()
    "Remove the Dired view keybinding."
    (when utils-dired--view-key-installed-p
      (define-key dired-mode-map (kbd "z") nil)
      (setq utils-dired--view-key-installed-p nil)))

  (defun utils-dired--apply-view-key ()
    "Apply keybinding state according to customization."
    (if utils-dired-enable-view-key
        (utils-dired--install-view-key)
      (utils-dired--remove-view-key)))

  ;; Apply once after Dired loads
  (with-eval-after-load 'dired
    (utils-dired--apply-view-key))

  ;; React dynamically to customization changes
  (add-variable-watcher
   'utils-dired-enable-view-key
   (lambda (_sym _newval _op _where)
     (utils-dired--apply-view-key)))
...
#+begin_src emacs-lisp
  ...
  ;; Depends ONLY on core-api (never core-session directly).
  ;;
  ;;; Code:

  (require 'subr-x)

  (defgroup utils-buffers nil
    "Automatic cleanup of temporary and dead-process buffers."
    :group 'utils)
  ...
  (defcustom utils-buffers-job-kind 'periodic
    "Scheduler lane used by core-session via core-api."
    :type '(choice (const periodic) (const idle))
    :group 'utils-buffers)
  ...
  (with-eval-after-load 'core-api
    (when (fboundp 'core-api-session-register-job)
      (core-api-session-register-job
       'utils-buffers-cleanup
       #'utils-buffers-cleanup
       :kind utils-buffers-job-kind
       :enabled t)))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Commentary:
  ;; Secure org-agenda-files cache using JSON (data-only, non-evaluating).
  ;;
  ;;; Code:
  ...
  (defcustom utils-org-agenda-cache-file
    (expand-file-name "org-agenda-files.cache"
                      (or (bound-and-true-p no-littering-var-directory)
                          user-emacs-directory))
    "Cache file for org-agenda-files (JSON format)."
    :type 'file
  ...
  (defun utils-org-agenda--valid-org-directory-p ()
    (and (boundp 'org-directory)
         ...
         (file-readable-p org-directory)))

  (defun utils-org-agenda--scan (dir)
    (when (file-directory-p dir)
      (seq-filter
       (lambda (file)
         (and (string-match-p "\\.org\\'" file)
       ...
       (directory-files-recursively dir "\\.org\\'" nil nil))))
    ...
    (when (file-readable-p utils-org-agenda-cache-file)
      (condition-case nil
          (with-temp-buffer
            ...
            (let ((data (json-parse-buffer :array-type 'list)))
              (when (and (listp data)
                         (seq-every-p #'stringp data))
                data)))
        (error nil))))

  (defun utils-org-agenda--save-cache (files)
    (let ((dir (file-name-directory utils-org-agenda-cache-file)))
      (unless (file-directory-p dir)
        (make-directory dir t)))
  ...
  If FORCE is non-nil, rebuild the cache."
    (when (utils-org-agenda--valid-org-directory-p)
