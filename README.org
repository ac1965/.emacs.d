...
| =utils/utils-notes-markdown.el=  | Prose-oriented Markdown notes under =my:d:org/notes=; consult-notes, rg, org-download glue.|

**** Directory tree (my:d:org)

The Org knowledge base and Markdown notes share a single root directory,
but are separated by semantics and tooling.

#+begin_example
  my:d:org/
  ├─ agenda/          ; Org agenda files (tasks, schedules)
  ├─ projects/        ; Long-lived Org project files
  ├─ journal/         ; org-journal entries
  ├─ roam/            ; org-roam graph (if enabled)
  ├─ notes/           ; Markdown prose notes (NOT part of org-agenda)
  │  ├─ inbox/
  │  ├─ work/
  │  ├─ life/
  │  ├─ ideas/
  │  └─ archive/
  └─ assets/          ; Shared attachments (images, files)
#+end_example

The =notes/= subtree is intentionally excluded from =org-agenda-files=
and is managed by =utils/utils-notes-markdown.el=.
...
**** Key settings (reference)
- *Loader*
  - =my:modules-verbose= :: non-nil → per-module timing + summary
