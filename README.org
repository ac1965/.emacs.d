...
#+begin_src emacs-lisp
  ;;; core/fixes.el --- Workarounds for Emacs 30.1 internal bugs -*- lexical-binding: t; -*-
  ;;
  ;; Fixes:
  ;;   - void-variable thisfile
  ;;   - void-variable f
  ;;
  ;; Safe for Emacs 30.2+ (no effect there).
  ;;
  ;;; Code:
  ...
  (leaf core-fixes
    :straight nil
    :init
    ;; --- Do NOT delay this; must run before any after-load hooks fire.
    (advice-remove 'do-after-load-evaluation #'my:fix-load-eval-bug)

    (defun my:fix-load-eval-bug (orig-fn file)
      "Fix unbound `thisfile`/`f` in Emacs 30.1 `do-after-load-evaluation`."
      (condition-case err
          ;; ensure both vars exist even if not bound
          (let ((thisfile (if (boundp 'thisfile) thisfile file))
                (f (if (boundp 'f) f nil)))
            (funcall orig-fn file))
        (void-variable
         ;; fallback: safe rebinding
         (let ((thisfile file)
               (f nil))
           (message "⚠️ [core/fixes] caught void-variable in after-load for %s" file)
           (funcall orig-fn file)))
        (error
         (message "⚠️ [core/fixes] suppressed internal after-load error: %S" err)
         (let ((thisfile file)
               (f nil))
           (ignore-errors (funcall orig-fn file))))))

    (advice-add 'do-after-load-evaluation :around #'my:fix-load-eval-bug)
    (message "[core/fixes] Loaded early: Emacs 30.1 after-load patch active."))
