...
#+begin_src emacs-lisp
  ...
  ;; - Availability detection
  ;; - Explicit major-mode remapping
  ;; - Grammar source management
  ;; - A single explicit and directory-safe grammar installation API
  ;;
  ;; Design rules:
  ;; - Grammar installation is NEVER implicit
  ;; - No installation during init / byte-compile / native-comp
  ;; - Installation directory is defined in early-init.el
  ;; - Internal Tree-sitter caches are cleared explicitly
  ;;
  ;;; Code:
  ...
  (defcustom core-treesit-auto-install-p nil
    "Allow explicit Tree-sitter grammar installation via core API.

  When nil, grammar installation is completely disabled."
    :type 'boolean
    :group 'core-treesit)

  ;; Disable Emacs built-in implicit auto-install (Emacs 30+)
  ...
  (defun core-treesit--apply-remap ()
    "Apply explicit Tree-sitter major-mode remapping."
  ...
  (defvar core-treesit--language-sources
    ...
    "Tree-sitter grammar sources managed by core.")

  ;; ---------------------------------------------------------------------------
  ;; Explicit grammar installation API
  ;; ---------------------------------------------------------------------------

  (defun core-treesit-install-grammar (lang)
    "Explicitly install Tree-sitter grammar for LANG.

  This is the ONLY supported entry point for grammar installation.

  Guarantees:
  - No implicit installation
  - Installation directory is forced to `my:d:treesit`
  - Internal Tree-sitter install directory cache is cleared"
    (interactive
     (list
      (intern
       (completing-read
        "Language: "
        (mapcar #'car core-treesit--language-sources)
        nil t))))
    (unless core-treesit-auto-install-p
      (user-error "Tree-sitter auto-install is disabled"))

    (unless (assoc lang core-treesit--language-sources)
      (user-error "Unknown Tree-sitter language: %s" lang))

    ;; Register grammar source
    (add-to-list 'treesit-language-source-alist
                 (assoc lang core-treesit--language-sources))

    ;; CRITICAL: clear internal install dir cache (Emacs 29–31)
    (when (boundp 'treesit--install-dir-cache)
      (setq treesit--install-dir-cache nil))

    ;; Force install directory
    (let ((treesit-install-dir my:d:treesit))
      (treesit-install-language-grammar lang)))
