...
#+begin_src emacs-lisp
  ...
  (leaf core-fixes
    :straight nil
    :init
    ;; 1) Define `thisfile` as a global dynamic variable.
    ;;    This permanently prevents "void-variable thisfile" errors in Emacs 30.1.
    (defvar thisfile nil
      "Dynamically bound to the file currently being loaded.
  Hard-defined to avoid `void-variable` errors on Emacs 30.1.")

    ;; 2) Ensure `thisfile` is always locally bound during `load`.
    ;;    This wraps `load` so that any file being loaded has a valid `thisfile`.
    ...
    ;; 3) Also bind `thisfile` during `require`.
    ;;    When the file name is not explicitly given, it falls back to `locate-library`.
    ...
    ;; 4) Final safeguard: even if `do-after-load-evaluation` misbehaves,
    ;;    make sure `thisfile` is always locally bound when it runs.
    (ignore-errors
    ...
    (defun my:guard-after-load (orig-fn file)
      (let ((thisfile (or (and (boundp 'thisfile) thisfile) file)))
        (condition-case _
          ...
          (void-variable
           ;; Last-resort recovery: rebind `thisfile` and retry safely.
           (let ((thisfile file)) (ignore-errors (funcall orig-fn file)))))))
