...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  (when (native-comp-available-p)
    (setq native-comp-async-report-warnings-errors 'silent
          ...
          (let ((e (getenv "EMACS_NATIVE_COMP_JOBS")))
            (if (and e (string-match-p "\\`[0-9]+\\'" e)) (string-to-number e) 4))
          native-comp-speed 2))

  ;; Ensure ELN dir exists before redirect
  ...
  (setq package-enable-at-startup nil
        package-user-dir my:d:elpa) ;; unify under .var/elpa/
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; Unbind disruptive or unused default keys
  (keymap-set global-map "C-z" nil)      ;; suspend-frame: often accidental
  (keymap-set global-map "C-x C-z" nil)  ;; suspend-frame alias
  (keymap-set global-map "M-z" nil)      ;; zap-to-char: destructive by default
  (keymap-set global-map "M-m" nil)      ;; back-to-indentation: rarely used, free key
  (keymap-set global-map "M-/" nil)      ;; dabbrev-expand: replaced by modern completion

  (eval-and-compile
    ;; Configure archives once; init.el is the single package init point.
    ...
    ;; Bootstrap leaf without use-package dependency
    (unless (package-installed-p 'leaf)
      (package-refresh-contents)
      (ignore-errors (package-install 'leaf)))

    (eval-when-compile (require 'leaf))
    (leaf leaf-keywords :ensure t
      :init (leaf blackout :ensure t)   ;; provides modeline hiding integrations
      :config (leaf-keywords-init))

    (leaf leaf-convert :ensure t))      ;; optional: migrate use-package -> leaf
  ...
  ;; Load README.org via Org Babel
  (let ((init-org-file (expand-file-name "README.org" my:d)))
    (when (file-exists-p init-org-file)
      ;; 1) Declare as special before Org defines defcustom (avoid lexical clash).
      (defvar org-confirm-babel-evaluate)

      ;; 2) Load Org and ob-tangle explicitly; autoload may be unavailable.
      (require 'org)         ;; safe: we only do this when README.org exists
      (require 'ob-tangle)   ;; defines `org-babel-load-file`

      ;; 3) Non-interactive tangle/load without prompts.
...
#+begin_src emacs-lisp :tangle README.el
  ...
  ;; $Lastupdate: 2025/09/01 10:00:31 $
