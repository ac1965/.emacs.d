...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  ;;
  ;;; Commentary:
  ;; Early startup optimizations + directory/layout setup for Emacs 30+.
  ;; Consolidates `my/ensure-directory-exists` and my:d* directory policy here.
  ;; Keeps package.el dormant; straight.el + leaf are bootstrapped in init.el.
  ;;
  ;;; Code:

  ;; Keep package.el dormant (straight handles packages).
  (setopt package-enable-at-startup nil)

  ;; We use `string-trim` / `when-let` at compile time.
  (eval-when-compile (require 'subr-x))

  ;;;; 0) GC & file-name-handler optimizations during startup --------------------
  ...
  ;;;; 1) Native compilation (defensive) -----------------------------------------
  (when (featurep 'native-compile)
    ;; Be quiet unless something is really wrong.
    (setopt native-comp-async-report-warnings-errors 'silent))

  ;;;; 2) Directory layout policy (moved from init.el) ---------------------------
  ...
  ;; Prefer the directory of early-init.el / init.el if available; otherwise fallback.
  (defvar my:d
  ...
  (defvar my:d:treesit-build
    (expand-file-name "tree-sitter-build" my:d:var))
  (defvar my:f:custom  (expand-file-name "custom.el"    my:d:etc))

  ;; Ensure directories exist as early as possible.
  (dolist (dir (list my:d:etc my:d:var my:d:lisp my:d:treesit))
    (my/ensure-directory-exists dir))

  ;; Keep ~/.emacs.d clean and stop the legacy auto-save-list dir creation.
  (setq user-emacs-directory my:d)
  (setq auto-save-list-file-prefix nil)   ;; don't create auto-save-list/

  ;; Tree-sitter dynamic module search path (if Emacs provides the var).
  (when (boundp 'treesit-extra-load-path)
    (add-to-list 'treesit-extra-load-path my:d:treesit))

  ;; straight.el: place all artifacts under .var/ (cache-like location).
  (setopt straight-base-dir my:d:var
          straight-use-package-by-default t
          straight-profiles '((nil . "default.el")))

  ;;;; 3) macOS + Homebrew libgccjit (optional) ----------------------------------
  (when (and (eq system-type 'darwin) (executable-find "brew"))
    (condition-case err
        (let* ((prefix (string-trim (shell-command-to-string "brew --prefix")))
               (gccjit (expand-file-name "lib/gcc/current" prefix)))
          (when (file-directory-p gccjit)
            (setenv "LIBRARY_PATH"
                    (concat gccjit
  ...
  ;;;; 4) Early UI defaults (optional; keep minimal) -----------------------------
  ...
  ;; For Emacs 29 (some window managers): undecorated frames.
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; License: GNU GPL v3 or later
  ;;
  ;; $Lastupdate: 2025/10/10 17:50:22 $
  ;;
  ;;; Commentary:
  ...
  ;; Tiny utilities to safely ensure packages with straight.el.
  (defun my/ensure-straight-packages (packages &optional quiet)
    "Ensure PACKAGES are installed via straight.el.
  PACKAGES is a list of package symbols or straight recipes.
  If QUIET is non-nil, suppress progress messages.
  Return an alist like ((PKG . :ok) (PKG . :error))."
    (unless (featurep 'straight)
      ;; Attempt to load straight without failing hard in non-straight setups.
      (require 'straight nil :noerror))
    (let ((report '()))
      (dolist (pkg packages (nreverse report))
        (condition-case err
            (progn
              (when (fboundp 'straight-use-package)
                (straight-use-package pkg))
              (push (cons pkg :ok) report)
              (unless quiet
                (message "[straight] OK: %s" pkg)))
          (error
           (push (cons pkg :error) report)
           (unless quiet
             (message "[straight] ERROR: %s -> %s"
                      pkg (error-message-string err))))))))

  (defcustom my:bootstrap-packages
    '(leaf leaf-keywords leaf-convert blackout org)
    "Default package set ensured by `my/ensure-bootstrap'."
    :group 'my
    :type '(repeat sexp))

  (defun my/ensure-bootstrap (&optional quiet)
    "Ensure `my:bootstrap-packages' via straight.el.
  If QUIET is non-nil, suppress progress messages.
  When called interactively, print a compact summary."
    (interactive "P")
    (let ((result (my/ensure-straight-packages my:bootstrap-packages quiet)))
      (when (called-interactively-p 'any)
        (message "Bootstrap result: %S" result))
      result))

  ;; Keep ~/.emacs.d clean with no-littering and stop auto-save-list dir creation
  (setq user-emacs-directory my:d)
  (setq auto-save-list-file-prefix nil)   ;; don't create auto-save-list/

  ;; Tree-sitter extra search path
  (when (boundp 'treesit-extra-load-path)
    (add-to-list 'treesit-extra-load-path my:d:treesit))
  ...
  (require 'org) ;; keep org available early

  ;;;; Customization group --------------------------------------------------------
  (defgroup my:lsp-modules nil
    "User customizations."
    :group 'convenience)
  ...
  (leaf cus-edit
    ...
    :custom `((custom-file . my:f:custom)))
...
#+begin_src emacs-lisp :tangle lisp/core/general.el
  ...
  ;;;; Auto-save (visited-mode) & backups ----------------------------------------
  ;; Backups go into hidden .var/; classic #...# auto-save settings are removed.
  (leaf files
    :straight nil
    :custom
...
*** ui/ui-modules.el
:PROPERTIES:
:CUSTOM_ID: ui-bundles-switch
:END:
This optional shim delegates UI setup to a bundle if you have one installed. Put this module **before** other UI modules (theme/modeline/etc.).
- =my:use:modules= can be ='none=, ='doom=, or ='nano=.
- Each bundle is expected to =provide= its feature: =(provide 'doom-modules)= or =(provide 'nano-modules)=.

#+begin_src emacs-lisp :tangle lisp/ui/ui-modules.el
  ;;; ui/ui-modules.el --- UI bundle switcher -*- lexical-binding: t; -*-
  ;;; Commentary:
  ;; Load a UI bundle (doom-modules or nano-modules) if requested and available.
  ;; Safe: does nothing if bundle isn't installed.
  ;;; Code:

  (eval-when-compile (require 'subr-x)) ;; when-let

  (defgroup my:ui-modules nil
    "UI bundle switcher."
    :group 'convenience)

  (defcustom my:use:modules 'none
    "Which UI bundle to use. One of: 'none, 'doom, 'nano."
    :type '(choice (const none) (const doom) (const nano))
    :group 'my:ui-modules)

  (defun my/require-soft (feature)
    "Try to (require FEATURE) quietly; return non-nil on success."
    (when feature
      (condition-case err
          (progn (require feature) t)
        (error (message "[ui-modules] Missing feature: %s. Ensure it is on `load-path' and `(provide '%s)'."
                        feature feature)
               nil))))

  (let ((ok
         (pcase my:use:modules
           ('doom (my/require-soft 'ui/ui-doom-modeline))
           ('nano (my/require-soft 'ui/ui-nano-modeline))
           (_     nil))))
    (when ok
      (message "[ui-modules] Loaded bundle: %s" my:use:modules)))

  (provide 'ui/ui-modules)
  ;;; ui/ui-modules.el ends here
#+end_src
