...
#+begin_src emacs-lisp
  ...
  (defvar my:bootstrap-packages-base
    ...
    "Base packages you’ll want to install in any environment.")
...
#+begin_src emacs-lisp
  ...
  ;; Use hooks or after-load to bind keys under `my:local-leader-key` for specific modes.

  ;; Example: EWW (web browsing) mode local leader keys
  (with-eval-after-load 'eww
    (define-key eww-mode-map (kbd "q") 'quit-window)  ;; for reference, not under leader
    ;; Local leader bindings in eww-mode:
    (define-key eww-mode-map (kbd "C-c SPC m s") #'my/eww-search)       ;; search query
    (define-key eww-mode-map (kbd "C-c SPC m o") #'eww-open-file)       ;; open local file in EWW
    (define-key eww-mode-map (kbd "C-c SPC m b") #'eww-list-bookmarks)  ;; list bookmarks
    (define-key eww-mode-map (kbd "C-c SPC m r") #'eww-readable)        ;; toggle readability view
    (define-key eww-mode-map (kbd "C-c SPC m u") #'my/eww-toggle-images) ;; toggle images on/off
    ;; (Note: `eww` (open URL) is generally a global action, so we don't bind it as local-only.)
  )

  ;; (Additional mode-specific bindings can be added similarly, e.g., for org-mode or programming modes.)
