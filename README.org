...
#+begin_src emacs-lisp
  ...
  ;; $Lastupdate: 2025/01/13 11:09:39 $
...
#+begin_src emacs-lisp
  ...
  ;; macOS-specific integration for environment variables
  ...
  ;; Organize Emacs configuration and cache files
  (leaf no-littering
    :ensure t
    :require t)

  ;; Load custom settings from a dedicated custom.el file
  (leaf cus-edit
    :custom `((custom-file . ,(concat no-littering-etc-directory "custom.el")))
    :config (ignore-errors (load custom-file)))

  ;; Save and restore desktop sessions and window layouts
  (leaf *desktop
    :custom
  ...
  ;; Enable structured editing with puni (handles parentheses, quotes, etc.)
  (leaf puni
    :ensure t
  ...
  ;; Efficient window management with ace-window
  (leaf ace-window
    :ensure t
  ...
  ;; Enable Tree-sitter for better syntax highlighting and parsing
  (leaf tree-sitter
    :ensure t
  ...
  (leaf tree-sitter-langs
    :ensure t
    :config (tree-sitter-langs-install-grammars))
#+end_src
