...
#+begin_src emacs-lisp
  ...
  ;;   magit-status、vterm/eshell/term/comint）に入ったら ASCII へ切り替え、
  ;;   `RET'/`g'/`d'/`s' などの操作キーが誤爆しないようにする。そのバッファから
  ;;   離れたら、入る直前の入力ソースへ自動復帰する。
  ...
  are covered too.  Dirvish and `wdired-mode' need no separate entry: both are
  built on top of `dired-mode' and are therefore already covered."
    :type '(repeat symbol)
    :group 'utils-ime)
