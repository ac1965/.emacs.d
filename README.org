...
Goals: faster startup, clean directory layout, native-comp tuning, and minimal UI.

- Disable `package.el` → use `straight.el` exclusively
- Raise GC thresholds during startup → restore afterwards
- Temporarily disable file-name handlers → restore afterwards
- Create standard directories: `.cache/`, `.etc/`, `.var/`, `lisp/`, `eln-cache/`, `tree-sitter/`
- Configure Native Compilation (silent async, cache under `.cache/eln-cache/`)
- On macOS, automatically set Homebrew `libgccjit` path
- Minimal UI (disable menu/tool/scroll bars after startup to avoid flicker)
- Initialize straight.el in `.cache/` with defaults
- Provide `early-init` feature for external `require`
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; $Lastupdate: 2025/09/26 14:16:38 $
