...
#+begin_src emacs-lisp
  ...
  ;; mode-line（`dirvish-mode-line-format'、core/core-editing.el の leaf dirvish
  ;; :custom で :right に `ime' を追加済み）を持つため、ウィンドウ最大化時など
  ;; 見た目上の手がかりが少ない状況では dirvish バッファに入った瞬間 IME 状態が
  ;; 分からなくなっていた。
  ;;
...
そのまま Python の =set= として持ち、 =/= verbatim と ~code~ を先に確定させて
保護領域とし、 *bold* はそれと重ならない候補だけを検査します。 =*scratch*= の
ように verbatim の内側にある =*= を誤って bold 候補と見なさないよう、直前・直後が
=/~ の場合は除外します（README.org 側の自動修正スクリプトと同じ判定規則です）。
...
#+begin_example
$ python3 scripts/check_emphasis.py README.org design_spec.org
...
複数ファイルを一度に検査できます。 =make check-emphasis= は =$(ORG)= （ =README.org= ）
のみを渡しますが、 =design_spec.org= も含めて検査したい場合は直接呼び出してください。
