...
#+begin_src emacs-lisp
  ...
  ;;
  ;; 注意: `ui-leader--bind-root'（ui-leader モジュール）と
  ;; `utils-ime--install-leader'（utils-ime モジュール、IME切替でリーダーを
  ;; 包む処理）は、いずれも本ファイル（personal オーバーレイ）より前に
  ;; ロードされ、その時点の既定値 "M-SPC" で global-map に一度結線済みである。
  ;; そのため `ui-leader-key' を setopt するだけでは実際のキーバインドは
  ;; 変わらない。旧バインドを明示的に解除してから新キーへ張り直す必要がある。
