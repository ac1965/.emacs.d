...
|Property|Value|
|--------|-----|
|Commit|34b582301f6849374caf2058d8cd45400735ae0e|
|Branch|master|
|System|aarch64-apple-darwin25.4.0|
|Date|2026-04-10 20:03:05 (JST)|
...
#+begin_src emacs-lisp
  ...
  (defun bootstrap--safe-load-file (file &optional noerror)
    "Load FILE safely, reporting but not raising errors by default.
  When NOERROR is non-nil, emit a message instead of signaling."
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 1. URL state directory layout
  ;;;    Must be configured before `url' is required anywhere.
  ;;; -------------------------------------------------------------------------
  ...
  (defvar core--url-state-dir
    ...
    "Root directory for url.el runtime state files.")
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 2. Bootstrap straight.el — deterministic package management
  ;;; -------------------------------------------------------------------------
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 3. leaf and leaf-keywords bootstrap
  ;;; -------------------------------------------------------------------------

  (dolist (pkg '(leaf leaf-keywords))
    (straight-use-package pkg))

  ;; `eval-and-compile' makes leaf macros available during byte-compilation
  ;; and during interpreted evaluation when init.el is not pre-compiled.
  (eval-and-compile
    (require 'leaf))
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 4. Org bootstrap
  ;;;    straight.el must manage Org to avoid version conflicts with built-ins.
  ;;; -------------------------------------------------------------------------
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 5. macOS login environment import (GUI and daemon sessions only)
  ;;; -------------------------------------------------------------------------
  ...
  ;;;    Expand thresholds during init, restore them when idle after startup.
  ;;; -------------------------------------------------------------------------

  (defconst core-default-read-process-output-max (* 4 1024 1024)
    "Enlarged process read buffer applied during Emacs startup.")

  (defvar core--orig-read-process-output-max
    (and (boundp 'read-process-output-max) read-process-output-max)
    "Original value of `read-process-output-max', saved before startup expansion.")
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 7. Core built-in policy settings
  ;;;    Configures UI-neutral defaults only.  No feature activation here.
  ;;;    Visual settings (themes, fonts, modeline) belong in the ui/ layer.
  ;;; -------------------------------------------------------------------------
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 8. Personal overlay (optional, isolated via safe-load)
  ;;;
  ;;;    my:modules-extra is declared here, before any personal files load,
  ;;;    so that personal/user.el may safely extend it via setq/append.
  ;;;    The defvar in modules.el is a no-op once this binding exists.
  ;;; -------------------------------------------------------------------------

  (defvar my:modules-extra nil
    "Extra module list appended by personal overlay files.")
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 9. Modules entrypoint — authoritative feature activation
  ;;; -------------------------------------------------------------------------
  ...
  ;;; -------------------------------------------------------------------------
  ;;; 10. Startup metrics (diagnostic, emitted after the event loop settles)
  ;;; -------------------------------------------------------------------------

  (defun core--announce-startup ()
    "Report startup elapsed time and GC count."
    (message "Emacs ready in %.2f seconds with %d GCs."
             (float-time (time-subtract after-init-time before-init-time))
             gcs-done))

  (defun core--schedule-startup-announcement ()
    "Schedule `core--announce-startup' after the event loop becomes idle."
