...
header-line globally and suppress the mode-line again."
  ...
  ;; Bind after after-init-hook so ui-keymap--startup-init has already run
  ;; ui-leader--define-maps, guaranteeing ui-leader-t-map exists as a keymap.
  ;; Depth 10 places this after ui-keymap--startup-init (depth 0 / default).
...
Music.app is not available or the playlist list is empty."
  ...
  ;; Define the hydra and bind it after init so hydra is loaded.
  ;; Depth 10 runs after ui-keymap--startup-init (depth 0), which means
  ;; ui-leader-a-map is already a live keymap when ui-keymap--bind is called.
  (add-hook 'after-init-hook
            (lambda ()
