...
#+begin_src emacs-lisp :tangle early-init.el
  ...
  ;; Keep frame/GUI decisions here to avoid flicker and relayouts.
  ;; a) Frame sizing & layout policy (must be set before first frame)
  ...
  ;; b) Default/initial frame parameters
  ;;    - Put common shape into default-frame-alist
  ;;    - Mirror for initial-frame-alist if you want the *very first* frame same.
  ...
  ;; c) Disable GUI bars from the very beginning (avoid flicker)
  (menu-bar-mode -1)
  (tool-bar-mode -1)
  (scroll-bar-mode -1)
...
#+begin_src emacs-lisp :tangle init.el
  ...
  ;; Must be set BEFORE url.el is loaded so defaults pick these locations.
  (setopt
   ;; url/ general config/root directory (Emacs uses this for several files)
   url-configuration-directory my:d:url
   ;; Explicit cookie/history file paths (safer across Emacs versions)
   url-cookie-file  (expand-file-name "cookies" my:d:url)
   url-history-file (expand-file-name "history" my:d:url)
   url-cache-directory (expand-file-name "cache" my:d:url))
  ...
  (leaf emacs
    :straight nil
    :doc "Core Emacs built-ins: startup suppression, initial buffers, font-lock caps."
    :init
    ;; Startup experience (safe to apply after early frame setup)
    ...
    ;; Font-lock caps; keep nil if you intentionally remove caps.
    (setopt font-lock-maximum-decoration nil
            font-lock-maximum-size nil)

    ;; Frame title: t → use default (buffer name etc.). Set here, not in early-init.
    (setopt frame-title-format t)
    ...
    :config
    ;; Window divider: values were set in early-init; turn the mode on here.
    (when (fboundp 'window-divider-mode)
      (window-divider-mode 1))

    ;; If you still want menu/scroll/tool bars off (double down, harmless):
  ...
  ;; Dired: display/listing settings (GNU ls integration)
  (leaf dired :straight nil
    :config
    (cond
     ;; Prefer GNU ls (gls) on macOS for rich listing options
...
#+begin_src emacs-lisp :tangle lisp/ui/ui-nano-modeline.el
  ...
  ;;     - Compatibility aliases for legacy callers (my/ui-enable-nano, etc.)
  ;;
  ;; Usage:
  ...
  ;;
  ;; Notes:
  ;;   - Face settings avoid :custom to prevent face-defface-spec pitfalls.
  ;;   - LSP/Eglot detection degrades gracefully when packages are absent.
  ;;   - Clock/Battery update every 60s via a lightweight timer (cancel on disable).
  ;;
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  (defcustom my/ui-nano-center-max 44
    "Max length for the center segment; longer text is truncated with ellipsis."
    :type 'integer
    :group 'my/ui-nano-modeline)

  ;; New toggles (LSP/clock/battery)
  ...
  (defcustom my/ui-nano-time-format "%H:%M"
    "Clock format string for `format-time-string`."
    :type 'string
    :group 'my/ui-nano-modeline)

  (defcustom my/ui-nano-show-battery t
    "If non-nil, show battery summary on the right (if supported)."
    :type 'boolean
    :group 'my/ui-nano-modeline)
  ...
  (defun my/ui-nano--lsp-eglot-status ()
    "Return a compact LSP/Eglot status string (OK when connected)."
    (when my/ui-nano-show-lsp
      (cond
       ...
       ;; lsp-mode
       ((featurep 'lsp-mode)
        (let* ((ws (and (boundp 'lsp--workspace-status-string)
                        (boundp 'lsp-mode)
  ...
  (defun my/ui-nano--battery ()
    ...
    (when (and my/ui-nano-show-battery (fboundp 'battery))
      (let* ((al (ignore-errors (battery)))
             ...
             (sym (cond
                   ;; Normalize states across OSes
                   ((member state '("charging" "AC" "AC Power")) (my/ui-nano--icon "battery_charging_full" "⚡" 'my/ui-nano-ok))
                   ((member state '("discharging" "Battery"))    (my/ui-nano--icon "battery_std" "🔋" 'my/ui-nano-face))
  ...
  (defun my/ui-nano--format ()
    "Return a format list usable for `mode-line-format`/`header-line-format`."
    ;; Each segment is computed inside its own :eval so that no free vars leak.
    (list
     ;; Left
     '(:eval
     ...
     ;; Center (true centering using align-to + measured width)
     '(:eval
     ...
     ;; Split filler to push the rest to the right margin
     "%="
  ...
  ;;;; Minor mode + periodic refresh timer ---------------------------------------
  ...
  (defun my/ui-nano--start-timer ()
    "Start a lightweight timer to refresh the modeline every 60 seconds."
  ...
  (defun my/ui-nano--uninstall ()
    ...
    ;; Clear saved copies to avoid stale state
  ...
  (define-minor-mode my/ui-nano-modeline-mode
    ...
    (if my/ui-nano-modeline-mode
        (progn
          ;; Avoid double composition with upstream nano-modeline
          ...
          ;; Start periodic refresh for clock/battery
  ...
  (defun my/ui-nano-modeline-toggle-placement ()
    ...
    (if my/ui-nano-modeline-mode
        (progn
          ;; Re-install to reflect new placement
          (dolist (buf (buffer-list))
            (with-current-buffer buf
              (my/ui-nano--uninstall)
              (my/ui-nano--install)))
  ...
  ;;; Some older modules expect these entry points:
  ;;; - my/ui-enable-nano
  ;;; - my/ui-disable-nano
  ;;; - my/ui-nano-modeline-mode-enable
  ;;; - my/ui-nano-modeline-mode-disable
