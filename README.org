...
#+begin_src emacs-lisp
  ...
  (dolist (it '((internal-border-width . 8)
    ...
    (push it initial-frame-alist))

  (setq default-frame-alist (assq-delete-all 'fullscreen default-frame-alist)
        initial-frame-alist (assq-delete-all 'fullscreen initial-frame-alist))
...
#+begin_src emacs-lisp
  ...
  (defun my/delete-old-backups ()
    ...
    (let ((backup-dir (concat (or (bound-and-true-p my:d:var)
                                  no-littering-var-directory)
                              "backup/"))
          (threshold (- (float-time) (* 7 24 60 60))))
...
** 2026-05 (thirty-fifth evaluation pass)
*** Fix BP: Six efficiency and maintainability refactors (no behaviour change)

🟠 Medium — efficiency / dead-code / duplication

- *[Fix BP-1] lisp/modules.el (=my/modules-load=)*: Replaced two sequential
  =seq-remove= passes (O(n)×2) with a single =seq-group-by #'modules--should-load-p=
  call.  The resulting property list is destructured with =assq= to obtain =final=
  and =skipped= lists in one traversal.

- *[Fix BP-2] lisp/modules.el (=modules--format-seconds=)*: Merged the
  sub-millisecond branch (=< sec 0.001=, format "=%.3fms=") into the general
  millisecond branch (=< sec 1.0=, format "=%.1fms=") via a two-arm =if=.
  Module load times below 1 ms are rare in practice and "0.1ms" is sufficient
  resolution for the diagnostic log.  Eliminates one dead =cond= arm.

- *[Fix BP-3] =init.el= (section 9 — Modules entrypoint)*: Removed the
  redundant =let*= block that re-computed =lisp-dir= and re-added it to
  =load-path=.  =early-init.el= already adds =my:d:lisp= (identical path) to
  =load-path= unconditionally at step 2.  Reduced to a direct =(require 'modules nil t)=.

- *[Fix BP-4] lisp/core/core-session-private.el (=core-session--jobs=)*:
  Removed the =core-session--jobs= function (dead code).  It allocated an
  intermediate list via =maphash=; it was replaced in Fix AK by direct
  =maphash= iteration in =core-session--run-all=.  No call sites remain.

- *[Fix BP-5] =early-init.el= (section 10 — Early UI defaults)*:
  Replaced two separate =setq default-frame-alist= / =setq initial-frame-alist=
  lines with a single =setq= form setting both variables.  The =dolist= loop
  uses =push= instead of =add-to-list= (=add-to-list= performs an O(n)
  membership check; duplicates cannot arise here because the alist is cleared
  immediately after by the =assq-delete-all= below).

- *[Fix BP-6] lisp/core/core-persistence.el (=my/delete-old-backups=)*:
  Replaced =(float-time (current-time))= with the zero-argument shorthand
  =(float-time)=, which is the canonical idiom since Emacs 25.  The
  =current-time= call is implicit and redundant.
