...
#+begin_src makefile :tangle Makefile :mkdirp no :padline no
  ...
  # している。
  distclean-deps:
	@echo "[distclean-deps] remove straight/ eln-cache .cache .etc .var (package clones & runtime caches)"
	@rm -rf "$(ROOT)/straight" "$(ROOT)/eln-cache" "$(ROOT)/.cache" "$(ROOT)/.etc" "$(ROOT)/.var"
  ...
  help:
	...
	@echo "  make distclean-deps           # straight/ eln-cache .cache .etc .var を削除（破壊的・低頻度用）"
	@echo ""
	@echo "  # 図版（dot / mermaid）"
