...
#+begin_src emacs-lisp
  ...
  (leaf aidermacs
    ...
    :init
    ;; [Fix BZ-2] Retrieve API keys from auth-source (~/.authinfo.gpg) instead
    ;; of getenv.  exec-path-from-shell no longer imports OPENAI_API_KEY or
    ;; OPENROUTER_API_KEY; they are available exclusively via auth-source at
    ;; runtime.  auth-source-search looks for a machine=api.anthropic.com entry
    ;; for the Claude/OpenRouter key and machine=api.openai.com for the OpenAI
    ;; key.  The first non-nil result wins; if neither entry exists, a warning
    ;; is displayed rather than silently misconfiguring the aider process.
    (require 'auth-source)
    (let* ((router-entry (car (auth-source-search
                                :host "openrouter.ai" :require '(:secret))))
           (openai-entry (car (auth-source-search
                                :host "api.openai.com" :require '(:secret))))
      ...
      (cond
       ...
       (t
        (display-warning 'aidermacs
                         "No API key found in auth-source. Add machine openrouter.ai or api.openai.com to ~/.authinfo.gpg."))))
    :config
