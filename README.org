...
|Property|Value|
|--------|-----|
|Commit|66069812be537adb7b1c50c4695f831a055d5b3d|
|Branch|master|
|System|aarch64-apple-darwin25.1.0|
|Date|2025-12-21 13:45:33 (JST)|
...
#+begin_src emacs-lisp
  ...
  ;; - Backups OFF; classic auto-save OFF (use auto-save-visited later)
  ;; - Early UI defaults to avoid flicker
  ;;
  ;;; Code:
  ...
  (defconst my:d:cache     (expand-file-name "emacs/"
  ...
  (dolist (dir (list my:d:var my:d:etc my:d:cache my:d:lisp
                     my:d:treesit my:d:url my:d:eww my:d:eln-cache))
  ...
  (when (eq system-type 'darwin)
    (let* ((brew-prefix
           ...
           (prefix-libgccjit
            (and brew-prefix (expand-file-name "opt/libgccjit" brew-prefix)))
           (prefix-gcc
            (and brew-prefix (expand-file-name "opt/gcc" brew-prefix)))
           (candidates
            (seq-filter
             #'file-directory-p
             (list
              (and prefix-libgccjit
                   (expand-file-name "lib/gcc/current" prefix-libgccjit))
              (and prefix-libgccjit
                   (expand-file-name "lib" prefix-libgccjit))
              (and prefix-gcc
                   (expand-file-name "lib/gcc/current" prefix-gcc))))))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;;; Commentary:
  ;; Entry point for Emacs 30.1+ configuration.
  ;;
  ;; - URL state paths (before url.el)
  ;; - straight.el bootstrap
  ;; - Early import of shell environment (macOS)
  ;; - Performance (GCMH / read-process-output-max)
  ;; - Core built-ins via leaf
  ;; - Personal overlay loader
  ;; - Deterministic module entrypoint
  ;;
  ;;; Code:
  ...
  (dolist (dir (list url-configuration-directory url-cache-directory))
    (when dir
      (make-directory dir t)))

  (require 'url) ;; straight bootstrap depends on url.el
    ...
    (unless (file-exists-p bootstrap-file)
      (condition-case err
          (let ((url-request-extra-headers
                 '(("User-Agent" . "Emacs straight.el bootstrap")))
                (buf nil))
            (setq buf
  ...
  ;;; 1.6) Base package installer helper ---------------------------------------

  (defun my/straight-install (packages)
    "Install PACKAGES via straight-use-package with simple timing/log."
    (let ((t0 (float-time)))
      (dolist (pkg packages)
        (let ((s (float-time)))
          (condition-case err
              (progn
                ...
                (message "OK  %-18s %4.2fs"
                         pkg (- (float-time) s)))
            (error
             (message "ERR %-18s %4.2fs %s"
                      pkg (- (float-time) s)
                      (error-message-string err))))))
  ...
  (defvar my:bootstrap-packages-base
    '(leaf
      ...
      imenu-list
      hydra)
    "Base packages required for core configuration.")
  ...
  (when (boundp 'read-process-output-max)
    (setq read-process-output-max (* 4 1024 1024))) ;; 4 MiB
  ...
  (leaf gcmh
    ...
    :custom
    ((gcmh-idle-delay . 5)
     (gcmh-high-cons-threshold . 16777216)) ;; 16 MiB
    :config
    (gcmh-mode 1))


  ;;; 3) Core built-ins (do NOT duplicate early-init) --------------------------
  ...
  (leaf emacs
    ...
    (dolist (key '("C-z" "C-x C-z" "C-x C-c" "M-z" "M-m"))
      (when (lookup-key (current-global-map) (kbd key))
        (keymap-global-unset key)))
    (setq confirm-kill-emacs #'y-or-n-p))


  ;;; 4) Modifier keys per OS --------------------------------------------------
  ...
  (let* ((root (or (and (boundp 'my:d)
                        (stringp my:d)
                        (file-directory-p my:d)
                        my:d)
                   user-emacs-directory))
         (lisp-dir (expand-file-name "lisp" root)))
    (add-to-list 'load-path lisp-dir)
    (defvar my:modules-extra nil)
    (require 'core/custom-ui-extras)
...
#+begin_src emacs-lisp
  ...
  ;;; SIS (macOS input method auto-switch) -------------------------------------
