...
#+begin_src emacs-lisp
  ...
  ;; nano-modeline: time and buffer encoding in the right segment
  ;;
  ;; Actual API of rougier/nano-modeline (verified from source):
  ;;
  ;;   (defun nano-modeline-prog-mode (&optional default)
  ;;     (funcall nano-modeline-position
  ;;              '((nano-modeline-buffer-status) " " ...)   ; left
  ;;              '((nano-modeline-cursor-position)           ; right
  ;;                (nano-modeline-window-dedicated))
  ;;              default))
  ;;
  ;; nano-modeline-position (= nano-modeline-header) passes the left/right
  ;; lists to nano-modeline--make, which builds header-line-format as a
  ;; (:eval ...) form.  Each element of the right list is evaluated every
  ;; frame via (apply (car el) (cdr el)), so adding a function sexp to the
  ;; right list gives automatic time updates.
  ;;
  ;; Extension strategy: use :override advice on nano-modeline-prog-mode and
  ;; nano-modeline-text-mode to prepend (ac1965--nano-time-encoding) to the
  ;; right list.
  ;;
  ;; Design constraints (cf. README coding rules):
  ;;   - Guarded by with-eval-after-load 'nano-modeline; no-op under
  ;;     my:use-ui 'doom or 'none.
  ;;   - No top-level require of nano-modeline.
  ;;   - All defun carry docstrings.
  ;;   - No defcustom/defvar modified; setopt/setq not needed.

  (defun ac1965--nano-time-encoding ()
    "Return a propertized \"HH:MM  CODING\" string for the nano-modeline right segment.
Called every frame inside nano-modeline--make's :eval block, so the
time updates automatically.  Uses `nano-modeline-face' with the
\\='secondary face role to match the standard right-segment style.
Coding system suffix modifiers (-unix, -dos, -mac, -with-signature) are stripped."
    (let* ((time-str (format-time-string "%H:%M"))
      ...
      (propertize (concat time-str "  " cs-name)
...
Passes the same left segment as the original, with `ac1965--nano-time-encoding'
prepended to the right segment.  DEFAULT has the same meaning as in the original."
    (funcall nano-modeline-position
...
DEFAULT has the same meaning as in the original."
    (funcall nano-modeline-position
             '((nano-modeline-buffer-status) " "
               (nano-modeline-buffer-name) " "
               (nano-modeline-git-info))
  ...
  (with-eval-after-load 'nano-modeline
    ;; Override the per-mode functions.  Using :override rather than
    ;; redefining the functions preserves the ability to remove with
    ;; (advice-remove ...) and keeps the originals intact for inspection.
    ...
    (dolist (buf (buffer-list))
      (with-current-buffer buf
        (cond ((derived-mode-p 'prog-mode) (ac1965--nano-prog-mode))
              ((derived-mode-p 'text-mode) (ac1965--nano-text-mode))))))

  ;; nano-modeline toggle: M-SPC t m
  ;;
  ;; Toggles nano-modeline header-line visibility for the current buffer.
  ;; When active, clears header-line-format (hides the modeline); when
  ;; inactive, re-runs the appropriate nano-modeline mode function to
  ;; restore it.  Bound into ui-leader-t-map ("t" submap = toggles) at
  ;; key "m" (mnemonic: modeline).
  ;;
  ;; ui-leader-t-map is defined by ui-leader.el which loads via my:modules
  ;; before personal/ac1965.el is evaluated, so the map is always present.
  ;; ui-keymap--bind guards against a missing map, matching the pattern
  ;; used throughout ui-keymap.el.

  (defun ac1965/nano-modeline-toggle ()
    "Toggle nano-modeline header-line display for the current buffer.
When the header-line is visible, hides it by setting
`header-line-format' to nil.  When hidden, restores it by calling the
appropriate nano-modeline mode function for the current major mode."
    (interactive)
    (if header-line-format
        (setq-local header-line-format nil)
      (cond
       ((derived-mode-p 'prog-mode) (ac1965--nano-prog-mode))
       ((derived-mode-p 'text-mode) (ac1965--nano-text-mode))
       (t (when (fboundp 'nano-modeline-prog-mode)
