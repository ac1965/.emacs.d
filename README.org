...
#+begin_src emacs-lisp
  ...
  (defun core-native--apply-report-setting (mode)
    ;; [Fix H] native-comp-async-report-warnings-errors is a defcustom.
    ;; Use setopt (not setq) to respect its setter and type validation.
    ;; early-init.el already uses setopt for the same variable; this
    ;; function must remain consistent.
    (pcase mode
      ('silent
...
*** Fix E: ui-theme.el — ui-nano-palette の hard require を optional に変更

*🟠 Medium — UI-replaceable 保証の違反*

- *[Fix E] ui/ui-theme.el*: =(require 'ui-nano-palette)= を
  =(require 'ui-nano-palette nil t)= に変更。
  =ui-theme= は =modules.el= の ui セクションに常時含まれるが、
  =ui-nano-palette= は =my:use-ui='nano= 選択時のみ意味を持つ。
  ハード require は =my:use-ui='doom= や ='none= の場合でも
  nano パレットを強制ロードし、UI-replaceable 設計を破壊していた。

  =ui-theme--apply-faces= はすでに =(fboundp 'my/nano-apply-faces)= で
  ガードされているため、optional require への変更に機能的な後退はない。

  =(with-eval-after-load 'ui-nano-palette ...)= を追加し、
  nano バンドルが後から有効化された場合にフェイスを自動再適用する。

  Commentary を更新し =ui-nano-palette= 依存関係のポリシーを明文化した。

*** Fix F: core-editing.el — Commentary の旧 dired-filter/dired-subtree 記述を削除

*🟠 Medium — Literate Programming「唯一の真実」原則の違反*

- *[Fix F] core/core-editing.el*: Commentary の
  =- extends Dired with dired-filter and dired-subtree= を
  =- replaces dired with dirvish (dirvish-override-dired-mode)= および
  =- configures dirvish attributes (subtree-state, vc-state, git-msg, file-size)=
  に置き換えた。
  =dired-filter → dirvish= 移行は 2026-03 Refactoring で実施済みだったが、
  Commentary が古い実装を参照したままだった。
  README.org は「唯一の真実」であるため、コードと一致しない Commentary は
  設計原則の直接違反となる。

*** Fix G: early-init.el — treesit-* 変数を setopt に統一

*🟡 Minor — setq/setopt 一貫性*

- *[Fix G] early-init.el*: =treesit-install-dir= と =treesit-extra-load-path= の
  設定を =setq= から =setopt= に変更。両変数は Emacs 29+ の =defcustom= であり、
  コーディングルール「=defcustom= 変数には =setopt= を使う」に従う。
  =boundp= ガードにより tree-sitter サポートなしのビルドでも安全。

*** Fix H: core-native.el — native-comp-async-report-warnings-errors を setopt に統一

*🟡 Minor — setq/setopt 一貫性*

- *[Fix H] core/core-native.el*: =core-native--apply-report-setting= 内の
  =native-comp-async-report-warnings-errors= への代入を
  =setq= から =setopt= に変更。
  =early-init.el= では同変数を =setopt= で設定しており、
  内部関数が =setq= で上書きするのは一貫性を欠いていた。
  =defcustom= のセッター関数・型検証を経由させる。

*** Fix I: personal-messages.el — 不要な runtime require を除去

*🟡 Minor — 起動コストと設計意図*

- *[Fix I] personal/personal-messages.el*: トップレベルの
  =(require 'org)= を削除。このファイルは =personal/user.el= の
  =(with-eval-after-load 'org ...)= 経由でロードされるため、
  ロード時点で =org= はすでに存在が保証される。
  冗長な =require= は遅延ロード戦略の意図を損なう。

  =(require 'subr-x)= を =(eval-when-compile (require 'subr-x))= に移動。
  =subr-x= のマクロ（=when-let*=, =string-trim= 等）はバイトコンパイル時に
  展開されるため、ランタイム require は不要。
