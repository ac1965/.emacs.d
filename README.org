...
このリポジトリは、モダンな Emacs 設定を実装したものである。

設定は Org Mode と Babel を用いたリテラルプログラミングシステムとして構成されている。
ソースコードブロックは Emacs Lisp モジュールへと tangle（展開）される。

#+begin_src mermaid :file tangle-flow.svg :exports results
flowchart LR
    ...
    tangle --> ei["early-init.el"]
    tangle --> ii["init.el"]
    tangle --> mo["lisp/modules.el"]
    tangle --> mods["lisp/&lt;layer&gt;/&lt;module&gt;.el\n(89+ files)"]

    style src  fill:#EEEDFE,stroke:#534AB7,color:#3C3489
    style T    fill:#E1F5EE,stroke:#0F6E56,color:#085041
    style ei   fill:#FAEEDA,stroke:#BA7517,color:#633806
    style ii   fill:#FAEEDA,stroke:#BA7517,color:#633806
    style mo   fill:#FAEEDA,stroke:#BA7517,color:#633806
    style mods fill:#FAECE7,stroke:#993C1D,color:#712B13
#+end_src

=README.org= が唯一の真実のソースであり、=.el= ファイルはすべて生成物である。
これにより、ドキュメントとコードの乖離が構造的に不可能になる。

主要な設計目標:

- 決定論的な起動（同一の入力から常に同一の結果）
- 明示的な依存関係管理（暗黙的な自動検出を排除）
- 削除安全なモジュールアーキテクチャ（任意モジュールを除去しても残りが動作する）
- Emacs 31+ との前方互換性
- 長期的な発展を見据えたメンテナビリティ
- LSP バックエンド非依存性（eglot / lsp-mode / lsp-bridge を切り替え可能）
...
この設定は厳格な 10 層アーキテクチャに従う。各層は固有の責務を持ち、
依存関係は一方向にのみ流れる。

#+begin_example
layer 10: personal      （ユーザー固有。他の層を変更しない）
layer  9: utils         （汎用ユーティリティ）
layer  8: dev           （言語・LSP・AI 支援）
...
layer  2: core          （ランタイム基盤 + policy）
layer  1: early-init    （package.el 無効化・GC 最適化）
#+end_example

ロード順序（上から下へ、依存関係は下向きにのみ流れる）:
...
起動は Emacs の標準初期化シーケンスに従い、3 段階で構成される。

*** early-init.el

パッケージ初期化より前に実行されるフェーズ。フレームが描画される前に実行されるため、
ここでの設定はフリッカー（画面のちらつき）を防ぐ。

責務:

- =package.el= の自動初期化を無効化
- GC（ガベージコレクション）とファイルハンドラーを最適化
- ディレクトリレイアウトを設定（=my:d:var=, =my:d:cache=, =my:d:etc=）
- ネイティブコンパイルのパスを準備
- tree-sitter のパスを準備
- 早期 UI デフォルト値を設定（ツールバー非表示等）
...
- =straight.el= をブートストラップ
- macOS ログイン環境変数をインポート
- =leaf= を初期化
- ランタイムパフォーマンス設定を適用
- 個人オーバーレイ（=personal/<user>.el=）をロード
- =modules.el= を起動

*** modules.el

オーケストレーション層。このファイルが「唯一の権威ある起動順序」を定義する。

責務:

- 正規のロード順序を定義（=core → ui → auth → completion → orgx → vcs → dev → utils=）
- モジュール単位のエラー分離（=condition-case= により一つの失敗が他を止めない）
- =my:modules-skip= による任意モジュールの安全なスキップ
- =my:modules-verbose= が非 nil のとき、モジュールごとのロード時間を報告
- 層間の依存方向の逆転を防止
...
|--------------+------|
| =lisp/=      | モジュールソースコード |
| =.var/=      | ランタイム状態（パッケージが書き込むデータ） |
| =.cache/=    | 一時キャッシュ（削除しても再生成される） |
| =.etc/=     | 外部リソース（テーマファイル等） |
...
ランタイムの基盤設定。すべての他の層が依存する最下位の機能層。

責務:

- 環境設定とディレクトリレイアウトの確立
- パス管理と =load-path= の構築
- ネイティブコンパイルと =eln-cache= の設定
- tree-sitter グラマー管理
- セッション永続化と履歴管理
- GC チューニングと起動パフォーマンス

コアモジュールは UI に依存してはならない。

*** policy

グローバル動作ポリシー（core 層の一部としてロードされる）。

責務:

- 空白文字・末尾空白のルール
- インデント標準とタブポリシー
- エディタ安全性制約（=require-final-newline= 等）

Policy モジュールは UI またはパッケージ機能を有効化してはならない。
...
=capf-org-src= ファミリーは協調動作し、Org ソースブロック内で言語固有の補完を提供する。
...
- アジェンダと TODO 設定
- ジャーナリング（org-journal）とノートキャプチャ
- org-roam 知識グラフ統合
- エクスポートバックエンド（ox-hugo, LaTeX/LuaLaTeX）
- ビジュアル強化（org-modern）
- 保存時の自動 tangle
...
- Magit 設定
- ガター差分インジケーター（diff-hl）
- GitHub/GitLab Forge 統合

VCS モジュールは dev 層に依存してはならない。
...
Org Babel + Claude API ワークフロー統合は =personal/personal-ai.el= に存在し、
HTTP トランスポートのために =utils/utils-claude.el= に依存する。

*** utils

層を横断して使用される汎用ユーティリティ。
...
- バッファ管理と ibuffer 設定
- dired と dirvish 統合
- Claude API HTTP クライアント（utils-claude）
...
Org Babel が Emacs Lisp ブロックをモジュールファイルに展開する。

- 各モジュールは =lisp/<layer>/<module>.el= に展開される
- ディレクトリは自動的に作成される（=:mkdirp yes=）
...
=reload= ターゲットは =*.elc= ファイルを削除してから再 tangle する。
「cp を忘れて古い README.org から tangle した」という失敗を防ぐ。
...
起動は毎回同一の結果を生成しなければならない。
=my:modules= リストが唯一の権威ある起動順序を定義する。

*** 明示的な依存関係

モジュールは依存関係を明示的に =require= する。暗黙的な自動検出は行わない。

*** 層の所有権

各層はドメインを所有し、他の層を変更しない。

*** 削除安全性

モジュールを削除してもシステムが壊れてはならない。
=condition-case= によるエラー分離がこれを実現する。

*** 前方互換性

Emacs 31+ での変更を先取りする。
=setopt=、=keymap-set=、=if-let*=、=when-let*= を優先的に使用する。
...
すべてのファイルはレキシカルバインディングを宣言し、=provide= シンボルはファイル名と
一致する。組み込みパッケージは =:straight nil= を使用する。=leaf= キーワード順序を
遵守する。公開 =defun= は docstring を持つ。=defcustom= には =setopt=、=defvar= には
=setq= を使用する。命名規則（=my/=、=<module>-=、=<module>--=、=my:d:*=）を遵守する。
=keymap-set= API（Emacs 29+）を使用する。

詳細は [[#modular-loader-and-core-suite][モジュールローダー & Core スイート]] を参照。

** ライセンス

この設定は GNU General Public License v3 の下でリリースされる。
...
この Emacs 設定は *モジュール設計* で **Emacs 30+** を対象とする。
各層は明確に定義された責務を持ち、動作の予測可能性、
UI の交換可能性、個人カスタマイズの分離を保証する。
...
フィーチャーを有効化する前に Emacs を準備する、クリーンで高速なブートストラップを提供する。
...
インフラが最初に確立され、フィーチャーロジックは後で起動される。
...
個人オーバーレイは意図的にユーザースコープである:

: personal/<login-name>.el

これはバイト単位の再現性よりも分離とマルチユーザー安全性を優先する。
...
**** 深掘り解説: early-init.el の設計

Emacs が =package.el= を初期化する前に実行される。フレームが描画される前に実行されるため、
ここでの設定はフリッカーを防ぐ。起動シーケンスの最初のフェーズとして、インフラ確立
のみを行い、フィーチャーのロードは一切行わない。

**ディレクトリ基盤の確立**

=file-chase-links= はシンボリックリンクを解決する。=~/.emacs.d= がシンボリックリンクで
あっても実体パスを取得できるため、設定が任意のディレクトリに relocatable となる。
=XDG_CACHE_HOME= を優先することで Linux の XDG 規格に準拠する（macOS では
=~/.emacs.d/.cache/emacs/= になる）。=defconst= を使用するのは後から変更されるべき
でないためである。

**straight.el のベースディレクトリ設定**

=straight-build-dir= にバージョン番号を含めることで（=build-31.0= 等）、Emacs を
バージョンアップした際にビルド済みパッケージが混在しない。この設定は *early-init.el で
行う必要がある* — =straight.el= 自身がブートストラップ前にこの変数を参照するから。

**GC チューニング戦略**

起動時に =file-name-handler-alist= を空にするのは重要な最適化である。このリストは
=require= や =load= の度に各エントリと照合されるため、多数のモジュールを読み込む際の
オーバーヘッドが無視できない。TRAMP のハンドラーも含まれるため、=after-init-hook= で
=core--orig-file-name-handler-alist= から復元する（復元を忘れると TRAMP が動作しない）。

後続の =core-runtime.el= で gcmh を使い、アイドル時の GC を管理する二段構えになっている。

**no-littering との互換性確保**

=no-littering.el= はロード時にこれらの変数を参照してパスを設定する。=early-init.el= で
事前に設定しておくことで、=no-littering= が読み込まれた瞬間に正しいパスが使われる。
順序が逆だと =no-littering= がデフォルトの =~/.emacs.d/= 以下にファイルを置いてしまう。

**早期 UI 設定（フリッカー防止）**

=default-frame-alist= と =initial-frame-alist= の両方に設定するのは、最初のフレーム
（initial）とその後のフレーム（default）で異なるパラメータが適用されるからである。
ツールバーを =early-init.el= で無効化しないと、GUI が描画されてから消えるため
視覚的なちらつきが発生する。
...
インフラ（=early-init.el=）を確立した後、フィーチャーのロジックを起動する段階。
責務は明確に分割されており、ブートストラップのみを行い、設定の本体は =modules.el= に委譲する。

**URL 状態ディレクトリ（=url.el= より前に設定）**

=url.el= は初回ロード時にこれらの変数のデフォルト値でファイルを作成しようとする。
=require 'url= の前に変数を設定しなければ、=~/.emacs.d/url/= にファイルが作られてしまう。
順序が重要であり、コメントにも "must precede url load" とある。

**straight.el ブートストラップ**

=(bound-and-true-p straight-base-dir)= を使うのは =early-init.el= で設定した値を
尊重するためである。=boundp= だけでは =nil= にバインドされた変数も =t= を返すので
=bound-and-true-p= が正確。初回起動時のみネットワークにアクセスし、以降はローカルの
=bootstrap.el= をロードするだけである。

**macOS 環境変数のインポート**

macOS の GUI アプリはシェルのプロファイルを読まないため、=PATH= が =/usr/bin:/bin= 程度
しかない。=exec-path-from-shell= はログインシェルを起動して環境変数を取得する。
=(daemonp)= の条件が重要 — =emacs --daemon= で起動した場合もこれが必要。

**起動時間報告**

=before-init-time= は Emacs が起動した時刻、=after-init-time= は =after-init-hook= が
完了した時刻。=gcs-done= は GC 回数のカウンター。この数字が大きい場合は
=early-init.el= の GC 閾値が低すぎる可能性がある。
...
この設定は厳格に層化され、明示的に順序付けられた
モジュールロードアーキテクチャを使用する。

共有動作は一元化されたエントリポイントからのみ有効化される。
モジュールは暗黙的に検出されたり、ディレクトリスキャンでロードされたりすることはない。

目標は決定論的で再現可能かつ検査可能な起動である。
...
**** 深掘り解説: modules.el の設計

中央オーケストレーション層。このファイルの設計が「削除安全性」と「決定論的起動」の
核心を実装する。

**モジュール定義とスキップ機能**

=my:modules-skip= と =my:modules-extra= を =defcustom= にすることで、=customize= UI
から設定でき、=personal/<user>.el= から =setopt= で上書きできる。
=my:modules-skip= に ='(dev-docker dev-music)= を追加すれば Docker/音楽関連モジュールを
スキップできる。=my:modules-extra= でパーソナルモジュールを末尾に追加できる。

**エラー分離ローディング（削除安全性の核心実装）**

=condition-case= が =error= を捕捉することで、一つのモジュールが失敗しても
後続のモジュールのロードを継続する。注意: =condition-case= は =error= シグナルのみ
捕捉する。=quit=（=C-g=）は伝播する — ユーザーによる中断は尊重すべき設計。

**負荷計測と診断**

=my:modules-verbose= が非 nil のとき、各モジュールの読み込み時間を ms 単位で報告する。
=%-24s= は 24 文字の左揃えフォーマットで、モジュール名の長さを揃えて読みやすくする。
=(current-time)= と =float-time=、=time-subtract= の組み合わせでサブミリ秒精度の
計測が可能。
...
互換性修正層。現時点では活性化された修正コードが存在しないが、将来のバージョン固有の
修正を追加するためのプレースホルダーとして機能する。

=core-fixes--emacs>== は =core-tools.el= にも同名で定義されている（意図的重複）。
=core-fixes= は最初にロードされるモジュールであり、=core-tools= のシンボルはまだ
存在しない。=defalias= で参照すると void-function エラーになるため、ロード順序の
制約を守るために重複定義を選択している。
...
***** 深掘り解説: core-session.el — ファサードパターン

外部モジュールに対する公開 API ファサードを提供し、実装詳細（=core-session-private.el=）
を隠蔽する。

#+begin_example
外部モジュール → core-session（公開 API）
                      ↓ require
              core-session-private（実装詳細）
#+end_example

タイマー管理では =run-at-time= による周期実行（定期ジョブ、600秒ごと）と
=run-with-idle-timer= によるアイドル実行（120秒アイドル後）の二車線設計を採用する。
=(require 'core-session-private)= を外部から呼ぶことを禁止することで、
実装詳細のカプセル化が保たれる。
...
***** 深掘り解説: core-gc.el — 段階的 GC 管理

フォーカスが外れたとき（他アプリに切り替えたとき）とミニバッファを閉じたとき
（コマンド完了後）に GC を実行する。これらのタイミングはユーザーが「一息ついている」
瞬間であり、GC による遅延を感じにくい。

=core-runtime.el= の gcmh との役割分担:
- =core-gc.el= → 明示的なトリガーポイントで GC
- =gcmh= → アイドルタイマーベースの自動 GC 管理（=focus-out-hook= と =minibuffer-exit-hook=）
...
***** 深掘り解説: core-history.el — セッション永続化

=:pre-setq= は =leaf= の展開時点で評価される（パッケージロード前）。バッククォート +
コンマにより =no-littering-var-directory= の値が展開時に評価される。

3 つのモードを =emacs-startup-hook= でまとめて有効化するのは、全モジュールのパス設定が
完了してからファイルを読み込むためである（=after-init-hook= ではパスが未確定の場合がある）。
...
***** 深掘り解説: core-switches.el — LSP/UI バックエンド選択

=autoload= で関数を宣言することで、実際にその関数が呼ばれるまで =dev-lsp-eglot.el= が
ロードされない。=core-switches.el= は =dev= 層のモジュールに依存することなく、後で
条件に応じてロードを発動できる（依存方向の逆転なし）。

=after-init-hook= で発動することで、=personal/<user>.el= で設定した =my:use-lsp= の値が
確実に参照される（個人設定がモジュールロード中に反映される）。
...
***** 深掘り解説: ui-health-modeline.el — セッション状態表示

=featurep= と =fboundp= の二重確認は、eglot が存在しない環境でのエラーを防ぐ。
=ignore-errors= は eglot がバッファに対応していない場合のエラーを抑制する。
この関数はモードライン更新の度に呼ばれるため、軽量である必要がある（=length=
の O(n) は許容範囲）。
...
***** 深掘り解説: ui-leader.el — リーダーキー基盤

Fix BR-1 で =C-c SPC= から =M-SPC= に変更された（3キー → 2キー）。
=M-SPC= は標準 Emacs で未使用、macOS に遮断されない、親指+スペースバーで
人間工学的に快適。

=make-sparse-keymap= は空のキーマップを作成する。スパースマップはバインディングが
少ない場合にメモリ効率が良い。=keymap-set= は Emacs 29+ の新 API（=define-key= +
=kbd= の組み合わせを置き換える）。

サブマップの連鎖: =M-SPC b l= → =ui-leader-map["b"]= → =ui-leader-b-map["l"]= →
=ibuffer= という二段引き。
...
***** 深掘り解説: ui-keymap.el — バインディング戦略

Fix BR のキーストローク削減の設計原則:

| 戦略 | 実装例 |
|------+--------|
| リーダー短縮 | =C-c SPC= → =M-SPC= (−1キー) |
| 高頻度コマンドを最上位に | =M-SPC RET= = consult-buffer (5→3キー) |
| 短い global 経路が存在する場合は削除 | =next-buffer= は =s-→= (2キー) の方が短い |

=ui-keymap--bind= は =map= をシンボルとして受け取り、=keymapp= でキーマップであること
を確認してから =keymap-set= する。この防御的チェックにより、=ui-leader.el= が
ロードされていない状態でも安全に動作する。
...
***** 深掘り解説: completion-capf-org-src.el — Org SRC ブロック補完

=org-in-src-block-p= でカーソル位置を確認し、=org-element-property= でブロックの
言語を取得する。Python のブロックでは Python の補完、Elisp ブロックでは Elisp の
補完が動く。

=capf-org-src= ファミリーの協調関係:
- =completion-capf-org-src.el= → ブロック検出とディスパッチ
- =completion-capf-org-src-lang.el= → 言語別 CAPF 選択
- =completion-corfu-org-src.el= → corfu との統合
- =completion-orderless-org-src.el= → orderless フィルタリングの適用
...
***** 深掘り解説: completion-lsp.el — バックエンド非依存 CAPF

LSP バックエンド非依存設計の補完側実装。=core-switches= で選択したバックエンドに
応じて、適切な CAPF 関数を =completion-at-point-functions= に追加する:
...
=personal/= ディレクトリはユーザーおよびデバイス固有のオーバーレイを含む。

これらのファイルは:

- *バージョン管理対象外*（または共有コミットから除外）
- *安全に編集可能*（他のユーザーに影響しない）
- *モジュール後にロード*（ローカルオーバーライドを可能にする）
