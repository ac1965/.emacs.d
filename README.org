...
#+begin_src emacs-lisp
  ...
  ;; Category: ui
  ;;
  ;; This module configures the visual theme layer.
  ;;
  ;; Responsibilities:
  ;;
  ;; - load nano-emacs theme
  ;; - configure nano faces
  ;; - adjust spacing and padding
  ;; - apply face overrides after theme activation
  ;;
  ;; The configuration is idempotent and safe to run multiple times.
  ...
  (defvar ui-theme--initialized nil
    "Non-nil once the theme configuration has been initialized.

  This flag prevents the theme initialization routine from running
  multiple times during the same Emacs session.")

  (defun ui-theme--apply ()
    "Initialize and apply the UI theme configuration.

  This function performs the following tasks:

  - Ensures the initialization runs only once
  - Registers hooks to reapply Nano faces after theme changes
  - Configures subtree face backgrounds
  - Loads Nano theme and supporting face configuration
  - Applies spacing adjustments for graphical displays

  The function is safe to run multiple times but will only execute
  the initialization logic once."
    (unless ui-theme--initialized
      (setq ui-theme--initialized t)

      ;; Reapply Nano faces when themes are enabled
      ...
      ;; Configure dired-subtree background colors
      ...
      ;; Load Nano layout and faces
      (require 'nano-layout)
      (require 'nano-faces)

      (nano-faces)

      ;; Strengthen emphasis faces
      ...
      ;; Apply palette adjustments
      (my/nano-apply-faces)

      ;; Ensure emphasis faces behave as expected
      (set-face-attribute 'bold nil :weight 'bold)
      (set-face-attribute 'italic nil :slant 'italic)

      ;; Configure pairing and parenthesis highlighting
...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; Leader key infrastructure providing hierarchical keymaps.
  ;;
  ;; This module defines the root leader key and a set of prefix
  ;; keymaps used throughout the UI layer.  It does not assign
  ;; commands; it only provides the map structure.
  ;;
  ;; Typical layout:
  ;;
  ;; C-c SPC
  ...
  ;; └─ h  help
  ;;
  ;; Command bindings are defined in other modules.
  ...
  (defcustom ui-leader-key "C-c SPC"
    "Root key sequence used for the leader key."
    :type 'string
    :group 'ui-leader)

  (defcustom ui-leader-local-suffix "m"
    "Suffix used for the local leader map."
    :type 'string
    :group 'ui-leader)

  (defcustom ui-leader-enable-p t
    "If non-nil, enable the leader key infrastructure."
    :type 'boolean
    :group 'ui-leader)
...
#+begin_src emacs-lisp
  ...
  (leaf aidermacs
    ...
    ;; No warning is emitted: comint is a fully supported backend and the
    ;; absence of vterm is a normal configuration, not an error condition.
...
#+begin_src emacs-lisp
  ...
  (defun my/vterm-apply-palette ()
    ...
    (cl-labels
      ...
      (safe-face-set 'vterm-color-black
                     :foreground nano-color-strong
                     :background 'unspecified)

      (safe-face-set 'vterm-color-red
                     :foreground nano-color-critical
                     :background 'unspecified)

      (safe-face-set 'vterm-color-green
                     :foreground "#10b981"
                     :background 'unspecified)

      (safe-face-set 'vterm-color-yellow
                     :foreground nano-color-popout
                     :background 'unspecified)

      (safe-face-set 'vterm-color-blue
                     :foreground nano-color-salient
                     :background 'unspecified)

      (safe-face-set 'vterm-color-magenta
                     :foreground "#a21caf"
                     :background 'unspecified)

      (safe-face-set 'vterm-color-cyan
                     :foreground "#0891b2"
                     :background 'unspecified)

      (safe-face-set 'vterm-color-white
