...
**** Purpose
Provide **static linting helpers** for Emacs Lisp modules using
=checkdoc= and =package-lint=.

**** What it does
- Runs =checkdoc= on the current buffer or directory
- Runs =package-lint= on individual files
- Reports issues without modifying files

**** Notes
- Intended for manual or CI-triggered use
