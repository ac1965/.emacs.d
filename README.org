...
Former C-c b/k bindings remain removed: reachable via M-SPC (2-3 keys).
C-c o/v/r were reintroduced deliberately (2026-07) after cross-checking
against the ty07.net memo: o/v duplicate no leader binding (find-file and
find-file-read-only were never given M-SPC equivalents), and r is a
genuinely different command (query-replace) from the already-leader-bound
consult-ripgrep, so none of the three re-create the redundancy that C-c
b/k avoided by living under M-SPC.
