...
- Maximal use of Emacs 31 built-ins (a built-in always beats a hand-written equivalent)
...
The Keychain backend (=auth-keychain=) is enabled by default on macOS.
The personal layer may override =auth-keychain-enable= before modules load.

*** orgx

Extended Org Mode functionality.  All orgx modules depend only on =core=,
=ui=, =auth=, and =completion=; no dependency on =vcs= or =dev=.

The layer contains eleven modules.  Two of them (=orgx-roam-ui=,
=orgx-brain=) are optional extras declared in =my:modules-extra= by
=personal/user.el=; the remaining nine are in the canonical =my:modules= list.

Responsibilities by module:

- =orgx-core= — Org directory layout, =defvar= path variables, agenda
  fallback phase 1 (=inbox.org=), TODO keywords, refile targets, capture
  templates (todo, note, journal, meeting, blog post), archive location,
  agenda phase 2 rebuild via =after-init-hook=
- =orgx-extensions= — =org-journal= (journal directory), =org-roam= (DB
  location, connector, autosync via 5-second idle timer, file-hash guard
  advice), =org-download= (image directory), =toc-org=, =org-cliplink=
- =orgx-visual= — =org-modern= visual variables (TODO faces, list/checkbox/
  priority symbols, heading stars), agenda grid, custom faces; configured
  once via =after-init-hook=; =org-modern-mode= enabled per buffer via
  =org-mode-hook=
- =orgx-fold= — interactive fold/unfold/toggle commands (=my/org-fold-subtree=,
  =my/org-unfold-subtree=, =my/org-toggle-fold=) using built-in =org-fold=
  with =outline-invisible-p= fallback for Emacs < 29; key bindings under
  =M-SPC m= and =C-c= aliases
- =orgx-export= — =ox-hugo= (TOML front-matter), =markdown-mode=,
  =markdown-preview-mode=, =edit-indirect=, =ob-mermaid= (Babel mermaid
  language; CLI path probed via =orgx-export--mermaid-cli-path=), =ob-dot=
  (Graphviz; optional, activated only when =ob-dot= is on load-path)
- =orgx-notes-markdown= — notebook-style Markdown notes under =my:d:org=;
  =markdown-mode= / =gfm-mode= with =pandoc= backend, =visual-fill-column=,
  =consult-notes= integration (=consult-notes-file-dir-sources= set via
  =setopt= in =:config=), =org-download= integration for images;
  commands =my/notes-new-note=, =my/notes-open-root=,
  =my/notes-consult-ripgrep=
- =orgx-auto-tangle= — automatic =org-babel-tangle= on =after-save-hook=
  for =README.org= only; custom implementation using =ob-tangle= directly
  (does NOT use the third-party =org-auto-tangle= package); guarded by a
  buffer-local in-progress flag to prevent recursive loops
- =orgx-auto-hugo= — automatic =ox-hugo= all-subtrees export triggered by
  =org-capture-after-finalize-hook= (fires on =C-c C-c=, not on save);
  aborted captures (=org-note-abort=) are skipped; target file detected via
  =my:f:capture-blog-file= at finalize time
- =orgx-typography= — =valign-mode= (pixel-precise table alignment) and
  =org-appear-mode= (auto-show emphasis/link/sub-superscript markers);
  optional extra appended to =my:modules-extra= by =personal/user.el=
- =orgx-roam-ui= — =org-roam-ui= graph visualiser served via =web-server=
  (MELPA package name: =simple-httpd=; feature symbol: =web-server=);
  packages installed at top level (outside =with-eval-after-load=) to ensure
  load-path availability; =provide 'simple-httpd= shim satisfies the upstream
  =(require 'simple-httpd)= in =org-roam-ui.el=; UI opened via 8-second idle
  timer after =org-roam= autosync (5-second timer) settles
- =orgx-brain= — =org-brain= mind-map view sharing =my:d:org-roam= as its
  note directory; command =my/brain-visualize=

Org Babel language registration: =mermaid= and =dot= (Graphviz, optional)
are explicitly registered in =orgx-export=.  =emacs-lisp=, =shell=, and
=python= are Org built-in defaults and require no explicit registration.

The =orgx= layer does not manage completion CAPF integration; that
responsibility belongs to the =completion= layer (=capf-org-src= family).
...
- Built-in =vc= policy (=vcs-core=): async check-in, published-history guard,
  =vc-auto-revert-mode=, VC Directory hygiene, smerge auto-enable
- Magit configuration
...
Certain modules have tangle targets but are intentionally absent from =my:modules=.
They are loaded on demand via =autoload= declarations in a coordinating module.
This pattern applies when the module must not load unconditionally at startup
(LSP backend selection, optional UI bundles).

| Module              | Declared in     | Trigger condition                       |
|---------------------+-----------------+-----------------------------------------|
| =dev-lsp-eglot=     | =core-switches= | =(my:use-lsp 'eglot)= via after-init   |
| =dev-lsp-mode=      | =core-switches= | =(my:use-lsp 'lsp)= via after-init     |
| =ui-doom-modeline=  | =core-switches= | =(my:use-ui 'doom)= via after-init     |
| =ui-nano-modeline=  | =core-switches= | =(my:use-ui 'nano)= via after-init     |
| =ui-nano-palette=   | =ui-theme=      | =(require 'ui-nano-palette nil t)=      |

These modules must not use =:require t= in their =leaf= declarations; =autoload=
entries guarantee load-path availability without triggering premature evaluation.
...
Shell-environment import is scoped to the minimum required variables.

On macOS, =exec-path-from-shell= launches a login shell to populate =exec-path=
and the process environment.  The shell invocation itself takes approximately
200–500 ms.  Two design decisions bound its cost:

1. =exec-path-from-shell-arguments= is set to ='("-l")= (login shell only,
   no interactive flag).  This halves startup time compared to the default
   ='("-l" "-i")= by skipping interactive-shell startup files.

2. =exec-path-from-shell-variables= is restricted to =("PATH" "LANG")=.
   API keys (=OPENAI_API_KEY=, =OPENROUTER_API_KEY=) are retrieved at call
   time from =~/.authinfo.gpg= via =auth-source= inside =utils-claude.el=.
   Removing them from the environment import list eliminates the only reason
   the shell needed to source login-profile files that export those keys.

These two measures together reduce the =exec-path-from-shell= contribution to
startup time to the irreducible cost of launching a minimal login shell.
...
*Required*
- Emacs 31 (=emacs-31= branch / =master=), built with native compilation.
  As of 2026-07 the newest Emacs 31 tag is =emacs-31.0.90= (pretest); the
  released stable series is 30.x.  Modules degrade gracefully on Emacs 30:
  every Emacs 31-only block is guarded by =(>= emacs-major-version 31)= or a
  =boundp= / =fboundp= check.
- Git
- GNU Make
- GCC 10+ (=libgccjit=)
...
This Emacs configuration uses a *modular design* targeting **Emacs 31** (degrading safely to Emacs 30).
...
#+begin_src emacs-lisp
  ...
  ;; Backups are stored under var/backup/ (no-littering path set in core-editing).
  ;; early-init enables versioned backups; paths are overridden after no-littering loads.
...
=exec-path-from-shell-variables= is restricted to =("PATH" "LANG")= (Fix BZ-1).
API keys (=OPENAI_API_KEY=, =OPENROUTER_API_KEY=) are retrieved from =~/.authinfo.gpg=
via =auth-source= at call time inside =utils-claude.el= and =dev-ai.el=.
This eliminates the need to source login-profile files that export those keys,
reducing the startup overhead from approximately 513 ms to the irreducible cost
of populating =exec-path= from =/etc/paths.d/=.
...
#+begin_src emacs-lisp
  ...
  ;; [Fix BZ-1] Reduced variable list to PATH and LANG only.
  ;; OPENAI_API_KEY and OPENROUTER_API_KEY are no longer imported via a
  ;; login shell; they are retrieved at call time from auth-source
  ;; (~/.authinfo.gpg) by utils-claude.el.  Removing them eliminates the
  ;; need to spawn an interactive login shell solely to export two env vars,
  ;; which was the primary source of the ~513 ms startup overhead on macOS.
  ;; exec-path-from-shell-arguments '("-l") retains the login-shell flag
  ;; (required on macOS to populate PATH from /etc/paths.d), but the shorter
  ;; variable list reduces the shell work.
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This module intentionally contains no active fix code.  It is the first
  ;; module loaded (see modules.el) and exists as a version-guarded hotfix slot.
  ;;
  ;;; Code:
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; Load-order note:
  ;; Loaded last in the core/ section so that all core
...
Runs via `after-init-hook' once all modules are loaded."
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31: battery-aware asynchronous native compilation
  ;; ---------------------------------------------------------------------------
  ;; On an Apple Silicon laptop, async native-comp is the single largest source
  ;; of background CPU (and therefore battery drain) after a package upgrade.
  ;; Emacs 31 lets us suspend it while running on battery; queued jobs resume
  ;; once the machine is back on AC power.
...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; Tree-sitter infrastructure, delegated to Emacs 31 built-ins.
  ;;
  ;; Emacs 31 provides two user options that replace the hand-written grammar
  ;; management of the previous implementation:
  ;;
  ;; - `treesit-enabled-modes'        derives `major-mode-remap-alist' from the
  ;;                                  `treesit-major-mode-remap-alist' tables that
  ;;                                  the ts-mode packages themselves publish.
  ;; - `treesit-auto-install-grammar' installs a missing grammar on demand, when a
  ;;                                  ts-mode is actually turned on.
  ;;
  ;; Consequently this module no longer carries a mode-remap table, a bulk
  ;; installer, or an `emacs-startup-hook' entry.  It declares only:
  ;;
  ;; - where grammars live (`treesit-extra-load-path')
  ;; - where they come from (`treesit-language-source-alist', Emacs 31 keyword
  ;;   form: :revision :source-dir :cc :c++ :commit)
  ;; - remaps for third-party legacy modes that ship no ts-mode remap table
  ;; - `core-treesit-report' for diagnostics
  ;;
  ;; On Emacs < 31 the module degrades to a no-op except for the source alist.
  ;;; Code:
...
Applied only when the corresponding grammar is actually installed.")
  ...
  (leaf treesit
    ...
    :custom
    ;; Emacs 31: enable every tree-sitter mode whose grammar is present.
    ((treesit-enabled-modes        . t)
     ;; Emacs 31: install a missing grammar on demand.  Use `always' for
     ;; unattended installation, `ask-dir' to choose the target directory.
     (treesit-auto-install-grammar . 'ask)
     (treesit-font-lock-level      . 4))
    :config
    ;; Emacs 31: `treesit-extra-load-path' is a user option, and its first
    ;; element is the default install directory for `treesit-auto-install-grammar'.
...
#+begin_src emacs-lisp
  ...
  (leaf saveplace :straight nil
    ...
    :custom
    ;; Emacs 31: periodic autosave of places.  This option installs a timer, so
    ;; it must be set with setopt (:custom), never with setq/:pre-setq.
  ...
  (leaf recentf :straight nil
    ...
    :custom
    ;; Emacs 31: periodic autosave (timer-installing option -> setopt), plus
    ;; quieter operation of `recentf-save-list' and `recentf-open-files'.
    ((recentf-autosave-interval       . 300)
     (recentf-show-messages           . nil)
     (recentf-suppress-open-file-help . t))
    :config
    ;; Emacs 31: skip files whose extension is in `completion-ignored-extensions'.
    (when (fboundp 'recentf-exclude-file-by-extension-p)
      (add-to-list 'recentf-exclude #'recentf-exclude-file-by-extension-p)))

  ;; Emacs 31: savehist merges external modifications of the history file
  ;; (multi-process safety: GUI + daemon + CLI Emacs sharing one history file).
  ;; That merge is active ONLY while `savehist-additional-variables' is nil, so
  ;; `my:desktop-ask-on-restore' is deliberately NOT persisted here; it belongs
  ;; to `custom-file' / core-session-private.el instead.
  (leaf savehist
    :straight nil
    :pre-setq
...
#+begin_src emacs-lisp
  ...
  ;; - implements the auto-save subsystem (auto-save-visited-mode + focus-out hook
  ;;   + versioned backups under no-littering-var-directory/backup/)
  ...
  ;;
  ;; 1. auto-save-visited-mode  — writes the visited file itself every
  ;;    `auto-save-visited-interval' seconds of idle time (Emacs 26.1+).
  ;;    Replaces the legacy #file# scratch-file approach (auto-save-default nil
  ;;    set in early-init).
  ;;
  ;; 2. core-editing--save-all-on-focus-out — saves all modified file-backed
  ;;    buffers when Emacs loses focus (focus-out-hook) or the selected window
  ;;    changes (window-selection-change-functions).  Covers the gap where the
  ;;    idle timer has not yet fired.
  ;;
  ;; 3. Versioned backups — enabled in early-init; backup-directory-alist is
  ;;    set here (after no-littering-var-directory is available) so all backup
  ;;    files are consolidated under var/backup/.
...
`add-hook' and `add-function' check for existing entries."
    ;; 1. Backup directory — set here so no-littering-var-directory is available.
    (setopt backup-directory-alist
            `(("." . ,(expand-file-name "backup/" no-littering-var-directory))))
    ;; 2. auto-save-visited-mode: write to the actual file, not a scratch file.
    (setopt auto-save-visited-interval core-autosave-interval
            auto-save-default          nil)
    (auto-save-visited-mode 1)
    ;; 3. focus-out-hook — zero-argument; fired when the OS takes focus away.
    (add-hook 'focus-out-hook #'core-editing--save-on-focus-out)
    ;; 4. after-focus-change-function — also zero-argument (Emacs calls it
    ;;    with no arguments; the frame is NOT passed as an argument).
    ;;    Remove any stale registration before adding to prevent duplicates
    ;;    across config reloads (e.g. make reload without restarting Emacs).
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31: editing built-ins
  ;; ---------------------------------------------------------------------------
  ;;
  ;; hideshow reached parity with third-party folding packages in Emacs 31:
  ;; `hs-cycle' (parent -> nested -> all), `hs-toggle-all', margin/fringe
  ;; indicators, and a hidden-line count next to the ellipsis.  Tree-sitter
  ;; based modes gained proper `hs-minor-mode' support in the same release, so
  ;; folding now works in *-ts-mode buffers without extra configuration.
  ;;
  ;; NOTE: `hs-special-modes-alist' is obsolete in Emacs 31, and hiding commands
  ;; now act on the innermost block containing point (set `hs-hide-block-behavior'
  ;; to `after-cursor' for the pre-31 behaviour).
  (leaf hideshow
    :straight nil
  ...
  ;; ElDoc: Emacs 31 can surface the `help-echo' text property at point, and can
  ;; show the first sentence of a called function's docstring while typing its
  ;; arguments -- documentation without leaving the buffer.
  (leaf eldoc
    :straight nil
    :custom
    ((eldoc-help-at-pt                          . t)
  ...
  ;; Scalar editing options introduced in Emacs 31.  Guarded so the module stays
  ;; loadable on Emacs 30 (delete-safety applies to versions as well as modules).
  (when (>= emacs-major-version 31)
    ;; C-w with no active region kills the previous word instead of signalling.
    (setopt kill-region-dwim 'emacs-word)
    ;; `delete-pair' leaves a mark at the deletion site.
    (setopt delete-pair-push-mark t)
    ;; Do not re-highlight the region on C-x C-x (mark is a position, not a UI).
    (setopt exchange-point-and-mark-highlight-region nil)
    ;; show-paren must not match parens inside comments/strings.
    (setopt show-paren-not-in-comments-or-strings t)
    ;; hl-line follows the window, not the buffer.
    (setopt global-hl-line-sticky-flag 'window)
    ;; Windows: quitting help/compilation buffers kills them; killing a buffer
    ;; lets `quit-restore-window' restore the previous layout.
    (setopt quit-window-kill-buffer '(help-mode compilation-mode messages-buffer-mode))
    (setopt kill-buffer-quit-windows t)
    ;; `C-h l' refreshes while displayed.
    (setopt view-lossage-auto-refresh t)
    ;; Emacs 31 replaces `find-function-setup-keys' with a real minor mode.
...
**Deprecation status (2026-03, Fix B):** This module is a no-op compatibility
shim.  All functional code was moved to =personal/user.el= when the architectural
rule prohibiting core modules from injecting upper-layer (=ui=, =orgx=) symbols
into =my:modules-extra= was enforced.  The shim remains to satisfy any
=( require 'core-custom-ui-extras nil t)= call in on-disk =.el= files generated
by a previous tangle pass.  After =make reload= (which retangles from this
=README.org=), no such call exists in the generated outputs and this file
becomes unreachable.  It will be removed in a future pass once all users have
run =make reload=.
...
#+begin_src emacs-lisp
  ...
  (defun ui-font-describe ()
    ...
    (let* ((default-family  (face-attribute 'default :family))
           ...
           ;; Read the emoji family from the defcustom/fallback value that
           ;; ui-font-apply passed to set-fontset-font.  The previous probe
           ;; (font-get (font-spec :script 'emoji) :family) queries font
           ;; availability rather than the registered fontset entry, so it
           ;; returns nil on macOS and reports "unknown".
  ...
  (defun ui-keyboard--apply ()
    ...
    (cond
     ((and ui-keyboard-map-c-h-to-del
      ...
      ;; [Fix C] define-key + kbd → keymap-set (Emacs 29+ API)
      ;; key-translation-map is a normal keymap and supports keymap-set.
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; This file:
  ;; - binds C-s, C-., s-z and M-x for high-frequency single-chord navigation
...
that benefit from single-chord access (no prefix).
  ...
  s-z   → undo-redo          (macOS-convention undo/redo; was C-? — Fix CO)
...
[Fix CO] C-? (undo-redo) replaced with s-z: aligns Emacs undo/redo with the
...
binding would add keymap surface without practical benefit."
    ...
    ;; [Fix B] global-set-key + kbd → keymap-global-set (Emacs 29+ API, Rule: prefer keymap-set)
    (keymap-global-set "C-."       #'other-window)
    (keymap-global-set "C-s"       #'consult-line)
    (keymap-global-set "M-x"       #'execute-extended-command)
    ;; [Fix CO] undo/redo moved from C-? (3-key chord: Ctrl+Shift+/) to s-z
    ;; (2-key chord: Cmd+Z), matching macOS-wide undo convention.
...
#+begin_src emacs-lisp
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31: system integration (macOS 10.9+)
  ;; ---------------------------------------------------------------------------
  ;; `system-taskbar-mode' drives the Dock icon: badge counts, a progress bar
  ;; overlay, and the "needs attention" bounce.  `system-sleep' blocks idle
  ;; sleep while a long job runs and provides pre-sleep / post-wake hooks.
  ;;
  ;; Both are useful for the compile-heavy workloads this configuration is used
  ;; for (native compilation, Emacs builds from source): the machine no longer
  ;; falls asleep mid-build, and remote (TRAMP) connections are torn down before
  ;; sleep rather than hanging on wake.
...
This module centralises all policy for =~/.authinfo.gpg= and the GPG/EasyPG
stack.  It has three responsibilities:

1. Configure =auth-source= backends so that the encrypted credential store is
   consulted before the plain-text fallback.
2. Provide =my/auth-gpg-verify= — an interactive diagnostic command that checks
   every service entry and reports which keys are present, which are missing,
   and what the decrypted secret prefix looks like (first four characters only).
3. Provide =my/auth-gpg-init-template= — a command that writes a starter
   =~/.authinfo= template and then encrypts it with the current GPG key.

The module intentionally loads after =auth-core= (which enables =epa-file=) and
must not depend on any layer above =auth=.

**Required =~/.authinfo.gpg= entries:**
...
The =login= field in each entry must exactly match the =:user= argument in the
corresponding =auth-source-search= call in =dev-ai.el= and =auth-gpg-services=.
A mismatch silently returns no results from =auth-source-search=.

All entries use the =machine= / =login= / =password= netrc format that
=auth-source= understands natively.  The file must be GPG-encrypted; EasyPG
decrypts it transparently on first access.

#+begin_src emacs-lisp
  ;;; auth/auth-gpg.el --- ~/.authinfo.gpg management and verification -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; Centralised policy for ~/.authinfo.gpg and the EasyPG/auth-source stack.
  ;;
  ;; This file:
  ;; - configures auth-source to prefer the encrypted credential store
  ;; - defines auth-gpg-services: the canonical list of service entries
  ;;   expected in ~/.authinfo.gpg (used by the verify command)
  ;; - provides my/auth-gpg-verify  — diagnostic: checks which entries exist
  ;; - provides my/auth-gpg-init-template — creates a starter authinfo file
  ;;
  ;; Load order: requires auth-core (epa-file must be enabled first).
  ;; Must not depend on ui, completion, orgx, vcs, dev, utils, or personal.
  ;;
  ;;; Code:
...
")
        (insert template))
      ;; Copy to the target path; EasyPG will encrypt on save because the
      ;; filename ends in .gpg and epa-file is active.
...
This module integrates the macOS Keychain with =auth-source= so that
credentials stored in the system keychain are available to all packages
that use =auth-source-search=.  It is a pure auth-layer module: it does
not depend on =ui=, =completion=, =orgx=, =vcs=, =dev=, =utils=, or =personal=.

The module is a no-op on non-macOS hosts.  The personal layer controls
whether it is active by setting =auth-keychain-enable= before this file
loads.  Because =personal/user.el= is loaded by =init.el= before =modules.el=
requires all modules, the =defcustom= default is honoured on every host and
the personal override takes effect without any load-order dependency.

**Implementation strategy**

macOS exposes Keychain lookup through the =security(1)= command-line tool.
Two sub-commands are relevant:

- =security find-internet-password= — for entries stored as Internet
  Passwords (host, port, protocol, account).
- =security find-generic-password= — for entries stored as Generic
  Passwords (service name, account).

This module defines a custom =auth-source= backend using the
=auth-source-backend= struct and the =:search-function= slot introduced in
Emacs 26.  The backend delegates each =auth-source-search= call to the
=security(1)= binary; results are cached by the standard =auth-source= cache
unless =auth-source-do-cache= is nil.

**Keychain entry format**

Entries used by this module must be stored as Generic Passwords (the most
portable form).  The =Account= field corresponds to the =:user= argument of
=auth-source-search=; the =Service= field corresponds to =:host=.

Create entries with Keychain Access.app or via the command line:
...
**Interaction with auth-gpg**

=auth-keychain= is appended to =auth-sources= after =auth-gpg= runs.  The
priority order is therefore:

1. =~/.authinfo.gpg= (managed by =auth-gpg=)
2. =~/.authinfo= plain-text fallback (managed by =auth-gpg= when present)
3. macOS Keychain (managed by this module)

This means the GPG-encrypted store takes precedence.  Credentials absent
from =~/.authinfo.gpg= are looked up in the Keychain automatically.

**Enabling and disabling**

Set =auth-keychain-enable= in =personal/device-darwin.el= (device level) and
optionally override it in =personal/ac1965.el= (login level) before
=modules.el= is required:

#+begin_example
;; personal/device-darwin.el — activate on this macOS machine:
(setopt auth-keychain-enable t)

;; personal/ac1965.el — login-level override (wins over device level):
(setopt auth-keychain-enable t)   ; confirm, or nil to disable
#+end_example

#+begin_src emacs-lisp
  ;;; auth/auth-keychain.el --- macOS Keychain auth-source backend -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; macOS Keychain integration for auth-source.
  ;;
  ;; This module registers a custom auth-source backend that delegates
  ;; credential lookups to the macOS `security(1)` command-line tool.
  ;;
  ;; Responsibilities:
  ;;
  ;; - Define `auth-keychain-enable': personal layer sets this before modules load.
  ;; - Register the Keychain backend with auth-source on after-init-hook.
  ;; - Provide `my/auth-keychain-verify': diagnostic command (lists entries).
  ;; - Remain entirely inert on non-macOS hosts.
  ;;
  ;; Load order: after auth-core and auth-gpg; must not depend on any layer
  ;; above auth.
  ;;
  ;;; Code:
...
layer.  The canonical control pattern is:
  ...
  ;; personal/device-darwin.el — activate on macOS at device level:
  (setopt auth-keychain-enable t)

  ;; personal/ac1965.el — override at login level (wins over device):
...
zero-argument lambda (the auth-source lazy secret convention)."
  ...
  (defun auth-keychain--make-backend ()
...
#+begin_src emacs-lisp
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31 completion engine
  ;; ---------------------------------------------------------------------------
  ;; - `completion-eager-display' / `completion-eager-update' would pop up and
  ;;   live-update the "*Completions*" buffer.  Vertico already owns that role;
  ;;   both are pinned to nil so a future change of the Emacs default cannot
  ;;   produce two competing candidate lists.
  ;; - `completion-pcm-leading-wildcard' makes the partial-completion style
  ;;   behave like substring (match anywhere), which is what `basic' fallback
  ;;   completion should do next to orderless.
  ;; - The `flex' style was reimplemented in Emacs 31 (faster, better ranking);
  ;;   it is now worth using directly for the file category rather than relying
  ;;   on an orderless dispatcher.
  (leaf emacs
    :straight nil
...
Removes itself from `window-setup-hook' after the first run."
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31: Completion Preview (built-in inline "ghost text")
  ;; ---------------------------------------------------------------------------
  ;; Completion Preview shows the top candidate inline, ahead of point, without
  ;; opening a popup.  Emacs 31 promoted `completion-preview-sort-function' to a
  ;; user option precisely so it can be aligned with a popup UI such as Corfu --
  ;; sharing the sort function means the previewed candidate is always the same
  ;; one Corfu would insert first, so the two interfaces never disagree.
  ;; `completion-preview-inhibit-functions' (new in 31) keeps it out of terminal
  ;; and REPL buffers, where the preview overlay fights with the process output.
  ...
  (leaf completion-preview
    ...
    :config
    ;; Align the preview's ranking with Corfu's.  When Corfu has no explicit
    ;; sort function, keep the Emacs default rather than inventing one.
...
#+begin_src emacs-lisp
  ...
  ;; ---------------------------------------------------------------------------
  ;; Emacs 31: throttle with timeout.el
  ;; ---------------------------------------------------------------------------
  ;; README.org is ~14k lines and tangles ~90 files; a burst of saves (auto-save
  ;; -visited-mode plus an explicit C-x C-s) used to run the whole tangle twice.
  ;; `timeout-throttle' installs an advice that collapses repeated calls inside
  ;; the interval into one.  The in-progress flag above is retained: throttling
  ;; limits frequency, it does not guarantee non-reentrancy.
...
#+begin_src emacs-lisp
  ...
  (leaf aidermacs
    ...
    :config
    ;; [Fix CF] API key lookup runs on after-init-hook so that
    ;; auth-gpg--configure-sources (which also runs on after-init-hook, but
    ;; is registered earlier — auth-gpg loads before dev-ai) has already
    ;; prepended ~/.authinfo.gpg to auth-sources.  Running the lookup in
    ;; :init would race against auth-gpg's hook registration.
    ;;
    ...
    ;;
    ;; :user values must match the `login' field in ~/.authinfo.gpg exactly.
...
#+begin_src emacs-lisp
  ...
  ;; Emacs 31 project.el:
  ;; - `project-prune-zombie-projects' removes vanished projects from the list
  ;; - `project-list-exclude' keeps scratch/temporary trees out of the list
  ;; - `project-mode-line-mode' shows the project name in the mode line
  ;; - new commands: project-root-find-file, project-find-matching-buffer,
  ...
  (leaf project
    ...
    ((project-prune-zombie-projects . t)
     ...
     ;; Show the project name in the mode line, but not for remote (TRAMP)
     ;; files, where computing it costs a round trip.
     (project-mode-line . 'non-remote)))

  ;; Emacs 31: `delete-trailing-whitespace-mode' is a buffer-local minor mode.
  ;; The previous global `before-save-hook' entry rewrote every buffer that was
  ;; ever saved -- including files in third-party repositories, producing
  ;; whitespace-only diffs.  Scope it to the modes we actually own instead.
  (leaf files
    :straight nil
  ...
  ;; NOTE (Emacs 31): `editorconfig-apply' is obsolete; `editorconfig-mode'
...
#+begin_src emacs-lisp
  ...
  ;; `my:d:eww' is defined in early-init.el (defconst); declare it here for the
  ;; byte-compiler only -- do not re-initialise it.
  (defvar my:d:eww)
...
Emacs 31 rewrote large parts of =vc= (over forty NEWS entries).  Several of the
operations that historically justified reaching for Magit are now available from
the built-in interface: multiple working trees, cherry-pick and revert,
incoming/outgoing diffs, asynchronous check-in, and stash deletion from VC
Directory.

This module does *not* remove Magit.  It enables the built-in layer so both are
available, and so the decision to drop Magit (or not) can be made from measured
experience rather than assumption.

#+begin_src emacs-lisp
  ;;; vcs/vcs-core.el --- Built-in VC policy (Emacs 31) -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; Built-in `vc' configuration, exploiting the Emacs 31 VC rewrite.
  ;;
  ;; Key bindings worth knowing (all built-in, no third-party packages):
  ;;
  ;;   C-x v v     vc-next-action            C-x v @   vc-revert
  ;;   C-x v b l   vc-print-fileset-branch-log
  ;;   C-x v b L   vc-print-root-branch-log
  ;;   C-x v R     vc-rename-file            C-x v !   (own input history)
  ;;   In VC Directory:  V  vc-dir-root-next-action
  ;;                     z d / D  delete Git stashes
  ;;
  ;; This module must not depend on the dev layer (layer invariant).
  ;;; Code:
  ...
  (leaf vc
    ...
    (;; Refuse to rewrite history that has already been published.  This is the
     ;; single most valuable new safety option in the Emacs 31 VC rewrite.
     (vc-allow-rewriting-published-history . nil)
     ;; Check in asynchronously; surface failures rather than swallowing them.
     (vc-async-checkin                 . t)
     (vc-display-failed-async-commands . t)
     ;; Distinguish incoming/outgoing commands with their own prefixes.
     (vc-use-incoming-outgoing-prefixes . t)
     ;; VC Directory hygiene.
     (vc-dir-auto-hide-up-to-date        . t)
     (vc-dir-save-some-buffers-on-revert . t)
     ;; Offer conflict resolution (smerge) when a merge leaves conflicts.
     (vc-resolve-conflicts . t)
     ;; Show the commit diff in the log-edit buffer before committing.
     (log-edit-hook . '(log-edit-show-files log-edit-show-diff)))
    :config
    ;; Emacs 31: revert buffers automatically when the working tree changes
    ;; underneath us (branch switch, rebase, pull performed outside Emacs).
    (when (fboundp 'vc-auto-revert-mode)
      (vc-auto-revert-mode 1)))

  ;; SMerge gained a `repeat-map' in Emacs 31: after the first conflict command,
  ;; the following keys repeat without the prefix.
  (leaf smerge-mode
    :straight nil
...
#+begin_src emacs-lisp
  ...
  ;; Emacs 31 renders diagnostics at end-of-line natively
  ;; (`flymake-show-diagnostics-at-end-of-line', values: short / t / fancy) and
  ;; chooses fringe vs. margin indicators automatically
  ;; (`flymake-indicator-type' = auto, which also works on TTY frames).
  ;; The sideline stack (sideline + sideline-flymake + sideline-flycheck) is
  ;; therefore removed: three packages replaced by two built-in user options.
  (leaf flymake
    :straight nil
    :custom
    ...
    :config
    ;; Emacs 29+ keymap API (Coding Rule: keymap-set, not define-key).
  ...
  ;; flycheck is retained ONLY as the diagnostics backend that lsp-mode expects
  ;; when `my:use-lsp' selects the lsp-mode backend.  It is never enabled
  ;; globally; with the eglot backend (the default) flymake is the sole checker.
...
| =personal/device-darwin.el= | macOS IME, mouse/scroll, Apple Music control |
...
#+begin_src emacs-lisp
  ...
  ;; [Fix BU] Locale and encoding initialisation.
  ;;
  ;; `set-language-environment' must precede `prefer-coding-system'.
  ;; Calling it with "Japanese" adjusts the internal coding-system priority
  ;; list for Japanese text detection (EUC-JP, Shift_JIS, UTF-8).
  ;; `prefer-coding-system' then promotes utf-8 to the head of that list,
  ;; ensuring UTF-8 wins for both read and write.
  ;;
  ;; `set-locale-environment "UTF-8"' (previous form) only set the process
  ;; locale; it did NOT adjust Emacs's internal coding-system priority list,
  ;; leaving Japanese file detection unreliable.
  ;;
  ;; Coding rule §6: these are not defcustom variables; plain setq / direct
  ;; function calls are correct here.
  (leaf my:locale
    :straight nil
    :init
  ...
  ;; ---------------------------------------------------------------------------
  ;; Auto-save exclusions (personal policy)
  ;; ---------------------------------------------------------------------------
  ;; Customise which files are never auto-saved.
  ;; Core defaults already exclude: gpg age enc pem p12 pfx.
  ;; Add personal exclusions here.

  ;; Exclude additional file-name patterns (full path matched):
...
#+begin_src emacs-lisp
  ...
  ;; - registers .ps1 → powershell-mode and .bas → visual-basic-mode
  ;;   (both derive from prog-mode; prog-mode-hook fires automatically)
  ;;; Code:
  ...
  ;; [Fix AM] ui-font-emoji: explicit value overrides the ui-font--system-emoji
  ;; fallback and is read directly by ui-font-describe for the startup log.
  ...
  ;; File-extension → prog-mode registrations
  ;;
  ;; Neither powershell-mode nor visual-basic-mode is loaded by modules.el
  ;; because they are machine-specific tools used only on this host.
  ;; Placing the leaf blocks here keeps the shared personal/user.el clean.
  ;;
  ;; powershell-mode (MELPA/jschaf) derives from prog-mode; .ps1 and .psm1
  ;; are the standard PowerShell script extensions.
  ;;
  ;; visual-basic-mode (MELPA/yoshiki-1) derives from prog-mode; .bas covers
  ;; VBA module files exported from Excel/Access.
  ...
  ;; nano-modeline IME indicator — cache via sis-change-hook
  ;;
  ;; sis--current is updated by sis-global-respect-mode's pre/post-command-hook
  ;; handlers (sis--save-to-buffer = sis--get).  sis-change-hook fires inside
  ;; sis--update-state when the value changes.  The cache variable is read by
  ;; the nano-modeline :eval block every frame — zero process overhead.
  ;;
  ;; defvar (not defvar-local): sis-change-hook fires in whichever buffer
  ;; triggered the IME switch.  A buffer-local variable would update only
  ;; that buffer; a global variable propagates to every buffer immediately.
...
Attached to `sis-change-hook'; called whenever sis detects an IME switch."
  ...
  ;;
  ;; Goal: show nano-modeline in every buffer regardless of major mode,
  ;; and hide the stock mode-line entirely.
  ;;
  ;; API facts (verified from rougier/nano-modeline source):
  ;;   nano-modeline-header (left right &optional default)
  ;;     default=nil  → setq-local  header-line-format  (current buffer only)
  ;;     default=t    → setq-default header-line-format  (all buffers, global)
  ;;
  ;; Strategy:
  ;;   1. ac1965--nano-apply-global calls nano-modeline-header with default=t,
  ;;      setting header-line-format globally for all present and future buffers.
  ;;   2. (setq-default mode-line-format nil) hides the stock mode-line globally.
  ;;   3. Both steps run inside with-eval-after-load 'nano-modeline, which fires
  ;;      after ui-nano--setup calls (require 'nano-modeline).  The per-mode
  ;;      hooks registered by ui-nano--setup (prog-mode-hook, text-mode-hook)
  ;;      then call the overridden functions with default=nil, which is harmless:
  ;;      setq-local on top of setq-default merely shadows for that buffer with
  ;;      the same value.
  ;;
  ;; Design constraints (cf. README coding rules):
  ;;   - Guarded by with-eval-after-load 'nano-modeline.
  ;;   - No top-level require of nano-modeline.
  ;;   - All defun carry docstrings.
  ;;   - No defcustom/defvar modified.
...
including those that never trigger a major-mode hook."
  ...
  ;; nano-modeline toggle: M-SPC t m
  ;;
  ;; Toggles nano-modeline header-line and mode-line visibility globally.
  ;; Hide: sets header-line-format and restores mode-line-format to default.
  ;; Show: calls ac1965--nano-apply-global to re-enable both.
...
is guaranteed to be a live keymap."
  ...
  ;; ---------------------------------------------------------------------------
  ;; macOS Keychain auth-source backend — login-level control
  ;; ---------------------------------------------------------------------------
  ;; personal/device-darwin.el sets auth-keychain-enable to t for all macOS
  ;; sessions.  This form gives the login account the final word.
  ;; Change to nil to disable Keychain lookup for this account specifically.
  ;;
  ;; Rule 6: auth-keychain-enable is a defcustom; use setopt.
  (setopt auth-keychain-enable t)
...
#+begin_src emacs-lisp
  ...
  ;;; Commentary:
  ;;
  ;; macOS device-specific settings: IME switching, mouse scrolling, Apple Music.
  ;;
  ;; This file:
  ;; - loads sis unconditionally; enables macism ISM and global modes when
  ;;   macism is present (sis--current stays available for nano-modeline)
  ;; - tunes mouse/trackpad scroll for a Logitech MX Ergo S:
  ;;   1-line steps, no momentum, 3-line scroll margin
  ;; - provides Apple Music control via AppleScript (play, pause, next, prev,
  ;;   toggle, current-track, playlist selection via completing-read)
  ;; - defines hydra-apple-music and binds it to M-SPC a a after init
  ;;; Code:
  ...
  ;;
  ;; sis is always loaded (no :when guard) so that sis--current is available
  ;; for the nano-modeline IME indicator regardless of whether macism is
  ;; installed.  The ISM backend (macism) and the global modes are enabled
  ;; only when macism is present; without it sis loads but stays inert.
  ;;
  ;; sis-global-respect-mode is required for the nano-modeline IME indicator:
  ;; it installs pre/post-command-hook handlers that call sis--save-to-buffer
  ;; (= sis--get) on every command, keeping sis--current up to date and
  ;; firing sis-change-hook when the source changes.  Without it,
  ;; sis-global-context-mode only fires on evil-insert-state-entry-hook
  ;; (irrelevant without evil), so sis--current stays nil permanently and
  ;; the nano-modeline indicator never updates.
  (leaf sis
    :straight t
    :require t
    :config
  ...
  ;;
  ;; Merged from personal/apple-music.el (now :tangle no).
  ;; All commands are no-ops on non-macOS systems via the darwin guard in
  ;; apple-music--applescript.  The hydra is defined after-init so that
  ;; hydra is guaranteed to be loaded (it is a straight.el package required
  ;; by ui-hydra.el, which runs via my:modules before after-init-hook fires).
  ;; Key: M-SPC a a (apps → Apple Music hydra).
    ...
    (when (eq system-type 'darwin)
      (let* ((raw (shell-command-to-string
        ...
        (when (and trimmed (not (string-empty-p trimmed)))
          ;; osascript returns a comma-separated list: "foo, bar, baz"
...
"
  ...
  ;; macOS Keychain auth-source backend
  ;; ---------------------------------------------------------------------------
  ;; Activate the Keychain backend on this macOS device.
  ;; personal/ac1965.el (loaded after this file) may override this to nil
  ;; to disable Keychain lookup for a specific login account.
  ;;
  ;; Rule 6: auth-keychain-enable is a defcustom; use setopt.
  (setopt auth-keychain-enable t)
...
** 2026-07 (dead-code elimination pass)

*** Fix DC: removal of unreferenced code (static reachability analysis)

Reachability analysis over all tangled =emacs-lisp= blocks (roots: =interactive=
commands, =org-babel-execute:*= dispatch targets, hook/keymap references,
=define-error= / =defface= / minor-mode definitions, and every symbol referenced
outside a definition body) identified 30 definitions with no reachable call site.
All of them were removed.

*Modules deleted (every symbol in the file was unreachable):*

| Module | Unreachable symbols | Note |
|--------+---------------------+------|
| =lisp/core/core-tools.el= | 8/8 | =core-tools-emacs>== was never called; =core-tools-ensure-directory= was an alias with no call site (all callers use =my/ensure-directory-exists= directly); =core-tools-feature-present-p= / =-function-present-p= were only used by =core-tools-require-if-present= / =-call-if-present=, which were themselves unreferenced (transitively dead); =core-tools-check-provide= and =core-tools-user-file-p= had no callers. |
| =lisp/core/core-api.el= | 3/3 | The three =core-api-session-*= wrappers had no call site.  The facade wrapped the =core-session= job API, which no module uses. |
| =lisp/utils/utils-path.el= | 3/3 | =utils-path-relative= / =-safe-name= had no callers; =utils-path-normalize= was used only by =utils-path-relative= (transitively dead). |
| =lisp/utils/utils-async.el= | 2/2 | =utils-async-run-script= had no callers; =utils-async-shell-command= was used only by it (transitively dead). |

Both entries were removed from =my:modules= in =modules.el=, and the =utils=
layer responsibilities in Layer Definitions were updated accordingly.

*Definitions deleted (module retained):*

- =core-fixes.el= — =core-fixes--emacs>== (private, zero call sites).  The module
  is retained as the documented version-guarded hotfix slot; the stale load-order
  Commentary describing the =core-tools-emacs>== duplication was removed with it.
- =ui-leader.el= — =ui-leader-local-key= (zero call sites).
- =ui-nano-palette.el= — =my/nano-set-palette-and-apply= and the three palette
  variables it was the sole consumer of (=nano-color-surface=, =nano-color-border=,
  =nano-color-highlight=); none of the three is referenced by any face definition.

*Duplicate definition removed:*

- =dev-navigation.el= — =my:d:eww= was re-defined with a value even though
  =early-init.el= already binds it with =defconst=.  Since =defvar= does not
  overwrite a bound symbol, the initialiser was dead.  Replaced with a bare
  =(defvar my:d:eww)= forward declaration, which is the correct pattern for
  byte-compiler visibility across compilation units (as used in
  =core-session-private.el=).

*Deliberately retained:*

- =(defvar my:modules-extra nil)= in =init.el= is *not* redundant with the
  =defcustom= in =modules.el=: the personal overlay (section 8) appends to it
  before =modules.el= is loaded (section 9), so the =defvar= must establish the
  binding first.
- Interactive commands that are reachable only via =M-x= (no key binding) were
  left in place; they are user-facing entry points, not dead code.

Net: -313 lines, 4 modules, 30 definitions.

** 2026-07 (Emacs 31 built-in adoption pass)

Target version raised from "Emacs 30+" to *Emacs 31* (=emacs-31.0.90= / =master=).
Every change below replaces hand-written configuration with a facility that Emacs
itself now guarantees; all Emacs 31-only code is guarded so the configuration
still loads on Emacs 30.

*Removed (superseded by built-ins)*

- *lisp/core/core-treesit.el*: deleted =core-treesit--mode-remap-alist=,
  =core-treesit-apply-remap=, =core-treesit-register-sources=,
  =core-treesit-install-grammar=, =core-treesit-install-all=,
  =core-treesit--startup-init= and its =emacs-startup-hook= entry
  (~155 lines → ~105 lines, and no startup hook).  Replaced by
  =treesit-enabled-modes= (t) and =treesit-auto-install-grammar= (ask).
  =treesit-language-source-alist= now uses the Emacs 31 keyword form
  (=:source-dir=); =treesit-extra-load-path= (a user option since 31) points at
  =var/tree-sitter/= and doubles as the grammar install directory.
  A small =core-treesit--legacy-remap= remains for third-party modes
  (=toml-mode=, =json-mode=, =yaml-mode=, =dockerfile-mode=) that publish no
  =treesit-major-mode-remap-alist=.
- *lisp/utils/utils-lint.el*: removed =sideline=, =sideline-flymake= and
  =sideline-flycheck= (three packages) in favour of Emacs 31's
  =flymake-show-diagnostics-at-end-of-line= (=short=) and
  =flymake-indicator-type= (=auto=, TTY-aware).  =flycheck= is retained but not
  enabled: it exists only as lsp-mode's diagnostics backend.
- *lisp/dev/dev-web-core.el*: the global =(before-save-hook . delete-trailing-whitespace)=
  is gone.  It rewrote *every* saved buffer, including files in third-party
  repositories.  Replaced by the Emacs 31 buffer-local
  =delete-trailing-whitespace-mode=, hooked into prog/org/conf/text modes only.
- *lisp/core/core-history.el*: =savehist-additional-variables= cleared.  Emacs 31
  merges external modifications of the history file only while that variable is
  nil; multi-process safety (GUI + daemon + CLI) outweighs persisting
  =my:desktop-ask-on-restore= there.

*Added (Emacs 31 built-ins)*

- *lisp/vcs/vcs-core.el* (new module, registered before =vcs-magit=): built-in
  =vc= policy — =vc-allow-rewriting-published-history= nil,
  =vc-async-checkin=, =vc-use-incoming-outgoing-prefixes=,
  =vc-dir-auto-hide-up-to-date=, =vc-resolve-conflicts=, =vc-auto-revert-mode=,
  and automatic =smerge-mode= on conflict markers.
- *lisp/core/core-editing.el*: =hideshow= with Emacs 31 indicators
  (=hs-show-indicators=, =hs-display-lines-hidden=, =hs-cycle=, =hs-toggle-all=;
  tree-sitter modes gained =hs-minor-mode= support in 31); =eldoc-help-at-pt=;
  =elisp-eldoc-funcall-with-docstring=; =kill-region-dwim=,
  =delete-pair-push-mark=, =show-paren-not-in-comments-or-strings=,
  =global-hl-line-sticky-flag= (window), =quit-window-kill-buffer=,
  =kill-buffer-quit-windows=, =find-function-mode=.
- *lisp/completion/completion-core.el*: =completion-eager-display= and
  =completion-eager-update= pinned to nil (Vertico owns candidate display);
  =completion-pcm-leading-wildcard= t.
- *lisp/completion/completion-corfu.el*: built-in =completion-preview-mode=,
  with =completion-preview-sort-function= aligned to =corfu-sort-function= so the
  inline preview and the Corfu popup never disagree, and
  =completion-preview-inhibit-functions= keeping it out of vterm/eshell/comint.
- *lisp/core/core-history.el*: =recentf-autosave-interval= and
  =save-place-autosave-interval= (300 s) — built-in timers replace ad-hoc saving;
  =recentf-exclude-file-by-extension-p= added to =recentf-exclude=.
- *lisp/core/core-native.el*: =native-comp-async-on-battery-power= nil — async
  native compilation is suspended on battery power (Apple Silicon laptops).
- *lisp/dev/dev-web-core.el*: =project-prune-zombie-projects=,
  =project-list-exclude=, =project-mode-line-mode=.
- *lisp/ui/ui-macos.el*: =system-taskbar-mode= (Dock badge/progress) and
  =system-sleep= (idle-sleep inhibition during compilation; TRAMP teardown before
  sleep) — both new in Emacs 31 and supported on macOS 10.9+.
- *lisp/orgx/orgx-auto-tangle.el*: =timeout-throttle= (new library =timeout.el=)
  collapses save bursts into a single tangle; the in-progress flag is retained
  because throttling limits frequency, not reentrancy.

*Compatibility notes (Emacs 31 incompatible changes)*

- =if-let= / =when-let= are obsolete; the configuration already uses the starred
  forms exclusively.
- =FOO-ts-mode-indent-offset= was renamed to =FOO-ts-indent-offset=.
- =hs-special-modes-alist= is obsolete; hiding commands now act on the innermost
  block containing point (=hs-hide-block-behavior= restores the old behaviour).
- =editorconfig-apply= is obsolete; =editorconfig-mode= is unaffected.
- =xterm-mouse-mode= is on by default in capable terminals; disable it explicitly
  if terminal-side copy/paste is preferred in CLI sessions.
- =site-start.el= is now loaded *before* =early-init.el=.
- Emacs 31 bundles Org 9.8.

:PROPERTIES:
:CUSTOM_ID: changelog
:END:

** 2026-05 (Nth evaluation pass)

*** Fix BU: personal/user.el — replace set-locale-environment with set-language-environment "Japanese"

🔴 Critical — encoding initialisation

- *[Fix BU] personal/user.el (leaf my:locale)*: Replaced
  =(set-locale-environment "UTF-8")= with =(set-language-environment "Japanese")=
  followed by =(prefer-coding-system 'utf-8)=.  =set-locale-environment= only
  configures the OS process locale; it does not adjust Emacs's internal
  coding-system priority list, leaving Japanese file detection unreliable and
  causing mojibake when opening UTF-8 encoded Japanese text files.
  =set-language-environment "Japanese"= adjusts the priority list to recognise
  EUC-JP, Shift_JIS, and UTF-8 as Japanese candidates; the subsequent
  =prefer-coding-system 'utf-8= call then promotes UTF-8 to the head of that
  list, ensuring deterministic UTF-8 read and write.  The call order
  (language-environment first, prefer-coding-system second) is mandatory;
  reversing it would allow =set-language-environment= to overwrite the
  UTF-8 preference.  The remaining calls (=set-default-coding-systems=,
  =set-terminal-coding-system=, =set-keyboard-coding-system=,
  =set-file-name-coding-system=, =set-selection-coding-system=, =setenv "LANG"=)
  are unchanged.  Inline comments added per the coding rule that all
  non-obvious side effects must be explained.

*** Fix BW: README.org Layer Definitions — correct eight factual errors in orgx section

🟠 Medium — documentation accuracy (implementation mismatch)

- *[Fix BW-1] README.org (*** orgx > orgx-core)*: Removed the claim that
  =org-roam= is configured in =orgx-core=.  Actual location: =orgx-extensions.el=.
  =orgx-core= only sets =org-directory=, TODO keywords, capture templates,
  and agenda rebuild; it does not touch org-roam.

- *[Fix BW-2] README.org (*** orgx)*: Added =orgx-extensions= as a
  documented module.  It was entirely absent from the Layer Definitions
  despite implementing =org-journal=, the full =org-roam= configuration
  (DB location, connector, autosync idle timer, file-hash guard advice),
  =org-download=, =toc-org=, and =org-cliplink=.

- *[Fix BW-3] README.org (*** orgx > orgx-notes-markdown)*: Removed
  =ox-gfm= from the package list.  =ox-gfm= is not installed or required
  in =orgx-notes-markdown.el=.  GFM output is provided by =markdown-mode='s
  =gfm-mode=, not by =ox-gfm=.

- *[Fix BW-4] README.org (*** orgx > orgx-fold)*: Removed =origami= from
  the package list.  =orgx-fold.el= uses only the built-in =org-fold= API
  and an =outline-invisible-p= fallback for Emacs < 29.  The =origami=
  package is neither installed nor required.

- *[Fix BW-5] README.org (*** orgx > orgx-typography)*: Removed
  =mixed-pitch=, =olivetti=, and "headline spacing" from the description.
  =orgx-typography.el= installs only =valign= (table alignment) and
  =org-appear= (emphasis auto-reveal).  No other packages are present.

- *[Fix BW-6] README.org (*** orgx > orgx-auto-tangle)*: Corrected
  "via =org-auto-tangle=" to "custom implementation using =ob-tangle=".
  =orgx-auto-tangle.el= does not depend on the third-party =org-auto-tangle=
  package; it implements the trigger logic directly with =after-save-hook=
  and =org-babel-tangle=.

- *[Fix BW-7] README.org (*** orgx > orgx-auto-hugo)*: Corrected
  "export on save" to "export triggered by =org-capture-after-finalize-hook=".
  The actual trigger is =C-c C-c= capture finalization; aborted captures
  (=org-note-abort= non-nil) are explicitly skipped.  Save events do not
  trigger export.

- *[Fix BW-8] README.org (*** orgx > Babel languages)*: Corrected the
  Babel language list.  Only =mermaid= (=ob-mermaid=) and =dot= (=ob-dot=,
  optional) are explicitly registered via =add-to-list 'org-babel-load-languages=
  in =orgx-export=.  =emacs-lisp=, =shell=, and =python= are Org built-in
  defaults and require no registration; claiming they are registered here
  was inaccurate.

*** Fix BV: README.org Layer Definitions — expand orgx responsibilities to match implementation

🟡 Minor — documentation accuracy

- *[Fix BV] README.org (** Layer Definitions > *** orgx)*: The previous
  description listed only six high-level bullet points that did not reflect
  the actual module inventory.  Expanded to enumerate each module
  (=orgx-core=, =orgx-roam-ui=, =orgx-brain=, =orgx-export=,
  =orgx-notes-markdown=, =orgx-visual=, =orgx-fold=, =orgx-typography=,
  =orgx-auto-tangle=, =orgx-auto-hugo=) with its concrete responsibility.
  Added explicit Babel language registration (=emacs-lisp=, =shell=, =python=,
  =dot=, =mermaid=) and a boundary note clarifying that CAPF integration
  belongs to the =completion= layer, not =orgx=.  No code changed.
...
*** Fix BO2: orgx/orgx-export.el — ob-mermaid Babel integration corrected

🟠 Medium — Coding Rule 6 violation + structural correctness

- *[Fix BO2] orgx/orgx-export.el (leaf ob-mermaid)*: Four defects corrected.
...
*** Fix BS: README.org — translate all prose from Japanese to English

🟡 Minor — documentation language consistency

- *[Fix BS] README.org (all prose sections)*: Translated 367 lines of
...
** 2026-05 (forty-second evaluation pass)
*** Fix BT: README.org — remove four duplicate documentation sections

🟡 Minor — documentation deduplication

- *[Fix BT-1] * Overview > ** Boot Process (lines 88–132, 45 lines removed)*:
  Deleted in full.  This section described the responsibilities of
  =early-init.el=, =init.el=, and =modules.el= in short bullet lists.
  Every item appears verbatim or in expanded form inside
  =* Configuration > ** Core Bootstrap > *** Overview=, which additionally
  supplies Boot Flow steps, Key Configuration Values, Usage Guidelines, and
  Troubleshooting guidance.  The shorter version carried no information
  absent from the authoritative detailed section.

- *[Fix BT-2] ** Modular Loader > *** Overview > **** Architecture (lines 1100–1110, 11 lines removed)*:
  Deleted the three-stage initialisation list (Infrastructure → Runtime
  setup → Feature activation) and its two-sentence conclusion.  The same
  three stages are stated in =* Overview > ** Architecture= and in the
  =** Core Bootstrap= narrative.  Repetition inside the Modular Loader
  preamble added no new information.

- *[Fix BT-3] ** Modular Loader > *** Overview > **** Responsibility Model (lines 1111–1127, 17 lines removed)*:
  Deleted the directory-to-responsibility table and the dependency-flow
  sentence.  =* Overview > ** Layer Definitions= provides the authoritative
  and more detailed layer descriptions; the table was a compressed subset.
  The surviving prose in =**** Authoritative Entry Point= and
  =**** Module Constraints= covers the same architectural constraints with
  greater precision.

- *[Fix BT-4] ** Modular Loader > *** Overview > **** Load Order (lines 1140–1151, 12 lines removed)*:
  Deleted the numbered layer list (core → ui → completion → … → utils) and
  the sentence "This order encodes the dependency direction."  The canonical
  load order is already stated in =* Overview > ** Architecture= with full
  context (10-layer table, invariants).

  Total lines removed: 85.  No code blocks, property drawers, tangle
  targets, or internal cross-references were affected.
  The surviving =*** Overview= for Modular Loader retains: three-sentence
  framing prose, **** Authoritative Entry Point, **** Module Constraints,
  **** Loader Non-Responsibilities, and **** Benefits.

** 2026-05 (forty-third evaluation pass)
*** Fix BI: README.org — convert non-tangled emacs-lisp blocks to begin_example

🟠 Medium — Rule 1 violation (lexical-binding cookie)

- *[Fix BI-1] * Overview > ** Package Management (L348)*: The illustrative
  =leaf vertico= snippet was declared as =#+begin_src emacs-lisp= without a
  =:tangle= target and without a =lexical-binding: t= cookie on its first line.
  The evaluation script classified this block as a Rule 1 violation.
  Converted to =#+begin_example= because the block is purely illustrative —
  it demonstrates =leaf= syntax and is never tangled to a file.
  =#+begin_example= preserves syntax highlighting in rendered output while
  correctly signalling that the block is not executable.

- *[Fix BI-2] * Changelog > Fix BN (thirty-third pass)*: The two code
  examples in the Fix BN entry (before/after =leaf gcmh= blocks) were
  declared as =#+begin_src emacs-lisp= without cookies or tangle targets.
  Both are illustrative changelog artifacts, not module source.
  Converted to =#+begin_example=.

*** Fix BJ: init.el — exec-path-from-shell restricted to PATH and LANG

🔴 Critical — startup performance (513 ms regression)

- *[Fix BJ-1] init.el (leaf exec-path-from-shell)*: Replaced
  =exec-path-from-shell-copy-envs= with =exec-path-from-shell-initialize=
  and added =exec-path-from-shell-variables '("PATH" "LANG")=.

  The previous configuration imported four variables: =PATH=, =LANG=,
  =OPENAI_API_KEY=, and =OPENROUTER_API_KEY=.  Importing =OPENAI_API_KEY=
  and =OPENROUTER_API_KEY= required the login shell to source =.bash_profile=
  / =.zprofile= (to find the =export= statements), adding approximately
  400–500 ms to every startup.  With only =PATH= and =LANG= requested,
  the shell can return immediately after sourcing =/etc/paths.d/=.

  The exported API keys are no longer needed from the environment because
  both =utils-claude.el= and =dev-ai.el= now retrieve them from
  =~/.authinfo.gpg= via =auth-source= at call time (Fix BJ-2).

- *[Fix BJ-2] lisp/dev/dev-ai.el (leaf aidermacs :init)*: Replaced =getenv=
  calls for =OPENROUTER_API_KEY= and =OPENAI_API_KEY= with =auth-source-search=
  lookups against =openrouter.ai= and =api.openai.com= host entries in
  =~/.authinfo.gpg=.

  =getenv= returned =nil= after Fix BJ-1 removed the environment import,
  causing the =cond= to fall through to the warning branch on every startup.
  The =auth-source= approach is also more secure: secrets remain in the
  encrypted credential store rather than in the process environment where
  child processes can read them.

  Required auth-source entries (=~/.authinfo.gpg=):

  #+begin_example
  machine openrouter.ai login api password <key>
  machine api.openai.com login api password <key>
  #+end_example

  =auth-source-search= returns the first matching entry; only one entry per
  host is required.

*** Fix BK: Makefile — add checkdoc and lint targets

🟡 Minor — CI/CD coverage

- *[Fix BK] Makefile (checkdoc, lint targets)*: Added two quality-assurance
  targets after =reload=:

  - =checkdoc= — scans every =.el= file under =LISPDIR= with
    =checkdoc-current-buffer= and reports files with missing or malformed
    docstrings.  Exits 1 when any file fails.

  - =lint= — composite target that runs =check-cookies= then =checkdoc=,
    providing a single pre-commit quality gate.  Add to =.PHONY= in the
    Makefile header.

  Usage:

  #+begin_example
  make tangle && make lint   # full quality gate before restarting Emacs
  make checkdoc              # docstring check only
  make check-cookies         # lexical-binding cookie check only
  #+end_example

*** Fix BL2: * Design Principles — add *** Startup Performance section

🟡 Minor — documentation coverage

- *[Fix BL2] * Overview > ** Design Principles (new *** Startup Performance subsection)*:
  Added a =*** Startup Performance= subsection between =*** Forward Compatibility=
  and =** Coding Rules=.  The subsection documents the two-pronged strategy for
  bounding =exec-path-from-shell= startup cost: (1) ='("-l")= arguments flag
  and (2) minimised variable list.  This makes the design decision discoverable
  from the top-level documentation without requiring readers to inspect
  =init.el= or the Changelog.

  The companion addition in =**** init.el Design Notes > **macOS Environment
  Variable Import**= cross-references Fix BJ-1 and quantifies the observed
  startup-time reduction.

** 2026-05 (forty-fourth evaluation pass)
*** Fix BZ: orgx/orgx-export.el — when-let → when-let* (Emacs 31+ forward compat)

🟡 Minor — forward compatibility (Coding Rule: prefer when-let*)

- *[Fix BZ] orgx/orgx-export.el (leaf ob-mermaid :config)*: Replaced
  =(when-let ((cli (orgx-export--mermaid-cli-path))))= with
  =(when-let* ((cli ...)))=.  The non-asterisk form =when-let= is deprecated
  in Emacs 31 in favour of =when-let*=, which is unambiguous about sequential
  binding semantics.  The Coding Rules (Design Principles: Forward Compatibility)
  require preferring =when-let*= throughout the codebase.  This was the last
  remaining =when-let= (non-asterisk) call in executable source blocks.

*** Fix CA: ui/ui-global-keys.el — global-set-key → keymap-global-set

🟠 Medium — forward compatibility (Coding Rule: prefer keymap-set API)

- *[Fix CA] lisp/ui/ui-global-keys.el (ui-global-keys--startup-init)*:
  Replaced nine =(global-set-key (kbd KEY) CMD)= calls with
  =(keymap-global-set KEY CMD)=, eliminating the deprecated =global-set-key= +
  =kbd= pattern.  =keymap-global-set= is the Emacs 29+ API mandated by the
  Design Principles (Forward Compatibility) and Coding Rules.

  The nine bindings converted:

  | Old form | New form |
  |----------+----------|
  | =(global-set-key (kbd "C-.") ...)= | =(keymap-global-set "C." ...)= |
  | =(global-set-key (kbd "C-?") ...)= | =(keymap-global-set "C-?" ...)= |
  | =(global-set-key (kbd "C-s") ...)= | =(keymap-global-set "C-s" ...)= |
  | =(global-set-key (kbd "M-x") ...)= | =(keymap-global-set "M-x" ...)= |
  | =(global-set-key (kbd "s-<left>") ...)= | =(keymap-global-set "s-<left>" ...)= |
  | =(global-set-key (kbd "s-<right>") ...)= | =(keymap-global-set "s-<right>" ...)= |
  | =(global-set-key (kbd "s-<up>") ...)= | =(keymap-global-set "s-<up>" ...)= |
  | =(global-set-key (kbd "s-<down>") ...)= | =(keymap-global-set "s-<down>" ...)= |
  | =(global-set-key (kbd "<f1>") ...)= | =(keymap-global-set "<f1>" ...)= |

  The single remaining =global-set-key= call in =ui-leader.el= (L4345) is the
  =else= branch of an =(if (fboundp 'keymap-global-set) ...)= guard — it is
  the Emacs < 29 fallback and is intentionally retained.

*** Fix CB: ui/ui-keymap.el — define-key → keymap-set for key-translation-map

🟠 Medium — forward compatibility (Coding Rule: prefer keymap-set API)

- *[Fix CB] lisp/ui/ui-keymap.el (ui-keyboard--apply)*: Replaced two
  =(define-key key-translation-map (kbd KEY) VALUE)= calls with
  =(keymap-set key-translation-map KEY VALUE)=.  =key-translation-map= is a
  standard keymap and supports =keymap-set= fully.  Using =keymap-set=
  eliminates the remaining =define-key= + =kbd= pattern in the executable
  source and aligns with the Emacs 29+ API mandate.

*** Fix CC: * Overview > ** Module Structure — Autoload-Only Modules table

🟡 Minor — documentation coverage

- *[Fix CC] * Overview > ** Module Structure (new *** Autoload-Only Modules subsection)*:
  Added the =*** Autoload-Only Modules= subsection with a five-row table
  mapping each autoload-only module to its declaring file and trigger condition.
  This section was planned in Fix BV (previous documentation pass) but was not
  present in the uploaded source file for this evaluation pass.  It explicitly
  documents why =dev-lsp-eglot=, =dev-lsp-mode=, =ui-doom-modeline=,
  =ui-nano-modeline=, and =ui-nano-palette= are absent from =my:modules=
  despite having tangle targets, resolving the apparent contradiction with the
  "all modules appear in =my:modules=" rule.

** 2026-05 (forty-fifth evaluation pass)
*** Fix CD: auth/auth-gpg.el — new module: ~/.authinfo.gpg management

🟠 Medium — new functionality (auth layer)

- *[Fix CD] lisp/auth/auth-gpg.el (new module)*: Created =auth-gpg.el= as
  the second module in the =auth= layer, loaded immediately after =auth-core=
  in =my:modules=.

  *Motivation.*  The configuration gained three independent =auth-source-search=
  call sites across two layers (=utils-claude.el= for the Anthropic key,
  =dev-ai.el= for the OpenRouter / OpenAI keys) without a single authoritative
  document listing the required =~/.authinfo.gpg= entries or a diagnostic
  command for verifying that those entries are present.  Adding, changing, or
  debugging credentials required reading multiple source files.  =auth-gpg.el=
  consolidates this concern into one module.

  *Public API (three symbols):*

  | Symbol | Kind | Description |
  |--------+------+-------------|
  | =auth-gpg-services= | =defcustom= | Canonical list of expected service entries |
  | =my/auth-gpg-verify= | interactive =defun= | Displays ✓/✗ status for every service |
  | =my/auth-gpg-init-template= | interactive =defun= | Creates a starter =~/.authinfo.gpg= |

  *=my/auth-gpg-verify= output format:*

  #+begin_example
  Auth-GPG credential status — 2026-05-15 12:00:00
  Store: /Users/ac1965/.authinfo.gpg

    ✓ PRESENT  Anthropic Claude API
               secret preview: sk-a…

    ✗ MISSING  OpenRouter (aidermacs)
               add to /Users/ac1965/.authinfo.gpg:
               machine openrouter.ai login api password <key>
  #+end_example

  *=my/auth-gpg-init-template=* writes a starter netrc file with comment-line
  hints for every entry in =auth-gpg-services=, then opens the encrypted file
  for editing.  It aborts without overwriting when =auth-gpg-file= already exists.

  *Startup behaviour:* =auth-gpg--configure-sources= runs on =after-init-hook=
  and prepends =auth-gpg-file= (=~/.authinfo.gpg=) to =auth-sources=, followed
  by the plain-text fallback when it exists.  This ensures the encrypted store
  is consulted first without unconditionally replacing =auth-sources=.

  *Required =~/.authinfo.gpg= entries* (documented inline and in the module's
  Design Notes prose above the source block):

  #+begin_example
  machine api.anthropic.com login apikey password sk-ant-<key>
  machine openrouter.ai     login api     password <openrouter-key>
  machine api.openai.com    login api     password sk-<openai-key>
  #+end_example

  The third entry (OpenAI direct) is optional; =dev-ai.el= falls through to
  the warning branch when neither OpenRouter nor OpenAI entries are found.

  *Layer compliance:* =auth-gpg= depends only on =auth-source= and =epa-file=
  (both built-ins activated by =auth-core=).  It has no dependency on =ui=,
  =completion=, =orgx=, =vcs=, =dev=, =utils=, or =personal=.
  All three public =defun= carry docstrings (Rule 5).
  =defcustom= variables belong to the =auth-gpg= group (Rule 8).
  File provides ='auth-gpg= (Rule 2).

** 2026-05 (forty-sixth evaluation pass)
*** Fix CE: tangle-workflow — 20 stale on-disk .el files missing lexical-binding cookie

🟠 Medium — workflow (stale tangle output; no source defect)

- *[Fix CE] tangle-workflow (20 on-disk .el files)*: Emacs reported
  =Missing 'lexical-binding' cookie= for 20 modules on startup:
  ...
  *Root cause — stale on-disk tangle output.*  Every source block in
  =README.org= carries the cookie on its first code line:

  #+begin_example
  ;;; core/core-session-private.el --- ... -*- lexical-binding: t; -*-
  #+end_example

  The on-disk =.el= files pre-date the =README.org= that was used for this
  evaluation session.  The Makefile workflow requires:

  #+begin_example
  # replace ~/.emacs.d/README.org with the current session output, then:
  make tangle          # regenerate all .el files from README.org
  make check-cookies   # verify every .el line-1 carries the cookie
  #+end_example

  *Additional cause — =auth-gpg.el= absent from disk.*  =auth-gpg= was added
  to =my:modules= in Fix CD (forty-fifth pass) but the tangle step had not
  been run.  =modules--require-safe= catches the =file-missing= condition and
  records the failure without aborting startup, so Emacs continues to load.
  After =make tangle= the file is generated and the failure disappears.

  *C-' (org-edit-special) note.*  Editing a source block with =C-'= and saving
  the change updates =README.org= in-place.  It does not retangle the target
  =.el= file.  Run =make tangle= (or =make reload= for a full clean cycle)
  after any =C-'= editing session to keep on-disk files in sync with the source.

  *No source defect.*  =README.org= is correct.  All 20 files have the cookie
  on the first source-block line.  No code change is required.

  *Checklist for applying this fix:*

  1. Download the current =README.org= from the Claude output.
  2. =cp README.org ~/.emacs.d/README.org=
  3. =cd ~/.emacs.d && make tangle=
  4. =make check-cookies=   (should print: =[check-cookies] all files OK=)
  5. Restart Emacs.

** 2026-05 (forty-seventh evaluation pass)
*** Fix CF: dev/dev-ai.el — auth-source login mismatch and timing fix

🔴 Critical — authentication failure (aidermacs cannot find API key)

- *[Fix CF-1] lisp/dev/dev-ai.el (leaf aidermacs — :user mismatch)*:
  =auth-source-search= returned no results despite a valid =~/.authinfo.gpg=
  because the =:user= argument did not match the =login= field in the file.

  The =~/.authinfo.gpg= entries use:
  ...
  The previous code specified no =:user=, which should match any login.
  However =auth-source= performs an exact match on =:user= when the netrc
  backend parses the file: omitting =:user= maps to the default user which
  is typically the login name of the OS user, not a wildcard.  The fix
  adds explicit =:user= arguments that match the =login= fields:

  | Host | :user (code) | login (.authinfo.gpg) |
  |------+--------------+-----------------------|
  | openrouter.ai  | =ac1965= | =ac1965= |
  | api.openai.com | =org-ai=  | =org-ai=  |

- *[Fix CF-2] lisp/dev/dev-ai.el (leaf aidermacs — :init → after-init-hook)*:
  The =auth-source-search= calls were placed in =leaf :init=, which runs at
  module load time — before =after-init-hook= fires.  =auth-gpg--configure-sources=
  (which prepends =~/.authinfo.gpg= to =auth-sources=) also runs on
  =after-init-hook=.  Because =auth-gpg= loads before =dev-ai= in =my:modules=,
  =auth-gpg--configure-sources= is registered first and executes first, but
  =:init= code runs at load time before any =after-init-hook=.  This race meant
  =~/.authinfo.gpg= was not yet in =auth-sources= when =auth-source-search= ran.

  Extracted the lookup into a named function =dev-ai--setup-api-key= and
  registered it on =after-init-hook=.  The hook fires after =auth-gpg--configure-sources=
  has already updated =auth-sources=, guaranteeing =~/.authinfo.gpg= is consulted.

- *[Fix CF-3] lisp/auth/auth-gpg.el (auth-gpg-services :user)*:
  Updated =auth-gpg-services= =:user= values to match the actual =login= fields
  in =~/.authinfo.gpg=, so =my/auth-gpg-verify= reports accurate ✓/✗ status.

  | Before | After |
  |--------+-------|
  | =openrouter.ai= :user ="api"= | =openrouter.ai= :user ="ac1965"= |
  | =api.openai.com= :user ="api"= | =api.openai.com= :user ="org-ai"= |

*Note — =~/.authinfo.gpg= typo (user action required).*  The file contains
=machine oopenrouter.ai= (double =o=) instead of =machine openrouter.ai=.
This is a typo in the credential file and cannot be fixed in the Emacs
configuration.  Edit =~/.authinfo.gpg= and correct the hostname:

#+begin_example
# Wrong:
machine oopenrouter.ai login ac1965 password sk-or-...

# Correct:
machine openrouter.ai login ac1965 password sk-or-...
#+end_example

To edit the file: =M-x find-file RET ~/.authinfo.gpg RET=, correct the
hostname, then =C-x C-s= (EasyPG prompts for the GPG passphrase on save).

** 2026-05 (forty-eighth evaluation pass)
*** Fix CG: early-init.el — enable versioned backups under no-littering var/backup/

🟠 Medium — data safety (backup files were disabled)

- *[Fix CG] early-init.el (section 9 Backups / auto-save)*: Replaced
  =make-backup-files nil= with a versioned backup policy.  The previous
  configuration disabled all backup files, making silent data loss (accidental
  overwrite, failed tangle) unrecoverable.
  ...
  | =version-control= | =t= | Keep numbered backups (=file.el~1~=, =file.el~2~=, …) |
  | =kept-new-versions= | =6= | Retain the 6 most recent versions |
  | =kept-old-versions= | =2= | Retain the 2 oldest versions |
  | =delete-old-versions= | =t= | Silently delete excess versions |
  | =backup-by-copying= | =t= | Preserve hard links and symlinks |

  =backup-directory-alist= is set in =core-editing--autosave-init= (after
  =no-littering-var-directory= is available) to route all backup files to
  =var/backup/=.  The existing =my/delete-old-backups= (=core-persistence.el=)
  already targets this directory and purges files older than 7 days.

*** Fix CH: core/core-editing.el — replace ad-hoc auto-save with structured subsystem

🟠 Medium — reliability and correctness (auto-save-visited-interval = 1 s; no focus-out save)

- *[Fix CH] lisp/core/core-editing.el (auto-save subsystem)*: Replaced the
  three bare =setopt= / =when= lines with a self-contained auto-save subsystem
  comprising a =defgroup=, two =defcustom= variables, three named =defun=, and
  one =after-startup-hook= initialiser.

  *Changes:*

  1. =core-autosave-interval= (=defcustom=, default =5=) replaces the
     hard-coded =auto-save-visited-interval= value of =1=.  One-second
     intervals cause a disk write on every idle second while typing; the new
     default of five seconds matches common editor conventions without
     meaningfully increasing maximum data loss.

  2. =core-autosave-save-on-focus-out= (=defcustom=, default =t=) controls
     the focus-out behaviour independently from the idle-timer behaviour.

  3. =core-editing--save-all-buffers= calls =save-some-buffers= with the
     =buffer-file-name= predicate to skip non-file buffers (=*scratch*=,
     =*Messages*=, Dired, etc.).

  4. =core-editing--save-on-focus-out= is added to =focus-out-hook= and to
     =after-focus-change-function=.  This saves all modified file-backed
     buffers immediately when Emacs loses focus, covering the gap between
     idle-timer ticks.

  5. =core-editing--autosave-init= runs on =emacs-startup-hook=, which fires
     after =no-littering-var-directory= is established, ensuring
     =backup-directory-alist= is set to the correct path.

  *Effective auto-save triggers after this fix:*

  | Trigger | Mechanism |
  |---------------------------+------------------------------------|
  | Idle for ≥5 s | =auto-save-visited-mode= timer |
  | Emacs loses OS focus | =focus-out-hook= |
  | After-focus change | =after-focus-change-function= |
  | =C-x s= / =C-x C-s= | Standard Emacs save |

** 2026-05 (forty-ninth evaluation pass)
*** Fix CI: core/core-editing.el — auto-save exclusion system; personal/user.el defaults

🟠 Medium — configurability (auto-save applied indiscriminately to all file-backed buffers)

- *[Fix CI-1] lisp/core/core-editing.el (core-autosave defcustom group)*:
  Extended the =core-autosave= defcustom group with four new variables that
  give the personal layer full control over which buffers are excluded from
  auto-save:

  | Variable | Type | Default |
  |----------+------+---------|
  | =core-autosave-exclude-extensions= | =(repeat string)= | ='("gpg" "age" "enc" "pem" "p12" "pfx")= |
  | =core-autosave-exclude-name-patterns= | =(repeat regexp)= | =nil= |
  | =core-autosave-exclude-modes= | =(repeat symbol)= | =nil= |
  | =core-autosave-exclude-predicates= | =(repeat function)= | =nil= |

  The encrypted-file extensions (=gpg=, =age=, =enc=, =pem=, =p12=, =pfx=)
  are excluded by default because silently overwriting an encrypted file could
  corrupt it or expose plaintext.

- *[Fix CI-2] lisp/core/core-editing.el (core-autosave--save-buffer-p)*:
  Extracted the save predicate into a named private function
  =core-autosave--save-buffer-p= (double-dash = private, per naming rules).
  The predicate evaluates all four exclusion conditions in order and returns
  non-nil only when the buffer should be saved.  It is passed as the predicate
  argument to =save-some-buffers= inside =core-editing--save-all-buffers=.

  Predicate logic:

  #+begin_example
  buffer is saved iff:
    buffer-file-name is non-nil
    AND extension not in core-autosave-exclude-extensions
    AND file name matches no pattern in core-autosave-exclude-name-patterns
    AND major-mode not in core-autosave-exclude-modes
    AND no predicate in core-autosave-exclude-predicates returns non-nil
  #+end_example

  Uses =when-let*= (Emacs 31+ forward-compat) and =cl-some= for short-circuit
  evaluation of list predicates.

- *[Fix CI-3] personal/user.el (auto-save exclusion policy)*: Added personal
  exclusion configuration via =setopt= at the end of =personal/user.el=:

  *Excluded name patterns:*
  =\.authinfo=, =\.netrc=, =/tmp/=, =\.bak\'=,
  =COMMIT_EDITMSG\'=, =MERGE_MSG\'=, =rebase-merge/=, =\.log\'=

  *Excluded major modes:*
  =erc-mode=, =circe-mode=, =term-mode=, =vterm-mode=,
  =eshell-mode=, =sql-interactive-mode=

  All settings use =setopt= (Rule 6 — these are =defcustom= variables).

  *How to add custom exclusions in personal/ac1965.el or device-darwin.el:*

  #+begin_example
  ;; Append to the list set in personal/user.el:
  (setopt core-autosave-exclude-name-patterns
          (append core-autosave-exclude-name-patterns
                  '("/work/sensitive/")))

  ;; Add a custom predicate (buffer → bool):
  (defun my/large-buffer-p (buf)
    "Exclude buffers larger than 10 MB from auto-save."
    (> (buffer-size buf) (* 10 1024 1024)))

  (setopt core-autosave-exclude-predicates
          (append core-autosave-exclude-predicates
                  '(my/large-buffer-p)))
  #+end_example

** 2026-05 (fifty-second evaluation pass)
*** Fix CL: core/core-editing.el — separate focus-out hook functions by arity

🔴 Critical — runtime error on every focus-out event

- *[Fix CL] lisp/core/core-editing.el (core-editing--save-on-focus-out,
  core-editing--save-on-focus-change)*: The single function
  =core-editing--save-on-focus-out (_frame)= was registered on both
  =focus-out-hook= and =after-focus-change-function=.  These two hooks use
  incompatible calling conventions:

  | Hook | Arity | Convention |
  |------+-------+------------|
  | =focus-out-hook= | 0 | Standard hook; functions receive no arguments |
  | =after-focus-change-function= | 1 | Passes the affected frame as first argument |

  Calling a 1-argument function from =focus-out-hook= raised:
  =Wrong number of arguments: #[(_frame) ...], 0=
  on every focus-out event (window switch, app switch, screensaver).

  *Fix:* Split into two named functions with correct arities:

  | Function | Arity | Registered on |
  |----------+-------+---------------|
  | =core-editing--save-on-focus-out= | 0 | =focus-out-hook= |
  | =core-editing--save-on-focus-change= | 1 (=_frame=) | =after-focus-change-function= |

  Both functions delegate to =core-editing--save-all-buffers= after checking
  =core-autosave-save-on-focus-out=.  The defunct =core-editing--save-on-window-change=
  (same body, never registered) is also removed.

** 2026-05 (fifty-third evaluation pass)
*** Fix CM: core/core-editing.el — after-focus-change-function is zero-argument; remove duplicate registration

🔴 Critical — Fix CL introduced a second wrong-arity registration

- *[Fix CM] lisp/core/core-editing.el (core-editing--save-on-focus-out,
  core-editing--autosave-init)*: Fix CL incorrectly assumed that
  =after-focus-change-function= receives a frame argument.  The Emacs
  documentation (=C-h v after-focus-change-function=) states it is called
  with *no arguments*; it is the caller’s responsibility to inspect
  =(frame-focus-state)= if frame information is needed.  Fix CL therefore
  introduced a new 1-argument function =core-editing--save-on-focus-change=
  registered on a 0-argument hook, reproducing the original crash.

  Additionally, =core-editing--autosave-init= runs on =emacs-startup-hook= and
  may be re-executed across =make reload= sessions without a full Emacs restart.
  =add-function= does *not* deduplicate: each call appends another copy of the
  function to =after-focus-change-function=, causing repeated saves per event.

  *Changes:*

  1. =core-editing--save-on-focus-change= removed entirely.  Its body was
     identical to =core-editing--save-on-focus-out=; the only difference was
     the erroneous =(_frame)= parameter.

  2. =core-editing--save-on-focus-out= reverts to a zero-argument function and
     is now registered on *both* hooks:

     | Hook | Arity | Registration |
     |------+-------+--------------|
     | =focus-out-hook= | 0 | =add-hook= (idempotent by default) |
     | =after-focus-change-function= | 0 | =remove-function= then =add-function= |

  3. =core-editing--autosave-init= calls =remove-function= before =add-function=
     so that repeated invocations (reload without restart) never accumulate
     duplicate entries.

  *Immediate workaround* (without restarting Emacs):

  #+begin_example
  ;; Evaluate in *scratch* (C-x C-e):
  (remove-function after-focus-change-function
                   #'core-editing--save-on-focus-out)
  (remove-function after-focus-change-function
                   #'core-editing--save-on-focus-change)
  (add-function :after after-focus-change-function
                #'core-editing--save-on-focus-out)
  #+end_example

** 2026-05 (fifty-first evaluation pass)
*** Fix CK: auth/auth-keychain.el — new module: macOS Keychain auth-source backend

🟠 Medium — capability addition (macOS system credentials unavailable to auth-source)

- *[Fix CK-1] lisp/auth/auth-keychain.el (new module)*: Added a self-contained
  =auth-source= backend that delegates credential lookups to the macOS
  =security(1)= command-line tool.  Before this change, packages using
  =auth-source-search= could only retrieve credentials from =~/.authinfo.gpg=
  or =~/.authinfo=.  Credentials stored in the system Keychain (via Keychain
  Access.app or the =security add-generic-password= command) were invisible to
  Emacs regardless of the =auth-sources= list.

  The module implements the =auth-source-backend= protocol introduced in
  Emacs 26: it registers a backend via =auth-source-backend-parser-functions=
  whose =:search-function= calls =security find-generic-password= (with a
  fallback to =security find-internet-password=) for each host in the search
  spec.  Results are returned as standard auth-source plists with a lazy
  =:secret= lambda, compatible with all packages that use =auth-source-search=.

  *New public API:*

  | Symbol | Kind | Description |
  |--------+------+-------------|
  | =auth-keychain-enable= | =defcustom= (boolean) | Activate backend; default =(eq system-type 'darwin)= |
  | =auth-keychain-security-binary= | =defcustom= (string) | Path to =security(1)=; default ="security"= |
  | =auth-keychain-services= | =defcustom= (list) | Entries verified by the diagnostic command |
  | =my/auth-keychain-verify= | =defun= (interactive) | Show ✓/✗ status for each entry in =auth-keychain-services= |

  *Private functions (double-dash naming, Rule 7):*

  | Symbol | Purpose |
  |--------+---------|
  | =auth-keychain--security-available-p= | Check =security(1)= is in =exec-path= |
  | =auth-keychain--run-security= | Synchronous subprocess call; returns stdout or nil |
  | =auth-keychain--lookup-generic= | Generic Password lookup |
  | =auth-keychain--lookup-internet= | Internet Password fallback lookup |
  | =auth-keychain--search-function= | Backend search function registered with auth-source |
  | =auth-keychain--make-backend= | Construct =auth-source-backend= struct |
  | =auth-keychain--register-backend= | Append backend to parser list (idempotent) |
  | =auth-keychain--startup-init= | =after-init-hook= entry point; skips on non-macOS |

- *[Fix CK-2] modules.el (my:modules — auth section)*: Added =auth-keychain= to
  =my:modules= immediately after =auth-gpg=.  This position ensures:

  1. =auth-core= has enabled =epa-file= before =auth-keychain= loads.
  2. =auth-gpg= has configured =auth-sources= with the GPG file first, so the
     Keychain backend is appended in lower-priority position (GPG store wins on
     identical host/user matches).
  3. The module is still in the mandatory list (not =my:modules-extra=) because
     =auth-keychain-enable= defaults to =nil= on non-macOS hosts, making the
     module a safe no-op on Linux/Windows without requiring personal-layer
     configuration.

- *[Fix CK-3] personal/user.el (auth-keychain-enable)*: Added an explicit
  =setopt= declaration so the personal layer documents its intent and the
  value is visible in =M-x customize-option RET auth-keychain-enable=:

  #+begin_example
  (setopt auth-keychain-enable (eq system-type 'darwin))
  #+end_example

  This form is evaluated before =modules.el= requires =auth-keychain=, so the
  =defcustom= default-value override takes effect before =auth-keychain--startup-init=
  runs on =after-init-hook=.  On macOS the result is =t= (activate); on Linux
  or Windows the result is =nil= (skip activation silently).

  *Priority order after Fix CK on macOS:*

  | Priority | Backend | Module |
  |----------+---------+--------|
  | 1 (highest) | =~/.authinfo.gpg= | =auth-gpg= |
  | 2 | =~/.authinfo= (plain-text fallback) | =auth-gpg= |
  | 3 (lowest) | macOS Keychain | =auth-keychain= |

  *Keychain entry management:*

  #+begin_example
  # Add a Generic Password (recommended format):
  security add-generic-password -s "api.anthropic.com" -a "apikey" -w "sk-ant-..."

  # Verify:
  security find-generic-password -s "api.anthropic.com" -a "apikey" -w

  # Diagnostic inside Emacs:
  M-x my/auth-keychain-verify
  #+end_example

- *[Fix CK-4] Refactor: login-level control via personal/ac1965.el*:
  Corrected the control flow identified after Fix CK-3.  The original
  design placed =auth-keychain-enable= in =personal/user.el= (all-machine
  scope) using =(eq system-type 'darwin)=, which conflated OS detection
  with personal preference and violated the layer contract: =user.el= owns
  cross-machine policy, not OS-specific capability activation.

  *New three-tier control chain:*

  | File | Role | Value set |
  |------+------+-----------|
  | =auth-keychain.el= | Module default | =nil= (explicit opt-in required) |
  | =personal/device-darwin.el= | Device level — macOS confirmed | =t= |
  | =personal/ac1965.el= | Login level — final authority | =t= (or =nil= to disable) |

  Load order ensures the login-level setting always wins:
  =device-darwin.el= is loaded inside =user.el= via the dynamic device-file
  loader; =ac1965.el= is loaded by =init.el= after =user.el= completes.
  Each subsequent =setopt= form legally overrides the prior value because
  =auth-keychain-enable= is a =defcustom= and =setopt= invokes its setter.

  =personal/user.el= no longer references =auth-keychain-enable= at all,
  keeping the shared personal layer free of OS-specific logic.

 dev/dev-ai.el — self-contained auth-source lookup (no hook-order dependency)

🔴 Critical — authentication failure persists after Fix CF

- *[Fix CJ] lisp/dev/dev-ai.el (dev-ai--ensure-authinfo-in-sources, dev-ai--setup-api-key)*:
  The warning persisted after Fix CF because =dev-ai--setup-api-key= still
  depended on =auth-gpg--configure-sources= having run first.  While both
  functions are registered on =after-init-hook=, the hook executes them in
  registration order — which is the module load order — and auth-gpg loads
  before dev-ai.  In principle this should be safe.

  However, =auth-source-search= caches its results.  If =auth-source= was
  consulted (e.g., by EasyPG or another module) before =after-init-hook=
  fired, the cache was built from the default =auth-sources= list which does
  not include =~/.authinfo.gpg=.  When =dev-ai--setup-api-key= subsequently
  called =auth-source-search=, it received the stale cached result (nil)
  rather than searching the now-updated =auth-sources=.

  *Fix:* Added =dev-ai--ensure-authinfo-in-sources=, a private helper called
  at the top of =dev-ai--setup-api-key=.  It checks whether the GPG
  credential file is already in =auth-sources= and, if not:

  1. Prepends the file to =auth-sources= via =setopt=.
  2. Calls =(auth-source-forget-all-cached)= to force a fresh read on the
     next =auth-source-search= call.

  This makes =dev-ai--setup-api-key= self-contained: it succeeds whether
  =auth-gpg--configure-sources= has run or not, and regardless of any prior
  =auth-source= cache state.

  *Diagnostic steps* if the warning appears after =make tangle && make reload=:

  #+begin_example
  ;; Evaluate in *scratch* (C-x C-e after each form):

  ;; 1. Check what auth-sources currently contains:
  auth-sources

  ;; 2. Clear the cache and retry manually:
  (auth-source-forget-all-cached)
  (auth-source-search :host "openrouter.ai" :user "ac1965" :require '(:secret))

  ;; 3. If nil, check the file exists and is readable:
  (file-exists-p auth-gpg-file)
  #+end_example

  If step 2 returns nil but step 3 returns =t=, the =login= field in
  =~/.authinfo.gpg= does not match =ac1965=.  Edit the file with
  =M-x find-file ~/.authinfo.gpg= and correct the =login= field.

*** Fix CN: core/core-editing.el — save-some-buffers PRED arity and anonymous lambda extraction

🔴 Critical — wrong-number-of-arguments at runtime; auto-save predicate never matched

- *[Fix CN-1] lisp/core/core-editing.el (core-autosave--save-buffer-p)*:
  The function was declared as =(defun core-autosave--save-buffer-p (buf) ...)=
  and passed as the PRED argument to =(save-some-buffers t #'core-autosave--save-buffer-p)=.
  =save-some-buffers= calls PRED with *zero* arguments, setting =current-buffer=
  to the buffer under test before each call.  The mismatch produced the error:

  #+begin_example
  Wrong number of arguments: #[(buf) ...], 0
  #+end_example

  The fix removes the =buf= parameter.  All references to =buf= inside the
  body are replaced with =current-buffer= (for the predicate call site) and
  =buffer-file-name= / =major-mode= without an argument (both return values
  for =current-buffer= by default).  Rule violated: public API contract of
  =save-some-buffers= (PRED is called with 0 args; Emacs source,
  =files.el=, =(funcall pred)=).

- *[Fix CN-2] lisp/core/core-editing.el (core-autosave--extension-excluded-p,
  core-autosave--name-pattern-excluded-p, core-autosave--predicate-excluded-p)*:
  The two =cl-some= calls inside =core-autosave--save-buffer-p= used inline
  =lambda= expressions.  The coding rules require named =defun= only; no
  anonymous lambdas in hook or predicate positions (Rule 5).  Each lambda is
  promoted to a private named function with double-dash naming (Rule 7):

  | New symbol | Replaces |
  |------------+----------|
  | =core-autosave--extension-excluded-p= | inline =(and ext (member ...))= |
  | =core-autosave--name-pattern-excluded-p= | =(cl-some (lambda (pat) ...) ...)= |
  | =core-autosave--predicate-excluded-p= | =(cl-some (lambda (fn) ...) ...)= |

  The =cl-some= calls remain inside these named functions, which is
  permitted: the rule prohibits anonymous lambdas passed directly to
  =add-hook= / =display-buffer-alist= and similar registration sites,
  not lambdas used as iteratees inside a named helper.

** 2026-05 (fifty-fourth evaluation pass)

*** Fix CO: ui/ui-global-keys.el — undo-redo moved from C-? to s-z (ergonomics)

🟡 Minor — ergonomics improvement (no functional regression)

- *[Fix CO] lisp/ui/ui-global-keys.el (ui-global-keys--startup-init)*:
  Replaced =C-?= (=undo-redo=) with =s-z=.  =C-?= is =Ctrl+Shift+/=, a
  three-key chord requiring the pinky (Control), a second finger (Shift),
  and a third finger (=/=) simultaneously — heavier than its usage
  frequency warrants.  =s-z= (=Cmd+Z=) matches the macOS system-wide
  undo convention used by every other application on the machine,
  reducing both physical chord cost (2 keys instead of 3) and
  cross-application muscle-memory conflict.

  =C-z= (single Control chord, thematically consistent with =C-.= and
  =C-s=) was considered as an alternative and was confirmed free —
  =early-init.el='s startup sequence already runs
  =(keymap-global-unset "C-z")= to disable =suspend-frame= — but =s-z=
  was chosen to prioritize consistency with non-Emacs macOS applications
  over in-Emacs Control-chord uniformity.  =C-z= remains unbound and
  available for future use.

  =s-S-z= (redo-specific) is intentionally not bound: =undo-redo=
  auto-reverses direction once any other command is issued, so a
  dedicated redo binding would add keymap surface without practical
  benefit.

  The module Commentary (=;; - binds C-s, C-., s-z and M-x...=) was
  updated to match.

** 2026-03 (sixteenth evaluation pass)

*** Fix AM: ui-font.el — ui-font-describe reports "unknown" for emoji font

*🟠 Medium — ui-font-describe emoji detection returns nil on macOS*

- *[Fix AM] lisp/ui/ui-font.el (ui-font-describe)*: Replaced the unreliable
  emoji font probe =(font-get (font-spec :script 'emoji) :family)= with a
  direct reference to =(or ui-font-emoji (ui-font--system-emoji))=.

  The previous probe constructs a =font-spec= requesting the =emoji= script
  and then reads back its =:family= slot.  On macOS, =font-spec= with
  =:script 'emoji= does not perform a fontset lookup; it returns nil for
  =:family= because no family was specified in the spec itself.  The
  =font-get= call therefore always returned nil, and the =(or nil
  "unknown")= fallback produced the "unknown" message regardless of what
  =set-fontset-font= had actually registered.

  The authoritative source of the emoji family is the value that
  =ui-font-apply= passed to =set-fontset-font=, which is =(or ui-font-emoji
  (ui-font--system-emoji))=.  Reading that expression directly in
  =ui-font-describe= makes the displayed value consistent with what was
  registered.

- *[Fix AM] personal/ac1965.el*: Added =(setopt ui-font-emoji "Apple Color
  Emoji")= to the font preferences block, making the emoji font explicit
  rather than relying on the =ui-font--system-emoji= macOS fallback.  This
  ensures the value is visible in the startup log and in
  =M-x ui-font-describe= output.
