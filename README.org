...
#+begin_src emacs-lisp
  ...
  ;; - LaTeX バックエンドの *.svg 画像リンクを \includegraphics から
  ;;   \includesvg に書き換える（org 標準機能では変換されないため）
  ;;
  ...
  ;;   同梱済みのため、追加インストールは不要。
  ;;
  ;;   *.svg を図として使う文書（design_spec.org 等）向けに、org-latex-pdf-process
  ;;   には -shell-escape を付与済み。文書側の #+LATEX_HEADER に
  ;;   \usepackage[inkscapelatex=false]{svg} を追加し、Inkscape CLI ではなく
  ;;   rsvg-convert（要: brew install librsvg）で SVG をレンダリングする構成を
  ;;   前提としている。\includegraphics → \includesvg の書き換えは
  ;;   orgx-export--svg-to-includesvg（下記）が担う。文書本体の画像リンクは
  ;;   [[file:foo.svg]] のままでよく、個々のリンクを書き換える必要はない。
  ;;; Code:
...
where npm global installs are not on PATH."
  ...
  (defun orgx-export--svg-to-includesvg (contents backend _info)
    "Rewrite \\includegraphics{FILE.svg} to \\includesvg{FILE} for the latex backend.
...
file extension. The svg package (loaded via #+LATEX_HEADER, rendered through
rsvg-convert under -shell-escape) only understands \\includesvg, not
\\includegraphics, so without this rewrite every *.svg figure silently fails
to compile. Runs on `org-export-filter-link-functions', so it only touches
the exported form of image links, never the buffer itself."
    (when (org-export-derived-backend-p backend 'latex)
      (replace-regexp-in-string
       "\\\\includegraphics\\(\\[[^]]*\\]\\)?{\\([^}]+\\)\\.svg}"
       (lambda (whole)
         (string-match
          "\\\\includegraphics\\(\\[[^]]*\\]\\)?{\\([^}]+\\)\\.svg}" whole)
         (concat "\\includesvg" (or (match-string 1 whole) "")
                 "{" (match-string 2 whole) "}"))
       contents)))

  (defun orgx-export--configure-latex ()
    "Register the ltjsarticle class, route PDF export through latexmk, and
rewrite *.svg image links to \\includesvg for the latex backend.
`ox-latex' is required lazily here (after `org' loads) rather than at
top level, mirroring `orgx-export--register-mermaid' above."
    (require 'ox-latex)
    ...
    (setopt org-latex-pdf-process
            '("latexmk -synctex=1 -halt-on-error -shell-escape -output-directory=%o %f"))
    (add-to-list 'org-export-filter-link-functions
                 #'orgx-export--svg-to-includesvg))
