...
Customise in personal/user.el with setopt."
  ...
  (defun core-autosave--save-buffer-p (buf)
    "Return non-nil when BUF should be auto-saved.
    ...
    (when-let* ((fname (buffer-file-name buf)))
      (let* ((ext  (file-name-extension fname))
        ...
        (and
         ...
         (not (and ext (member (downcase ext)
                               core-autosave-exclude-extensions)))
         ;; 2. Name pattern not excluded
         (not (cl-some (lambda (pat) (string-match-p pat fname))
                       core-autosave-exclude-name-patterns))
         ;; 3. Major mode not excluded
         (not (memq mode core-autosave-exclude-modes))
         ;; 4. Custom predicates
         (not (cl-some (lambda (fn) (funcall fn buf))
