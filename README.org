...
*** Fix AJ, AK (applied): 10件の効率性・重複廃止リファクタリング

この評価パスで確定した修正 (Fix AJ, AK) を添付の README.org に適用した。
前回パスの出力ファイルには反映済みだったが、添付ファイルへの再適用が必要だった。

*🟠 Medium — completion-corfu--init: 3連続 add-to-list O(n)×3*

- *[Fix AJ] lisp/completion/completion-corfu.el (completion-corfu--init)*:
  3つの連続する =(add-to-list 'completion-at-point-functions ...)= を
  単一の =setq= + =append= に置き換えた（O(n)×3 → O(1)）。
  優先順位（elisp-symbol > file > dabbrev > 既存）を明示する。
  =corfu-margin-formatters= への追加は =cl-pushnew= に変更し重複登録を防ぐ。

*🟠 Medium — core-native: O(n) length 呼び出し × 7箇所*

- *[Fix AJ] lisp/core/core-native.el*: =core-native--count= 変数を追加し
  =core-native--warnings= リストの長さを常時追跡する。
  =core-native--cap-collect=, =core-native--maybe-notice-threshold=,
  =core-native--collect= (=cl-incf= で同期), =core-native-clear-warnings=
  (=--count 0= でリセット), =core-native-warnings-count=,
  =core-native-show-warnings=, =core-native-save-warnings= の7箇所で
  =(length core-native--warnings)= を =core-native--count= 参照に置換した。

*🟠 Medium — core-native--apply-report-setting: 4アームに同一 boundp+setopt 重複*

- *[Fix AK] lisp/core/core-native.el (core-native--apply-report-setting)*:
  4つの =pcase= アームそれぞれに同一の =boundp= ガード付き =setopt= ブロック
  が存在していた。2ステップ構造に整理し、=boundp= ガードを1箇所に統合した。
  値は =(if (eq mode 'silent) 'silent t)= で決定し、コレクター起動は
  =memq= と catch-all の2条件で制御する。

*🟠 Medium — core-treesit-apply-remap: dolist + add-to-list O(n)×10*

- *[Fix AJ] lisp/core/core-treesit.el (core-treesit-apply-remap)*:
  =major-mode-remap-alist= への10エントリ追加を =cl-union :test #'equal=
  の単一マージに置き換えた。スタートアップで実行されるため起動時間短縮に寄与。
...
- *[Fix AK] lisp/core/core-treesit.el (core-treesit-register-sources)*:
  =treesit-language-source-alist= へのソース登録を =cl-union= の単一マージに
  置き換えた。=install-all= 経由でスタートアップ時に実行される。

*🟠 Medium — core-switches: 同一3分岐 cond が enable-ui/enable-lsp に4回重複*

- *[Fix AK] lisp/core/core-switches.el*: =core-switches--load-and-call=
  ヘルパーを抽出した。=enable-fn=, =library=, =not-found-msg= の3引数で
  fboundp → locate-library+load → エラーメッセージの3段フォールバックを
  実装する。=enable-ui= と =enable-lsp= の各アームが1行になった。

*🟠 Medium — dev-lsp-bridge: 同一 cond ディスパッチが3コマンドに重複*

- *[Fix AK] lisp/dev/dev-lsp-bridge.el*: =dev-lsp--dispatch= ヘルパーを
  追加した。=eglot-fn=, =lsp-fn=, =fallback-fn=, =error-msg= の4引数で
  ディスパッチを統一。各コマンドが1行になりロジックの重複が解消された。

*🟠 Medium — core-session--run-all: タイマー毎に中間リスト確保*

- *[Fix AK] lisp/core/core-session-private.el (core-session--run-all)*:
  =core-session--jobs= 経由の中間リスト確保を廃止し、=maphash= でハッシュ
  テーブルを直接イテレートする方式に変更した。

*🟠 Medium — utils-org-agenda--scan: ディレクトリあたり directory-files 2回*

- *[Fix AJ] lisp/utils/utils-org-agenda.el (utils-org-agenda--scan)*:
  =directory-files= を1回の呼び出しにまとめ、=seq-filter= で =.org= ファイル
  とサブディレクトリに分割する方式に変更。各ディレクトリの I/O を半減させる。

*🟠 Medium — my/create-scratch-buffer: 起動時に既存バッファを破壊*

- *[Fix AJ] lisp/core/core-buffers.el (my/create-scratch-buffer)*:
  =erase-buffer= による無条件リセットを廃止し、=fundamental-mode= かつ空
  バッファの場合のみ初期化するよう変更した。=desktop-save= 復元内容や
  =--eval= 書き込み内容を保護する。
