...
#+begin_src emacs-lisp
  ...
  ;; nano-modeline: global header-line, suppressed mode-line
  ;;
...
Evaluated every frame inside nano-modeline--make's :eval block, so
the clock and IME state update automatically without any timer.
Coding system suffix modifiers (-unix, -dos, -mac, -with-signature) are stripped.
IME state is read from `sis--current' when sis is loaded:
  \\='english → \"A\"  (Roman / ASCII input)
  \\='other   → \"あ\"  (Japanese Kotoeri)
  nil/unknown → \"\"   (sis not active)"
    (let* ((time-str (format-time-string "%H:%M"))
           ...
           (ime-str  (when (boundp 'sis--current)
                       (pcase sis--current
                         ('english " A")
                         ('other   " あ")
                         (_        "")))))
      (propertize (concat time-str "  " cs-name (or ime-str ""))
