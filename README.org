...
本層は 11 モジュールで構成されます。 =my:modules= に含まれる正規モジュールは 8 つです。
残る 3 つ（ =orgx-typography= 、 =orgx-brain= 、 =orgx-roam-ui= ）はオプション拡張です。
このうち =orgx-typography= と =orgx-brain= は =personal/user.el= が =my:modules-extra= に
宣言します。 =orgx-roam-ui= は tangle されますが、既定では =my:modules-extra= に含めません
（Org を起動時に引き込まないため。理由は当該モジュールと =personal/user.el= を参照）。
