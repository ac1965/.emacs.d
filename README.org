...
#+LATEX_HEADxER: \usepackage{luatexja}
...
本層は 12 モジュールで構成されます。 =my:modules= に含まれる正規モジュールは 8 つです。
残る 4 つ（ =orgx-typography= 、 =orgx-brain= 、 =orgx-roam-ui= 、 =orgx-citar= ）はオプション
拡張です。このうち =orgx-typography= 、 =orgx-brain= 、 =orgx-citar= は =personal/user.el= が
=my:modules-extra= に宣言します。 =orgx-roam-ui= は tangle されますが、既定では
=my:modules-extra= に含めません（Org を起動時に引き込まないため。理由は当該モジュールと
=personal/user.el= を参照）。
...
- =orgx-export= — =ox-hugo= （TOML フロントマター）、 =markdown-mode= 、
  ...
  オプション扱いで、 =ob-dot= が load-path 上にある場合のみ有効化）、LaTeX/PDF
  （ =ltjsarticle= クラスを =org-latex-classes= に登録し、PDF 生成は =latexmk=
  経由で =~/.latexmkrc= の LuaLaTeX-ja デフォルトに委ねる）
...
- =orgx-auto-hugo= — =org-capture-after-finalize-hook= を契機とする =ox-hugo= の
  ...
  =my:f:capture-blog-file= で判定する。あわせて =org-capture-mode-map= に
  =C-c C-p= を割り当て、確定前のエントリを =markdown-preview-mode= で
  ブラウザプレビューできる（実 content ディレクトリには一切書き込まない）
...
- =orgx-roam-ui= — =simple-httpd= + =websocket= 上で動く =org-roam-ui= グラフ
  ...
  =my:modules-extra= に加えた場合、モジュールのロード時に依存を導入し、 =org-roam= の
  ロード後にアイドルタイマー（8 秒）で UI を開く。ただし既定ではこのリストに含めない
- =orgx-brain= — =my:d:org-roam= をノートディレクトリとして共有する =org-brain=
  マインドマップビュー。コマンドは =my/brain-visualize=
- =orgx-citar= — =citar= + =org-cite= による文献引用管理。 =citar-bibliography= は
  =my:d:org-biblio= （ =my:d:org= 配下、新設）を参照し、ホームディレクトリ直書きの
  パスをハードコードしない。 =citar-embark= が =embark-act= 経由の文献アクション
  （開く／挿入など）を追加する
...
点に注意してください。同様に、ローカル Ollama モデルへのルーター（=~/.local/bin/localllm=）を
Org-roam / コーディングバッファの双方から呼び出す統合は =personal/personal-llm.el= にあり、
HTTP クライアントを介さずプロセス起動のみで完結するため =utils= 層には依存しません。
...
- =utils-ime= — macism による入力ソース制御（リーダー／dired・magit-status・ターミナル系／ミニバッファで英字へ、離脱時は自動復帰）
...
| =orgx-roam-ui=    | orgx  | org-roam-ui グラフビジュアライザ（既定では無効。下記参照） |
| =orgx-brain=      | orgx  | org-brain マインドマップビュー |

=orgx-roam-ui= は上の表に挙げてありますが、既定の =my:modules-extra= からは外しています。
このモジュールを有効にすると org-roam-ui がモジュールのロード時に依存を導入し、
=org-roam= → =org= を起動直後に引き込むため、Org の遅延ロードが崩れます。グラフビジュアライザを
常用する場合のみ、 =personal/user.el= の =my:modules-extra= に =orgx-roam-ui= を戻してください。
...
*初回ブートストラップ（Makefile が無い場合）*

=Makefile= も =scripts/reload.py= も =early-init.el= / =init.el= / =lisp/= も、すべて
この README.org からの tangle 生成物であり =.gitignore= 対象。=git clone= 直後は
どれも存在しないため、 =make reload= はおろか =python3 scripts/reload.py= すら
まだ使えない「鶏と卵」状態になる。

これを破る唯一の依存ゼロな入口は、組み込み Org だけで完結する生の Emacs バッチ
コマンド（外部パッケージ不要。straight や leaf のブートストラップより前の段階
なので、それらにも依存しない）。

#+begin_example
emacs -Q --batch --eval '(require (quote ob-tangle))' \
                  --eval '(org-babel-tangle-file "README.org")'
#+end_example

これを一度実行すれば =Makefile= ・ =scripts/reload.py= ・ =lisp/*= 一式が生成される。
以後は通常どおり =make reload= （またはこのコマンド単体の再実行）で回せる。
...
参照してください。LICENSE 自身も本文書からの tangle 生成物であり（下記ソースブロックを
=make tangle= が =early-init.el= 等と同様に書き出す）、ソース・オブ・トゥルースは常に
本節である。

#+begin_src text :tangle LICENSE :mkdirp no :padline no
...
|Property|Value|
|--------|-----|
|Commit|f0430371c8d5b1e172670cdbc59f4d298be64a29|
|Branch|master|
|System|aarch64-apple-darwin25.6.0|
|Date|2026-09-13 21:01:00 (JST)|
...
|Options|CC=/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang CXX=/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang++ AR=/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/ar RANLIB=/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/ranlib NM=/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/nm --with-ns --with-native-compilation=aot --with-tree-sitter --with-json --with-gnutls --without-imagemagick --with-modules --prefix=/Users/ac1965/.local 'CFLAGS=-O3 -arch arm64 -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk' 'CPPFLAGS=-I/opt/homebrew/opt/libgccjit/include -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk -I/opt/homebrew/include' 'LDFLAGS=-L/opt/homebrew/opt/libgccjit/lib -isysroot /Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk -L/opt/homebrew/lib -framework AppKit'|
...
- =gcmh-high-cons-threshold= :: 128 MiB
...
=XDG_CACHE_HOME= を尊重するのは Linux の XDG 仕様に従うためです。未設定時のフォールバックは
=$HOME/.cache/emacs/= です。GUI（Finder/Dock/Spotlight）から起動した =Emacs.app= には
=XDG_CACHE_HOME= が渡らないため、フォールバックが =.zshenv= の既定値（=$HOME/.cache=）と
食い違うとターミナル起動時と異なるキャッシュ場所を見てしまいます。かつてはリポジトリ内の
=user-emacs-directory/.cache/= にフォールバックしていましたが、それが原因で GUI 起動時だけ
straight のパッケージ群が見つからず再ブートストラップが走る不具合があったため、
=$HOME/.cache/= に揃えています。
...
起動時に =file-name-handler-alist= を空にし、 =gc-cons-threshold= を引き上げるのは、地味ですが
効果の大きい最適化です。すべての =require= と =load= が =file-name-handler-alist= を参照するので、
モジュール数が増えるほどオーバーヘッドが積み上がります。

この 2 つの設定は、 =early-init.el= の *最初*（ディレクトリ作成より前）に置いてください。
以前は末尾寄りの「8) 起動性能」に置いていましたが、起動プロファイル計測（Changelog参照）で、
起動時に発生する GC がすべて early-init.el の実行中（ディレクトリ作成や straight/native-comp の
設定の最中、つまり閾値引き上げより *前*）に起きていることが判明しました。閾値を早い位置に
移したことで、以降の起動シーケンス全体がこの恩恵を受けます。

このリストには TRAMP のハンドラが入っています。 =after-init-hook= で
...
#+begin_src emacs-lisp
  ...
  ;; ---------------------------------------------------------------------------
  ;; 1) 起動性能（GC / file-name-handler-alist）
  ;; ---------------------------------------------------------------------------
  ;;
  ;; 起動プロファイル計測（Changelog参照）で、起動時に発生する 4 回の GC が
  ;; すべて early-init.el の実行中（ディレクトリ作成やstraight/native-comp設定の
  ;; 最中）に起きており、以降の起動シーケンス（straight/leaf のブートストラップ、
  ;; [modules] ループ）では 1 回も GC が起きていないことが判明した。
  ;;
  ;; 原因は、この GC 閾値引き上げが従来「8) 起動性能」として、ディレクトリ作成
  ;; （旧2〜3）や straight/Homebrew/native-comp の設定（旧4〜6）より *後* に
  ;; 置かれていたことである。それらの処理は Emacs の既定の GC 閾値（通常
  ;; 800KB 程度）のまま実行されており、GC が走っていた。
  ;;
  ;; 閾値の引き上げを early-init.el の最初（ディレクトリ作成より前）に移動し、
  ;; 以降のセクションすべてがこの恩恵を受けるようにした。
  ...
  ;; ---------------------------------------------------------------------------
  ;;
  ;; `straight-check-for-modifications' の既定値は
  ;; `(find-at-startup find-when-checking)' であり、`find-at-startup' は起動の
  ;; たびに管理下の全パッケージへ `find' によるファイル変更チェックを走らせる。
  ;; 起動プロファイル計測（Changelog参照）で、straight のブートストラップだけで
  ;; 620ms（起動全体の約半分）を占めることが判明した。管理下のパッケージを
  ;; エディタの外から手動編集する運用ではないため、起動時の全件スキャンは
  ;; 実質的に無駄である。
  ;;
  ;; `find-at-startup' を外し、ファイル保存時のみチェックする `check-on-save'
  ;; に絞る。手動で clone 先を直接編集した場合は自動検出されなくなるが、
  ;; その場合は `M-x straight-rebuild-package' または `straight-pull-all' で
  ;; 明示的にリビルドすればよい。
  ;;
  ;; `straight-check-dependency-versions' の既定値 `warn' は、Package-Requires
  ;; に書かれたバージョン文字列を `version<' で単純比較するだけである。MELPA
  ;; 経由なら Package-Version は commit 日時から自動生成されるため意味を持つが、
  ;; straight.el は git からソースをそのまま使うため、パッケージが static な
  ;; `;; Version:' ヘッダー（例: restclient.el の "1.0"）を持っていると、依存側の
  ;; 宣言（restclient-jq.el の "20200502.831" など）と噛み合わずに誤検知する。
  ;; 個別パッケージ単位で無視する仕組みは straight.el に存在しないため、
  ;; 全体を `nil'（旧来の既定動作）にして無効化する。

  (setopt straight-base-dir my:d:cache
          straight-use-package-by-default t
          straight-vc-git-default-clone-depth 1
          straight-check-for-modifications '(check-on-save find-when-checking)
          straight-check-dependency-versions nil
          straight-build-dir
  ...
  ;; native-comp-deferred-compilation は Emacs 29.1 で廃止され、
  ;; native-comp-jit-compilation に改名された（旧名を参照すると obsolete 警告が出る）。
  ;; JIT コンパイルは既定で有効だが、意図を明示するために設定する。
  (when (boundp 'native-comp-jit-compilation)
    (setopt native-comp-jit-compilation t))

  ;; 上の native-comp-eln-load-path のブロックとは独立に、非同期ネイティブコンパイルの
  ;; 警告/エラー通知を抑制する（eln-load-path が未束縛の環境でも確実に適用するため）。
  (when (boundp 'native-comp-async-report-warnings-errors)
    (setq native-comp-async-report-warnings-errors 'silent))

  ;; ---------------------------------------------------------------------------
  ;; 8) no-littering との互換性
  ;; ---------------------------------------------------------------------------
  ...
  (defvar core--did-fullscreen nil
    "非 daemon 起動時、フルスクリーンを適用済みかどうか。")

  (defun core--fullscreen-frame (frame)
    "FRAME がグラフィカルなら、それをフルスクリーンにする。"
    (when (display-graphic-p frame)
      (set-frame-parameter frame 'fullscreen 'fullboth)))

  ;; window-setup-hook は --daemon 起動時、実フレームが無い状態で走ってしまう
  ;; （デーモンはクライアント接続を待つだけで、起動時にフレームを作らない）。
  ;; そのため非 daemon 時のみこの関数を使い、初回のグラフィカルフレームに
  ;; 限って（core--did-fullscreen で）適用する。
  (defun core--fullscreen-once ()
    "非 daemon 起動時、初回のグラフィカルフレームをフルスクリーンにする。"
    (unless core--did-fullscreen
      (let ((f (selected-frame)))
        (when (display-graphic-p f)
          (setq core--did-fullscreen t)
          (core--fullscreen-frame f)))))

  ;; daemon 起動時は core--fullscreen-once ではなく core--fullscreen-frame を
  ;; 直接 after-make-frame-functions に繋ぐ。初回のみに限定すると、2 枚目以降の
  ;; `emacsclient -c` で作られるフレームが最大化されないままになってしまうため、
  ;; daemon が生きている限り、新しいグラフィカルフレームが繋がるたびに毎回適用する。
  (if (daemonp)
      (add-hook 'after-make-frame-functions #'core--fullscreen-frame)
...
#+begin_src emacs-lisp
  ...
  (let* ((base (or (bound-and-true-p straight-base-dir)
                   user-emacs-directory))
         (repo-dir
          (expand-file-name "straight/repos/straight.el" base))
         (bootstrap-file
          (expand-file-name "bootstrap.el" repo-dir)))
    (unless (file-exists-p bootstrap-file)
      ;; ネットワークアクセスは初回のみ。raw.githubusercontent.com の単一ファイル
      ;; 取得は CDN 側のレート制限（429）で頻繁に失敗し、その本文をそのまま Lisp
      ;; として読もうとして invalid-read-syntax で落ちていた。git clone は別の
      ;; レート制限系統かつ成功/失敗が明確なため、こちらに一本化する。
      (unless (executable-find "git")
        (user-error "[straight] bootstrap failed: git not found"))
      (make-directory (file-name-directory repo-dir) t)
      (unless (zerop (call-process "git" nil "*straight-bootstrap*" t
                                    "clone" "--depth" "1" "--branch" "develop"
                                    "https://github.com/radian-software/straight.el.git"
                                    repo-dir))
        (user-error "[straight] bootstrap failed: git clone error (see *straight-bootstrap* buffer)")))
  ...
  ;; early-init.el 8) で no-littering-etc-directory / no-littering-var-directory を
  ;; defvar 済みだが、それだけでは no-littering.el 本体は一切ロードされない。
  ;; save-place / recentf / savehist / tramp / transient / desktop / tree-sitter の
  ;; ように各モジュールが手作業で no-littering-var-directory を使ってパスを
  ;; 組み立てている変数はこれで正しく var/ 配下に収まるが、それ以外の
  ;; 100 以上の変数（bookmark-default-file、project-list-file、
  ;; url-configuration-directory、forge-database-file、treemacs-persist-file 等）は
  ;; no-littering 本体が持つ既知変数の自動リダイレクト表に頼っており、本体が
  ;; ロードされていなければ一切効かない。各パッケージ自身の既定値のまま、
  ;; ~/.emacs.d/ 直下や、パッケージによっては .emacs.d の外（XDG 系ディレクトリ等）
  ;; に書き出されてしまう。
  ;;
  ;; straight/leaf のブートストラップ直後、他のどのパッケージよりも先にロードする。
  ;; no-littering 本体は自身が持つ既知変数のデフォルト値を setq で直接上書きする
  ;; 実装なので、対象パッケージ自体がまだロードされていなくても問題ない
  ;; （対象パッケージが後でロードされたとき、上書き済みの defcustom 値を読む）。
  (leaf no-littering
    :straight t
    :require t
    :init
    ;; no-littering.el は require 時に no-littering-etc-directory /
    ;; no-littering-var-directory を参照してディレクトリを決定する。
    ;; early-init.el 8) で先に defvar 済みであることが前提。
    (unless (and (boundp 'no-littering-etc-directory)
                 (boundp 'no-littering-var-directory))
      (error "no-littering: etc/var directory vars must be set in early-init.el before this loads")))

  ;;; -------------------------------------------------------------------------
  ;;; 5. Org（組み込みを使用。straight では導入しない）
  ;;; -------------------------------------------------------------------------
  ...
  (defvar core--announce-startup-done nil
    "Non-nil once the startup-ready message has been shown (daemon mode only).")

  (defun core--announce-startup ()
    "Report Emacs startup time and GC count to *Messages*."
    (message "Emacs ready in %.2f seconds with %d GCs."
             (float-time (time-subtract after-init-time before-init-time))
             gcs-done))

  ;; run-with-idle-timer をここで直接呼ぶと、init.el の実行中（emacs-startup-hook が
  ;; 発火する *前*）にアイドルタイマーキューへ即座に登録されてしまう。一方、
  ;; ui-font.el / ui-nano-modeline.el / ui-visual-aids.el（pulsar）等の遅延ロード
  ;; モジュールは emacs-startup-hook から自分自身のアイドルタイマー
  ;; （run-with-idle-timer 0 nil #'<module>--activate-*）を登録する。同じ遅延0秒の
  ;; アイドルタイマーは登録順に発火するため、ここで直接登録すると「Emacs ready...」が
  ;; Fonts applied: や [ui] nano-modeline initialized. より *先* に出てしまう
  ;; （起動完了サマリのつもりが、実際には最初に出力される）。
  ;;
  ;; 対策として、タイマーの登録自体を emacs-startup-hook の末尾（depth 100）へ回す。
  ;; こうすることで、他のモジュールが emacs-startup-hook 内で自分のアイドルタイマーを
  ;; 登録し終えた *後に* このタイマーが登録されるため、同じアイドルタイマーキューの
  ;; 中でも最後に発火するようになる。これは非 daemon 起動（通常の `emacs' 起動）に
  ;; 対する対策である。
  (defun core--schedule-announce-startup ()
    "Schedule `core--announce-startup' after other emacs-startup-hook timers.
Registering the idle timer directly at init.el load time would queue it
before the deferred-loading modules (ui-font, ui-nano-modeline,
ui-visual-aids, etc.) register their own zero-delay idle timers from
`emacs-startup-hook', causing the ready message to print first instead
of last.  Deferring the registration itself to the tail of
`emacs-startup-hook' (depth 100) ensures those modules queue their
timers first."
    (run-with-idle-timer 0 nil #'core--announce-startup))

  ;; daemon モードでは上記の emacs-startup-hook 対策は無意味である。daemon 自体は
  ;; 最初のクライアントが繋がるずっと前に初期化を終えるため、その時点で
  ;; 「Emacs ready...」を出しても、まだ誰も見ていない *Messages* に記録されるだけで、
  ;; Fonts applied: / [ui] nano-modeline initialized. / When done with this frame,
  ;; type C-x 5 0（`emacsclient -c' の実接続時にのみ出る）はすべて、実際にクライアント
  ;; フレームが接続した *ずっと後* に出る。つまり「起動完了」を報告するタイミングとして
  ;; 意味があるのは daemon 初期化完了時ではなく、人間が実際にフレームを見られるように
  ;; なった瞬間である。
  ;;
  ;; そこで daemon モードでは、メッセージの登録先を after-make-frame-functions に
  ;; 切り替え、最初のクライアントフレームが接続した時点まで表示自体を遅らせる。
  ;; after-init-time / gcs-done は daemon 初期化完了時点で既に確定した値なので、
  ;; 「N.NN 秒」が指す内容（起動所要時間）そのものは変わらない。変わるのは
  ;; メッセージを *出すタイミング* だけである。
  ;;
  ;; ui-font.el / ui-nano-modeline.el の daemon 分岐と同じ after-make-frame-functions
  ;; に乗るが、それらより後に発火するよう depth 100 を指定する。さらに
  ;; nano-modeline 側は `run-with-timer 0 nil' で 1 ティック遅らせてから
  ;; 「[ui] nano-modeline initialized.」を出す（`ui-nano--setup-after-frame-realized'
  ;; を参照。色の問い合わせにはフレーム実体化後のもう 1 ティックが必要なため）ので、
  ;; こちらも同じだけ遅らせないと、depth 100 でも nano-modeline より先に出てしまう
  ;; 可能性がある。複数フレームで繰り返し表示されないよう、初回発火時に自分自身を
  ;; フックから取り除く（dirvish の自己解除型 advice と同じパターン）。
  (defun core--announce-startup-on-first-frame (&optional _frame)
    "Show the startup-ready message once the first client frame connects.
In daemon mode the daemon itself becomes ready long before any
`emacsclient -c' connects, so printing immediately at init time lands
in a log nobody is watching.  `after-init-time' and `gcs-done' are
already fixed at that point, so the reported duration is unaffected;
only the timing of the print is deferred, to when a human can actually
see the frame.  Runs once only (removes itself from
`after-make-frame-functions' on first invocation)."
    (unless core--announce-startup-done
      (setq core--announce-startup-done t)
      (remove-hook 'after-make-frame-functions #'core--announce-startup-on-first-frame)
      (run-with-timer 0 nil #'core--announce-startup)))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'core--announce-startup-on-first-frame 100)
    (add-hook 'emacs-startup-hook #'core--schedule-announce-startup 100))
...
#+begin_src emacs-lisp
  ...
  ;;;; 信頼するコンテンツ（Emacs 30.1 以降）----------------------------------

  ;; Emacs 30.1 は `trusted-content' を導入した。バッファの内容を「実行してよい」と
  ;; みなすかどうかを判定する仕組みである。`flymake-mode' が有効な Emacs Lisp バッファでは
  ;; `elisp-flymake-byte-compile' がバイトコンパイルを走らせるが、バイトコンパイルは
  ;; マクロ展開と `eval-when-compile' を評価する。つまり *コードを実行する*。
  ;; 信頼できない内容に対してこれを行うのは危険なため、Emacs は `trusted-content-p' が
  ;; nil のバッファでバックエンドを無効化し、警告を出す。
  ;;
  ;; 自分で管理している設定ファイルは信頼してよい。これを宣言しないと、lisp/ や personal/
  ;; 配下の .el を編集するたびに flymake のバイトコンパイル・バックエンドが無効化される。
  ;;
  ;; 仕様上の注意:
  ;; - ディレクトリを指定する要素は末尾が "/" でなければならない。
  ;; - 値は abbreviate-file-name 形式で与える。
  ;; - init.el（`user-init-file'）は Emacs が無条件に信頼するため、列挙する必要はない。
  ;; - `trusted-content-p' は buffer-file-truename が nil のバッファを決して信頼しない。
  ;;   したがって *scratch* はここに何を書いても信頼されない（`:all' のみが例外だが、
  ;;   docstring が「gaping security hole」と明記しており採用しない）。
  ;;   *scratch* で flymake を有効化しない方針については utils-lint.el を参照。

  (defun core-policy--trusted-dir (dir)
    "Return DIR as an abbreviated directory name suitable for `trusted-content'."
    (abbreviate-file-name (file-name-as-directory (expand-file-name dir))))

  (when (boundp 'trusted-content)
    (setopt trusted-content
            (list (core-policy--trusted-dir my:d:lisp)
                  (core-policy--trusted-dir
...
#+begin_quote
*既知の問題:* =core-session-register-job= を実際に呼び出している箇所は、本設定全体のどこにも現状ありません。したがって =core-session--jobs-table= は常に空で、定期・アイドル
両方のタイマーは空のハッシュテーブルを律儀に走査し続けているだけの状態です。実害はありま
せんが、将来のジョブ登録用の拡張ポイントとして温存するか、スケジューラ自体を削除するかは
未決です。
#+end_quote
...
- =core-gc.el= — 明示的な契機での GC（ =after-focus-change-function= と =minibuffer-exit-hook= ）
- =gcmh= （ =core-runtime.el= ）— アイドルタイマーによる自動 GC 管理

前者が「切れ目を狙う」担当、後者が「暇なときに片づける」担当だと考えると分かりやすいです。
実際には 3 層構成です。起動直後は =early-init.el §1= の GC ガードが閾値を一時的に
512MiB まで緩め、=after-init-hook= で 128MiB に戻します。これが無いと、起動処理の
大半が既定の閾値（800KB 程度）のまま走ってしまいます（詳細は
[[#early-init-el][early-init.el]] を参照）。
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; 本ファイルの責務:
  ;; - `after-focus-change-function'（フォーカス喪失時）と `minibuffer-exit-hook' で
  ;;   `garbage-collect' を起動する
  ...
  ;; focus-out-hook は Emacs 27.1 で廃止された（handle-focus-out は今も実行するが、
  ;; docstring が「タイミングによっては呼ばれない」と警告している）。後継は
  ;; after-focus-change-function である。ただしフォーカスの取得時にも呼ばれるため、
  ;; frame-focus-state で「失った」ことを判定しなければならない。
  ;;
  ;; after-focus-change-function は「フック」ではなく単一の関数を保持する変数なので、
  ;; add-hook ではなく add-function を用いる。設定のリロードをまたいだ重複登録を防ぐため、
  ;; 追加前に古い登録を除去する。
  (defun core-gc--on-focus-change ()
    "Run garbage collection when Emacs loses focus.
  `after-focus-change-function' fires for both focus-in and focus-out, so the
  focus state must be checked.  `frame-focus-state' returns nil when the frame
  has lost focus."
    (unless (frame-focus-state)
      (core-gc--collect)))

  (remove-function after-focus-change-function #'core-gc--on-focus-change)
  (add-function :after after-focus-change-function #'core-gc--on-focus-change)
...
自動保存サブシステム（3 機構の合流）の詳細は以下のコードの Commentary を参照してください。
...
#+begin_src emacs-lisp
    ...
    ;;   起動時間短縮のため、有効化自体は `dired-noselect' への自己解除型
    ;;   :around advice で、最初に dired が呼ばれた瞬間まで遅延させる
    ...
    ;;    （after-focus-change-function）、または選択ウィンドウが変わったとき
  ...
  Called from `after-focus-change-function', which uses a zero-argument calling
  convention and fires for both focus-in and focus-out.  `frame-focus-state'
  returns nil when the frame has lost focus, so saving is restricted to that case."
      (when (and core-autosave-save-on-focus-out
  ...
  `add-hook' and `add-function' check for existing entries."
      ...
      ;; 3. after-focus-change-function — フォーカス変化時の唯一の登録先。
      ;;    以前は focus-out-hook にも登録していたが、focus-out-hook は Emacs 27.1 で
      ;;    廃止されている。しかも両方に登録していたため、フォーカスを失うたびに保存が
      ;;    二重に走っていた（handle-focus-out は focus-out-hook を実行したうえで
      ;;    after-focus-change-function も呼ぶ）。
      ;;
      ;;    after-focus-change-function はフォーカスの取得時にも呼ばれる。
      ;;    core-editing--save-on-focus-out は frame-focus-state で「失った」ことを
      ;;    確認してから保存する（以前はこの判定が無く、フォーカス取得時にも保存が走っていた）。
      ;;
      ;;    これは「フック」ではなく単一の関数を保持する変数なので add-function を用いる。
    ...
    (leaf paredit
      ;; straight の既定レシピは :repo に https://paredit.org/paredit.git を指定して
      ;; いる。作者個人の git サーバーで、GitHub 等より不安定・到達不能になりやすく、
      ;; 実際に `git clone` が失敗する事例があったため、GitHub 上のミラー
      ;; （emacsmirror）を明示的に指定して回避する。
    ...
    ;; dirvish は leaf の :global-minor-mode が :require t を伴うため、
    ;; core-editing のロード時に即時 require されていた（起動プロファイル
    ;; 計測で core-editing 全体の 25〜30ms の主要因の一つ）。
    ;;
    ;; pulsar/apheleia/project.el と違い、dirvish は単純な emacs-startup-hook +
    ;; アイドルタイマーでは遅延できない。`dirvish-override-dired-mode' は
    ;; 「dired を呼んだら代わりに dirvish を開く」という置き換えそのものなので、
    ;; ユーザーが最初に dired を開く *前* に有効化されていなければならない。
    ;; 固定秒数のアイドルタイマーでは、起動直後に dired を呼ばれた場合に
    ;; 間に合わない。
    ;;
    ;; そこで `dired-noselect' に自己解除型の :around advice を仕込む。
    ;; `dired'、`dired-jump'、`project-dired'、`find-file' のディレクトリ分岐など、
    ;; dired を呼ぶほぼ全ての経路は最終的に `dired-noselect' を通るため、
    ;; ここ一箇所へのアドバイスで束ねられる。
    (defun core-editing--activate-dirvish ()
      "dirvish を require し、override モードと表示設定を適用する。
`core-editing--ensure-dirvish' から一度だけ呼ばれる。"
      (require 'dirvish)
      ;; leaf :custom は暗黙に require するため、dirvish-attributes 等は
      ;; require 後にここで setopt する。
      (setopt
       dirvish-attributes '(subtree-state vc-state git-msg file-size)
       ;; dirvish は自前の mode-line パイプラインを持ち、編集バッファの
       ;; nano-modeline（M-SPC 経由ではなく起動時にグローバル適用される方）を
       ;; 経由しない。そのため IME（sis）状態がウィンドウ最大化時に判別できなくなる
       ;; 問題があった。:right に `ime' セグメントを追加することで、dirvish 自身の
       ;; mode-line 上に IME 表示を出す。セグメント本体（`ime'）の定義は
       ;; personal/ac1965.el 側（sis-change-hook のキャッシュ `ac1965--sis-indicator'
       ;; の定義箇所）に置く — `dirvish-mode-line-format' はセグメント名のリストに
       ;; すぎず、実際に mode-line が描画されるタイミング（dirvish バッファを開いた
       ;; とき、= init 完了よりずっと後）までにセグメント関数が登録されていれば
       ;; よいため、この setopt の位置とセグメント定義の位置の前後関係は問わない。
       dirvish-mode-line-format '(:left (sort symlink) :right (omit yank index ime)))
    ...
    (defun core-editing--ensure-dirvish (_orig-fn &rest args)
      "`dired-noselect' の初回呼び出し時に dirvish を有効化してから呼び直す。

`:around' アドバイスとして一度だけ動作する自己解除型のブートストラップである。
_ORIG-FN（このアドバイスを追加した時点の `dired-noselect'）を直接呼ぶのではなく、
自分自身のアドバイスを除去したうえで `core-editing--activate-dirvish' を実行し、
シンボル `dired-noselect' を改めて `apply' で呼び直している。

理由: `dirvish-override-dired-mode' 自身も `dired-noselect' に advice を
追加する。_ORIG-FN はこのアドバイスを追加した時点のスナップショットであり、
それをそのまま呼ぶと、今まさに処理している最初の呼び出しだけ dirvish の
advice を素通りしてしまう（2 回目以降の呼び出しからしか dirvish の UI に
ならない）。シンボル経由で呼び直せば、その時点の最新の advice チェーンを
確実に通る。"
      (advice-remove 'dired-noselect #'core-editing--ensure-dirvish)
      (core-editing--activate-dirvish)
      (apply #'dired-noselect args))

    (advice-add 'dired-noselect :around #'core-editing--ensure-dirvish)
    ...
    ;; 現在行のハイライト。global-hl-line-sticky-flag（下の Emacs 31 ブロック内）
    ;; はこのモードの「複数ウィンドウでの追従挙動」を調整するだけの設定であり、
    ;; モード自体を有効化するものではない。以前は sticky-flag だけが設定され、
    ;; global-hl-line-mode 自体を有効化する行が無かったため、通常の編集バッファで
    ;; 現在行がハイライトされていなかった。hl-line-mode は Emacs 31 より前から
    ;; 存在するため、Emacs 31 限定ブロックには含めない。
    (leaf hl-line
      :straight nil
    ...
    (when (>= emacs-major-version 31)
      ...
      ;; show-paren はコメント／文字列内では、対応が取れない括弧だけを非強調にする
      ;; （'all にすると常に非強調、nil で常に強調）。値はクォートしたシンボルであること。
      (setopt show-paren-not-in-comments-or-strings 'on-mismatch)
      ;; hl-line はバッファではなくウィンドウに追従する
      ;; （global-hl-line-mode 自体の有効化は上の leaf hl-line を参照）。
...
#+begin_src emacs-lisp
  ...
  ;;   `after-make-frame-functions' にフックする（daemon 対応）
  ;; - グラフィカルフレームで `global-ligature-mode' を有効化する
  ;;   （初回のグラフィカルフレーム作成時に一度だけ。daemon 対応）
  ;;; Code:
  ...
  ;; leaf の :when は「モジュールのロード時」に一度だけ評価される。通常起動では
  ;; その時点で既にグラフィカルフレームがあるため (display-graphic-p) は機能するが、
  ;; --daemon 起動時はモジュールのロード自体がフレームの無い状態で行われるため、
  ;; :when (display-graphic-p) は常に nil と評価され、ligature が永久にロードされない
  ;; という daemon 対応バグになっていた。フォント適用（上の ui-font--apply-to-frame）と
  ;; 同じ「初回のグラフィカルフレームで一度だけ有効化する」パターンに揃える。
  (leaf ligature
    :straight (ligature :type git :host github :repo "mickeynp/ligature.el")
    :require nil)

  (defvar ui-font--ligature-activated-p nil
    "Non-nil once ligature has been configured and enabled.")

  (defun ui-font--activate-ligature ()
    "ligature を require し、prog-mode 用の定義とグローバル有効化を行う。
`ui-font--maybe-activate-ligature' から一度だけ呼ばれる。"
    (require 'ligature)
    ;; prog-mode 用にリガチャを登録してから、グローバルに有効化する。
    ;; global-ligature-mode はグローバルなマイナーモードであるため、prog-mode-hook に
    ;; 追加してはならない — 繰り返し有効化するのは冗長であり、デバッグを混乱させる。
    (ligature-set-ligatures 'prog-mode '("->" "=>" "::" "===" "!=" "&&" "||"))
    (global-ligature-mode 1))

  (defun ui-font--maybe-activate-ligature (&optional frame)
    "FRAME がグラフィカルなら、初回に限り ligature を有効化する。
`after-make-frame-functions' （daemon）と `after-init-hook' （通常起動）の
両方から呼ばれる。通常起動の =after-init-hook= には引数が渡らないため、
FRAME は省略可能にし、その場合は `selected-frame' を見る。"
    (when (and (not ui-font--ligature-activated-p)
               (display-graphic-p (or frame (selected-frame))))
      (setq ui-font--ligature-activated-p t)
      (ui-font--activate-ligature)))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'ui-font--maybe-activate-ligature)
    (add-hook 'after-init-hook #'ui-font--maybe-activate-ligature))
...
#+begin_src emacs-lisp
  ...
  (defcustom nano-color-selection "#dbeafe"
  ...
  (defun my/nano-apply-faces ()
    ...
    ;; 現在行のハイライト（global-hl-line-mode）。
    ;; dirvish-hl-line と同じ色を使い、バッファの種類を問わず見た目を揃える。
    ;; この設定が抜けていたため、global-hl-line-mode を有効化しても
    ;; 通常の編集バッファでは現在行の背景色が付かなかった。
    (ui-nano--safe-face
     'hl-line
  ...
  ;; dirvish は dired-noselect 初回呼び出し時まで require されない
  ;; （core/core-editing.el の `core-editing--ensure-dirvish' 参照）。
  ;; そのため起動時の `my/nano-apply-faces' 呼び出し時点では
  ;; `dirvish-hl-line' 等がまだ defface されておらず、`ui-nano--safe-face'
  ;; の facep チェックに弾かれて黙ってスキップされていた。dirvish は
  ;; カーソルを非表示にしてこのフェイスのオーバーレイだけでカレント行を
  ;; 示す設計のため、パレット未適用のままだと現在行が全く見えなくなる。
  ;; dirvish が実際に require されたタイミングで一度だけ再適用する。
...
#+begin_src emacs-lisp
  ...
  ;; nano-modeline.el は defface（nano-modeline-active 等）の値を、
  ;; (face-foreground 'default) / (face-background 'header-line nil t) を
  ;; backquote でロード時に一度だけ焼き込む実装になっている。
  ;; --daemon 起動時（実フレームが無い状態）で (require 'nano-modeline) すると、
  ;; これらの呼び出しが nil を返し、その nil がフェイスの :foreground / :background
  ;; に永久に焼き込まれてしまう（defface は一度評価されると再評価されない）。
  ;; 後から emacsclient -c で本物のグラフィカルフレームが繋がっても直らず、
  ;; "Unable to load color unspecified-fg/unspecified-bg" のエラーになる。
  ;;
  ;; ligature / core--fullscreen-once と同じ「初回のグラフィカルフレームまで
  ;; require を遅らせる」パターンで、require そのものを FRAME の
  ;; display-graphic-p 確認より後ろへ動かす。
  (defun ui-nano--setup (&optional frame)
  ...
  ;; after-make-frame-functions は、フレームの生成中（ウィンドウシステムが
  ;; まだ実際の色を確定させる前）に同期的に走る。ui-font-apply のように
  ;; :foreground/:height を明示的に「設定」するだけなら問題ないが、
  ;; nano-modeline の defface は (face-foreground 'default) のように現在の
  ;; 色を「問い合わせる」ため、この早すぎるタイミングでは値がまだ確定して
  ;; おらず nil が返る（display-graphic-p のガードだけでは防げなかった、
  ;; 実機検証で判明した問題）。run-with-timer で 1 ティック遅らせ、フレームが
  ;; 完全に実体化してから実行することで回避する。
  (defun ui-nano--setup-after-frame-realized (frame)
    "フレームが完全に実体化してから `ui-nano--setup' を呼ぶ。"
    (run-with-timer 0 nil #'ui-nano--setup frame))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'ui-nano--setup-after-frame-realized)
    (add-hook 'after-init-hook #'ui-nano--setup))
...
#+begin_src emacs-lisp
  ...
  (defvar ui-theme--initialized-p nil
    "Non-nil once `ui-theme--init' has run.
デーモンモードでのみ、一度きりの初期化を守るために参照する（後述）。")

  ;; --daemon 起動時、`emacs-startup-hook' は実フレームが無い状態で走る。
  ;; `ui-theme--init' は nano-faces / nano-theme を require・適用するが、
  ;; nano-faces 内部の色計算（`face-foreground' / `face-background' 等）は
  ;; 実フレームが無いと nil を返し、その nil がフェイスに永久に焼き込まれる
  ;; （nano-modeline.el の defface と同種の問題。詳細は Changelog を参照）。
  ;; 通常起動（ターミナルを含む）では従来どおり `emacs-startup-hook' から
  ;; 無条件に呼ぶ（ターミナルでも 256 色パレットとして意味があるため、
  ;; display-graphic-p では絞らない）。daemon の場合のみ、初回のグラフィカル
  ;; フレームができるまで遅延させる。
  (defun ui-theme--init-once-graphic (&optional frame)
    "`ui-theme--init' を、実際にグラフィカルなフレームができてから一度だけ呼ぶ。
daemon 用のエントリポイント。"
    (unless ui-theme--initialized-p
      (when (display-graphic-p (or frame (selected-frame)))
        (setq ui-theme--initialized-p t)
        (ui-theme--init))))

  ;; after-make-frame-functions はフレーム実体化の途中（色がまだ確定して
  ;; いない状態）で同期的に走る。nano-faces の (face-foreground/background ...)
  ;; はこのタイミングでは nil を返す（ui-nano-modeline.el と同じ問題、
  ;; 実機検証で判明。詳細は Changelog を参照）。run-with-timer で 1 ティック
  ;; 遅らせ、フレームが完全に実体化してから呼ぶ。
  (defun ui-theme--init-after-frame-realized (frame)
    "フレームが完全に実体化してから `ui-theme--init-once-graphic' を呼ぶ。"
    (run-with-timer 0 nil #'ui-theme--init-once-graphic frame))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'ui-theme--init-after-frame-realized)
...
#+begin_src emacs-lisp
  ...
  (leaf desktop
    ...
    (let ((dir (concat no-littering-var-directory "desktop/")))
      ...
      ;;
      ;; desktop-restore-frames は既定で t である。--daemon 起動時（実
      ;; ディスプレイ接続が無い状態）でもこれが働き、desktop-read が
      ;; フレームセットを復元しようとする。復元されたフレームは、実際には
      ;; 表示に接続していないにもかかわらず、以前の GUI セッションで保存された
      ;; フレームパラメータ（display / window-system 等）を持つため、
      ;; (display-graphic-p FRAME) が true を返してしまう「ゾンビフレーム」に
      ;; なる。これが nano-modeline / ui-theme / vertico-posframe 等の
      ;; after-make-frame-functions ガードを素通りさせ、色計算が壊れる
      ;; 直接の原因だった（個別パッケージ側をいくら直しても、この根本原因を
      ;; 直さない限り再発する）。daemon では実フレームは emacsclient の接続に
      ;; よってのみ作られるべきなので、daemon 起動時に限り無効化する。
      ;; バッファの復元（desktop-restore-eager）には影響しない。
...
#+begin_src emacs-lisp
  ...
  ;; leaf の :if は「モジュールのロード時」に一度だけ評価される。--daemon
  ;; 起動時はその時点で実フレームが無いため (display-graphic-p) は常に nil
  ;; となり、treemacs パッケージ自体が永久にロードされない（ligature・
  ;; vertico-posframe と同じ daemon 対応バグ）。treemacs はユーザーが
  ;; M-x treemacs で明示的に呼ぶまで自動起動しないパッケージであり、
  ;; ターミナルでも（アイコンは無いが）動作するため、display-graphic-p での
  ;; 事前ゲートは不要である。:if を撤去し、:require nil による通常の遅延
  ;; ロード（初回コマンド呼び出し時に本体がロードされ、そのとき初めて
  ;; カスタム変数とフックが with-eval-after-load 経由で適用される）に変更した。
  (leaf treemacs
    :straight t
    :require nil)
  ...
  ;; leaf の :if (memq window-system '(mac ns)) は、グローバル変数
  ;; window-system をモジュールのロード時に一度だけ読む。--daemon 起動時は
  ;; その時点で実フレームが無いため window-system は nil となり、pbcopy が
  ;; 永久に有効化されない（ligature 等と同じ daemon 対応バグ）。グローバル
  ;; 変数ではなく、実際に作られたフレームの種別を (framep frame) で個別に
  ;; 確認する形に変更した。
  (leaf pbcopy
    :straight t
    :require nil)

  (defvar ui-utils--pbcopy-activated-p nil)

  (defun ui-utils--maybe-activate-pbcopy (&optional frame)
    "FRAME が macOS の GUI（mac/ns）なら pbcopy を一度だけ有効化する。"
    (unless ui-utils--pbcopy-activated-p
      (when (memq (framep (or frame (selected-frame))) '(mac ns))
        (setq ui-utils--pbcopy-activated-p t)
        (require 'pbcopy)
        (turn-on-pbcopy))))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'ui-utils--maybe-activate-pbcopy)
    (add-hook 'after-init-hook #'ui-utils--maybe-activate-pbcopy))
...
#+begin_src emacs-lisp
  ...
  ;; Emacs 30 未満のみ MELPA から単体パッケージを導入する。Emacs 30 以降は組み込みの
  ;; ため何もしない（straight には一切触れさせない）。有効化（which-key-mode 1）と設定は
  ;; 両バージョンとも下記の `ui-which-key--startup-init' が一元的に行う。
  ;;
  ;; version<= による文字列比較ではなく整数の emacs-major-version を使う。
  ;; "30.0.90"（プレリリース）のようなバージョン文字列は固定文字列 "30.0" との比較結果が
  ;; 予測しにくいのに対し、整数比較は曖昧さがない。
  ;;
  ;; 以前は `(leaf which-key :straight nil/t :if (...))' という2つの leaf 宣言に
  ;; 分けていたが、Emacs 30 環境（which-key が組み込みの場合）で実機検証したところ
  ;; "Symbol's value as variable is void: which-key" というエラーで ui-which-key.el
  ;; 自体の読み込みが失敗することが判明した。leaf-keywords.el の :straight 拡張と
  ;; :if の組み合わせ方に起因すると見られるが、正確な内部要因までは特定できていない。
  ;; leaf マクロを経由せず素の elisp でバージョン分岐すれば、この問題自体を回避できる。
...
Submap commands reach 4 keystrokes.  Redundant paths via global keys are removed."
    ...
    (when (fboundp 'org-roam-capture)
      (ui-keymap--bind 'ui-leader-o-map "n" #'org-roam-capture))        ; NEW
  ...
  ;; `z' — dirvish 一覧を保持したまま file-at-point を別ウィンドウに RO 表示する。
  ;;
  ;; 上の `ui-keymap--dirvish-open-read-only'（"O" にバインド）は
  ;; `dired-find-file-other-window' + `read-only-mode' の組み合わせだが、これは
  ;; 「編集可能なバッファを開いてから明示的に read-only にする」経路であり、
  ;; view-mode 特有の挙動（"q" で閉じてウィンドウ配置ごと元に戻る、末尾に
  ;; カーソルを置いても編集は一切できない、等）を持たない。
  ;;
  ;; `view-file-other-window' は最初から view-mode で開くため、「素早く中身を
  ;; 確認して q で戻る」用途に向いている。dired 標準の "v"（`dired-view-file'）は
  ;; カレントウィンドウを置き換えてしまい、dirvish の一覧が隠れてしまうため、
  ;; その代替として "z" に別ウィンドウ版を割り当てる。
  (defun ui-keymap--dirvish-view-file-other-window ()
    "View file-at-point read-only in another window via `view-file-other-window'.
  Unlike `dired-view-file' (bound to the dired-native \"v\"), this keeps the
  current dirvish listing window intact instead of replacing it, and opens
  the target buffer directly in `view-mode' (not merely `read-only-mode'
  layered on top of a normal buffer)."
    (interactive)
    (view-file-other-window (dired-get-filename nil t)))

  ;; view-mode バッファの出現位置を固定する。
  ;;
  ;; `view-file-other-window' は内部的に `switch-to-buffer-other-window' →
  ;; `display-buffer' を通るため、`display-buffer-alist' の対象になる。指定が
  ;; ないと分割方向がフレームの状態（既存ウィンドウ数・フォーカス位置）に依存して
  ;; 毎回揺れる。フレーム最大化運用（ウィンドウ分割の目視手がかりが少ない）と
  ;; 特に相性が悪いため、常に右側の専用サイドウィンドウに固定する。
  ;;
  ;; `dedicated . t' により、view-mode を抜けた後（"q" で `view-mode' が
  ;; バッファを kill/quit する際）にこのサイドウィンドウ自体も片付く。
  (add-to-list 'display-buffer-alist
               '((lambda (buf _action)
                   (with-current-buffer buf (derived-mode-p 'view-mode)))
...
local-leader path M-SPC m t/s/d is retained as aliases for discoverability."
    (with-eval-after-load 'dirvish
      (when (boundp 'dirvish-mode-map)
        ...
        ;; "z" は dired 標準では `dired-do-compress'（マークしたファイルの圧縮）に
        ;; 割り当てられている。dirvish セッションでは頻度が低いと判断し、
        ;; view-mode での別ウィンドウ RO 表示に明示的に上書きする。
        (keymap-set dirvish-mode-map "z"   #'ui-keymap--dirvish-view-file-other-window)
        (keymap-set dirvish-mode-map "q"   #'dirvish-quit)))

    (with-eval-after-load 'org
      (when (boundp 'org-mode-map)
        ;; ローカルリーダーのエイリアス: <leader> m t/s/d はグローバルの
        ;; <leader> o t/s/d と同義。ハードコードの "M-SPC" ではなく
        ;; ui-leader-key を直接参照することで、personal/device-*.el が
        ;; リーダーキーを変更した環境（例: WSL の C-c SPC）にも追従する。
        (keymap-set org-mode-map (concat ui-leader-key " m t") #'org-todo)
        (keymap-set org-mode-map (concat ui-leader-key " m s") #'org-schedule)
        (keymap-set org-mode-map (concat ui-leader-key " m d") #'org-deadline))))
...
#+begin_src emacs-lisp
  ...
  ;; - macOS 風のフレーム／ウィンドウ操作をバインドする（s-o/s-m/s-w/s-./s-j）
  ;; - 単発ファイル操作（C-c o/v）、置換・行移動（C-c r/M-g g）、
  ;;   コメント・整列（C-c ;/:/M-a）、スクロール（C-s-up/down、
  ;;   s-wheel-up/down）、文字サイズ（C-+/C--）をバインドする
...
that benefit from single-chord access (no prefix), plus a curated set of
frame/window/editing conveniences documented on the ty07.net cheat-sheet
post (備忘録: Emacsのショートカットキー).
  ...
  <f1>    → help-command
  C-c c   → org-capture        (Org community standard; global)
  s-o     → find-file-other-frame
  s-m     → make-frame-command
  s-w     → delete-frame
  s-.     → window-toggle-split-direction (guarded by fboundp; Emacs 27+)
  s-j     → find-file-other-window
  C-c o   → find-file          (re-added: direct chord faster than M-SPC for
                                 the single highest-frequency file action)
  C-c v   → find-file-read-only
  C-c k   → kill-buffer-and-window (distinct from M-SPC k = my/smart-kill-buffer;
                                 the latter save-confirms then kills the buffer
                                 only, this one also closes the window)
  C-c r   → query-replace      (distinct from M-SPC s r = consult-ripgrep;
                                 this is in-buffer string replacement, not
                                 a project search — no overlap in practice)
  M-g g   → consult-goto-line
  C-c ;   → comment-region
  C-c :   → uncomment-region
  C-c M-a → align-regexp
  C-s-↑   → scroll-down-command      (reveal text above point, same window)
  C-s-↓   → scroll-up-command        (reveal text below point, same window)
  s-<wheel-up>   → scroll-other-window-down
  s-<wheel-down> → scroll-other-window
  C-+     → text-scale-increase
  C--     → text-scale-decrease
Former C-c b binding remains removed: reachable via M-SPC (2-3 keys).
C-c o/v/k/r were reintroduced deliberately (2026-07) after cross-checking
against the ty07.net memo: o/v duplicate no leader binding (find-file and
find-file-read-only were never given M-SPC equivalents), k is a distinct
command (kill-buffer-and-window vs. leader's save-confirming
my/smart-kill-buffer), and r is a genuinely different command
(query-replace) from the already-leader-bound consult-ripgrep, so none of
the four re-create the redundancy that C-c b avoided by living under
M-SPC alone.
...
binding would add keymap surface without practical benefit."
    ...
    ;; Org キャプチャ — コミュニティ標準の C-c c を全バッファで有効化。
    ;; org-capture は任意のバッファから起動できる必要があるため
    ;; orgx-core.el ではなくここ（グローバルキー）に置く。
    (keymap-global-set "C-c c"     #'org-capture)

    ;; macOS 風のフレーム／ウィンドウ操作（ty07.net 備忘録より）
    (keymap-global-set "s-o"       #'find-file-other-frame)
    (keymap-global-set "s-m"       #'make-frame-command)
    (keymap-global-set "s-w"       #'delete-frame)
    ;; window-toggle-split-direction は Emacs 27 以降の同梱コマンド。
    ;; 古いバージョンでも起動時エラーにならないよう fboundp で防御する。
    (when (fboundp 'window-toggle-split-direction)
      (keymap-global-set "s-." #'window-toggle-split-direction))
    (keymap-global-set "s-j"       #'find-file-other-window)

    ;; ファイル操作 — 高頻度な単発操作のため直接和音を再導入。
    ;; consult-find（M-SPC s f）はプロジェクト内あいまい検索用であり、
    ;; パスが分かっている単発オープンには find-file の直接和音が速い。
    (keymap-global-set "C-c o"     #'find-file)
    (keymap-global-set "C-c v"     #'find-file-read-only)
    ;; M-SPC k の my/smart-kill-buffer（保存確認してから kill、ウィンドウは残す）
    ;; とは別物。こちらはウィンドウごと閉じる kill-buffer-and-window。
    (keymap-global-set "C-c k"     #'kill-buffer-and-window)

    ;; 置換・行移動
    ;; C-c r は query-replace（バッファ内文字列置換）。
    ;; M-SPC s r の consult-ripgrep（プロジェクト横断検索）とは役割が異なる。
    ...
    (keymap-global-set "C-c M-a"   #'align-regexp)

    ;; スクロール — 現在のウィンドウ（C-s-↑/↓）と他のウィンドウ（s-wheel-up/down）
    (keymap-global-set "C-s-<up>"        #'scroll-down-command)
    (keymap-global-set "C-s-<down>"      #'scroll-up-command)
    (keymap-global-set "s-<wheel-up>"    #'scroll-other-window-down)
    (keymap-global-set "s-<wheel-down>"  #'scroll-other-window)
...
#+begin_src emacs-lisp
  ...
  ;;   （起動時間短縮のため、有効化自体は emacs-startup-hook 経由でアイドル
  ;;   タイマーに遅延させる）
  ...
  ;; pulsar は起動プロファイル計測で 16〜22ms を占めていた。leaf の :custom /
  ;; :config は暗黙に :require t を伴うため、モジュールのロード時に pulsar が
  ;; 即時 require されていたことが原因である。pulsar はナビゲーション系
  ;; コマンドの後にカーソル位置を一瞬光らせるだけの視覚効果であり、起動完了の
  ;; 一瞬後に有効化されても体感差が無い。`emacs-startup-hook' からアイドル
  ;; タイマー経由で遅延有効化する（org-roam の autosync などで既に使っている
  ;; パターンと同じ）。パッケージの導入自体は `:straight t' がここで保証する
  ;; ので、`:require nil' で「導入はするがロードはしない」状態にする。
  (leaf pulsar
    :straight t
    :require nil)

  (defun ui-visual-aids--activate-pulsar ()
    "pulsar を require し、`pulsar-global-mode' を有効化する。
`pulsar-delay' 等は defcustom であり、require 前に setopt すると void-variable
になるため、設定は require の直後に行う。"
    (require 'pulsar)
    (setopt pulsar-delay 0.04
            pulsar-pulse t
  ...
  (defun ui-visual-aids--schedule-pulsar ()
    "起動完了後、アイドルタイマーで `ui-visual-aids--activate-pulsar' を呼ぶ。"
    (run-with-idle-timer 0 nil #'ui-visual-aids--activate-pulsar))

  (add-hook 'emacs-startup-hook #'ui-visual-aids--schedule-pulsar)
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; system-taskbar-mode のバックエンド判定（system-taskbar--set-back-end）は
  ;; (boundp 'ns-version-string) というビルド種別のチェックであり、実行時の
  ;; フレーム有無とは無関係。'ns バックエンドの system-taskbar--enable は
  ;; フレームの存在を一切確認せず無条件に t を返す実装になっている
  ;; （system-taskbar.el 本体で確認済み）。そのため display-graphic-p による
  ;; daemon 向けの遅延ガードは不要と判断し、単純な無条件呼び出しに戻した。
...
SECRET may be a string or a zero-argument function (auth-source lazy form)."
  ...
  (defun auth-gpg--lookup-all (services)
    "Return an alist of (SVC . ENTRY) for SERVICES via a single auth-source-search.
ENTRY is nil when no matching entry was found for SVC's :host and :user.

A single combined search is deliberate: `auth-source-backends' has no
cache of its own and rebuilds every backend — including re-decrypting
`auth-gpg-file' via epa-file — on every call.  Looking up each service
with its own `auth-source-search' call used to trigger one Touch ID
prompt per service on macOS (three services, three prompts in immediate
succession, later ones prone to fail); see dev-ai.el's :config comment
for the same bug diagnosed in a different module."
    (let ((entries (auth-source-search
                    :host    (mapcar (lambda (svc) (plist-get svc :host)) services)
                    :max     (length services)
                    :require '(:secret))))
      (mapcar (lambda (svc)
                (cons svc
                      (seq-find (lambda (e)
                                  (and (equal (plist-get e :host) (plist-get svc :host))
                                       (equal (plist-get e :user) (plist-get svc :user))))
                                entries)))
              services)))
...
Looks up all services with a single `auth-gpg--lookup-all' call so only
one Touch ID prompt (on macOS) is needed, not one per service."
    (interactive)
    (let ((buf (get-buffer-create "*Auth-GPG Status*"))
          (results (auth-gpg--lookup-all auth-gpg-services)))
      (with-current-buffer buf
        (read-only-mode -1)
        (erase-buffer)
...
#+begin_quote
*落とし穴（実機で踏んだ）— 独自バックエンドの無限再帰:* かつての実装は
=security(1)= を =call-process= で直接呼ぶ独自の =auth-source-backend= を
=auth-source-backend-parser-functions= に登録していた。その登録関数
（ =auth-keychain--make-backend= ）が、実際のバックエンド構築を eieio の
コンストラクタで直接行う代わりに、ディスパッチャである =auth-source-backend-parse=
自身を再帰的に呼び出していた。 =auth-source-backend-parse= は
=auth-source-backend-parser-functions= を先頭から順に試すため、
=auth-keychain--make-backend= が呼ばれる → 中で再度 =auth-source-backend-parse= を
呼ぶ → リストの中にまだ =auth-keychain--make-backend= 自身が残っている →
また呼ばれる、という無限再帰になる。 =M-x my/auth-gpg-verify= を実行して初めて
=auth-source-search= が呼ばれた際に "excessive-lisp-nesting" で発覚した
（ =eieio--perform-slot-validation= が延々とスタックに積まれるバックトレースが
決め手になった）。

調査の過程で、この Emacs（32.0.50、開発版）には *組み込みの* macOS Keychain
バックエンド =auth-source-backends-parser-macos-keychain= が既に存在すること
が判明した。 =security(1)= を独自に呼ぶコードは、この組み込み機能が入る前に
書かれたものだったと見られる。無限再帰バグを個別に直すのではなく、独自実装を
丸ごと廃止して組み込みバックエンドに委譲する形に置き換えた。
#+end_quote

*実装方針 — 組み込みの macOS Keychain バックエンドに委譲する*

Emacs 本体の =auth-source.el= が提供する =macos-keychain-internet= /
=macos-keychain-generic= の2種類のうち、 =macos-keychain-internet= を使う。
理由は =:host= の意味づけにある。

- =macos-keychain-internet= — =:host= はそのまま =security find-internet-password -s HOST= の
  =-s= （サービス名）に渡る。 =auth-gpg--lookup-all= や =dev-ai--setup-api-key= など、他の
  auth-source バックエンドと同じ「 =:host= = 対象ホスト」という素直な意味づけになる。
- =macos-keychain-generic= — =:host= は =-c HOST= （Keychain の「creator」フィールド）に
  マップされ、 =-s= （サービス名）には代わりに =:port= を渡さなければならない。
  他のコードが素直に =:host= を渡すと *サイレントに何も見つからない* 、はまりやすい罠。

このねじれ（ =auth-source-macos-keychain-search-items= 内の
=(if keychain-generic "-c" "-s")= 分岐）があるため、Keychain エントリは
Internet Password として保存する必要がある。

*Keychain エントリの形式*

エントリは Internet Password として保存してください。対応関係は次のとおりです。
...
#+begin_example
security find-internet-password -s "api.anthropic.com" -a "apikey" -w
#+end_example

*auth-gpg との優先順位*

=auth-keychain= は =auth-gpg= の実行後に =auth-sources= の末尾へ =macos-keychain-internet=
シンボルを追加します。したがって参照される順序は次のとおりです。

1. =~/.authinfo.gpg= （ =auth-gpg= が管理）
2. =~/.authinfo= の平文フォールバック（存在する場合のみ。 =auth-gpg= が管理）
3. macOS Keychain（組み込みバックエンド。本モジュールは有効化のみを担う）

つまり GPG 暗号化ストアが常に優先されます。 =~/.authinfo.gpg= に無い資格情報だけが、自動的に
Keychain から拾われる、という関係です。 =auth-sources= の末尾に追加するため、 =dev-ai.el= の
=:host= 検索などグローバルな =auth-source-search= 呼び出しからも、GPG に無ければ自動的に
Keychain へフォールスルーします。
...
#+begin_src emacs-lisp
    ...
    ;;
    ;; 本モジュールは、Emacs 組み込みの macOS Keychain バックエンド
    ;; （`auth-source-backends-parser-macos-keychain'、Internet Password 種別）を
    ;; `auth-sources' に追加するだけの薄いラッパーである。`security(1)' を独自に
    ;; 呼ぶバックエンドは持たない。
    ;;
    ;; 責務:
    ;;
    ;; - `auth-keychain-enable' を定義する。personal 層がモジュールのロード前に設定する。
    ;; - after-init-hook で `macos-keychain-internet' を `auth-sources' の末尾に追加する。
  ...
  report consistent status across both backends."
    ...
    (defun auth-keychain--lookup (host user)
      "Look up an Internet Password for HOST and USER via the built-in macOS
  Keychain auth-source backend.  Returns the secret string, or nil when no
  matching entry exists.  Scoped to the Keychain only (via a let-bound
  `auth-sources') so the result reflects the Keychain alone, independent of
  whichever entries `auth-gpg' may also have registered."
  ...
  Opens a *Auth-Keychain Status* buffer listing each service entry as:
    ✓ PRESENT  — Internet Password found; first four characters of secret shown.
    ✗ MISSING  — No matching Internet Password in the Keychain.
    → Hint line with the `security add-internet-password' command to add it."
      (interactive)
      ...
      (let ((buf (get-buffer-create "*Auth-Keychain Status*")))
        (with-current-buffer buf
          ...
          (insert (format "Auth-Keychain credential status — %s\n"
                          (format-time-string "%Y-%m-%d %H:%M:%S")))
          (insert "Backend: built-in auth-source-macos-keychain (Internet Password items)\n\n")
          (dolist (svc auth-keychain-services)
            (let* ((label  (plist-get svc :label))
                   (host   (plist-get svc :host))
                   (user   (plist-get svc :user))
                   (secret (auth-keychain--lookup host user)))
              (if secret
                  (insert (format "  ✓ PRESENT  %s\n             secret preview: %s…\n"
                                  label
                                  (if (>= (length secret) 4)
                                    ...
                                    secret)))
                (insert (format "  ✗ MISSING  %s\n             add with:\n             security add-internet-password -s \"%s\" -a \"%s\" -w \"<secret>\"\n"
                                label host user)))
              (insert "\n")))
          (read-only-mode 1)
          (goto-char (point-min)))
    ...
    (defun auth-keychain--startup-init ()
      "Conditionally register the built-in macOS Keychain auth-source backend.
  Runs on `after-init-hook'.  Skips registration when `auth-keychain-enable'
  is nil (personal layer opt-out) or `system-type' is not darwin."
      (when (and auth-keychain-enable
                 (eq system-type 'darwin))
        (add-to-list 'auth-sources 'macos-keychain-internet t)
        (message "auth-keychain: macOS Keychain backend registered (built-in).")))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; 本ファイルの責務:
  ;; - 二重初期化を避けるため、`window-setup-hook' で vertico、marginalia を
  ;;   ちょうど一度だけ初期化する
  ;; - posframe 表示はグラフィカルセッションでのみ、初回のグラフィカル
  ;;   フレームができた時点で一度だけ有効化する（daemon 対応。vertico/
  ;;   marginalia の初期化とは独立に扱う）
  ...
  (defun completion-vertico--init ()
    "Initialize Vertico and Marginalia exactly once.
  Guards repeated invocation with `completion-vertico--initialized'.
  These are frame-independent (minibuffer completion works without a
  display), so unlike posframe below, no daemon branch is needed here.
  Removes itself from `window-setup-hook' after first run."
  ...
  ;; vertico-posframe だけは vertico-mode / marginalia-mode と切り離して
  ;; 扱う。--daemon 起動時、上の completion-vertico--init は実フレームの無い
  ;; window-setup-hook で走ってしまうため、その中で単純に (display-graphic-p)
  ;; を見るだけだと daemon では常に nil と判定され、しかも
  ;; completion-vertico--initialized が立って自分自身をフックから除去して
  ;; しまうため、後から emacsclient -c で本物のフレームが繋がっても
  ;; 二度と再試行されない（posframe が永久に効かないバグ）。
  ;;
  ;; ligature / nano-modeline と同じパターンで、初回のグラフィカルフレームが
  ;; できるまで独立して遅延させる。
  (defvar completion-vertico--posframe-initialized nil)

  (defun completion-vertico--activate-posframe ()
    "posframe / vertico-posframe を require し、有効化する。"
    (require 'posframe)
    (require 'vertico-posframe)
    (setopt vertico-posframe-border-width 2)
    (vertico-posframe-mode 1)
    (require 'vertico-multiform nil t))

  (defun completion-vertico--maybe-init-posframe (&optional frame)
    "FRAME が初回のグラフィカルフレームなら vertico-posframe を有効化する。"
    (unless completion-vertico--posframe-initialized
      (when (display-graphic-p (or frame (selected-frame)))
        (setq completion-vertico--posframe-initialized t)
        (completion-vertico--activate-posframe))))

  ;; nano-modeline / ui-theme と同じ理由（after-make-frame-functions はフレーム
  ;; 実体化の途中で走り、色がまだ確定していない）で、posframe の子フレーム背景色
  ;; も同種の影響を受け得る。直接の破損は未確認だが、同じパターンで予防的に
  ;; 1 ティック遅らせておく。
  (defun completion-vertico--init-posframe-after-frame-realized (frame)
    "フレームが完全に実体化してから `completion-vertico--maybe-init-posframe' を呼ぶ。"
    (run-with-timer 0 nil #'completion-vertico--maybe-init-posframe frame))

  (if (daemonp)
      (add-hook 'after-make-frame-functions #'completion-vertico--init-posframe-after-frame-realized)
    (add-hook 'window-setup-hook #'completion-vertico--maybe-init-posframe))
  ...
  ;; leaf の :if は「モジュールのロード時」に一度だけ評価される。--daemon
  ;; 起動時はその時点で実フレームが無いため (display-graphic-p) は常に nil
  ;; となり、vertico-posframe パッケージ自体が永久にロードされなかった
  ;; （ligature が :when で抱えていたのと同じ daemon 対応バグ）。
  ;; :if を撤去して :require nil にし、実際の require と有効化は上の
  ;; completion-vertico--activate-posframe に委ねる。
...
#+begin_src emacs-lisp
  ...
  ;; - Embark 本体のグローバルキーバインドを設定する（s-/、s-;、C-<f2>）
  ;;; Code:
  ...
  (leaf embark
    ...
    ;; s-/ act、s-; dwim、C-<f2> でバインディング一覧（ty07.net 備忘録より）。
    ;; パッケージ固有のコマンドのため、汎用の ui-global-keys.el ではなく
    ;; ここ（Embark 自身の leaf ブロック）にまとめる。
    :bind (("s-/"    . embark-act)
           ("s-;"    . embark-dwim)
           ("C-<f2>" . embark-bindings)))
  (leaf embark-consult
    :straight t
...
#+begin_src emacs-lisp
  ...
  ;; Org のディレクトリパス変数。
  ;;
  ;; 上書きの契約: personal/<user>.el は init.el のステップ 9 により modules.el より
  ...
  (defvar my:d:org-biblio (expand-file-name "biblio" my:d:org))
  ...
  (my/ensure-directory-exists my:d:org-biblio)
  ...
  (leaf org
    ...
    ;; Page Bundle化済み（2026-07-30〜）のため、EXPORT_FILE_NAME は常に "index" 固定。
    ;; 記事固有のスラッグは org-id-new で生成し EXPORT_HUGO_BUNDLE に入れる
    ;; （all-posts.org における既存記事と同じ UUID 由来スラッグの慣習に合わせている）。
    ;; EXPORT_HUGO_COVER という独自プロパティ名は ox-hugo が認識しない（front matter に
    ;; 変換されない）ため使わない。任意キーをfront matterに追加したい場合、ox-hugo が
    ;; 唯一受け付ける経路は EXPORT_HUGO_CUSTOM_FRONT_MATTER（plist形式）であるため、
    ;; ここに :cover "" を空文字のプレースホルダーとして常に挿入しておく。
    ;; 記事固有のカバー画像を用意した場合は、実ファイルを
    ;; assets/img/cover/custom/<EXPORT_HUGO_BUNDLE>.jpg に置いた上で、この値を
    ;; :cover "custom/<EXPORT_HUGO_BUNDLE>.jpg" に書き換える（deploy.sh の Tier1: 明示指定）。
    ;; 空文字のままなら deploy.sh 側で post_front_matter_cover() が空文字を返し、
    ;; 従来通り Tier2（タグ）→Tier3（季節）→Tier4（デフォルト）へフォールスルーする
    ;; ため、既存の自動選択の挙動は変わらない。
    ...
    (setq org-capture-templates
          `(("t" "Todo" entry (file+headline ,(expand-file-name "gtd.org" org-directory) "Inbox")
            ...
            ("b" "Blog post" entry
             ...
             "* TODO %^{Title}\n:PROPERTIES:\n:EXPORT_HUGO_BUNDLE: %(org-id-new)\n:EXPORT_FILE_NAME: index\n:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :cover \"\"\n:EXPORT_DATE: %<%Y-%m-%d %H:%M:%S>\n:EXPORT_HUGO_TAGS: %^{Tags}\n:EXPORT_HUGO_CATEGORIES: %^{Category}\n:EXPORT_HUGO_LASTMOD:\n:END:\n%?"))))

  ;; org-emphasis-regexp-components の既定値（PRE/POST）は半角の空白・句読点のみを
  ;; 想定しており、全角の「」『』（）、。・ 等は含まれない。本ドキュメントのような
  ;; 和文中心の文章では、和文の直後に =code= や *bold* を続け、その直後にまた
  ;; 全角の閉じ括弧・句読点が続く書き方がごく普通に発生する
  ;; （例: 「=README.org=（設定の本体）は」）。この全角文字が PRE/POST の集合に
  ;; 無いため、Org は強調マーカーを強調として認識せず、リテラルの "=" 等の文字が
  ;; そのまま表示されてしまう（scripts/check_emphasis.py が検出する不具合の実体）。
  ;;
  ;; 対処として、和文でよく使う開き括弧類を PRE に、閉じ括弧・読点類を POST に
  ;; 追加する。org-emphasis-regexp-components の :set 関数は org-set-emph-re
  ;; であり、setopt はこれを正しく呼び出して org-emphasis-re / org-verbatim-re を
  ;; 再生成する（手動で org-set-emph-re を呼び直す必要はない）。
  ;;
  ;; ここだけ leaf の :config 本体から外し、with-eval-after-load 'org で明示的に
  ;; 遅延させている。理由: leaf の :config は org.el のロードを待たない
  ;; （org-todo-keywords 等の setq がここまで問題なく動いていたのは、setq が
  ;; 未束縛のシンボルにも新規束縛を作れる — defcustom を待つ必要がない — ためで
  ;; あって、:config 自体が遅延されているわけではない）。この customization は
  ;; org-emphasis-regexp-components の *既存の値* を nth で読んで拡張するため、
  ;; org.el が実際にロードされ defcustom の初期値が入った後でなければ
  ;; 「Symbol's value as variable is void」で失敗する。すぐ下の「アーカイブ先」と
  ;; 同じ理由・同じパターンである。
  ;;
  ;; scripts/check_emphasis.py の PRE_OK / POST_OK もこれと同じ文字集合を
  ;; 使うよう更新してある。二重管理だが、Python 側は Emacs 本体が無い環境でも
  ;; 実行できることが目的（Appendix を参照）であり、Elisp の値を実行時に import
  ;; することはできないため、手作業での同期が必要になる。
  (with-eval-after-load 'org
    (setopt org-emphasis-regexp-components
            (list (concat (nth 0 org-emphasis-regexp-components) "「『（【、。・")
...
#+begin_src emacs-lisp
  ...
  ;;   `org-roam-db-autosync-mode' は起動時に自動起動しない（Org の遅延ロードを保つため）。
  ;;   有効化は対話コマンド my/org-roam-autosync-enable による
  ;; - org-roam-capture-templates を設定し、ノード新規作成時にドメイン×種別タグと
  ;;   :ID:/:ROAM_REFS:/:STATUS: の PROPERTIES drawer を強制する。タグ語彙・命名規則等の
  ;;   運用ポリシー本体は roam ノート側（meta-knowledge-base-conventions、
  ;;   id: cba75214-acbb-4b04-ab07-46bd63cb3e36）を正とし、ここにはコードのみを置く。
  ...
  (defun my/org-roam-autosync-enable ()
    "Load org-roam and enable `org-roam-db-autosync-mode'.
  org-roam is no longer synced automatically at startup.  Enabling autosync
  requires loading org-roam, which in turn loads Org; doing that on an idle
  timer defeated Org's deferred loading.  This command performs the same
  enable step on demand.  Note that `org-roam-node-find' and the other entry
  points are autoloaded, so they trigger a one-off DB sync on first use even
  without persistent autosync."
    (interactive)
    (condition-case err
        (progn
          (require 'org-roam)
          (when (fboundp 'org-roam-db-autosync-mode)
            ...
            (message "[org-roam] autosync enabled")))
      (error
  ...
  (leaf org-roam
    ...
    ;; capture template: ドメイン×種別タグ運用（meta-knowledge-base-conventions 参照）。
    ;; ファイル名は `<domain>-<slug>.org' 相当になるよう日付プレフィックス+スラッグで生成し、
    ;; :ID:/:ROAM_REFS:/:STATUS: を PROPERTIES drawer として強制する。
    ;; タグ語彙（ドメイン一覧・種別一覧）自体はこのファイルでは管理しない — 増減の度に
    ;; tangle/コミットが発生するのを避けるため、正規リストは roam ノート側に置く。
    (setopt org-roam-capture-templates
            '(("d" "domain note" plain
               ...
               (file+head
                "%<%Y%m%d>-${slug}.org"
                ":PROPERTIES:\n:ID: %(org-id-uuid)\n:ROAM_REFS: %^{出典URL（複数可、スペース区切り、省略可）}\n:STATUS: n/a\n:END:\n#+TITLE: ${title}\n#+FILETAGS: :%^{ドメインタグ}:%^{種別タグ}:\n#+CATEGORY: %\\1\n#+DATE: %U\n")
               :unnarrowed t)))

    ;; 以前はここで (run-with-idle-timer 5 nil #'orgx--org-roam-autosync-enable) を
    ;; 呼び、起動 5 秒後に autosync を有効化していた。しかし org-roam-db-autosync-mode を
    ;; 有効化するには org-roam のロードが必要であり、org-roam は org を require する。
    ;; つまりこのタイマーが「起動直後に Org を読み込む」経路そのものだった。
    ;; 自動起動をやめ、M-x my/org-roam-autosync-enable による対話起動に切り替えた。
    ;; org-roam-node-find などのエントリポイントは autoload されているため、初回呼び出し
    ;; 時に一度だけ DB が同期される。常時 autosync が要る場合のみ上記コマンドを使う。
    )
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'org
    ...
    (defun my/org-unfold-subtree ()
      ...
      (interactive) (org-fold-show-subtree))
...
#+begin_src emacs-lisp
    ...
    ;; - 日本語 PDF 文書向けに ltjsarticle クラスを org-latex-classes に登録し、
    ;;   PDF 生成を org 標準の 3 回連続コンパイルではなく latexmk に委ねる
    ;; - LaTeX バックエンドの *.svg 画像リンクを、事前に rsvg-convert で
    ;;   生成した同名 *.pdf を指すよう \includegraphics の拡張子を書き換える
    ;;   （org 標準機能では変換されず、svg パッケージは Inkscape 本体を要求する
    ;;   ため使わない。経緯は下記の依存注記を参照）
    ;;
    ...
    ;;
    ;; LaTeX/PDF（LuaLaTeX-ja）の依存に関する注記:
    ;;
    ;;   dotfiles 側で ~/.latexmkrc のグローバルデフォルトを LuaLaTeX-ja に切り替え
    ;;   済み（$pdf_mode = 4、$lualatex 経由で PDF を直接生成。詳細は dotfiles
    ;;   リポジトリの README.md「MacTeX」節を参照）。これに合わせて、以下の
    ;;   org-latex-pdf-process は %latex 置換を経由せず常に `latexmk' を呼ぶため、
    ;;   個々の文書の #+LATEX_COMPILER は PDF 生成段では参照されない。pLaTeX 等
    ;;   別エンジンへ文書単位で切り替えたい場合は、latexmk の magic comment
    ;;   （% !TEX program = platex）を .tex 側に置くこと。
    ;;   mactex-no-gui（TeX Live full scheme）に LuaTeX-ja・原ノ味フォントは
    ;;   同梱済みのため、追加インストールは不要。
    ;;
    ;;   *.svg を図として使う文書（design_spec.org 等）向けの取り扱い:
    ;;   当初 svg パッケージ + -shell-escape + \includesvg 書き換えを試したが、
    ;;   svg パッケージは変換手段として常に Inkscape 本体の CLI を要求し、
    ;;   inkscapelatex=false は「Inkscape に LaTeX テキストを処理させるか」の
    ;;   切替に過ぎない。mactex-no-gui 環境には Inkscape が無いため、実際に
    ;;   ビルドすると `Inkscape version not detected' で失敗する（design_spec.org
    ;;   のビルドログで確認済み）。そのため svg パッケージ／shell-escape は使わず、
    ;;   SVG を事前に（LaTeX の外側で）rsvg-convert（要: brew install librsvg。
    ;;   CLI のみで GUI 不要）で同名の *.pdf に変換しておき、通常の graphicx の
    ;;   \includegraphics で読み込む方式に変更した:
    ;;     for f in svg/*.svg; do rsvg-convert -f pdf -o "${f%.svg}.pdf" "$f"; done
    ;;   さらに、org-latex-link（ox-latex.el 本体）が *.svg リンクを検出すると、
    ;;   フィルタが動く前に自前で \includegraphics{*.svg} を \includesvg{*}
    ;;   （拡張子も除去済み）へ書き換えることが実機検証で判明したため、
    ;;   orgx-export--svg-to-pdf-graphics（下記）は \includegraphics{*.svg} では
    ;;   なく \includesvg{*} をマッチ対象にしている。文書本体の画像リンクは
    ;;   [[file:foo.svg]] のままでよく、個々のリンクを書き換える必要はないが、
    ;;   対応する *.pdf を事前に生成しておくこと。
    ;;; Code:

    (eval-when-compile (require 'leaf))

    ;; Emacs 本体 international/latexenc.el の既知の欠陥への防御。
    ;;
    ;; `latexenc-find-file-coding-system'（file-coding-system-alist の
    ;; *.tex/*.ltx/*.dtx/*.drv エントリ。Emacs 標準機能で AUCTeX 不要）は、バッファ内に
    ;; \usepackage[...]{inputenc} が見つからない場合、TeX-master ローカル変数などから
    ;; 主ファイルを推測しようとする。この探索で `TeX-master'（AUCTeX 変数。未インストール
    ;; なら unbound）と `tex-main-file'（tex-mode.el の defcustom。既定値 nil）の双方が
    ;; 得られないと、file が nil のまま (decode-coding-string file 'utf-8) を呼んでしまい
    ;; "Wrong type argument: stringp, nil" で例外になる（Emacs 側 latexenc.el 未修正）。
    ;;
    ;; 下記の ltjsarticle（LuaLaTeX-ja）でエクスポートした *.tex はこの条件に確実に合致
    ;; する: LuaLaTeX-ja は fontspec/luatexja で UTF-8 を扱うため inputenc を使わず、
    ;; org の LaTeX エクスポートも TeX-master コメントを付与しない。エクスポート結果を
    ;; デバッグのため Emacs で開くたびに再現するため、TeX-master 探索自体を無効化して
    ;; 回避する。`latexenc-dont-use-TeX-master-flag' は defcustom ではない素の defvar
    ;; なので setq が正しい（コーディング規則 §6。personal/user.el の my:locale と同じ
    ;; 扱い）。org のロード完了を待つ orgx-export--configure-latex 内ではなく、ここ
    ;; （モジュール先頭のトップレベル）で設定するのは、org 未ロードの時点で *.tex を
    ;; 開いても latexenc-find-file-coding-system 自体は独立に発火し再現するため。
    ...
    ;; :custom による先行登録は ox-hugo のロード時に反映される。
    ;;
    ;; 以前との違い: Changelog の「Org の遅延ロード」節にはこの :require t 削除が
    ;; 既に完了したと記載されていたが、実際のコードには反映されていなかった
    ;; （orgx-auto-hugo.el 側は同じ理由で正しく遅延化されていたのに、こちらだけ
    ;; 直し忘れていたと見られる）。ox-hugo は ox を、ox は org を require するため、
    ;; :require t を残したままだと「Org の遅延ロード」の効果がここから漏れて
    ;; 無に帰していた。
    ;;
    ;; 落とし穴（実機で踏んだ）: 上の理由で :require t を外したところ、
    ;; `M-x org-hugo-export-wim-to-md' のような直接呼び出しは autoload 経由で
    ;; 問題なく動くのに、`C-c C-e' の対話的ディスパッチメニューに Hugo の項目が
    ;; 出なくなった（`C-c C-e H A' が選べない）。原因は、`org-export-dispatch' が
    ;; メニューを組み立てる際に `org-export-registered-backends' を直接見るため。
    ;; この一覧への登録（`org-export-define-derived-backend'）は ox-hugo.el が
    ;; 実際にロードされた副作用として起きるのであって、個々のコマンドに付いている
    ;; autoload cookie では発生しない。つまり「関数呼び出しの autoload」と
    ;; 「バックエンドのメニュー登録」は別物で、後者は :require t を外すと
    ;; 起動時だけでなく *ずっと* 登録されないままになる。
    ;;
    ;; 対処: `org-export-dispatch' そのものに :before advice を付け、メニューを
    ;; 開く直前にだけ ox-hugo を require する。通常起動時のコストはそのまま避けつつ、
    ;; 対話的なディスパッチメニューでも Hugo が正しく選べるようにする。
    (leaf ox-hugo :straight t :custom ((org-hugo-front-matter-format . "toml")))

    (defun orgx-export--ensure-ox-hugo-before-dispatch (&rest _)
      "Require `ox-hugo' before `org-export-dispatch' builds its backend menu.
  `org-export-dispatch' reads `org-export-registered-backends' directly; the
  Hugo backend is only added there as a side effect of loading ox-hugo.el
  itself (via `org-export-define-derived-backend'), not by the autoload
  cookies on its individual interactive commands. Without this advice,
  `C-c C-e H A' silently omits the Hugo entry even though calling
  `org-hugo-export-wim-to-md' directly works fine."
      (require 'ox-hugo))

    (advice-add 'org-export-dispatch :before #'orgx-export--ensure-ox-hugo-before-dispatch)
  ...
  where npm global installs are not on PATH."
    ...
    ;; ox-latex は Org 同梱（straight 不要）。ob-mermaid と同じ理由（上記参照）で、
    ;; トップレベルの require は避け、org のロード後に遅延登録する。
    ;; add-to-list — 冪等。setopt — org-latex-pdf-process は defcustom。
    ;;
    ;; SVG 埋め込み方式の変更履歴（2026-07、2 段階の訂正）:
    ;;
    ;; (1) 当初 \usepackage[inkscapelatex=false]{svg} + -shell-escape +
    ;;     \includegraphics{*.svg} を \includesvg{*} へ書き換えるフィルタで試したが、
    ;;     inkscapelatex=false は Inkscape に LaTeX テキストを処理させるかどうかの
    ;;     切替に過ぎず、svg パッケージは変換手段として常に Inkscape 本体の CLI を
    ;;     要求する。mactex-no-gui 環境には Inkscape が無いため `Inkscape version
    ;;     not detected' で実際に失敗した（design_spec.org のビルドログで確認）。
    ;;     svg パッケージ／shell-escape を使わない事前変換方式（下記）に変更した。
    ;;
    ;; (2) (1) の書き換え後も、design_spec.org の実機ビルドで \includesvg が
    ;;     \includegraphics に変換されず `Undefined control sequence' で失敗した。
    ;;     原因は org-latex-link（ox-latex.el）が *.svg リンクに対して、フィルタが
    ;;     動く前に自前で \includegraphics{*.svg} を \includesvg{*}（拡張子も除去
    ;;     済み）へ書き換えていたこと — つまりフィルタは常に \includegraphics{*.svg}
    ;;     ではなく \includesvg{*} を受け取っており、旧フィルタの正規表現が一致対象
    ;;     を持たず素通りしていた。加えて、replace-regexp-in-string に関数 REP を
    ;;     渡す場合でも、返り値に含まれる `\' はデフォルトでは literal 扱いされず
    ;;     `Invalid use of \` in replacement text' で例外になることも判明した
    ;;     （返り値に \linewidth のような \ を含む文字列が混ざる場合に発生）。
    ;;     下記は両方を修正済み: \includesvg{*} を直接マッチし、
    ;;     replace-regexp-in-string の LITERAL 引数を明示的に t にしている。

    (defun orgx-export--svg-to-pdf-graphics (contents backend _info)
      "Rewrite \\includesvg{FILE} to \\includegraphics{FILE.pdf} for the latex backend.
  ox-latex natively rewrites *.svg image links from \\includegraphics{FILE.svg}
  to \\includesvg{FILE} (extension already stripped) before any
  `org-export-filter-link-functions' filter sees the text — this happens
  unconditionally for any link matching `org-latex-inline-image-rules', whether
  or not the svg package (and its Inkscape dependency, deliberately avoided
  here; see the change history comment above) is actually loaded. Instead,
  ,*.svg figures are expected to be pre-rendered to a sibling *.pdf via
  rsvg-convert (brew install librsvg):

    for f in svg/*.svg; do rsvg-convert -f pdf -o \"${f%.svg}.pdf\" \"$f\"; done

  This filter redirects \\includesvg back to the ordinary graphicx
  \\includegraphics pipeline (no shell-escape needed) pointing at the
  pre-rendered PDF. LITERAL is passed as t explicitly: even with a function
  REP, `replace-regexp-in-string' does not treat the returned string as
  literal by default, and the replacement here can itself contain backslashes
  (e.g. \\linewidth carried over from the original options), which would
  otherwise trigger \"Invalid use of `\\' in replacement text\"."
      (when (org-export-derived-backend-p backend 'latex)
        (replace-regexp-in-string
         ...
         (lambda (whole)
           (string-match
            "\\\\includesvg\\(\\[[^]]*\\]\\)?{\\([^}]+\\)}" whole)
           (concat "\\includegraphics" (or (match-string 1 whole) "")
                   "{" (match-string 2 whole) ".pdf}"))
         contents
         nil t)))

    (defun orgx-export--configure-latex ()
      "Register the ltjsarticle class, route PDF export through latexmk, and
  redirect *.svg image links to their pre-rendered *.pdf sibling for the latex
  backend. `ox-latex' is required lazily here (after `org' loads) rather than
  at top level, mirroring `orgx-export--register-mermaid' above."
      (require 'ox-latex)
      ...
      ;; ~/.latexmkrc（LuaLaTeX-ja がデフォルト、$pdf_mode = 4）に依存関係解決と
      ;; 再コンパイル回数の判断を一任する。bibtex/makeindex も upbibtex/upmendex
      ;; として .latexmkrc 側で処理されるため、ここでは何も指定しない。
      ;; shell-escape は使わない（svg パッケージ運用を撤回したため不要。任意の
      ;; シェルコマンド実行を許す -shell-escape は使わずに済むなら使わない）。
      (setopt org-latex-pdf-process
              '("latexmk -synctex=1 -halt-on-error -output-directory=%o %f"))
      (add-to-list 'org-export-filter-link-functions
                   #'orgx-export--svg-to-pdf-graphics))

    (with-eval-after-load 'org
      (orgx-export--configure-latex))
...
保存時の自動 tangle と、=orgx-auto-hugo.el= が担うキャプチャ確定時の自動エクスポートは、
どちらも Org 本体を遅延ロードしたまま実現している。両者の合流点をまとめて図示する。
...
#+begin_src emacs-lisp
  ...
  ;; あわせて、確定前のキャプチャ中に手動で 1 回だけ呼び出す、ブラウザでの
  ;; 簡易プレビュー（markdown-preview-mode 活用）。
  ;;
  ;; 本ファイルの責務:
  ...
  ;;   Org の遅延ロードを保つため）
  ;; - キャプチャ中、`org-capture-mode-map' のキー 1 つで現在のエントリを
  ;;   markdown-preview-mode でブラウザプレビューできるようにする（確定はしない）
  ;;
  ;; 設計の根拠:
  ;;
  ...
  ;; プレビュー機能の設計根拠:
  ;;
  ;; プレビューは「確定（エクスポート）とは独立した、副作用のない下書き確認」でなければ
  ;; ならない。そのため `org-hugo-export-wim-to-md' のようなファイル書き出し系の
  ;; コマンドは使わない。`org-hugo-base-dir' はキャプチャ対象ファイル（
  ;; `my:f:capture-blog-file'）自身の #+HUGO_BASE_DIR キーワードで既に決まっている
  ;; ことが多く、これを一時ディレクトリへ動的束縛で上書きしようとしても、キーワード側が
  ;; 優先されて実ファイルへ書き出されてしまう危険がある。
  ;;
  ;; 代わりに `org-export-as' を直接呼び、SUBTREEP を指定して現在のエントリだけを
  ;; 文字列として取り出す。これはディスクに一切触れず、`org-hugo-base-dir' の解決にも
  ;; 依存しない。得られた文字列は、キャプチャセッションごとに 1 つ用意する使い捨ての
  ;; スクラッチファイルへ書き出し、そのバッファ上で `markdown-preview-mode' を有効化する。
  ;; 実ブログの content ディレクトリは常に無傷のままになる。
  ;;
  ;; スクラッチファイルの置き場所はキャプチャセッションのプロパティリストに
  ;; `org-capture-put'/`org-capture-get' で保持する。バッファローカル変数ではなく
  ;; セッションプロパティを使うのは、キャプチャバッファが確定・中断のどちらでも
  ;; `org-capture-after-finalize-hook' の実行前後で kill される可能性があり、
  ;; バッファローカル変数だけでは後始末のタイミングで参照できない場合があるため。
  ;;
    ;;; Code:
  ...
  (declare-function markdown-preview-mode "markdown-preview-mode" (&optional arg))

  (defun orgx-auto-hugo-preview--scratch-file ()
    "Return this capture session's scratch preview file, creating it on first use.
  The path (and its containing temp directory) is stashed via `org-capture-put'
  under :orgx-hugo-preview-file so repeated calls to
  `orgx-auto-hugo-preview-capture' within the same capture session reuse the
  same file and `markdown-preview-mode' buffer/server instead of spawning a
  new one on every keystroke."
    (or (org-capture-get :orgx-hugo-preview-file)
        (let* ((dir (file-name-as-directory (make-temp-file "orgx-hugo-preview-" t)))
               (file (expand-file-name "preview.md" dir)))
          (org-capture-put :orgx-hugo-preview-file file)
          file)))
  ...
  Uses `org-export-as' with the `hugo' backend and SUBTREEP non-nil to export
  only the current entry to a string, entirely in memory — this deliberately
  avoids `org-hugo-export-wim-to-md' and `org-hugo-base-dir', since the
  capture target file may already set #+HUGO_BASE_DIR, which would take
  precedence over any dynamic let-binding and could export straight into the
  real content directory. The resulting string is written to a per-session
  scratch file (see `orgx-auto-hugo-preview--scratch-file') and previewed
  from there.

  Intended to be bound in `org-capture-mode-map'; signals a `user-error'
  outside a capture buffer.

  `markdown-preview-mode' reads the preview buffer's *window* when it pushes
  an update to the browser (`markdown-preview--send-preview-to', used for
  scroll-sync). Called on a buffer that was never actually displayed — as
  happens with plain `find-file-noselect' — it errors with \"Wrong type
  argument: number-or-marker-p, nil\" instead of pushing the update (the
  scratch file itself still gets written correctly; only the push fails).
  To avoid that, the scratch buffer is popped into a real window for the
  duration of the update, then focus is restored to the capture buffer so
      ...
      (when md-string
        (let* ((capture-buf (current-buffer))
          ...
          (save-selected-window
            ...
            (insert md-string)
            (save-buffer)
            ;; The mode's own initial "send to browser" can still throw the
            ;; error above in edge cases (e.g. the websocket client has not
            ;; connected yet); by that point the server has already started
            ;; and the buffer-local mode is already on, so it is safe to
            ;; swallow and let the next save (or a manual browser reload of
            ;; the printed URL) pick up the content.
            (ignore-errors
  ...
  (defun orgx-auto-hugo-preview--cleanup ()
    "Delete this capture session's scratch preview directory, if one was created.
  Runs on `org-capture-after-finalize-hook' for both finalize and abort;
  harmless when no preview was ever requested during the session."
    (when-let* ((file (org-capture-get :orgx-hugo-preview-file))
                (dir (file-name-directory file)))
      (when (file-directory-p dir)
        (delete-directory dir t))))

  (add-hook 'org-capture-after-finalize-hook #'orgx-auto-hugo-preview--cleanup)
...
#+begin_src emacs-lisp
  ...
  (leaf valign :straight t :after org :require t :hook (org-mode-hook . valign-mode))

  ;; 巨大バッファ（README.org など）では valign の再描画コストが無視できないため、
  ;; 一定サイズを超えるバッファでは無効化する。
...
*** orgx/orgx-citar.el
:PROPERTIES:
:CUSTOM_ID: orgx-citar
:header-args:emacs-lisp: :tangle lisp/orgx/orgx-citar.el
:END:

#+begin_src emacs-lisp
  ;;; orgx/orgx-citar.el --- citar / org-cite による文献引用管理 -*- lexical-binding: t; -*-
  ...
  ;;; Commentary:
  ;;
  ;; citar は org-cite のバックエンドとして文献データベース（CSL-JSON）を検索・挿入・
  ;; フォローする UI を提供する。citar-embark はそこに Embark 経由の文脈依存アクション
  ;; （開く、引用挿入など）を追加する。
  ;;
  ;; 本ファイルの責務:
  ;; - citar-bibliography を my:d:org-biblio（orgx-core.el が定義する my:d:org 配下の
  ;;   ディレクトリ）配下のパスとして解決する。ホームディレクトリを直書きした
  ;;   固定パスは持たない
  ;; - org-cite-*-processor をすべて citar に統一する
  ;; - citar-embark-mode を有効化する
  ;;
  ;; ロード経路: personal/user.el が `my:modules-extra' に追加する。
  ;; 本モジュールは正規の `my:modules' リストに現れてはならない。
  ;;
  ...
  (leaf citar
    ...
    :config
    ;; citar-bibliography と org-cite-global-bibliography は defcustom だが、
    ;; 値に expand-file-name の実行時評価式（前者）と citar-bibliography への参照
    ;; （後者）を含む。leaf の :custom はこの種の値を正しく扱えない —
    ;; leaf-normalize-list-in-list が「(a b c) 形の値は複数変数へ同じ値を配る
    ;; ための省略記法」とみなして分解してしまい、citar-bibliography・list・
    ;; expand-file-name・my:d:org-biblio を別々の変数として customize-set-variable
    ;; してしまう（Attempt modify list エラー付き）。org-todo-keywords 等と同じ
    ;; コーディング規則 §6 の例外に該当するため setopt を使う。
    ;; :custom の生成コードは :config より前に展開されるため、org-cite-global-bibliography
    ;; の代入も citar-bibliography 設定後のここに置く必要がある。
    (setopt citar-bibliography
            (list (expand-file-name "gov-standards.json" my:d:org-biblio)))
...
LSP バックエンド（eglot / lsp-mode）を切り替えても、キーバインドやコマンド呼び出し側が
影響を受けないようにするファサードです。詳細は以下の Commentary を参照してください。
...
API キー検索を =aidermacs-run= への advice に載せ替えるまでの経緯は、下記の :config
コメント（落とし穴その 1・その 2）に詳しい。処理の流れを図で示す。
...
#+begin_src emacs-lisp
  ...
  ;; dev-ai--api-key-configured を defun より前に置く。dev-ai--setup-api-key が
  ;; 実行時に参照するため、インタプリタ実行時の void-variable エラーを避ける
  ;; （aidermacs-extra-args と同じ理由。上のコメント参照）。
  (defvar dev-ai--api-key-configured nil
    "Non-nil once `dev-ai--setup-api-key' has run in this session.")
  ...
  (leaf aidermacs
    ...
    :config
    ;; API キーの参照は `aidermacs-run' への :before advice で行う。after-init-hook
    ;; では *ない*。
    ;;
    ;; 落とし穴その1（実機で踏んだ）: `add-hook' は APPEND 引数を渡さない限り新しい
    ;; 関数をフックリストの *先頭* に追加する（同じ DEPTH 内では後から登録した関数
    ;; ほど先に走る）。当初 after-init-hook DEPTH 10 で登録していたが、これは
    ;; auth-core（epa-file-enable）／auth-gpg（~/.authinfo.gpg を auth-sources へ
    ;; 追加）の既定 DEPTH（0）より後に走ることは保証できても、それ以外の問題までは
    ;; 解決しなかった（落とし穴その2参照）。
    ;;
    ;; 落とし穴その2（実機で踏んだ）: after-init-hook で毎起動時に無条件でキーを
    ;; 検索する設計そのものが問題だった。 =auth-source-backends= （auth-source.el
    ;; 組み込みマクロ）はキャッシュを持たず、呼ばれるたびに =auth-sources= の
    ;; 全エントリを再パースする——=~/.authinfo.gpg= も呼ばれるたびに epa-file 経由で
    ;; 再復号される。旧実装は openrouter.ai と api.openai.com を別々に2回
    ;; =auth-source-search= していたため、Emacs 起動直後に GPG 復号（macOS では
    ;; Touch ID 生体認証）が短時間に2回連続で走ってしまい、2回目が
    ;; "No secret key" で失敗する（LAContext の生体評価は極端に短い間隔で連続実行
    ;; できないため）という、毎起動ごとに再現する形で表面化した。
    ;;
    ;; 対処（この2つを両方やって初めて解決する）:
    ;;   1. 検索を1回の =auth-source-search= にまとめる（:host にリストを渡し、
    ;;      結果を host で振り分ける）。無条件の2回検索を1回に減らす。
    ;;   2. after-init-hook を完全にやめ、`aidermacs-run' への :before advice に
    ;;      変更する。これにより GPG/Touch ID 認証は「実際に aidermacs を使う
    ;;      瞬間」だけに発生し、通常の Emacs 起動では一切発生しない。
    ;;      `dev-ai--api-key-configured' で 1 セッション 1 回だけに絞り、
    ;;      `aidermacs-run' を呼ぶたびに再認証が走らないようにする。
    ;;
...
Called by `dev-ai--setup-api-key'."
    ...
    (defun dev-ai--setup-api-key (&rest _ignore)
      "Configure OPENAI_API_KEY and OPENAI_API_BASE from auth-source.
Reads ~/.authinfo.gpg via a *single* `auth-source-search' call for:
  machine openrouter.ai login ac1965  (preferred)
  machine api.openai.com login org-ai (fallback)
A single call is deliberate: `auth-source-backends' rebuilds every backend
(including re-decrypting ~/.authinfo.gpg via epa-file) on every call, with
no cache of its own.  Two back-to-back calls used to trigger two Touch ID
prompts in immediate succession, and the second reliably failed with
\"No secret key\" (see the :config comment above).
Calls `dev-ai--ensure-authinfo-in-sources' first so the lookup succeeds
even if `auth-gpg--configure-sources' has not added the file yet.
Added as `:before' advice on `aidermacs-run' (not `after-init-hook'), so
this — and the GPG/Touch ID prompt it triggers — only happens when
aidermacs is actually invoked, never on ordinary Emacs startup.
Runs at most once per session, guarded by `dev-ai--api-key-configured'."
      (unless dev-ai--api-key-configured
        (setq dev-ai--api-key-configured t)
        (dev-ai--ensure-authinfo-in-sources)
        (require 'auth-source)
        (let* ((entries (auth-source-search
                          :host '("openrouter.ai" "api.openai.com")
                          :max 2
                          :require '(:secret)))
               (router-entry (seq-find (lambda (e) (equal (plist-get e :host) "openrouter.ai"))
                                        entries))
               (openai-entry (seq-find (lambda (e) (equal (plist-get e :host) "api.openai.com"))
                                        entries))
...
#+begin_src emacs-lisp
  ...
  ;;   （起動時間短縮のため、project.el の customize と editorconfig-mode の
  ;;   有効化は、いずれも emacs-startup-hook 経由でアイドルタイマーに遅延させる）
  ;;; Code:
  ...
  ;;
  ;; project.el は Emacs 組み込みだが、leaf の :custom は暗黙に :require t を
  ;; 伴うため、この :custom を書くだけで起動時に project.el 全体が同期ロード
  ;; されていた（起動プロファイル計測で 12〜13ms、ログの
  ;; "Loading project...done" がこれに当たる）。project-find-file 等の
  ;; コマンド自体は Emacs 本体の autoload で既に利用可能なので、customize は
  ;; ロード後まで遅延して問題ない。`emacs-startup-hook' からアイドルタイマーで
  ;; 遅延させる。
  (leaf project :straight nil :require nil)

  (defun dev-web-core--activate-project ()
    "project.el を require し、カスタム変数を設定する。
`project-prune-zombie-projects' はビルドによって型が boolean と alist の
どちらにもなり得るため、setopt の前に `custom-type' を確認し、boolean の
ときだけ t を設定する。alist 型のビルドでは既定値に委ね、詳しい形は
`C-h v project-prune-zombie-projects' で個別に確認すること
（誤った型で setopt すると custom 型検証の警告が出る）。"
    (require 'project)
    (setopt project-list-exclude '("\\`/tmp/" "\\`/private/var/folders/")
            project-mode-line 'non-remote)
    (let ((ty (get 'project-prune-zombie-projects 'custom-type)))
      (when (or (eq ty 'boolean) (eq (car-safe ty) 'boolean))
        (setopt project-prune-zombie-projects t))))

  (defun dev-web-core--schedule-project ()
    "起動完了後、アイドルタイマーで `dev-web-core--activate-project' を呼ぶ。"
    (run-with-idle-timer 0 nil #'dev-web-core--activate-project))

  (add-hook 'emacs-startup-hook #'dev-web-core--schedule-project)
  ...
  ;;
  ;; editorconfig も project.el と同じ理由（:global-minor-mode は :require t を
  ;; 伴う）で起動時コストになっていたため、同様にアイドルタイマーへ遅延させる。
  (leaf editorconfig :straight t :require nil)

  (defun dev-web-core--activate-editorconfig ()
    "editorconfig を require し、`editorconfig-mode' をグローバルに有効化する。"
    (require 'editorconfig)
    (editorconfig-mode 1))

  (defun dev-web-core--schedule-editorconfig ()
    "起動完了後、アイドルタイマーで `dev-web-core--activate-editorconfig' を呼ぶ。"
    (run-with-idle-timer 0 nil #'dev-web-core--activate-editorconfig))

  (add-hook 'emacs-startup-hook #'dev-web-core--schedule-editorconfig)
...
#+begin_src emacs-lisp
  ...
  ;; apheleia は起動プロファイル計測で 14ms 前後を占めていた（:require t が
  ;; モジュールのロード時に即時 require させていた）。フォーマッタ登録と
  ;; `apheleia-global-mode' の有効化を、`emacs-startup-hook' からのアイドル
  ;; タイマーで遅延させる。保存時整形という性質上、起動完了直後に有効化されれば
  ;; 実用上の差は無い。
  (leaf apheleia
    :straight t
    :require nil)

  (defun dev-format--activate-apheleia ()
    "apheleia を require し、フォーマッタ登録と `apheleia-global-mode' を有効化する。"
    (require 'apheleia)
  ...
  (defun dev-format--schedule-apheleia ()
    "起動完了後、アイドルタイマーで `dev-format--activate-apheleia' を呼ぶ。"
    (run-with-idle-timer 0 nil #'dev-format--activate-apheleia))

  (add-hook 'emacs-startup-hook #'dev-format--schedule-apheleia)
...
#+begin_src emacs-lisp
  ...
  ;;
  ;; 以前との違い: :config は my:modules のロード時（＝毎回の起動時）に無条件で
  ;; 走っていたため、emms-all / emms-default-players / emms-mode-line-mode の初期化に
  ;; 実機で 6 秒以上かかることが判明した（原因は emms-all が引き込む多数のサブ
  ;; ライブラリの初期化コストと見られるが、正確な内訳までは特定していない）。
  ;; music を使わないセッションでも毎回この税金を払うのは割に合わないため、
  ;; docker / eglot / lsp / tempel と同じ :commands パターンに変更し、実際に
  ;; EMMS のコマンドを呼んだ瞬間まで :config の実行を遅延させる。dired 統合の
  ;; キーバインド登録自体は :commands が作る autoload だけで足りる（emms 本体は
  ;; まだロードされない）ため、:config の外に出して無条件で早期に行う。
  (leaf emms
    :straight t
    :if dev-music-enable-p
    :commands (emms emms-play-dired emms-play-directory emms-play-directory-tree
               emms-play-file emms-play-playlist emms-smart-browse)
    :pre-setq
  ...
  ;; emms-play-dired は :commands による autoload スタブなので、ここで束縛しても
  ;; emms 本体はまだロードされない（キーを押した瞬間に初めてロードされる）。
  (when dev-music-enable-p
...
:PROPERTIES:
:CUSTOM_ID: utils-ime
:header-args:emacs-lisp: :tangle lisp/utils/utils-ime.el
:END:

**** utils-ime.el — 入力ソース（IME）制御

日本語 IME をかな入力にしたままだと、リーダーキー =M-SPC= の後続打鍵
（ =b l= など）や =dired= / =magit-status= /ターミナル系バッファでのナビゲーション・
操作キー（ =RET= / =g= / =d= / =s= など）が IME に composed されて誤動作する。
本モジュールは =macism= を同期呼び出しして、次の契機で OS の入力ソースを
ASCII（英字）へ切り替える。

- *リーダーキー押下時* — =ui-leader-key= を =menu-item= の =:filter= で包み、
  キー押下の瞬間に ASCII 化してから =ui-leader-map= を返す。プレフィックスキーの
  性質は保たれるため、 which-key と =describe-key= は従来どおり機能する。単発の
  ディスパッチ用途のため、復帰処理は行わない。
- *対象メジャーモード進入／離脱時* — =utils-ime-target-modes= （既定:
  =dired-mode= =magit-status-mode= =vterm-mode= =eshell-mode= =term-mode=
  =comint-mode= ）に該当するバッファがカレントになった瞬間に ASCII へ切り替え、
  そのモードから離れて対象外のバッファへ戻った瞬間に、入る直前の入力ソースへ
  自動復帰する。 =buffer-list-update-hook= でカレントバッファの遷移を監視し、
  対象／非対象の境界をまたいだときだけ切替・復帰を行う。 =wdired-mode= は
  =dired-mode= 上のマイナーモードなので、 =dired-mode= を対象化するだけで
  wdired 編集中も自動的にカバーされる（個別フック不要）。dirvish も同様に
  =dired-mode= 土台のため自動的に対象になる。
- *プログラミング言語バッファ進入／離脱時*（追加分） — =utils-ime-target-modes=
  に =prog-mode= を加えることで、Python／Elisp／JS など、ほとんどの
  プログラミング言語メジャーモードが =derived-mode-p= 経由で自動的に対象になる。
  =org-edit-special=（ =C-c '= ）で開く Org Babel のソース編集バッファも、
  その言語自身のメジャーモード（ =python-mode= 等）を持つ独立バッファであるため、
  この一行追加だけで =buffer-list-update-hook= が既存の仕組みのまま検知し、
  新規フックなしに自動的にカバーされる。 =org-mode= 自体は =prog-mode= の
  派生ではないため、本文（日本語散文）側は影響を受けない。なおコード中に
  日本語コメントを書く場合は、その間だけ手動で IME に切り替える必要がある。
- *ミニバッファ使用時* — =M-x= や =find-file= などでミニバッファに入った瞬間に
  ASCII へ切り替え、抜けた瞬間に入る直前の入力ソースへ復帰する
  （ =minibuffer-setup-hook= / =minibuffer-exit-hook= ）。

=macism= が見つからない環境（非 macOS ／未インストール）では全て no-op になる。
ASCII のソース ID は =utils-ime-ascii-source= で変更でき、既定は
=com.apple.keylayout.ABC= （ =macism= を引数なしで実行すると現在値を確認できる）。
復帰先は固定値ではなく、対象コンテキストに入る直前に =macism= で取得した実際の
入力ソースを保存しておき、離脱時にそこへ戻す。

#+begin_src emacs-lisp
  ;;; utils/utils-ime.el --- IME（入力ソース）制御ユーティリティ -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; macism を用いて OS の入力ソースを ASCII（英字）へ切り替え、対象コンテキストを
  ;; 離れたら元の入力ソースへ復帰するユーティリティ。
  ;;
  ;; 目的:
  ;; - リーダーキー（`ui-leader-key'）を押した瞬間に ASCII へ切り替え、後続の
  ;;   1 打鍵（`b l' など）が日本語 IME に composed されず、確実に届くようにする。
  ;;   （単発ディスパッチのため復帰は行わない）
  ;; - `utils-ime-target-modes' に列挙したメジャーモード（dired/dirvish、
  ;;   magit-status、vterm/eshell/term/comint、`prog-mode' 派生の全言語モード）
  ;;   に入ったら ASCII へ切り替え、`RET'/`g'/`d'/`s' などの操作キーの誤爆や、
  ;;   コード入力時の意図しない日本語 composition を防ぐ。そのバッファから
  ;;   離れたら、入る直前の入力ソースへ自動復帰する。`prog-mode' を含めたことで、
  ;;   Org Babel のソース編集バッファ（`org-edit-special' が開く独立バッファ、
  ;;   実体は言語自身のメジャーモード）も同じ仕組みで自動的にカバーされる。
  ;; - ミニバッファ（`M-x'、`find-file' など）に入ったら ASCII へ切り替え、抜けたら
  ;;   入る直前の入力ソースへ自動復帰する。
  ;;
  ;; 本ファイルの責務:
  ;; - `utils-ime-to-ascii': macism を同期呼び出しして ASCII 入力ソースへ切り替える。
  ;; - `utils-ime--install-leader': リーダーキーを `menu-item' の `:filter' で包み、
  ;;   キー押下時に ASCII 化してから `ui-leader-map' を返す。これにより which-key と
  ;;   `describe-key' の挙動はプレフィックスキーとして従来どおり維持される。
  ;; - `utils-ime--track-buffer-change': `buffer-list-update-hook' 上で、カレント
  ;;   バッファが `utils-ime-target-modes' の境界をまたいだときに ASCII 化／復帰する。
  ;; - `utils-ime--minibuffer-setup' / `utils-ime--minibuffer-exit':
  ;;   `minibuffer-setup-hook' / `minibuffer-exit-hook' でミニバッファの
  ;;   ASCII 化／復帰を行う。
  ;;
  ;; 設計上の注意:
  ;; - macism は同期呼び出し（`call-process' の DESTINATION を nil とし待機）とする。
  ;;   非同期（DESTINATION 0）だと、切替完了前にユーザーが次を打鍵し IME に取られる
  ;;   競合が起きうるため。macism は数十 ms で完了するため実用上の遅延は無視できる。
  ;; - macism が見つからない環境（非 macOS／未インストール）では全て no-op。
  ;;   `utils-ime-available-p' で存在を確認し、なければ何もしない。
  ;; - `ui-leader' 未ロードでも安全なよう `with-eval-after-load' と `boundp' で防御する。
  ;; - `buffer-list-update-hook' は高頻度で発火するが、`utils-ime--in-target-p' で
  ;;   ガードし、対象／非対象の境界をまたいだ瞬間だけ macism を呼ぶため実用上のコスト
  ;;   は無視できる。
  ;; - `wdired-mode' はマイナーモードで、有効化してもバッファのメジャーモードは
  ;;   `dired-mode' のまま変わらない。よって `dired-mode' を対象に含めておくだけで
  ;;   wdired 編集中も自動的に ASCII のままになり、個別のフックは不要である。
  ;;
  ;;; Code:

  (defgroup utils-ime nil
    "Input-source (IME) control via the macism CLI."
    :group 'utils)

  (defcustom utils-ime-macism-executable (executable-find "macism")
    "Absolute path to the `macism' executable, or nil if unavailable.
  When nil, every command in this module is a no-op."
    :type '(choice (const :tag "Disabled" nil) file)
    :group 'utils-ime)

  (defcustom utils-ime-ascii-source "com.apple.keylayout.ABC"
    "Input-source ID used for ASCII (English) input.
  Common values: \"com.apple.keylayout.ABC\" or \"com.apple.keylayout.US\".
  Run `macism' with no arguments in a shell to print the current source ID."
    :type 'string
    :group 'utils-ime)

  (defcustom utils-ime-enable-leader-p t
    "If non-nil, switch to ASCII when the leader key is pressed."
    :type 'boolean
    :group 'utils-ime)

  (defcustom utils-ime-target-modes
    '(dired-mode magit-status-mode vterm-mode eshell-mode term-mode comint-mode
      prog-mode)
    "Major modes that force ASCII input on entry and restore the previous
  input source on exit.  Matched via `derived-mode-p', so modes derived from
  any of these (e.g. `shell-mode' or `sql-interactive-mode' via `comint-mode',
  or `python-mode'/`emacs-lisp-mode'/etc. via `prog-mode') are covered too.
  Dirvish and `wdired-mode' need no separate entry: both are built on top of
  `dired-mode' and are therefore already covered.  Including `prog-mode' also
  covers Org Babel source-edit buffers opened via `org-edit-special' (C-c '),
  since each such buffer's major mode is the source language's own mode;
  `org-mode' itself does not derive from `prog-mode', so ordinary prose
  buffers are unaffected.  Typing Japanese comments inside a covered buffer
  still requires switching the OS input source back manually for that span."
    :type '(repeat symbol)
    :group 'utils-ime)

  (defcustom utils-ime-enable-mode-tracking-p t
    "If non-nil, switch to ASCII on entering a buffer whose major mode is in
  `utils-ime-target-modes', and restore the previous input source when the
  selected buffer leaves that set."
    :type 'boolean
    :group 'utils-ime)

  (defcustom utils-ime-enable-minibuffer-p t
    "If non-nil, switch to ASCII while the minibuffer is active (`M-x',
  `find-file', etc.) and restore the previous input source on exit."
    :type 'boolean
    :group 'utils-ime)

  (defvar utils-ime--in-target-p nil
    "Non-nil while the selected buffer is considered inside a target context.")

  (defvar utils-ime--saved-source nil
    "Input source captured immediately before entering a target-mode context.
  Nil when not currently inside one.")

  (defvar utils-ime--minibuffer-saved-source nil
    "Input source captured immediately before the minibuffer was activated.")

  (defun utils-ime-available-p ()
    "Return non-nil when macism can be used on this system."
    (and (eq system-type 'darwin)
         (stringp utils-ime-macism-executable)
         (file-executable-p utils-ime-macism-executable)))

  (defun utils-ime--current-source ()
    "Return the current OS input-source ID via macism, or nil on failure.
  Also nil when macism is unavailable."
    (when (utils-ime-available-p)
      (ignore-errors
        (with-temp-buffer
          (when (zerop (call-process utils-ime-macism-executable nil t nil))
            (string-trim (buffer-string)))))))

  (defun utils-ime--switch-to (source)
    "Switch the OS input source to SOURCE via macism.
  No-op when SOURCE is nil or macism is unavailable.  Synchronous by design
  (see Commentary)."
    (when (and source (utils-ime-available-p))
      (ignore-errors
        (call-process utils-ime-macism-executable nil nil nil source))))

  (defun utils-ime-to-ascii ()
    "Switch the OS input source to `utils-ime-ascii-source' via macism.
  No-op when macism is unavailable.  Errors are trapped so a failure never
  blocks command dispatch.  Synchronous by design (see Commentary)."
    (interactive)
    (utils-ime--switch-to utils-ime-ascii-source))

  (defun utils-ime--install-leader ()
    "Wrap the leader key so a keypress first switches to ASCII input.
  The leader map stays a real prefix (which-key/`describe-key' keep working)
  because the `menu-item' `:filter' returns `ui-leader-map' after the switch.
  This is a one-shot dispatch aid, so the previous input source is not saved
  or restored afterwards."
    (when (and utils-ime-enable-leader-p
               (boundp 'ui-leader-key) (stringp ui-leader-key)
               (boundp 'ui-leader-map) (keymapp ui-leader-map))
      (define-key global-map (kbd ui-leader-key)
                  `(menu-item "leader+ascii" ,ui-leader-map
                              :filter ,(lambda (_)
                                         (utils-ime-to-ascii)
                                         ui-leader-map)))))

  (defun utils-ime--target-buffer-p ()
    "Return non-nil if the current buffer's major mode is in
  `utils-ime-target-modes'."
    (apply #'derived-mode-p utils-ime-target-modes))

  (defun utils-ime--track-buffer-change ()
    "Switch to/from ASCII when the selected buffer crosses a target-mode
  boundary.  Installed on `buffer-list-update-hook', which fires on every
  buffer-selection change regardless of how it happened (`find-file',
  `quit-window', `other-window', `dirvish-quit', jumping from dired into
  `magit-status', ...).  Guarded by `utils-ime--in-target-p' so macism is only
  invoked exactly on entry/exit transitions, not on every command."
    (when (and utils-ime-enable-mode-tracking-p
               (not (minibufferp)))
      (let ((target (utils-ime--target-buffer-p)))
        (cond
         ((and target (not utils-ime--in-target-p))
          (setq utils-ime--saved-source (utils-ime--current-source))
          (setq utils-ime--in-target-p t)
          (utils-ime-to-ascii))
         ((and (not target) utils-ime--in-target-p)
          (setq utils-ime--in-target-p nil)
          (utils-ime--switch-to utils-ime--saved-source)
          (setq utils-ime--saved-source nil))))))

  (defun utils-ime--minibuffer-setup ()
    "Switch to ASCII when the minibuffer is activated, saving the prior source."
    (when utils-ime-enable-minibuffer-p
      (setq utils-ime--minibuffer-saved-source (utils-ime--current-source))
      (utils-ime-to-ascii)))

  (defun utils-ime--minibuffer-exit ()
    "Restore the input source saved by `utils-ime--minibuffer-setup'."
    (when (and utils-ime-enable-minibuffer-p utils-ime--minibuffer-saved-source)
      (utils-ime--switch-to utils-ime--minibuffer-saved-source)
      (setq utils-ime--minibuffer-saved-source nil)))

  ;; リーダー基盤のロード後にキーを包む。ui-leader が既に provide 済みなら即時実行。
  (with-eval-after-load 'ui-leader
    (utils-ime--install-leader))

  ;; メジャーモード境界の追跡。dirvish・wdired は dired-mode 上に成り立つため、
  ;; `utils-ime-target-modes' に `dired-mode' を含めるだけで自動的にカバーされる。
  (add-hook 'buffer-list-update-hook #'utils-ime--track-buffer-change)

  ;; ミニバッファの ASCII 化／復帰。
  (add-hook 'minibuffer-setup-hook #'utils-ime--minibuffer-setup)
  (add-hook 'minibuffer-exit-hook #'utils-ime--minibuffer-exit)

  (provide 'utils-ime)
  ;;; utils/utils-ime.el ends here
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; - ファイルを訪問している `prog-mode' バッファで flymake を有効化する
  ;;   （*scratch* 等の非ファイルバッファは除外する。理由は下記）
  ...
  ;; flymake はファイルを訪問しているバッファでのみ有効化する。
  ;;
  ;; 理由: `lisp-interaction-mode'（*scratch*）は `prog-mode' 派生なので、
  ;; prog-mode-hook に flymake-mode を直接繋ぐと *scratch* でも有効になる。しかし
  ;; `elisp-flymake-byte-compile' は Emacs 30.1 以降 `trusted-content-p' を確認し、
  ;; buffer-file-truename が nil のバッファ（= ファイルを訪問していないバッファ）を
  ;; 決して信頼しない。結果として起動のたびに次の警告が出る:
  ;;
  ;;   Disabling backend elisp-flymake-byte-compile because
  ;;   (user-error "Disabling elisp-flymake-byte-compile in *scratch* (untrusted content)")
  ;;
  ;; これは Emacs の設計どおりの動作である。バイトコンパイルはマクロを展開し
  ;; `eval-when-compile' を評価する（= コードを実行する）ため、貼り付けたばかりの
  ;; スニペットに対して自動実行するのは危険だからである。
  ;;
  ;; 対処として `trusted-content' に `:all' を設定する方法があるが、docstring が
  ;; 「gaping security hole」と明記しており採用しない。*scratch* で flymake を
  ;; 有効化しなければ済む話である。診断は C-x C-e / C-j での評価時に得られる。
  ;;
  ;; 信頼するディレクトリ（lisp/ と personal/）の宣言は core-policy.el にある。
  (defun utils-lint--enable-flymake-maybe ()
    "Enable `flymake-mode' in `prog-mode' buffers that visit a file.
  Buffers without a file (notably *scratch*) can never satisfy `trusted-content-p',
  so `elisp-flymake-byte-compile' would only disable itself with a warning."
    (when buffer-file-name
      (flymake-mode 1)))
  ...
  (leaf flymake
    ...
    :hook ((prog-mode-hook . utils-lint--enable-flymake-maybe))
    :config
  ...
  ;; python-flymake は pyflakes / flake8 / ruff / pylint のいずれかが PATH に
  ;; 存在することを要求する。見つからない場合は Python バッファを開くたびに
  ;; "Cannot find a suitable checker" 警告を出して無効化される。
  ;; eglot が有効なバッファでは LSP が診断を担当するため python-flymake は
  ;; 不要かつ重複になる。以下のフックで両方のケースを処理する。
  (defun utils-lint--python-flymake-setup ()
    "Configure python-flymake backend for the current Python buffer.
When eglot is managing the buffer, removes python-flymake from
`flymake-diagnostic-functions' (LSP already provides diagnostics).
When eglot is absent and no external checker is on PATH, sets
`python-flymake-command' to nil to suppress the warning silently."
    (when (and (derived-mode-p 'python-mode 'python-ts-mode)
      ...
      (if (and (fboundp 'eglot-managed-p) (eglot-managed-p))
          ;; eglot がバックエンドを提供するので python-flymake を除去
          (remove-hook 'flymake-diagnostic-functions #'python-flymake t)
        ;; LSP なし: 外部チェッカーが PATH になければ警告なしで無効化
        (unless (or (executable-find "pyflakes")
                    (executable-find "flake8")
                    (executable-find "ruff")
                    (executable-find "pylint"))
          (setq-local python-flymake-command nil)))))

  (add-hook 'python-mode-hook    #'utils-lint--python-flymake-setup)
  (add-hook 'python-ts-mode-hook #'utils-lint--python-flymake-setup)
...
Claude API（=api.anthropic.com/v1/messages=）への同期・非同期 HTTP クライアントです。
...
#+begin_src emacs-lisp
  ...
  ;; 表示上の選択であり、personal 層に属する。本ファイルは init.el のステップ 9 で、
  ...
  ;;
  ;;
  ;; これは `modules.el' が require される前に走る: init.el はステップ 9 で
  ;; personal/user.el をロードし、ステップ 7 で modules を require するため、
  ;; my/modules-load が最終リストを走査する時点で my:modules-extra は構築済みである。
  ;; orgx-roam-ui は意図的に含めない。org-roam-ui はモジュールのロード時に
  ;; (straight-use-package 'org-roam-ui) を行い、with-eval-after-load 'org-roam 経由で
  ;; アイドルタイマーを仕込む。これが org-roam -> org を起動直後に引き込む経路になるため、
  ;; Org の遅延ロードを優先してここから除外した。UI が必要になったら M-x org-roam-ui-open
  ;; を実行する（autoload されているのでその場でロードされる）。
  ;; グラフを常用する運用に戻す場合は orgx-roam-ui をこのリストへ戻す。
  ...
  ;; WSL 判定ユーティリティ
  ;;
  ;; WSL (Windows Subsystem for Linux) 上の `system-type' は素の Linux と同じ
  ;; `gnu/linux' であり、`system-type' だけでは区別できない。WSL は環境変数
  ;; `WSL_DISTRO_NAME'（wsl.exe が各ディストロ起動時に設定する）で確実に判定できる。
  ;;
  ;; この関数はデバイスファイルのロード（直後）と、後段の `personal/ac1965.el'
  ;; （`my:d:org*' 等のパス分岐）の両方から参照されるため、両者より前に位置する
  ;; 本ファイル（`personal/user.el'）内で定義する。
  (defun my:wsl-p ()
    "現在 WSL 上で動作していれば non-nil を返す."
    (and (eq system-type 'gnu/linux)
         (getenv "WSL_DISTRO_NAME")))

  ;; デバイス固有の設定をロードする
  ;;
  ;; WSL は `system-type' が `gnu/linux' と衝突するため、`my:wsl-p' を優先判定し、
  ;; 該当すれば `device-wsl.el' を、そうでなければ従来どおり
  ;; `device-<system-type>.el' をロードする。
  (let* ((personal-dir
  ...
  ;; personal-llm: ローカル LLM ルーター（~/.local/bin/localllm）統合
  ;;
  ;; personal-messages と異なり org 本体には依存しない（Org-roam バッファからの
  ;; 呼び出しでもリージョン／バッファ文字列を渡すだけで org 固有関数を使わない）
  ;; ため、`with-eval-after-load 'org' で遅延させる必要はない。`locate-library'
  ;; でガードし、ファイルが無ければ静かにスキップする。
  (when (locate-library "personal-llm")
    (require 'personal-llm nil t))
...
#+begin_src emacs-lisp
  ...
  ;; - `my:d:org*' のパスを、macOS では /Users/ac1965/Documents/org/ に、
  ;;   WSL では /home/ac1965/org/ に上書きする（`my:wsl-p' で分岐）
  ...
  ;; 本ファイルは init.el のステップ 9 で、modules.el が走るより前にロードされるため、
  ...
  ;;
  ;; マシン（ログイン）単位ではなく OS 単位で値を変える必要があるため、`my:wsl-p'
  ;; （personal/user.el で定義済み。本ファイルは user.el の後にロードされるため
  ;; 参照可能）で分岐する。WSL 側は macOS の /Users/ac1965/ に対応するホームである
  ;; /home/ac1965/ を使い、org ルート配下のサブディレクトリ構成（journal/、
  ;; org-roam/）は macOS 側とそろえてミラーする。my:f:capture-blog-file も同じ
  ;; 規則でホームパスだけを置き換えているが、WSL 側に blog プロジェクト
  ;; （~/Projects/blog/mysite/）を未 clone の場合は個別に上書きするか、
  ;; orgx-core.el 側の (bound-and-true-p my:f:capture-blog-file) ガードに委ねて
  ;; 未設定のままにしてよい。
  (if (my:wsl-p)
      (setq my:d:org              "/home/ac1965/org/"
            my:d:org-journal      "/home/ac1965/org/journal/"
            my:d:org-roam         "/home/ac1965/org/org-roam/"
            my:f:capture-blog-file "/home/ac1965/Projects/blog/mysite/all-posts.org")
    (setq my:d:org              "/Users/ac1965/Documents/org/"
          ...
          my:f:capture-blog-file "/Users/ac1965/Projects/blog/mysite/all-posts.org"))
  ...
  ;;
  ;; org-roam-ui はここでは require せず :straight のみで導入されるため、
  ;; バイトコンパイル時にこれらの defcustom を未知の自由変数として警告される。
  ;; bare defvar でコンパイラに「特殊変数である」とだけ伝え、警告を抑止する。
  (defvar org-roam-ui-port)
  (defvar org-roam-ui-browser-function)
  (defvar org-roam-ui-open-on-start)
  (defvar org-roam-ui-sync-theme)
  (defvar org-roam-ui-follow)
  (defvar org-roam-ui-update-on-save)
  ...
  (with-eval-after-load 'org-roam-ui
    ...
    (setq org-roam-ui-browser-function
        (lambda (_url)
         (browse-url (format "http://127.0.0.1:%d/"
                       org-roam-ui-port))))
    ;; 自動起動
  ...
  ;; python-mode 初期化時 (define-derived-mode python-mode 本体) で
  ;; python-indent-guess-indent-offset-verbose を呼ぶタイミングが
  ;; hack-local-variables より早いため、ファイルローカル変数
  ;; (# -*- python-indent-offset: ... -*-) では抑制できない。
  ;; 実測確認済み（Emacs 29.3, ob-tangle :comments link 環境）。
  ;; 値自体は変えず、"Can't guess python-indent-offset" の
  ;; メッセージだけを黙らせる。
  ...
  ;; dirvish 用 IME mode-line セグメント — nano-modeline とキャッシュを共有する
  ;;
  ;; dirvish は編集バッファの nano-modeline パイプラインを通らない独自の
  ;; mode-line（`dirvish-mode-line-format'、core/core-editing.el の
  ;; `core-editing--activate-dirvish' で :right に `ime' を追加済み）を持つため、
  ;; ウィンドウ最大化時など見た目上の手がかりが少ない状況では dirvish バッファに
  ;; 入った瞬間 IME 状態が分からなくなっていた。
  ;;
  ;; ここでは新たに osascript/macism を呼ぶのではなく、上で定義済みの
  ;; `ac1965--sis-indicator'（sis-change-hook が更新するグローバルキャッシュ）を
  ;; そのまま読むだけのセグメントを `dirvish-define-mode-line' で登録する。
  ;; プロセス起動コストはゼロで、編集バッファ側と表示内容も常に一致する。
  ;;
  ;; `dirvish-define-mode-line' はマクロであり dirvish がロード済みである必要が
  ;; あるが、my:modules の起動順は core-editing（dirvish 本体）→ ... →
  ;; personal（このファイル）なので通常は既にロード済みである。順序変更に対して
  ;; 堅牢にするため、念のため `with-eval-after-load' で包む。
  ;;
  ;; マクロ定義そのものはコンパイル時に展開されるため、eval-when-compile で
  ;; dirvish を require しておかないと、バイトコンパイラが
  ;; `dirvish-define-mode-line' を普通の関数呼び出しと誤解し、第1引数 `ime' を
  ;; （クォートされていない）自由変数として評価しようとして警告を出す。
  ;;
  ;; NOERROR (第2引数 nil、第3引数 t) が必須の理由:
  ;; `eval-when-compile' はバイトコンパイル時にしか特別扱いされない —
  ;; 本ファイルのようにインタプリタ実行（非コンパイル）される .el を
  ;; `load' すると、Emacs のロード時 eager macro-expansion がこの
  ;; トップレベルフォームを即座に展開しようとし、その際 `eval-when-compile'
  ;; は単なる `progn' として本文を「今すぐ」評価する。personal/ac1965.el は
  ;; init.el ステップ9で、modules.el（ステップ10、core-editing の
  ;; `(leaf dirvish :straight t :require nil)' が dirvish を実際にインストール
  ;; する箇所）より前にロードされるため、この時点では dirvish はまだ
  ;; load-path 上に存在しない。NOERROR なしだと `(require 'dirvish)' が
  ;; file-missing で失敗し、本ファイルの残り（下の `dirvish-define-mode-line'
  ;; 登録を含む）が一切ロードされなくなる。結果として `dirvish-ime-ml' が
  ;; 定義されないまま dirvish 側の `dirvish-mode-line-format' には `ime' が
  ;; 残るため、dirvish バッファに入るたびに
  ;; `(void-function dirvish-ime-ml)' が再描画のたびに発生する。
  ;; NOERROR で「失敗したら nil を返すだけ」にしておけば、この時点では
  ;; 単に require をあきらめて後続の `with-eval-after-load' 登録に進み、
  ;; 実際に dirvish がロードされたとき（初回 dired 起動時）に遅延展開
  ;; されて正しく `dirvish-ime-ml' が定義される。
  (eval-when-compile (require 'dirvish nil t))

  (with-eval-after-load 'dirvish
    (dirvish-define-mode-line ime
      "Show the cached sis IME indicator (`ac1965--sis-indicator') in dirvish's
  own mode-line. Mirrors the editing-buffer nano-modeline segment so IME
  state (English / Japanese) stays legible while browsing in dirvish,
  including when the frame is maximized. Reads the shared cache only —
  no subprocess is spawned here; `ac1965--sis-update-indicator' (attached to
  `sis-change-hook') is what keeps the cache current."
      (when (boundp 'ac1965--sis-indicator)
        (string-trim ac1965--sis-indicator))))
  ;;
  ...
  ;; nano-modeline-position はトップレベルで require しない nano-modeline
  ;; パッケージの内部変数（関数を保持する）であり、bare defvar でコンパイラに
  ;; 存在を伝えておかないと自由変数警告が出る。
  (defvar nano-modeline-position)

  (defun ac1965--nano-time-encoding ()
    "Return a propertized \"HH:MM  CODING  IME\" string for the
  nano-modeline right segment.
  ...
  Coding system suffix modifiers (-unix, -dos, -mac, -with-signature) are
  stripped."
  ...
  and `ui-leader-o-map' are guaranteed to be live keymaps."
    (ui-keymap--bind 'ui-leader-t-map "m" #'ac1965/nano-modeline-toggle)
    (ui-keymap--bind 'ui-leader-o-map "g" #'ac1965/org-hugo-set-tags)          ; NEW
    (ui-keymap--bind 'ui-leader-o-map "f" #'ac1965/open-capture-blog-file))   ; NEW
  ...
  ;; ---------------------------------------------------------------------------
  ;; all-posts.org（Hugo ブログ）の既存タグ補完付きタグ設定
  ;; ---------------------------------------------------------------------------
  ;; my:f:capture-blog-file（本ファイル上部で all-posts.org に上書き済み）を走査し、
  ;; 過去に使ったインラインタグを補完候補にした状態で org-set-tags-command を呼ぶ。
  ;;
  ;; キャプチャ用の *Capture* バッファ自体には他の見出しが無いため、補完対象は
  ;; カレントバッファではなく my:f:capture-blog-file を直接開いて集める。

  (defun ac1965--org-hugo-tag-alist-from-buffer ()
    "`my:f:capture-blog-file' 内の既存インラインタグを org-tag-alist 形式で返す.
  ファイルが未設定・未存在の場合は nil を返す。"
    (let (tags)
      (when (and (bound-and-true-p my:f:capture-blog-file)
                 (file-exists-p my:f:capture-blog-file))
        (with-current-buffer (find-file-noselect my:f:capture-blog-file)
          (org-map-entries
           (lambda ()
             (dolist (tag (org-get-tags nil t))
               (unless (member tag tags) (push tag tags)))))))
      (mapcar #'list tags)))

  (defun ac1965/org-hugo-set-tags ()
    "all-posts.org の既存タグ補完付きで、現在の見出しにタグを設定する."
    (interactive)
    (let ((org-tag-alist (ac1965--org-hugo-tag-alist-from-buffer)))
      (call-interactively #'org-set-tags-command)))

  ;; ---------------------------------------------------------------------------
  ;; all-posts.org（Hugo ブログ）を直接開くショートカット
  ;; ---------------------------------------------------------------------------
  ;; ブログ記事の一覧・既存タグの確認・手動編集のために、キャプチャを経由せず
  ;; my:f:capture-blog-file を直接開きたい場面向け。個人設定の上書きが未反映
  ;; （orgx-core.el の既定値 blog.org のまま）だったり、パス自体が存在しない
  ;; 環境で呼んだ場合は find-file を黙って失敗させず、user-error で理由を示す。

  (defun ac1965/open-capture-blog-file ()
    "`my:f:capture-blog-file'（ox-hugo 単一ファイルのブログソース）を開く."
    (interactive)
    (if (and (bound-and-true-p my:f:capture-blog-file)
             (file-exists-p my:f:capture-blog-file))
        (find-file my:f:capture-blog-file)
      (user-error "my:f:capture-blog-file が未設定か、ファイルが存在しません: %s"
                  (bound-and-true-p my:f:capture-blog-file))))

  ;; ---------------------------------------------------------------------------
  ;; ブログ投稿キャプチャの確定前にタグ設定を挟む
  ;; ---------------------------------------------------------------------------
  ;; org-capture-before-finalize-hook はキャプチャバッファがまだ生きている
  ;; （= このタイミングならタグを付けても org-capture-after-finalize-hook 経由の
  ;; orgx-auto-hugo による ox-hugo 自動エクスポートに、付けたタグが反映される）
  ;; 段階で走る。テンプレートキーが "b"（orgx-core.el の "Blog post"）のときだけ動作する。

  (declare-function org-capture-get "org-capture" (&optional prop-name local))

  (defun ac1965--org-capture-set-blog-tags ()
    "ブログ投稿キャプチャ（テンプレートキー \"b\"）の確定前にタグを設定する.
タグ選択（fast tag selection / ミニバッファ補完）中に C-g で抜けると
`quit' シグナルが送出されるが、それをここで捕捉し、タグ無しのまま
キャプチャの確定処理へ進む。捕捉しないと `quit' が
`org-capture-before-finalize-hook' の実行そのものを中断し、
キャプチャ自体が失敗する（テンプレート `b' 以外の副作用や後続フックにも
影響しうる）。C-g がそのまま「タグを選ばない」の選択肢になる。"
    (when (equal (org-capture-get :key) "b")
      (condition-case nil
          (ac1965/org-hugo-set-tags)
        (quit (message "タグ設定をスキップしました")))))

  (add-hook 'org-capture-before-finalize-hook #'ac1965--org-capture-set-blog-tags)

  ;; ---------------------------------------------------------------------------
  ;; （任意）ブログ投稿テンプレートの :EXPORT_HUGO_TAGS: %^{Tags} プロンプトを外す
  ;; ---------------------------------------------------------------------------
  ;; orgx-core.el の "b" テンプレートは :EXPORT_HUGO_TAGS: %^{Tags} で別途タグ入力を
  ;; 求める。上のフックでインラインタグを付けるなら二重にタグ入力を求めることになる
  ;; ので、"b" エントリだけをここで上書きし、EXPORT_HUGO_TAGS の入力欄を外す。
  ;;
  ;; 実行タイミングに関する注記:
  ;;   orgx-core.el の org-capture-templates 設定は (leaf org :config ...) の中にあり、
  ;;   with-eval-after-load 'org として登録される。この登録は modules.el 経由で
  ;;   本ファイルより「後」（init.el ステップ10）に行われる。
  ;;   一方 with-eval-after-load の同一 feature に対するコールバックは登録順
  ;;   （FIFO）で実行されるため、本ファイル（ステップ8でロード）が単純に
  ;;   with-eval-after-load 'org を使うと、orgx-core より「先」に走ってしまい、
  ;;   後から orgx-core 側の setq に上書きされて負ける。
  ;;   これを避けるため、登録そのものを after-init-hook（全モジュールのロード完了後）
  ;;   まで遅らせる。こうすると with-eval-after-load 'org への登録が orgx-core より
  ;;   確実に後になり、実際に org がロードされたときの実行順も orgx-core → 本関数、
  ;;   の順になって上書きが効く。

  (defun ac1965--org-capture-templates-override ()
    "org-capture-templates の \"b\"（Blog post）エントリを、EXPORT_HUGO_TAGS の
  プロンプト無しの版に上書きする。ac1965--org-capture-set-blog-tags によるインライン
  タグ設定と重複しないようにするため。EXPORT_HUGO_COVER という独自プロパティ名は
  ox-hugo が認識しないため使わない。ox-hugo が唯一受け付ける任意キー用の経路である
  EXPORT_HUGO_CUSTOM_FRONT_MATTER（plist形式）に :cover \"\" を空文字のプレースホルダー
  として挿入する（deploy.sh の Tier1 明示指定用。空文字のままなら従来通りTier2以降へ
  フォールスルーする）。"
    (with-eval-after-load 'org
  ...
  (add-hook 'after-init-hook #'ac1965--org-capture-templates-override 20)
...
#+begin_src emacs-lisp
  ...
  ;; leaf の :when (display-graphic-p) は「モジュールのロード時」に一度だけ
  ;; 評価される。--daemon 起動時はその時点で実フレームが無いため常に nil と
  ;; なり、これらのスクロール設定が永久に適用されない（ligature 等と同じ
  ;; daemon 対応バグ）。パッケージの require は不要な単なる setopt なので、
  ;; 初回のグラフィカルフレームができた時点で一度だけ適用する関数に変更した。
  (defvar ac1965--mouse-configured-p nil)

  (defun ac1965--maybe-configure-mouse (&optional frame)
    "FRAME がグラフィカルなら、初回に限りマウス／スクロール設定を適用する。"
    (unless ac1965--mouse-configured-p
      (when (display-graphic-p (or frame (selected-frame)))
        (setq ac1965--mouse-configured-p t)
  ...
  (if (daemonp)
      (add-hook 'after-make-frame-functions #'ac1965--maybe-configure-mouse)
    (add-hook 'after-init-hook #'ac1965--maybe-configure-mouse))
...
Music.app is not available or the playlist list is empty."
  ...
  ;; hydra-apple-music/body は defhydra が実行時に定義する関数であり、
  ;; コンパイル時には存在しないため、コンパイラに存在を伝えておく。
  (declare-function hydra-apple-music/body "hydra")
...
*** personal/device-wsl.el
:PROPERTIES:
:CUSTOM_ID: device-wsl
:header-args:emacs-lisp: :tangle personal/device-wsl.el
:END:

#+begin_src emacs-lisp
  ;;; personal/device-wsl.el --- WSL デバイス固有の設定 -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; WSL (Windows Subsystem for Linux) デバイス固有の設定:
  ;; Windows クリップボードとの連携、既定ブラウザの起動、Explorer 連携。
  ;;
  ;; `personal/user.el' の `my:wsl-p' によって WSL 判定済みの状態でのみ
  ;; ロードされる（`device-darwin.el' の macOS 判定と対になる位置づけ）ため、
  ;; 本ファイル内では改めて `my:wsl-p' や `system-type' のガードを行わない。
  ;;
  ;; 本ファイルの責務:
  ;; - リーダーキーを `M-SPC' から `C-c SPC' へ切り替える（Alt+Space は Windows
  ;;   側のシステムショートカットと衝突しやすく、人間工学的にも押しにくいため）
  ;; - `win32yank.exe' が存在すれば、kill-ring と Windows クリップボードを
  ;;   `interprogram-cut-function' / `interprogram-paste-function' 経由で橋渡しする
  ;; - `wslview'（wslu パッケージ）が存在すれば、`browse-url' の既定を
  ;;   Windows 側の既定ブラウザ起動に切り替える
  ;; - 現在のディレクトリを Explorer で開くコマンドを提供する
  ;; - 現在のファイルパスを Windows 形式（`wslpath -w'）に変換してコピーするコマンドを提供する
  ;; - hydra-wsl を定義し、init 後にリーダー a w（本ファイルの変更により
  ;;   WSL では C-c SPC a w）へバインドする
  ;;
  ;; 前提（Emacs の外側で用意するもの、本ファイルの範囲外）:
  ;; - win32yank.exe: `winget install --id=equalsraf.win32yank' 等で Windows 側に導入し、
  ;;   WSL 側の $PATH からも解決できるようにしておく
  ;;   （WSLg 由来の /mnt/c 経由 PATH で通常は解決される。`which win32yank.exe' で確認）
  ;; - wslu: `sudo apt install wslu'（`wslview' を含む）
  ;; - GPG pinentry: `~/.gnupg/gpg-agent.conf' で `pinentry-program' を
  ;;   `pinentry-curses'（または WSLg 利用時は `pinentry-gtk'）に設定する
  ;;   （macOS の pinentry-touchid に相当する処理だが、gpg-agent 側の設定であり
  ;;   Emacs Lisp では制御しないため、ここにはコードを置かない）
  ;;; Code:

  ;; 本ファイルは `leaf' マクロを使わない（`defvar'/`defun'/`setq' のみ）ため、
  ;; `device-darwin.el' と異なり `(eval-when-compile (require 'leaf))' は不要。
  ;; `-Q --batch' 単体ロードでも straight.el のブートストラップなしに読み込める。

  ;; ---------------------------------------------------------------------------
  ;; リーダーキーの代替 — M-SPC → C-c SPC
  ;; ---------------------------------------------------------------------------
  ;; Alt+Space はWindowsのシステムメニュー・ショートカットと衝突しやすく、
  ;; また人間工学的にも押しにくいため、WSLでは3打鍵に戻る形にはなるが
  ;; 衝突が少なく確実な `C-c SPC' を使う。
  ;;
  ;; 注意（読み込み順序）: 本ファイル（personal/device-wsl.el）は
  ;; `personal/user.el' の内部から、`(require 'modules)'（ui 層・
  ;; `ui-leader' を含む）より *前* に同期的にロードされる。つまり本ファイルの
  ;; トップレベルが評価される時点では `ui-leader-key' も `ui-leader-map' も
  ;; まだ `boundp' ですらない。ここでトップレベルに直接リバインド処理を
  ;; 書くと `when' の条件が黙って不成立になり、後から `ui-leader--bind-root'
  ;; が既定値 "M-SPC" で結線した状態が最終的に残ってしまう（実機で発生した
  ;; 不具合）。
  ;;
  ;; そのため `with-eval-after-load' で 'ui-leader を待ち、`ui-leader.el' が
  ;; ロードされ `ui-leader--bind-root' が "M-SPC" で一度結線した *直後* に
  ;; 確実に実行されるようにする。これなら `personal/user.el' → `modules.el'
  ;; という実際のロード順序に依存せず、常に「結線の後」に上書きできる。
  ;; （`utils-ime--install-leader' の IME切替ラッパーは macism 依存＝macOS
  ;;  限定の機能であり、WSL 上ではもともと不活性なので、ここでは単純な
  ;;  `ui-leader-map' への張り替えでよい。）

  (with-eval-after-load 'ui-leader
    (when (and (boundp 'ui-leader-key) (stringp ui-leader-key)
               (boundp 'ui-leader-map) (keymapp ui-leader-map))
      (keymap-global-unset ui-leader-key)   ; 旧 "M-SPC" を解除
      (setopt ui-leader-key "C-c SPC")
      (if (fboundp 'keymap-global-set)
          (keymap-global-set ui-leader-key ui-leader-map)
        (global-set-key (kbd ui-leader-key) ui-leader-map))))
  ...
  ;; ---------------------------------------------------------------------------
  ;; win32yank.exe は標準入出力経由でテキストを受け渡すだけの単純なブリッジである。
  ;; コピー（cut）は非同期プロセスへ標準入力として書き込み、ペースト（paste）は
  ;; 同期的に標準出力を読む。win32yank.exe が見つからない場合は双方とも no-op のままとし、
  ;; Emacs 標準の（WSL 内では機能しない）kill-ring 動作にフォールバックする。

  (defvar device-wsl--win32yank-executable (executable-find "win32yank.exe")
    "win32yank.exe のパス。見つからなければ nil。
Windows クリップボードとの橋渡しに使う。
未インストールなら `winget install --id=equalsraf.win32yank' 等で導入する。")

  (defun device-wsl--clipboard-cut (text &optional _push)
    "TEXT を win32yank.exe 経由で Windows クリップボードへコピーする.
`interprogram-cut-function' として使う。"
    (when device-wsl--win32yank-executable
      (let ((proc (make-process :name "win32yank-cut"
                                 :command (list device-wsl--win32yank-executable
                                                "-i" "--crlf")
  ...
  (defun device-wsl--clipboard-paste ()
    "win32yank.exe 経由で Windows クリップボードの内容を返す.
`interprogram-paste-function' として使う。取得失敗時は nil を返す。"
    (when device-wsl--win32yank-executable
      (let ((result (shell-command-to-string
                     (concat (shell-quote-argument device-wsl--win32yank-executable)
                             " -o --lf"))))
        (unless (string-empty-p result) result))))

  (when device-wsl--win32yank-executable
    (setq interprogram-cut-function   #'device-wsl--clipboard-cut
          interprogram-paste-function #'device-wsl--clipboard-paste))

  ;; ---------------------------------------------------------------------------
  ;; ブラウザ起動 — wslview
  ;; ---------------------------------------------------------------------------
  ;; org-roam-ui 等が `browse-url' を呼ぶ際、既定の `browse-url-default-browser' は
  ;; WSL 内に GUI ブラウザが無ければ失敗する。wslview（wslu パッケージ）を経由すると
  ;; Windows 側の既定ブラウザで URL が開く。

  (defvar device-wsl--wslview-executable (executable-find "wslview")
    "wslview のパス。見つからなければ nil。
未インストールなら `sudo apt install wslu' で導入する。")

  (when device-wsl--wslview-executable
    (setq browse-url-generic-program  device-wsl--wslview-executable
  ...
  (defun device-wsl-open-in-explorer ()
    "現在のバッファのディレクトリを Windows Explorer で開く.
`dired-mode' 中はそのディレクトリを、それ以外は `default-directory' を対象とする。"
    (interactive)
    (let ((dir (expand-file-name default-directory)))
      (if (executable-find "explorer.exe")
          (call-process "explorer.exe" nil 0 nil dir)
        (message "[device-wsl] explorer.exe が見つかりません（$PATH に Windows 側パスが含まれているか確認）"))))

  (defun device-wsl-copy-windows-path ()
    "現在のファイル（無ければ `default-directory'）のパスを Windows 形式に変換し、
`wslpath -w' の結果を kill-ring へコピーする。"
    (interactive)
    (let ((file (or (buffer-file-name) default-directory)))
      (if (executable-find "wslpath")
          (let ((winpath (string-trim
                          (shell-command-to-string
                           (format "wslpath -w %s" (shell-quote-argument (expand-file-name file)))))))
            (kill-new winpath)
            (message "[device-wsl] Windows path copied: %s" winpath))
        (message "[device-wsl] wslpath コマンドが見つかりません"))))

  ;; ---------------------------------------------------------------------------
  ;; macOS Super(s-)キーバインドの置き換え
  ;; ---------------------------------------------------------------------------
  ;; WSL では Super キーのイベントが実用上届かない
  ;; （ターミナルは Super を伝える手段を持たず、WSLg 環境でも Windows キーは
  ;;   Windows 側にシステムショートカットとして横取りされることが多い）ため、
  ;; `lisp/ui/ui-global-keys.el' が定義する `s-' 系バインドは WSL 上では
  ;; 事実上デッドコードになる（無害だが発火しない）。
  ;;
  ;; ここでは共有ファイル（ui-global-keys.el）には一切手を入れず、WSL 専用の
  ;; 代替バインドを本ファイルに追加登録する。方針は頻度で2段階に分ける:
  ;;
  ;;   高頻度クラスタ（undo/redo、バッファ間移動、他ウィンドウのスクロール）
  ;;     → C-M-（Control+Alt 同時押し）、または C-z（core init で既に unset
  ;;       済みのため衝突なく再利用できる。lisp/ui/ui-global-keys.el の
  ;;       "s-z was selected..." のコメントを参照）
  ;;   低頻度クラスタ（フレーム／ウィンドウ管理）
  ;;     → 既存の `ui-leader-w-map'（WSL では C-c SPC w）に統合
  ;;
  ;; C-M-<left>/<right>/<up>/<down> は vanilla Emacs・org-mode いずれの既定
  ;; バインドとも衝突しない（sexp 移動は C-M-f/C-M-b であり arrow 版は無い。
  ;; org の見出し操作は素の M-<arrow> であり C-M- ではない）ことを確認済み。

  (defun device-wsl--global-keys-init ()
    "macOS の Super(s-)系グローバルキーを WSL 用に代替登録する.
Runs on `after-init-hook'."
    ;; undo/redo — macOS の s-z、Windows 標準の Ctrl+Z 規約とも一致する
    (keymap-global-set "C-z" #'undo-redo)
    ;; バッファ間移動 — 元 s-<left>/<right>/<up>/<down>
    (keymap-global-set "C-M-<left>"  #'previous-buffer)
    (keymap-global-set "C-M-<right>" #'next-buffer)
    (keymap-global-set "C-M-<up>"    #'beginning-of-buffer)
    (keymap-global-set "C-M-<down>"  #'end-of-buffer)
    ;; 他ウィンドウのスクロール — 元 s-<wheel-up>/<wheel-down>
    (keymap-global-set "C-M-<wheel-up>"   #'scroll-other-window-down)
    (keymap-global-set "C-M-<wheel-down>" #'scroll-other-window)
    ;; 現在ウィンドウのスクロール — 元 C-s-<up>/<down>（元々3キー和音のため、
    ;; C-c プレフィックス［逐次2打鍵］へ格下げしても体感の重さは同程度）
    (keymap-global-set "C-c <up>"   #'scroll-down-command)
    (keymap-global-set "C-c <down>" #'scroll-up-command))

  (add-hook 'after-init-hook #'device-wsl--global-keys-init 10)

  ;; ---------------------------------------------------------------------------
  ;; フレーム／ウィンドウ管理 — ui-leader-w-map への統合
  ;; ---------------------------------------------------------------------------
  ;; 元 s-o/s-m/s-w/s-./s-j。既存の ui-leader-w-map（既存の割り当て:
  ;; s/v/d/o/=/w/r/R/m/M）とは文字が重複しない。`ui-leader--define-maps' が
  ;; 済んでいる必要があるため、`device-wsl--hydra-init' と同じ理由で
  ;; after-init-hook depth 10 で実行する。

  (defun device-wsl--leader-window-keys-init ()
    "フレーム／ウィンドウ管理コマンドを `ui-leader-w-map' に追加登録する.
Runs on `after-init-hook' at depth 10, after `ui-keymap--startup-init'
(depth 0) has called `ui-leader--define-maps'."
    (ui-keymap--bind 'ui-leader-w-map "f" #'find-file-other-frame)  ; f = frame
    (ui-keymap--bind 'ui-leader-w-map "j" #'find-file-other-window) ; 元 s-j を継承
    (ui-keymap--bind 'ui-leader-w-map "n" #'make-frame-command)     ; n = new
    (ui-keymap--bind 'ui-leader-w-map "k" #'delete-frame)           ; k = kill
    ;; window-toggle-split-direction は Emacs 27 以降の同梱コマンド。
    (when (fboundp 'window-toggle-split-direction)
      (ui-keymap--bind 'ui-leader-w-map "t" #'window-toggle-split-direction))) ; t = toggle

  (add-hook 'after-init-hook #'device-wsl--leader-window-keys-init 10)

  ;; ---------------------------------------------------------------------------
  ;; hydra-wsl — リーダー a w（WSL では C-c SPC a w）
  ;; ---------------------------------------------------------------------------
  ;; `device-darwin--apple-music-init' と同じ理由（hydra と `ui-leader-a-map' の
  ;; 準備完了を待つ）により after-init-hook depth 10 で実行する。
  ;; `ui-leader-a-map' の "a" は device-darwin.el 側で Apple Music に割り当てられて
  ;; いるが、両ファイルは排他的にロードされる（同一マシンで両方は読み込まれない）ため、
  ;; キーの重複は実際には発生しない。ここでは記憶しやすさを優先し "w"（Windows）とする。

  ;; hydra-wsl/body は defhydra が実行時に定義する関数であり、コンパイル時には
  ;; 存在しないため、コンパイラに存在を伝えておく。
  (declare-function hydra-wsl/body "hydra")

  (defun device-wsl--hydra-init ()
    "`hydra-wsl' を定義し、leader キーへバインドする.
Runs on `after-init-hook' at depth 10, after `ui-keymap--startup-init'
(depth 0) has called `ui-leader--define-maps', so `ui-leader-a-map' is
guaranteed to be a live keymap. Requires hydra at runtime; no-op if
hydra is not installed."
    (when (require 'hydra nil t)
      (defhydra hydra-wsl (:hint nil :color blue)
        "
^WSL / Windows 連携^
[_e_] Explorerで開く   [_p_] Windowsパスをコピー   [_q_] quit
"
        ("e" device-wsl-open-in-explorer)
        ("p" device-wsl-copy-windows-path)
        ("q" nil))
      (ui-keymap--bind 'ui-leader-a-map "w" #'hydra-wsl/body)))

  (add-hook 'after-init-hook #'device-wsl--hydra-init 10)

  (provide 'device-wsl)
  ;;; personal/device-wsl.el ends here
#+end_src
...
Subsequent lines: describe arguments and return value.\n\n%s" src)))
  ...
  (defun my/claude-analyze-require-graph ()
    "Analyze require statements in the current buffer and report layer
violations.
Detects upward-layer dependencies relative to the 10-layer architecture:
early-init → core → ui → auth → completion → orgx → vcs → dev → utils
→ personal.
...
*** personal/personal-llm.el
:PROPERTIES:
:CUSTOM_ID: personal-llm
:header-args:emacs-lisp: :tangle personal/personal-llm.el
:END:

#+begin_src emacs-lisp
  ;;; personal/personal-llm.el --- ローカル LLM ルーター統合 -*- lexical-binding: t; -*-
  ;;
  ;; Copyright (c) 2021-2026
  ;; Author: YAMASHITA, Takao
  ;; License: GNU GPL v3 or later
  ...
  ;;; Commentary:
  ;;
  ;; =~/.local/bin/localllm= （ローカル Ollama モデルへのルーター）を Emacs から呼び出すための
  ;; 薄い統合層。プロンプトの内容に応じてどのモデル（qwen3:14b / qwen3-coder /
  ;; LFM2.5-8B-A1B / qwen2.5vl）へ振り分けるかの判定ロジックは、すべて
  ;; =~/.local/bin/localllm= 側の責務であり、本ファイルはそれに一切関与しない。
  ;; 本ファイルの責務は次の3点のみ:
  ;;
  ;;   1. プロンプトをルータープロセスの標準入力へ送る
  ;;   2. 応答をストリーミングで `personal-llm-output-buffer' へ挿入する
  ;;   3. Org-roam / コーディングバッファ向けの薄いラッパーコマンドを提供する
  ;;
  ;; 公開 API:
  ;;   `my/llm-run'                    — 任意のプロンプトをルーターへ送る（対話コマンド）
  ;;   `my/llm-run-from-org-roam'      — 現在の Org-roam ノート（リージョンか全体）を送る。
  ;;                                     タスクは指定しない — ノートは雑多な内容を含み得るため
  ;;                                     ルーター側のルールベース自動判定に委ねる。
  ;;   `my/llm-run-from-coding-buffer' — ポイント位置の defun（かリージョン）を
  ;;                                     --task coding 固定で送る。自動判定を待たず
  ;;                                     確実に qwen3-coder へ振り分けるため。
  ;;
  ;; 実行ファイルの場所は `personal-llm-script-path' で変更できる。
  ;; 本モジュールは org を要求しない（Org の遅延ロード方針を壊さない）。
  ;;
  ;; スクリプト名を単に "llm" にしなかったのは、Homebrew の llm（datasette 版、
  ;; 既定で OpenAI API を叩く）と PATH 上で衝突するため。本モジュールは
  ;; `expand-file-name' で解決した絶対パスを直接 `make-process' に渡すので
  ;; その衝突の影響を受けないが、同じ理由でユーザーがターミナルから素の
  ;; "llm" と打つと Homebrew 版に奪われる（実際に発生し、OpenAI の課金エラーが
  ;; 出た）。以後は "localllm" という別名を使うことでこれを避けている。
  ;;
  ;;; Code:
  ...
  (defgroup personal-llm nil
    "Local Ollama model router integration (~/.local/bin/localllm)."
    :group 'tools
    :prefix "personal-llm-")

  (defcustom personal-llm-script-path "~/.local/bin/localllm"
    "Path to the local LLM router script invoked by `my/llm-run'."
    :type 'file
    :group 'personal-llm)

  (defcustom personal-llm-output-buffer "*LLM Router Output*"
    "Name of the buffer used to display streamed LLM router output."
    :type 'string
    :group 'personal-llm)

  (defconst personal-llm--valid-tasks '(general coding agent vision)
    "Task symbols accepted by the ~/.local/bin/localllm router's --task flag.")

  ;;; ── 非公開ヘルパー ────────────────────────────────────────────────────────

  (defun personal-llm--ensure-script ()
    "Return the expanded, executable path to the router script.
Signals `user-error' if `personal-llm-script-path' does not point to an
executable file."
    (let ((script (expand-file-name personal-llm-script-path)))
      (unless (file-executable-p script)
        (user-error "LLM router script not found or not executable: %s" script))
      script))

  (defun personal-llm--task-string (task)
    "Return TASK (a symbol) as the string expected by the router's --task flag.
Signals `user-error' if TASK is not one of `personal-llm--valid-tasks'."
    (unless (memq task personal-llm--valid-tasks)
      (user-error "Unknown LLM task %S (expected one of %S)"
                  task personal-llm--valid-tasks))
    (symbol-name task))

  (defun personal-llm--defun-at-point ()
    "Return the top-level defun source string surrounding point, or nil."
    (save-excursion
      (condition-case nil
          (progn
  ...
  (defun personal-llm--sentinel (proc event)
    "Report completion of the router PROC via EVENT in the echo area."
    (when (memq (process-status proc) '(exit signal))
      (message "[llm] %s" (string-trim event))))

  (defun personal-llm--prepare-output-buffer (task)
    "Pop to `personal-llm-output-buffer', appending a heading for TASK.
TASK is a symbol or nil (rendered as \"auto\" in the heading).
Returns the buffer, with point at its end."
    (let ((buf (get-buffer-create personal-llm-output-buffer)))
      (with-current-buffer buf
        (unless (derived-mode-p 'org-mode)
          (org-mode))
        (goto-char (point-max))
  ...
  (defun personal-llm--start-process (prompt task image-path)
    "Start the router process for PROMPT, streaming into its output buffer.
TASK, when non-nil, is validated and forwarded as the router's --task flag.
IMAGE-PATH, when non-nil, is expanded and forwarded as --image."
    (let* ((script (personal-llm--ensure-script))
           (task-str (and task (personal-llm--task-string task)))
           (command (append (list script)
                            (when task-str (list "--task" task-str))
                            (when image-path
                              (list "--image" (expand-file-name image-path)))))
           (buf (personal-llm--prepare-output-buffer task))
           (proc (make-process :name "personal-llm"
                               :buffer buf
                               :command command
                               :noquery t
                               :sentinel #'personal-llm--sentinel)))
  ...
  (defun my/llm-run (prompt &optional task image-path)
    "Send PROMPT to the local LLM router and stream the response.
TASK, when non-nil, must be one of the symbols `general', `coding',
`agent', or `vision'; it is forwarded as the router's --task flag,
overriding its own rule-based auto-detection. When TASK is nil, the
router decides from PROMPT's content (or from IMAGE-PATH's presence).
IMAGE-PATH, when non-nil, is forwarded as the router's --image flag.

Interactively, PROMPT is the active region if any, otherwise it is
read from the minibuffer; TASK and IMAGE-PATH are left nil. Call this
function from Lisp (see `my/llm-run-from-coding-buffer') to pin a task
or attach an image."
    (interactive
    ...
    (when (string-empty-p (string-trim prompt))
      (user-error "Nothing to send to the LLM router"))
    (message "[llm] sending to router (task: %s)..." (or task "auto"))
    (personal-llm--start-process prompt task image-path))

  (defun my/llm-run-from-org-roam ()
    "Send the current Org-roam note to the LLM router.
Sends the active region if any, otherwise the whole buffer. The task
is left unspecified so the router's rule-based detection picks it —
Org-roam notes span general writing, coding notes, and action items
alike, so pinning a single task here would waste that detection."
    (interactive)
    (my/llm-run
     (if (use-region-p)
         (buffer-substring-no-properties (region-beginning) (region-end))
  ...
  (defun my/llm-run-from-coding-buffer ()
    "Send the current defun (or active region) to the LLM router as `coding'.
Pinning the task bypasses the router's rule-based auto-detection so the
prompt is always routed to the coding model, regardless of how much
prose the snippet happens to mix in."
    (interactive)
    (let ((src (if (use-region-p)
                   (buffer-substring-no-properties (region-beginning) (region-end))
                 (personal-llm--defun-at-point))))
      (unless src
        (user-error "No region active and no defun found at point"))
      (my/llm-run src 'coding)))

  (provide 'personal-llm)
  ;;; personal/personal-llm.el ends here
#+end_src
...
#+begin_src makefile :tangle Makefile :mkdirp no :padline no
  ...
  # リポジトリ直下に puppeteer-config.json があれば自動採用する（$(wildcard ...) は
  # ファイルが無ければ空文字になるので、無い環境では今まで通り渡さずフォールバックする）。
  # コマンドラインで PUPPETEER_CONFIG=... を指定すれば ?= なのでいつでも上書きできる。
  PUPPETEER_CONFIG ?= $(wildcard puppeteer-config.json)
  ...
  # ---- 既定ターゲット（引数なし）--------------------------------------------------
  .PHONY: all onepass-init onepass-q clean distclean distclean-deps show-files echo-paths tangle reload check-cookies check-tangle check-emphasis figures clean-figures dot-tangle dot-svg mmd-tangle mmd-svg diagrams clean-diagrams changelog-tangle help
  all: onepass-init
  ...
  # を解決する。
  #
  # スコープの注記: この一連の変数を参照するのは onepass-q・echo-paths・
  # distclean-deps だけである。reload（= clean tangle check-cookies）・
  # tangle 単体・clean はいずれも参照しない。STRAIGHT_BASE_DIR /
  # MY_D_CACHE_DIR は再帰展開変数（?=）なので、$(shell ...) はどこかの
  # ターゲットが実際にこの変数を参照したときにのみ評価される — 参照されなければ、
  # straight や leaf が未インストールの環境で `make reload` 等を実行しても
  # この行が原因で失敗することはない。
  #
  # 注意（$XDG_CACHE_HOME）: early-init.el の my:d:cache は
  # `$XDG_CACHE_HOME/emacs/`（未設定時は `$HOME/.cache/emacs/`）に解決され、
  # straight-base-dir にはこの my:d:cache がそのまま渡る（straight.el 自身が
  # その配下に `straight/` を作る）。以前はここで straight-base-dir の値を
  # そのまま「straight ディレクトリそのもの」として使っており、`straight/`
  # の一段が欠落していた（LEAF_DIR / LEAFKW_DIR が実際には存在しないパスを
  # 指し、onepass-q の leaf 注入が静かに失敗する不具合があった）。また未束縛時
  # フォールバックも $XDG_CACHE_HOME を無視して $HOME/.emacs.d/straight に
  # 固定されていた。両方とも my:d:cache の定義（early-init.el 3) 基本ディレクトリ
  # 参照）に合わせて修正済み。
  STRAIGHT_BASE_DIR ?= $(shell \
    if [ -f "$(EARLY)" ]; then \
      $(EMACS_Q) -l "$(EARLY)" \
        --eval "(princ (expand-file-name \"straight\" \
                         (if (boundp 'straight-base-dir) straight-base-dir user-emacs-directory)))"; \
    else \
      printf "%s" "$${XDG_CACHE_HOME:-$$HOME/.cache}/emacs/straight"; \
    fi)
  LEAF_DIR   := $(STRAIGHT_BASE_DIR)/repos/leaf.el
  LEAFKW_DIR := $(STRAIGHT_BASE_DIR)/repos/leaf-keywords.el

  # MY_D_CACHE_DIR : early-init.el の my:d:cache（straight-base-dir の親であり、
  # eln-cache や一部パッケージ（dirvish 等）のキャッシュも同居する、この設定専用の
  # XDG キャッシュルート）。distclean-deps が実キャッシュ全体を一括削除するために
  # 使う。STRAIGHT_BASE_DIR から逆算せず、my:d:cache を直接問い合わせる（両者の
  # 親子関係という前提に依存しないため）。
  MY_D_CACHE_DIR ?= $(shell \
    if [ -f "$(EARLY)" ]; then \
      $(EMACS_Q) -l "$(EARLY)" \
        --eval "(princ (if (boundp 'my:d:cache) my:d:cache \
                          (expand-file-name \"emacs\" \
                            (or (getenv \"XDG_CACHE_HOME\") \
                                (expand-file-name \".cache\" (getenv \"HOME\"))))))"; \
    else \
      printf "%s" "$${XDG_CACHE_HOME:-$$HOME/.cache}/emacs"; \
    fi)
  ...
  echo-paths:
	@echo "ROOT=$(ROOT)"; \
  	...
  	echo "MY_D_CACHE_DIR=$(MY_D_CACHE_DIR)"
  ...
  # distclean は README.org からの tangle 生成物一式を削除し、fresh clone 相当に戻す。
  # 対象は早期に消えても実害のない「ソースから常に再生成できるもの」だけ（early-init.el
  # / init.el / lisp/ / personal/ / scripts/ / LICENSE / Makefile 自身、および svg/ 配下の
  # *.dot / *.mmd / *.pdf 中間生成物）。svg/*.svg は git 管理下の成果物なので対象外。
  # ChangeLog は README.org からの通常 tangle ではなく make changelog-tangle（ox-ascii
  # エクスポート）由来の別の生成物だが、同じく常に再生成できるため distclean の対象。
  # straight/ の package clone や eln-cache 等の依存物・実行時キャッシュは distclean には
  # 含めない（再取得・再コンパイルに数分〜かかり、distclean を打つたびに発生させたくない
  # ため）。それらを消したい場合は distclean-deps を使う。
  #
  # 注意: Makefile 自身も削除されるため、実行後は make が一切使えなくなる。再び使うには
  # Tangle 節の「初回ブートストラップ」コマンド（emacs -Q --batch tangle）が必要。
  distclean: clean
	@echo "[distclean] remove stray *.eln"
	@find "$(ROOT)" -type f -name '*.eln' -delete
	@echo "[distclean] remove tangle-generated files (early-init.el init.el lisp/ personal/ scripts/ LICENSE svg intermediates)"
	@rm -f "$(EARLY)" "$(INIT)" "$(ROOT)/LICENSE"
	@rm -rf "$(LISPDIR)" "$(PERSONALDIR)" "$(ROOT)/scripts"
	@{ [ -d "$(SVGDIR)" ] && find "$(SVGDIR)" \( -name '*.dot' -o -name '*.mmd' -o -name '*.pdf' \) -delete; } 2>/dev/null || true
	@echo "[distclean] remove ChangeLog (from README.org Changelog section via make changelog-tangle)"
	@rm -f "$(ROOT)/ChangeLog"
	@echo "[distclean] remove Makefile (re-bootstrap: see README.org Tangle section)"
	@rm -f "$(ROOT)/Makefile"

  # distclean-deps は distclean よりさらに踏み込んだ、破壊的な操作。straight/ のパッケージ
  # clone・ネイティブコンパイルキャッシュ・no-littering の実行時状態を削除する。再取得や
  # 再コンパイルに数分〜十数分かかりうるため distclean には含めず、独立したターゲットに
  # している。
  #
  # 実キャッシュ（straight/ の package clone・eln-cache）は $XDG_CACHE_HOME/emacs/
  # （未設定時は $HOME/.cache/emacs/）配下にあり、$(ROOT) の外なので $(MY_D_CACHE_DIR)
  # を使って削除する（$(ROOT)/straight や $(ROOT)/.cache ではない — 以前はここが
  # 誤っており、$XDG_CACHE_HOME を考慮していなかった）。$(ROOT)/eln-cache は
  # dot-tangle 等の `-Q` バッチ実行が early-init.el を経由せず作る別物の副産物
  # （$HOME/.emacs.d が本リポジトリへのシンボリックリンクであるため、-Q の既定
  # user-emacs-directory がここを指す）なので、これも別途削除する。$(ROOT)/.etc /
  # $(ROOT)/.var（no-littering-etc-directory / -var-directory）はリポジトリ直下
  # のままなので従来どおり。
  distclean-deps:
	@echo "[distclean-deps] remove $(MY_D_CACHE_DIR) (straight/ + eln-cache/ + other per-package caches under XDG_CACHE_HOME)"
	@rm -rf "$(MY_D_CACHE_DIR)"
	@echo "[distclean-deps] remove stray $(ROOT)/eln-cache (from -Q batch runs; ~/.emacs.d symlinks here) and $(ROOT)/.etc $(ROOT)/.var"
	@rm -rf "$(ROOT)/eln-cache" "$(ROOT)/.etc" "$(ROOT)/.var"

  # ---- dot-tangle : README.org の dot ソースブロックを svg/*.dot として書き出す ---------
  # Appendix の :header-args:dot: :tangle svg/*.dot に従い、org-babel-tangle-file の
  # LANG 引数に "dot" を渡して dot 言語のブロックだけを対象に書き出す。design_spec.org は
  # 廃止済み（内容は README.org の Modular Loader & Core Suite 内の各モジュール節、および
  # Appendix に統合された）ため、対象は $(ORG) のみでよい。
  dot-tangle: $(ORG)
	@echo "[dot-tangle] $(ORG) -> $(SVGDIR)/*.dot"
	@$(EMACS_Q) \
     --eval "(require 'org)" \
     --eval "(require 'ob-core)" \
     --eval "(setq org-confirm-babel-evaluate nil noninteractive t)" \
     --eval "(org-babel-tangle-file \"$(ORG)\" nil \"dot\")"
  ...
  # ---- mmd-tangle : README.org の mermaid ソースブロックを svg/*.mmd として書き出す ------
  # :header-args:mermaid: :tangle svg/*.mmd に従い、LANG 引数に "mermaid" を渡して
  # tangle する。mermaid ブロックを持つのは README.org（tangle-flow.mmd）のみ。
  mmd-tangle: $(ORG)
	@echo "[mmd-tangle] $(ORG) -> $(SVGDIR)/*.mmd"
	@$(EMACS_Q) \
     --eval "(require 'org)" \
     --eval "(require 'ob-core)" \
     --eval "(setq org-confirm-babel-evaluate nil noninteractive t)" \
     --eval "(org-babel-tangle-file \"$(ORG)\" nil \"mermaid\")"
   ...
   for f in $$(find "$(SVGDIR)" -name '*.mmd' | sort); do \
     ...
     if [ ! -f "$$svg" ] || [ "$$f" -nt "$$svg" ]; then \
       ...
       if [ -n "$(PUPPETEER_CONFIG)" ]; then \
         "$(MMDC)" -i "$$f" -o "$$svg" -b white -p "$(PUPPETEER_CONFIG)" || { \
  ...
  # ---- changelog-tangle : README.org の Changelog 節を ChangeLog として書き出す ------
  # Changelog 節（:CUSTOM_ID: changelog）は見出し・表・強調記法を含む通常の Org
  # 構造であり、内容をソースブロックへ複製すると二重管理になる。そのため tangle では
  # なく、その subtree だけを ox-ascii でプレーンテキストへエクスポートする。
  # GNU 系ディストリビューションの慣習にならい、出力名は拡張子なしの ChangeLog と
  # する。$(ORG) 全体の tangle / reload には含めず、独立したターゲットにしている。
  changelog-tangle: $(ORG)
	@echo "[changelog-tangle] $(ORG)#changelog -> ChangeLog"
	@$(EMACS_Q) \
     --eval "(require 'org)" \
     --eval "(require 'ox-ascii)" \
     --eval "(setq org-confirm-babel-evaluate nil noninteractive t)" \
     --eval "(with-current-buffer (find-file-noselect \"$(ORG)\") \
               (let ((pos (org-find-property \"CUSTOM_ID\" \"changelog\"))) \
                 (unless pos (error \"CUSTOM_ID changelog not found\")) \
                 (goto-char pos) \
                 (let ((org-export-with-broken-links t) \
                       (org-ascii-charset 'utf-8)) \
                   (org-export-to-file 'ascii \"$(ROOT)/ChangeLog\" nil t)))))"
  ...
  help:
	...
	@echo "  make distclean                # clean + tangle 生成物一式を削除（fresh clone 相当。"
	@echo "                                #   Makefile 自身も消えるので要再ブートストラップ）"
	@echo "  make distclean-deps           # XDG キャッシュ配下（straight/ eln-cache 等）+ .etc .var を削除（破壊的・低頻度用）"
	@echo ""
	@echo "  # 図版（dot / mermaid）"
	@echo "  make diagrams                 # dot-svg + mmd-svg をまとめて実行（推奨はこれ）"
	@echo "  make dot-tangle               # README.org の dot ブロックを svg/*.dot へ"
	...
	@echo ""
	@echo "  # ChangeLog（GNU 系ディストリビューションの慣習にならった拡張子なしファイル。"
	@echo "  #   LICENSE は README.org の License 節に埋め込み済みなので make tangle で生成される）"
	@echo "  make changelog-tangle         # README.org の Changelog 節を ChangeLog へエクスポート"
	@echo ""
	@echo "  # 静的チェック"
  ...
  # 本文書（および他の org 文書）の LaTeX/PDF エクスポートは、SVG を
  ...
  # 変換を実行）。rsvg-convert コマンドが無ければ、その場でインストール手順を
  # 案内して失敗する（CI では未インストールを黙って無視しない）。
  # diagrams（dot/mmd -> svg）を先に実行し、変換元の *.svg が揃っていることを保証してから
  # svg -> pdf の変換に入る。
  #
  # mmdc（Mermaid）が出力する *.svg はラベルをすべて <foreignObject> 内の HTML
  # （<div>/<span>）として埋め込む。librsvg は foreignObject を描画できない
  # （実機で svg/tangle-flow.pdf のラベルが全て空白になることを確認済み）ため、
  # 同名の *.mmd が存在する *.svg（= Mermaid 由来）だけは rsvg-convert を使わず、
  # Puppeteer（実ブラウザエンジン）で HTML を正しく描画できる mmdc 自身に
  # -o *.pdf -f（pdfFit）で直接 PDF を吐かせる。dot（Graphviz）由来の *.svg は
  # ネイティブな <text> 要素だけで構成されるため、従来どおり rsvg-convert で問題ない。
  # mmd-svg と同じ理由で mmdc 側の Chrome 解決に失敗しうるため、$(PUPPETEER_CONFIG)
  # が設定されていれば同様に -p で渡す。
  figures: diagrams
	...
	@count=0; \
  	 for f in $$(find "$(SVGDIR)" -name '*.svg' | sort); do \
  	   ...
  	   if [ ! -f "$$pdf" ] || [ "$$f" -nt "$$pdf" ]; then \
  	     if [ -f "$$mmd" ]; then \
  	       command -v "$(MMDC)" >/dev/null 2>&1 || { \
  	         echo "[figures] $(MMDC) not found (needed for Mermaid-sourced $$mmd). Install with: npm install -g @mermaid-js/mermaid-cli"; \
  	         exit 1; \
  	       }; \
  	       echo "  $$mmd -> $$pdf (via mmdc, foreignObject-safe)"; \
  	       if [ -n "$(PUPPETEER_CONFIG)" ]; then \
  	         "$(MMDC)" -i "$$mmd" -o "$$pdf" -f -p "$(PUPPETEER_CONFIG)" || exit 1; \
  	       else \
  	         "$(MMDC)" -i "$$mmd" -o "$$pdf" -f || exit 1; \
  	       fi; \
  	     else \
  	       echo "  $$f -> $$pdf"; \
  	            ...
  	            (let ((bad 0)) \
  	              (org-babel-map-src-blocks nil \
  	                (when (and (string= lang \"emacs-lisp\") \
  	                           (not (equal (org-entry-get (point) \"CUSTOM_ID\" t) \"changelog\"))) \
  ...
  # 直後が半角空白・タブ・- – — ( " ' { （直前側）／ . , : ! ? ; ' " ) } [ （直後側）・
  # 行頭・行末のいずれかでなければ強調は成立しない。orgx-core.el（leaf org :config）で
  # 「」『』（）【】、。・ を PRE/POST に追加済みだが、それ以外の全角文字（！？：；等）は
  # まだこの集合に含まれない。和文の直後に =code= を続けるとマーカーが素通しされ、
  # リテラルの "=" 文字としてそのまま表示される（シンタックスハイライトも失われる）。
  # あわせて、Org の強調構文がネストをサポートしない点も検出する。*bold* の中に
  # 完全な =verbatim= / ~code~ が入っている場合（例: "*なぜ =foo= なのか*"）、内側の
  # マーカーは独立して再マッチされず literal 文字のまま表示される。
  # tangle には影響しないため、いずれも見た目の欠陥として気づかれにくい。
  #
...
* Appendix: check_emphasis.py — 効かない Org 強調記法の検出スクリプト
:PROPERTIES:
:CUSTOM_ID: appendix-check-emphasis-py
:header-args:python: :tangle scripts/check_emphasis.py :mkdirp yes :comments link
:END:

=make check-emphasis= （ =make lint= にも含まれる）が呼び出す検査本体です。

*なぜ Elisp ではなく Python なのか*

最初の実装は Elisp（ =scripts/check-emphasis.el= 、 =--load= で読み込む方式）でした。
しかし本リポジトリの開発環境には Emacs 本体が無く、実機で 2 件の不具合が見つかるまで
気づけませんでした。 =:comments link= が =lexical-binding= クッキーを 1 行目から
追い出す問題と、 =--load FILE -- ARGS= における =command-line-args-left= の ="--"=
処理の問題です。いずれも Elisp のバッチモード特有の癖であり、実行できない環境で
書いたコードでは検証しきれませんでした。

Python は本リポジトリの開発環境で実際に実行・検証できます。ロジックは Elisp 版と
同一で、 =scripts/reload.py= と同じく標準ライブラリのみに依存します。

*検出方法*

=org-emphasis-regexp-components= が定める「直前・直後に許される文字」の集合を
そのまま Python の =set= として持ちます。orgx-core.el の =(leaf org :config)= で
この変数に和文の括弧・読点（「」『』（）【】、。・）を追加しているため、
=PRE_OK= / =POST_OK= もその追加分を含めた集合にしてあります。Elisp 側の値を
実行時に import することはできないため、orgx-core.el を変更した場合は
=PRE_OK= / =POST_OK= を手作業で同期させてください。 =verbatim= と ~code~ を
先に確定させて保護領域とし、次に *bold* 候補を、保護領域との位置関係で 3 通りに
分けて判定します。

- 保護領域が bold 候補を完全に包含する（例: =*scratch*= の =*= ）—
  保護領域内の文字列がたまたま =*text*= の形をしているだけで、この =*= は
  そもそも実在する強調マーカーではない。検査対象から除外する。
- bold 候補が保護領域を完全に包含する（例: 「なぜ =foo= を使うのか」を
  bold で囲んだ場合）—
  Org の強調構文はネストしない。=org-emph-re= はマーカー種別ごとに 1 本の
  正規表現で、最初に見つかった外側のマッチが行の該当範囲を丸ごと消費するため、
  内側の =foo= は独立した verbatim として再マッチされず、"=" 記号がそのまま
  literal 文字として表示される。これは違反として報告する。
- 保護領域と bold 候補が部分的にしか重ならない（例: =if-let*= と =when-let*=
  のように、識別子自体に含まれるリテラルの =*= が離れた 2 つの verbatim
  スパンをまたいで =*...*= の形に見えてしまう）— どちらの包含関係でもない
  ため、実在する強調マーカーの組ではないと判断し、除外する。

コードブロック本文・見出し行・drawer/keyword/table 行（先頭が =:= =#= =|= ）は
検査対象から除外します。

*実行例*

#+begin_example
$ python3 scripts/check_emphasis.py README.org
[check-emphasis] scanning README.org for ineffective emphasis markers ...
  README.org:249: =my:modules= に含まれる
[check-emphasis] FAILED: 1 ineffective marker(s)
#+end_example

複数ファイルを一度に検査できる作りになっています。 =make check-emphasis= は =$(ORG)=
（ =README.org= ）のみを渡しますが、将来ドキュメントが分割された場合などに備えて、
複数の =.org= ファイルをまとめて検査できる余地を残してあります。

#+begin_src python
  #!/usr/bin/env python3
  """Detect Org emphasis markers that org-emphasis-regexp-components silently rejects.

  By default, Org requires the character immediately before an opening marker and
  immediately after a closing marker to belong to a specific set (ASCII whitespace,
  "-", en/em dash, and a few punctuation marks -- see PRE_OK / POST_OK below).
  Fullwidth characters (、。（）「」 etc.) are not in that set, so =verbatim=, ~code~,
  and *bold* spans adjacent to Japanese prose are silently rendered as literal text
  instead of being fontified -- a defect that does not affect tangling and is easy
  to miss by eye.

  This mirrors the detection rule used to auto-fix README.org during development:
  =verbatim= and ~code~ spans are resolved first and treated as protected regions
  (a bare = or ~ is always literal, never a bold delimiter). *bold* candidates are
  then checked only outside those protected regions, and are skipped when
  immediately adjacent to = or ~ (so the asterisks inside =*scratch*= are not
  mistaken for a bold marker).

  Usage:
      python3 scripts/check_emphasis.py README.org [other.org ...]

  Exit status: 0 if no violations were found in any file, 1 otherwise.
  """
  ...
  # Characters allowed immediately before an emphasis marker's opening char.
  # Extends Org's ASCII-only default with the Japanese punctuation added to
  # org-emphasis-regexp-components in orgx-core.el (leaf org :config). Keep
  # these two sets in sync by hand -- see the comment next to that setopt call.
  PRE_OK = set(" \t-\u2013\u2014(\"'{" "「『（【、。・")

  # Characters allowed immediately after an emphasis marker's closing char.
  POST_OK = set(" \t-\u2013\u2014.,:!?;'\")}[" "」』）】「『（【、。・")

  # Only these three markers are checked. "/" (italic) is deliberately excluded:
  # path-like strings such as "lisp/core/core-gc.el" are extremely common in this
  # document and are not meant as italics, so including "/" would produce a flood
  # of false positives.
  _MARKERS = "=~*"

  _BLOCK_START_RE = re.compile(r"^[ \t]*#\+begin_(src|example)", re.IGNORECASE)
  _BLOCK_END_RE = re.compile(r"^[ \t]*#\+end_(src|example)", re.IGNORECASE)
  _HEADING_RE = re.compile(r"^[ \t]*\*+ ")
  _DRAWER_KEYWORD_TABLE_RE = re.compile(r"^[ \t]*[:#|]")


  def _marker_span_re(marker: str) -> re.Pattern[str]:
      """Build a regexp matching MARKER CONTENT MARKER.

      CONTENT must not start or end with whitespace or MARKER itself, and must
      not contain MARKER internally. This mirrors the border/body rules implied
      by org-emphasis-regexp-components.
      """
      m = re.escape(marker)
      return re.compile(rf"{m}([^\s{m}][^{m}\n]*?[^\s{m}]|[^\s{m}]){m}")


  _SPAN_RE = {mk: _marker_span_re(mk) for mk in _MARKERS}


  def _collect_spans(line: str, marker: str) -> list[tuple[int, int]]:
      """Return all (start, end) spans in LINE matching MARKER, unfiltered."""
      spans = []
      pos = 0
      rx = _SPAN_RE[marker]
      while True:
  ...
  def _check_span(line: str, a: int, b: int) -> bool:
      """Return True if the span [a, b) has a disallowed border character."""
      pre = line[a - 1] if a > 0 else None
      post = line[b] if b < len(line) else None
      bad_pre = pre is not None and pre not in PRE_OK
      bad_post = post is not None and post not in POST_OK
      return bad_pre or bad_post


  def _contains(outer: tuple[int, int], inner: tuple[int, int]) -> bool:
      """True if span INNER is strictly contained within span OUTER."""
      return outer[0] <= inner[0] and inner[1] <= outer[1]


  def scan_line(line: str) -> list[tuple[int, int]]:
      """Return the list of (start, end) spans in LINE with an ineffective marker."""
      violations: list[tuple[int, int]] = []
      protected: list[tuple[int, int]] = []

      for marker in ("=", "~"):
          for a, b in _collect_spans(line, marker):
              if _overlaps(a, b, protected):
                  continue
              protected.append((a, b))
              if _check_span(line, a, b):
                  violations.append((a, b))

      for a, b in _collect_spans(line, "*"):
          # Case 1: the "*...*" candidate is itself entirely inside an
          # already-confirmed =verbatim= or ~code~ span (e.g. "=*scratch*=").
          # The asterisks are literal content there, not real bold markers,
          # so this candidate is spurious and must not be checked at all.
          if any(_contains((s, e), (a, b)) for s, e in protected):
              continue
          # Case 2: a =verbatim= or ~code~ span is entirely inside this bold
          # candidate's content (e.g. "*foo =bar= baz*"). Org emphasis does
          # not nest: org-emph-re is one combined regex per marker type, and
          # the first (outer) match consumes the whole span in a single
          # font-lock pass, so the inner marker is never independently
          # re-matched -- it renders as literal characters, not monospace.
          # This is a genuine violation even though PRE/POST of the outer
          # "*...*" may themselves be perfectly valid.
          if any(_contains((a, b), (s, e)) for s, e in protected):
              violations.append((a, b))
              continue
          # Case 3: partial overlap with a protected span (crosses a
          # boundary without full containment either way) -- e.g. two
          # separate =verbatim= spans whose content happens to end/start
          # with a literal "*" (identifiers like if-let*, when-let*, or
          # glob patterns like svg/01_*.svg). The "*" characters here are
          # literal content, not real bold markers; treat as spurious and
          # skip, matching Case 1.
          if _overlaps(a, b, protected):
              continue
          if a > 0 and line[a - 1] in "=~":
              continue
          if b < len(line) and line[b] in "=~":
              continue
          if _check_span(line, a, b):
              violations.append((a, b))
  ...
  def scan_file(path: Path) -> int:
      """Scan PATH and print any ineffective emphasis markers found.

      Skips source/example block bodies, heading lines, and lines beginning with
      a drawer, keyword, or table marker (: # |). Returns the violation count.
      """
      bad = 0
      in_block = False
      ...
      for lineno, line in enumerate(text.split("\n"), start=1):
          ...
          if _BLOCK_END_RE.match(line):
              in_block = False
              continue
          if in_block or _HEADING_RE.match(line) or _DRAWER_KEYWORD_TABLE_RE.match(line):
              continue
          for a, b in scan_line(line):
              bad += 1
              print(f"  {path}:{lineno}: {line[a:min(len(line), a + 40)]}")
      return bad


  def main(argv: list[str]) -> int:
      if not argv:
          print("usage: check_emphasis.py FILE.org [FILE.org ...]", file=sys.stderr)
          return 2
      ...
      for arg in argv:
          path = Path(arg)
          if not path.is_file():
              print(f"[check-emphasis] no such file: {path}", file=sys.stderr)
              return 2
          print(f"[check-emphasis] scanning {path} for ineffective emphasis markers ...")
          total += scan_file(path)

      if total > 0:
          print(f"[check-emphasis] FAILED: {total} ineffective marker(s)", file=sys.stderr)
          return 1
      print("[check-emphasis] ok")
      return 0
...
* Appendix: check_fboundp_guards.py — fboundp ガードの整合性検出スクリプト
:PROPERTIES:
:CUSTOM_ID: appendix-check-fboundp-guards-py
:header-args:python: :tangle scripts/check_fboundp_guards.py :mkdirp yes :comments link
:END:

=make check-fboundp-guards= （ =make lint= にも含まれる）が呼び出す検査本体です。

*何を検出するためのツールか*

=(when (fboundp 'X) ... #'X ...)= という形——つまり X をコマンドとして
キーバインドやフックに登録している箇所——で、X が実際に解決できない状態を検出する。
このパターンは =condition-case= のようにエラーを出して落ちるわけではなく、
=fboundp= が =nil= を返すだけで *黙って何も登録されない* 。実機で動かして
初めて気づく、という壊れ方をする。

このリポジトリで実際にこの形のバグが 2 件見つかっている: =dev-music.el= を
=leaf= の =:commands= 経由の遅延ロードに変えたとき、素の =emms= コマンド自体を
=:commands= リストに入れ忘れていたケースと、 =restart-emacs= パッケージの
=leaf= 宣言自体がどこにも存在しないのに =(fboundp 'restart-emacs)= だけ
書かれていたケース。どちらも起動時にエラーは出ず、該当のキーバインドが
静かに機能しないだけだった。

*検査の絞り込み方*

単なる機能検出のための =fboundp= （例: =(when (fboundp 'treesit-available-p) ...)= ）は
対象外にしている。 =X= が本文中で =#'X= として sharp-quote 参照されている
——つまり「コマンドとして束縛している」——ケースだけに絞ることで、
組み込み関数のバージョン互換チェックのような正当な用途を誤検出しないようにしている。

X の解決可否は、次のいずれかを満たすかで判定する。

1. 同じコーパス内に =(defun X ...)= / =(defalias 'X ...)= がある（ローカル定義）
2. どこかの =leaf= の =:commands (...)= に =X= が載っている（autoload 経由）
3. =X= の接頭辞と一致する =leaf= パッケージが =:commands= を持たない
   （＝ eager load されている）
4. =`X'= という Emacs docstring 引用記法の直後に "autoload" という語がある
   （ =org-roam-ui-open= のような、パッケージ側の =;;;###autoload= を
   コメントで明記しているケース）
5. =fboundp= チェックの近くに「Emacs NN 以降」のようなバージョン言及がある
   （組み込みコマンドのバージョン互換チェックだと判断する）

*限界*

完全な elisp パーサーではなく正規表現ベースの静的解析なので、誤検出
（false positive）はあり得る。実際、このツールを作った際に =find_autoload_hints=
の初版実装で =emms= という語が =dev-music.el= 中に大量に出現するせいで
"autoload" という語と偶然近接し、意図的に再現させた =emms= バグを見逃す
という誤検出を自分自身で踏んだ。判定条件を =`symbol'= という引用記法必須に
絞ることで直している。指摘された箇所は最終的に人間が目視で確認すること
——このツールは「洗い出しの手間を減らす」ためのものであり、無条件に
信頼できる完全な保証ではない。

#+begin_src python
  #!/usr/bin/env python3
  """check_fboundp_guards.py --- fboundp ガードで参照されるコマンドシンボルが、
  実際に解決可能（ローカル定義 / leaf の :commands 経由の autoload / 同名 leaf
  パッケージの eager load / 明示的な autoload コメント）かどうかを検査する。

  対象パターンは (when (fboundp 'X) ... #'X ...) / (if (fboundp 'X) ... #'X ...) の
  形だけに絞る。X が sharp-quote で参照されている、つまり「コマンドとして登録・束縛
  している」箇所だけが対象で、単なる機能検出のための fboundp（例:
  (when (fboundp 'treesit-available-p) ...)）は対象外。前者は解決できていないと
  「エラーは出ないが黙って何も起きない」バグになりやすく、後者は素朴な分岐に過ぎない
  ため。

  このスクリプトは静的なテキスト解析であり、完全な elisp パーサーではない。
  誤検出（false positive）はあり得るので、指摘された箇所は最終的に人間が確認すること。
  """
  ...
  # fboundp ガード + sharp-quote 参照のペアを拾う。
  # 例: (when (fboundp 'emms) ... #'emms ...)
  FBOUNDP_GUARD_RE = re.compile(
      r"\((?:when|if)\s+\(fboundp\s+'([A-Za-z0-9_/:*+=<>!?-]+)\)(.*?)\)\s*(?:\)|$)",
      re.DOTALL,
  )

  DEFUN_RE = re.compile(r"\(def(?:un|macro|subst)\s+([A-Za-z0-9_/:*+=<>!?-]+)")
  DEFALIAS_RE = re.compile(r"\(defalias\s+'([A-Za-z0-9_/:*+=<>!?-]+)")
  LEAF_RE = re.compile(r"\(leaf\s+([A-Za-z0-9_/:*+=<>!?-]+)\b(.*?)(?=\n\s*\(leaf\s|\Z)", re.DOTALL)
  COMMANDS_RE = re.compile(r":commands\s*\(([^)]*)\)", re.DOTALL)


  def collect_corpus_text(paths: list[Path]) -> str:
      chunks = []
      for p in paths:
          if p.is_file():
  ...
  def find_local_defs(text: str) -> set[str]:
      names = set(DEFUN_RE.findall(text))
      names |= set(DEFALIAS_RE.findall(text))
      return names


  def find_commands_symbols(text: str) -> set[str]:
      symbols: set[str] = set()
      for m in COMMANDS_RE.finditer(text):
          symbols |= set(m.group(1).split())
      return symbols


  def find_eager_leaf_prefixes(text: str) -> set[str]:
      """:commands を持たない leaf パッケージ名を集める（= eager load とみなす）。"""
      prefixes = set()
      for m in LEAF_RE.finditer(text):
          name, body = m.group(1), m.group(2)
          # 次の leaf までの body には後続 leaf の中身が混ざらないよう、
          # 直近の 400 文字程度に絞って :commands の有無だけ見る。
          head = body[:2000]
          if ":commands" not in head:
              prefixes.add(name)
      return prefixes


  def symbol_covered_by_eager_leaf(symbol: str, prefixes: set[str]) -> bool:
  ...
  def find_autoload_hints(text: str, symbol: str) -> bool:
      """`symbol' という Emacs docstring 引用記法の直後（100 文字以内）に "autoload" が
      あれば、ドキュメント上で明示的に autoload されると宣言されているとみなす。
      単に symbol という文字列が autoload という単語の近くに出現するだけでは
      誤検出になりやすいため（symbol が頻出する語のときに特に）、
      引用記法を要求して的を絞る。
      """
      pattern = re.compile(r"`" + re.escape(symbol) + r"'")
      for m in pattern.finditer(text):
          window = text[m.end() : m.end() + 100]
          if "autoload" in window:
              return True
      return False


  VERSION_COMMENT_RE = re.compile(r"Emacs\s*\d+")


  def has_version_compat_comment(text: str, match_start: int, lookback: int = 400) -> bool:
      """fboundp チェックの直前に「Emacs NN 以降」等のバージョン言及があれば、
      意図的な組み込みコマンドのバージョン互換チェックとみなして除外する。
      このリポジトリでは version-gate されたビルトインコマンドに
      「Emacs 27 以降の同梱コマンド」のような注記を付ける慣習が既にあるため、
      新しい抑制コメント記法を追加するのではなく、既存の書き方をそのまま検出する。
      """
      window = text[max(0, match_start - lookback) : match_start]
      return bool(VERSION_COMMENT_RE.search(window))


  def main(argv: list[str]) -> int:
      if len(argv) < 2:
          print("usage: check_fboundp_guards.py <lisp-dir> [<lisp-dir> ...]", file=sys.stderr)
          return 2
      ...
      for root in roots:
          if root.is_file():
              el_files.append(root)
          elif root.is_dir():
              el_files.extend(sorted(root.rglob("*.el")))

      if not el_files:
          print(f"[check-fboundp-guards] no .el files found under {roots}", file=sys.stderr)
          return 1

      print(f"[check-fboundp-guards] scanning {len(el_files)} file(s) ...")

      corpus = collect_corpus_text(el_files)
      local_defs = find_local_defs(corpus)
      commands_symbols = find_commands_symbols(corpus)
      eager_prefixes = find_eager_leaf_prefixes(corpus)

      findings: list[tuple[str, str, int]] = []  # (file, symbol, line)

      for path in el_files:
          text = path.read_text(encoding="utf-8", errors="replace")
          for m in FBOUNDP_GUARD_RE.finditer(text):
              symbol, body = m.group(1), m.group(2)
              # sharp-quote でそのシンボル自身を参照しているものだけを対象にする
              if f"#'{symbol}" not in body and f"#'{symbol})" not in body:
                  continue

              resolved = (
                  symbol in local_defs
                  or symbol in commands_symbols
                  or symbol_covered_by_eager_leaf(symbol, eager_prefixes)
                  or find_autoload_hints(corpus, symbol)
                  or has_version_compat_comment(text, m.start())
              )
              if not resolved:
                  line_no = text.count("\n", 0, m.start()) + 1
                  findings.append((str(path), symbol, line_no))

      if findings:
          print(f"[check-fboundp-guards] {len(findings)} unresolved guard(s) — needs manual review:")
          for file, symbol, line in findings:
              print(f"  {file}:{line}: (fboundp '{symbol}) — パッケージ宣言 / :commands / ローカル定義が見つからない")
          print(
              "[check-fboundp-guards] NOTE: このスクリプトは静的なテキスト解析なので誤検出があり得る。"
              " 指摘箇所は目視で確認すること。"
          )
          return 1

      print("[check-fboundp-guards] ok")
      return 0
...
* Appendix: reload.py — make reload 相当の Python 実装
:PROPERTIES:
:CUSTOM_ID: appendix-reload-py
:header-args:python: :tangle scripts/reload.py :mkdirp yes :comments link
:END:

=make reload= と同じことを行うスタンドアロンの Python スクリプトです。 =make= や
=README.org= 内の Makefile 定義に依存せず、 =python3 scripts/reload.py= だけで
「古い =.elc= の削除 → tangle → lexical-binding クッキー検証」を実行します。

用途は、 =make= が使えない環境や、 =reload= を別のツール（エディタのタスク、CI、
ファイル監視スクリプトなど）から呼びたい場合です。挙動は Makefile の =reload=
ターゲット（ =clean= → =tangle= → =check-cookies= ）を忠実に再現します。

** 挙動

=make reload= の 3 ステップと一対一で対応します。

| ステップ | Makefile | reload.py |
|----------+----------+-----------|
| clean | =lisp/= と =personal/= 配下の =*.elc= を削除 | =clean()= |
| tangle | =emacs -Q --batch= で =org-babel-tangle-file= | =tangle()= |
| check-cookies | 各 =.el= の 1 行目に =lexical-binding: t= があるか検証 | =check_cookies()= |

*設計上の注記*

- tangle は =make= と同じく =emacs -Q --batch= を呼びます。組み込み Org（Emacs 32.0.50 は
  Org 9.8.7 同梱）で tangle が完結するため、外部パッケージは不要です。
- =EMACS= 環境変数で Emacs バイナリを差し替えられます。macOS の self-contained NS ビルドでは
  =~/.local/bin/emacs= がバンドル外では動かないため、既定値を
  =/Applications/Emacs.app/Contents/MacOS/Emacs= とし、存在すればそれを優先します。
  環境変数が明示されていればそちらを尊重します。
- clean は「削除に失敗しても続行」します（Makefile の =|| true= と同じ思想）。tangle 前に
  =.elc= が残っていると新しい =.el= を古いバイトコンパイル結果が覆い隠すため、削除は
  best-effort でも実行する価値があります。
- check-cookies で 1 件でも欠落があれば終了コード 1 で終わります。CI に組み込めます。

** 使い方

#+begin_example
# 既定（カレントディレクトリを root として実行）
python3 scripts/reload.py

# Emacs バイナリを差し替える
EMACS=/opt/homebrew/bin/emacs python3 scripts/reload.py

# 別の README.org / ルートを指定
python3 scripts/reload.py --root ~/.emacs.d --org README.org

# tangle だけ / 検証だけ
python3 scripts/reload.py --only tangle
python3 scripts/reload.py --only check-cookies
#+end_example

実行後は Makefile の =reload= と同様、Emacs を再起動して新しい =.el= を読み込みます。

** reload.py

#+begin_src python
  #!/usr/bin/env python3
  """make reload equivalent: clean *.elc -> tangle README.org -> check lexical-binding cookies.

  This mirrors the Makefile `reload` target (clean -> tangle -> check-cookies)
  without requiring `make`.  It is intentionally dependency-free (standard library
  only) so it can run in minimal environments and be invoked from editors, CI, or
  file-watchers.
  """
  ...
  # --- Emacs binary resolution ------------------------------------------------
  # On macOS the self-contained NS build installs a $prefix/bin/emacs that does
  # not work outside the .app bundle, so prefer the in-bundle binary.  An explicit
  # EMACS environment variable always wins.
  _MACOS_APP_EMACS = "/Applications/Emacs.app/Contents/MacOS/Emacs"


  def resolve_emacs() -> str:
      """Return the Emacs binary to use for batch tangling."""
      env = os.environ.get("EMACS")
      if env:
          return env
      if sys.platform == "darwin" and Path(_MACOS_APP_EMACS).is_file():
          return _MACOS_APP_EMACS
      found = shutil.which("emacs")
      if found:
          return found
      # Fall back to the app path so the error message is actionable.
      return _MACOS_APP_EMACS


  # --- Step 1: clean ----------------------------------------------------------
  def clean(lisp_dir: Path, personal_dir: Path) -> int:
      """Delete every *.elc under lisp/ and personal/.  Best-effort (never fatal).

      Stale .elc files shadow freshly tangled .el files, so removing them is worth
      doing even if some deletions fail.  Returns the number of files removed.
      """
      removed = 0
      for base in (lisp_dir, personal_dir):
          if not base.is_dir():
              continue
          for elc in base.rglob("*.elc"):
              try:
                  elc.unlink()
                  removed += 1
              except OSError as err:
                  print(f"  WARN: could not delete {elc}: {err}", file=sys.stderr)
      print(f"[clean] removed {removed} .elc file(s) under "
            f"{lisp_dir} and {personal_dir}")
      return removed


  # --- Step 2: tangle ---------------------------------------------------------
  def tangle(emacs: str, org: Path) -> None:
      """Tangle ORG with `emacs -Q --batch`, matching the Makefile tangle target.

      Uses the built-in Org (Emacs 32.0.50 ships Org 9.8.7), so no third-party
      packages are required.  Exits the process on failure.
      """
      if not org.is_file():
          sys.exit(f"[tangle] ERROR: {org} not found")
      ...
      cmd = [
          ...
          "'((emacs-lisp . t)))",
          "--eval", "(setq org-confirm-babel-evaluate nil noninteractive t)",
          "--eval", f'(org-babel-tangle-file "{org}")',
      ]
      try:
          subprocess.run(cmd, check=True)
      except FileNotFoundError:
          sys.exit(f"[tangle] ERROR: Emacs binary not found: {emacs}\n"
                   f"          set EMACS=/path/to/emacs and retry")
      except subprocess.CalledProcessError as err:
          sys.exit(f"[tangle] ERROR: emacs exited with status {err.returncode}")


  # --- Step 3: check-cookies --------------------------------------------------
  def check_cookies(lisp_dir: Path) -> int:
      """Verify every .el under lisp/ has `lexical-binding: t` on line 1.

      Returns 0 when all files pass, 1 when any file is missing the cookie.
      """
      if not lisp_dir.is_dir():
          print(f"[check-cookies] {lisp_dir} does not exist; skipping")
          return 0

      print(f"[check-cookies] scanning {lisp_dir} ...")
      fail = 0
      for el in sorted(lisp_dir.rglob("*.el")):
          try:
              with el.open(encoding="utf-8", errors="replace") as handle:
                  line1 = handle.readline().rstrip("\n")
          except OSError as err:
              print(f"  ERROR reading {el}: {err}")
              fail = 1
              continue
          if "lexical-binding: t" not in line1:
              print(f"  MISSING: {el}")
              print(f"    line1: {line1}")
              fail = 1

      if fail:
          print("[check-cookies] FAILED: some files lack the lexical-binding cookie")
      else:
          print("[check-cookies] ok")
      return fail
  ...
  def main(argv: list[str] | None = None) -> int:
      parser = argparse.ArgumentParser(
          description="make reload equivalent (clean -> tangle -> check-cookies)")
      parser.add_argument(
          "--root", type=Path, default=Path.cwd(),
          help="repository root containing README.org (default: current dir)")
      parser.add_argument(
          "--org", default="README.org",
          help="Org file to tangle, relative to --root (default: README.org)")
      parser.add_argument(
          "--lisp", default="lisp",
          help="lisp directory, relative to --root (default: lisp)")
      parser.add_argument(
          "--personal", default="personal",
          help="personal directory, relative to --root (default: personal)")
      parser.add_argument(
          "--only", choices=("clean", "tangle", "check-cookies"),
          help="run only one step instead of the full reload")
      args = parser.parse_args(argv)

      root = args.root.expanduser().resolve()
      org = (root / args.org).resolve()
      lisp_dir = (root / args.lisp).resolve()
      personal_dir = (root / args.personal).resolve()
      emacs = resolve_emacs()

      if args.only == "clean":
          clean(lisp_dir, personal_dir)
          return 0
      if args.only == "tangle":
          tangle(emacs, org)
          return 0
      if args.only == "check-cookies":
          return check_cookies(lisp_dir)

      # Full reload: clean -> tangle -> check-cookies.
      clean(lisp_dir, personal_dir)
      tangle(emacs, org)
      rc = check_cookies(lisp_dir)
      if rc == 0:
          print(f"[reload] on-disk modules now reflect {org}; restart Emacs to load.")
      return rc
...
* Appendix: claude_org_roam_export.py — Claude チャットの org-roam 化
:PROPERTIES:
:CUSTOM_ID: appendix-claude-org-roam-export
:header-args:python: :tangle scripts/claude_org_roam_export.py :mkdirp yes :comments link
:END:

Claude.ai のデータエクスポート（ =conversations.json= ）を org-roam ノードに変換する
スタンドアロンの Python スクリプトです。1 会話につき 1 つの =.org= ノードファイルを、
=my:d:org-roam= （既定では =~/org/roam/= 配下）に書き出します。

会話の =uuid= をそのまま org-roam の =:ID:= に再利用するため、再エクスポートしても
同じノードが更新され、重複しません。

** データエクスポートの手順

1. Claude.ai の Settings → Privacy → Export data を実行する。
2. メールで ZIP アーカイブが届く。展開すると =user.json= 、 =conversations.json= 、
   =projects.json= 、 =artifacts.json= が含まれる。
3. 本スクリプトは =conversations.json= を入力に取る。

** 生成されるノードの形

- ファイル名は =YYYYMMDDHHMMSS-<スラッグ>.org= （ =created_at= 由来のタイムスタンプ + タイトル）。
- ファイルレベルの =:PROPERTIES:= に =:ID:= （会話 uuid）、 =:CREATED:= 、 =:UPDATED:= 、
  =:MODEL:= 、 =:SOURCE: Claude.ai= を書き込む。
- =#+title:= に会話名（無ければ最初のメッセージの先頭行）、 =#+filetags:= にタグを付ける。
- 既定では 1 メッセージにつき =** User= / =** Claude= の見出しを立てる。
  =--no-heading-per-message= を付けると、見出しを立てず本文を連結する。

スラッグ生成は ASCII に加えてひらがな・カタカナ・CJK 統合漢字を保持するので、
日本語タイトルでも読めるファイル名になります。

** 使い方

#+begin_example
# 基本: conversations.json を my:d:org-roam の下に変換
python3 scripts/claude_org_roam_export.py conversations.json ~/org/roam/claude

# タグを付ける（繰り返し可。既定は :claude:）
python3 scripts/claude_org_roam_export.py conversations.json ~/org/roam/claude \
    --tag claude --tag import

# 見出しを立てず本文を連結する
python3 scripts/claude_org_roam_export.py conversations.json ~/org/roam/claude \
    --no-heading-per-message

# uuid を再利用せず新しい ID を振る（重複更新をしたくない場合）
python3 scripts/claude_org_roam_export.py conversations.json ~/org/roam/claude \
    --fresh-uuid
#+end_example

変換後、Emacs で =M-x org-roam-db-sync= を実行すると新しいノードが org-roam の
データベースに登録されます。本設定では org-roam の autosync を起動時に有効化しない
（ =my/org-roam-autosync-enable= による対話起動）ため、 =org-roam-db-sync= を明示的に
呼ぶか、autosync を有効化してから変換結果を反映してください。

*出力先に関する注記*

出力ディレクトリは引数で指定します。本設定の =my:d:org-roam= （ =personal/<login>.el= で
上書き可能）配下を指定すれば、既存の org-roam ノードと同じ場所に並びます。org-roam の
ノードは 1 ファイル 1 ノードが基本なので、Claude 由来のノードだけをサブディレクトリ
（例: =~/org/roam/claude= ）にまとめておくと、後で一括削除や再エクスポートがしやすくなります。

** claude_org_roam_export.py

#+begin_src python
  #!/usr/bin/env python3
  """Convert a Claude.ai data export (conversations.json) into org-roam nodes.

  One org-roam node file is written per conversation.  The conversation uuid is
  reused as the :ID:, so re-running the export updates existing nodes in place
  rather than creating duplicates.

  Data export procedure on Claude.ai:
      Settings -> Privacy -> Export data.  A ZIP archive is delivered by
      email.  After extraction it contains user.json, conversations.json,
      projects.json and artifacts.json.  This script consumes
      conversations.json.

  Usage:
      python3 claude_org_roam_export.py conversations.json ~/org/roam/claude
      python3 claude_org_roam_export.py conversations.json ~/org/roam/claude \\
          --no-heading-per-message --tag claude --tag import

  After conversion, run `M-x org-roam-db-sync` in Emacs so the new nodes are
  registered in the org-roam database.
  """
  ...
  SENDER_NAMES = {"human": "User", "assistant": "Claude"}
  ...
  def org_timestamp(value: str | None) -> str | None:
      """Return an inactive Org timestamp for an ISO 8601 string, or None."""
      dt = parse_time(value)
      return dt.strftime("[%Y-%m-%d %a %H:%M]") if dt else None


  def message_text(message: dict) -> str:
      """Return the textual body of a message.

      The top-level ``text`` field is preferred.  When it is empty, the text
      blocks inside the ``content`` array are concatenated.
      """
  ...
  def slugify(title: str) -> str:
      """Return a filesystem-safe slug, preserving kana and CJK characters."""
      ascii_only = re.sub(r"[^0-9A-Za-z\u3040-\u30ff\u4e00-\u9fff]+", "-",
                          (title or "").lower())
      trimmed = ascii_only.strip("-")
      return trimmed or "untitled"


  def conversation_title(conversation: dict) -> str:
      """Return a non-empty title for a conversation."""
      name = (conversation.get("name") or "").strip()
      if name:
          return name
      messages = conversation.get("chat_messages") or []
      if messages:
          body = message_text(messages[0]).strip()
  ...
  def node_id(conversation: dict, reuse_uuid: bool) -> str:
      """Return the org-roam ID to assign to a conversation."""
      conv_uuid = conversation.get("uuid")
      if reuse_uuid and conv_uuid:
          return conv_uuid
      return str(uuidlib.uuid4())


  def output_filename(conversation: dict, title: str, directory: Path) -> Path:
      """Return an absolute output path following YYYYMMDDHHMMSS-slug.org."""
      created = parse_time(conversation.get("created_at")) or datetime.now()
      stamp = created.strftime("%Y%m%d%H%M%S")
      return directory / f"{stamp}-{slugify(title)}.org"


  def render_message(message: dict, heading_per_message: bool) -> str:
      """Return an Org fragment for one message."""
      label = SENDER_NAMES.get(
          message.get("sender"),
          (message.get("sender") or "unknown").capitalize())
      stamp = org_timestamp(message.get("created_at"))
      body = message_text(message).strip()
      if heading_per_message:
          parts = [f"** {label}"]
          if stamp:
              parts += [":PROPERTIES:", f":CREATED: {stamp}", ":END:"]
          parts += ["", body, ""]
          return "\n".join(parts)
      header = f"*{label}:*" + (f" {stamp}" if stamp else "")
      return "\n".join([header, "", body, ""])


  def render_conversation(conversation: dict, tags: list[str], reuse_uuid: bool,
                          heading_per_message: bool) -> str:
      """Return the full Org document string for a conversation."""
      title = conversation_title(conversation)
      lines = [
          ":PROPERTIES:",
          f":ID:       {node_id(conversation, reuse_uuid)}",
      ]
      created = org_timestamp(conversation.get("created_at"))
      updated = org_timestamp(conversation.get("updated_at"))
      ...
      lines += [":SOURCE:   Claude.ai", ":END:", f"#+title: {title}"]
      if tags:
          lines.append("#+filetags: :" + ":".join(tags) + ":")
      lines.append("")
      for message in conversation.get("chat_messages") or []:
          lines.append(render_message(message, heading_per_message))
          lines.append("")
      return "\n".join(lines).rstrip() + "\n"


  def export(json_file: Path, directory: Path, tags: list[str], reuse_uuid: bool,
             heading_per_message: bool) -> int:
      """Convert every conversation in json_file into a node file.

      Return the number of node files written.
      """
      ...
      for conversation in conversations:
          title = conversation_title(conversation)
          path = output_filename(conversation, title, directory)
          content = render_conversation(conversation, tags, reuse_uuid,
                                        heading_per_message)
  ...
  def main(argv: list[str] | None = None) -> int:
      ...
      parser.add_argument("directory", type=Path,
                          help="output directory for .org nodes")
      parser.add_argument("--tag", dest="tags", action="append", default=None,
                          help="filetag to apply (repeatable; default: claude)")
      parser.add_argument("--fresh-uuid", dest="reuse_uuid", action="store_false",
                          help="generate new IDs instead of reusing Claude uuids")
      parser.add_argument("--no-heading-per-message", dest="heading_per_message",
                          action="store_false",
                          help="concatenate messages instead of one heading each")
      parser.set_defaults(reuse_uuid=True, heading_per_message=True)
      args = parser.parse_args(argv)

      if not args.json_file.exists():
          parser.error(f"JSON file not found: {args.json_file}")
      tags = args.tags if args.tags is not None else ["claude"]

      count = export(args.json_file, args.directory, tags,
                     args.reuse_uuid, args.heading_per_message)
      print(f"claude-org-roam: wrote {count} node(s) into {args.directory}")
      print("Next: run M-x org-roam-db-sync in Emacs to register the nodes.")
      return 0
...
* Appendix: tangle-flow.mmd — tangle フローの図解ソース（Mermaid）
:PROPERTIES:
:CUSTOM_ID: appendix-tangle-flow-mmd
:header-args:mermaid: :tangle svg/tangle-flow.mmd :mkdirp yes
:END:

Overview 節の [[file:svg/tangle-flow.svg]] は元々 Mermaid Live Editor 等で作った手動アセットで、
本文中に再現可能なソースが存在しなかった。ここに Mermaid ソースを置き、 =make mmd-svg= で
同じファイルへ再レンダリングできるようにする。

#+begin_src mermaid
  flowchart LR
      src["README.org<br/>(single source of truth)"]

      subgraph tangle["org-babel-tangle"]
          extract["extract<br/>src blocks"]
      end

      ei["early-init.el"]
      ii["init.el"]
      mo["lisp/modules.el"]
      mods["lisp/&lt;layer&gt;/&lt;module&gt;.el<br/>(89+ files)"]
      ...
      classDef src fill:#EEEDFE,stroke:#534AB7,color:#3C3489
      classDef tangle fill:#E1F5EE,stroke:#0F6E56,color:#085041
      classDef config fill:#FAEEDA,stroke:#BA7517,color:#633806
      classDef modules fill:#FAECE7,stroke:#993C1D,color:#712B13

      class src src
      class extract tangle
      class ei,ii,mo config
      class mods modules
#+end_src

=make mmd-svg= は =mmdc= （ =npm install -g @mermaid-js/mermaid-cli= ）を使って
=svg/tangle-flow.mmd= を =svg/tangle-flow.svg= へレンダリングする。サンドボックス環境で
Puppeteer が Chrome を自動検出できない場合は、 =PUPPETEER_CONFIG= に =executablePath= を
書いた JSON ファイルのパスを渡すこと（Makefile 冒頭のコメント参照）。

* Appendix: emacs_config_layer_architecture.dot — レイヤーアーキテクチャ図の近似ソース
:PROPERTIES:
:CUSTOM_ID: appendix-emacs-config-layer-architecture-dot
:header-args:dot: :tangle svg/emacs_config_layer_architecture.dot :file svg/emacs_config_layer_architecture.svg :cmdline -Tsvg
:END:

Architecture 節の [[file:svg/emacs_config_layer_architecture.svg]] も手動アセットだった。
Graphviz で近い内容を再現したソースをここに置く。ただし元絵は座標を手動で調整した精密な
レイアウト（サブ層の横並びの間隔など）なので、Graphviz の自動配置に置き換わる分、
見た目は完全には一致しない。あくまで内容（10 層構成・依存関係・key tenets）を
再現するためのものである。

#+begin_src dot
  digraph emacs_config_layer_architecture {
    rankdir=TB;
    bgcolor="white";
    fontname="Noto Sans CJK JP";
    node [shape=box style="rounded,filled" fontname="Noto Sans CJK JP" fontsize=11];
    edge [color="#73726C"];

    ei [label="early-init.el\nGC deferral / dirs / package.el off / macOS toolchain / early UI" fillcolor="#E1F5EE" color="#0F6E56" fontcolor="#085041"];
    ii [label="init.el\nstraight.el / leaf / org / exec-path-from-shell / gcmh / personal overlay" fillcolor="#E1F5EE" color="#0F6E56" fontcolor="#085041"];
    mo [label="modules.el — orchestration only\nordered require / skip+extra hooks / per-module timing / error isolation" fillcolor="#EEEDFE" color="#534AB7" fontcolor="#3C3489"];

    subgraph cluster_layers {
      label="Layers";
      style=invis;
      {rank=same; core; ui; completion; orgx; dev; vcs; utils;}
      core       [label="core\nruntime"     fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      ui         [label="ui\nvisual"        fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      completion [label="completion\nvertico etc" fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      orgx       [label="orgx\norg ext"     fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      dev        [label="dev\ntooling"      fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      vcs        [label="vcs\nmagit"        fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      utils      [label="utils\nhelpers"    fillcolor="#E6F1FB" color="#185FA5" fontcolor="#0C447C"];
      core -> ui -> completion -> orgx -> dev -> vcs -> utils [style=invis];
    }

    personal [label="personal/ — optional overlay, loaded last\nuser.el / <login>.el / must not mutate earlier layers" fillcolor="#F1EFE8" color="#5F5E5A" fontcolor="#444441"];
    note [label="Dependency rule: upper layers may use lower;\nlower must never use upper" shape=note fillcolor="#FAEEDA" color="#854F0B" fontcolor="#633806"];
    tenets [label="Key tenets: deterministic / explicit deps / layer ownership /\ndelete-safe modules / public-API only / setopt for defcustom" shape=note fillcolor="#F1EFE8" color="#5F5E5A" fontcolor="#444441"];

    ei -> ii -> mo -> core;
    utils -> personal;
    personal -> note [style=dashed arrowhead=none];
    note -> tenets [style=dashed arrowhead=none];
  }
#+end_src

* Appendix: emacs_keybindings_cheatsheet_concept.dot — キーバインド概念図
:PROPERTIES:
:CUSTOM_ID: appendix-emacs-keybindings-cheatsheet-concept-dot
:header-args:dot: :tangle svg/emacs_keybindings_cheatsheet_concept.dot :file svg/emacs_keybindings_cheatsheet_concept.svg :cmdline -Tsvg
:END:

ui 節が参照する [[file:svg/emacs_keybindings_cheatsheet.svg]] は、フォントをアウトライン化して
ベクター化した文書（テキスト要素を持たない、キーバインド一覧の全ページ）であり、Graphviz や
Mermaid で再現できる種類の図ではない。この Appendix はその代替ソースではなく、
=ui-leader.el= の =M-SPC= リーダー構成（13 サブマップ）とグローバル単発和音、Embark を
簡略化した概念図として別ファイルに書き出すためのものである。 =emacs_keybindings_cheatsheet.svg=
そのものは今後も手動管理のアセットのままとする。

#+begin_src dot
  digraph keybindings_concept {
    rankdir=TB;
    bgcolor="white";
    fontname="Noto Sans CJK JP";
    node [shape=box style="rounded,filled" fontname="Noto Sans CJK JP" fontsize=11];
    ...
    root [label="Keybindings" fillcolor="#F1EFE8" color="#5F5E5A" fontcolor="#2C2C2A"];

    leader [label="M-SPC leader\n13 submaps: a b c e g h m o p q s t w" fillcolor="#E1F5EE" color="#0F6E56" fontcolor="#04342C"];
    global [label="グローバル単発和音\ns-* (macOS) / C-c* / M-g g" fillcolor="#FAEEDA" color="#854F0B" fontcolor="#633806"];
    embark [label="Embark\ns-/ act / s-; dwim / C-<f2> bindings" fillcolor="#EEEDFE" color="#534AB7" fontcolor="#3C3489"];

    root -> leader;
    root -> global;
    root -> embark;

    la [label="a apps" fillcolor="#F1EFE8" color="#5F5E5A" fontcolor="#2C2C2A"];
    lb [label="b buffer"]; lc [label="c code"]; le [label="e embark/eval"];
    lg [label="g git"]; lh [label="h help"]; lm [label="m major-mode"];
    lo [label="o org"]; lp [label="p project"]; lq [label="q quit"];
    ls [label="s search"]; lt [label="t toggle"]; lw [label="w window/frame"];

    leader -> la; leader -> lb; leader -> lc; leader -> le; leader -> lg;
    leader -> lh; leader -> lm; leader -> lo; leader -> lp; leader -> lq;
    leader -> ls; leader -> lt; leader -> lw;
  }
#+end_src

本文中の =svg/NN_*.svg= はすべて Graphviz（ =dot= ）で生成している。全 13 枚分の
=.dot= ソースを、通し番号順にここへまとめて置く。各コードブロックは =:tangle= で
=svg/NN_*.dot= として書き出せる。SVG 自体を再生成したいときは、tangle 後に
=dot -Tsvg svg/NN_*.dot -o svg/NN_*.svg= を実行するか（=make dot-svg= が一括で行う）、
Org バッファ内で =C-c C-c= すれば =:file= 指定に従って同じ場所へ直接出力される。日本語
ラベルが =dot= のデフォルトフォントで文字化け・box からのはみ出しを起こす環境では、
=fontname= を ="Noto Sans CJK JP"= 等の CJK 対応フォントに差し替えること（以下の図は
すべてこのフォントで生成済み）。それぞれの図は [[#modular-loader-and-core-suite][Modular
Loader & Core Suite]] 内の対応するモジュール節（起動フロー・=core-session.el= ・
=core-gc.el= ・自動保存・auth-gpg.el・completion-capf-org-src.el・dev-lsp-bridge.el・
utils-claude.el・orgx-auto-tangle.el・ui-leader.el・dev-ai.el・personal Overview）から
インライン画像として参照されている。

* Appendix: 01_boot_flow.dot — 起動シーケンス（3フェーズ）
:PROPERTIES:
:CUSTOM_ID: appendix-01-boot-flow-dot
...
* Appendix: 02_condition_case.dot — modules.el の condition-case 隔離
:PROPERTIES:
:CUSTOM_ID: appendix-02-condition-case-dot
...
* Appendix: 03_session_facade.dot — core-session の Facade パターン
:PROPERTIES:
:CUSTOM_ID: appendix-03-session-facade-dot
...
* Appendix: 04_gc_strategy.dot — GC Strategy 3層協調
:PROPERTIES:
:CUSTOM_ID: appendix-04-gc-strategy-dot
...
* Appendix: 05_autosave_flow.dot — Auto-Save 3機構の合流
:PROPERTIES:
:CUSTOM_ID: appendix-05-autosave-flow-dot
...
* Appendix: 06_auth_resolution.dot — auth 解決フロー（二段構えの安全策）
:PROPERTIES:
:CUSTOM_ID: appendix-06-auth-resolution-dot
...
* Appendix: 07_capf_dispatch.dot — capf-org-src ファミリーのディスパッチ
:PROPERTIES:
:CUSTOM_ID: appendix-07-capf-dispatch-dot
...
* Appendix: 08_lsp_agnostic.dot — dev Layer の LSP非依存設計
:PROPERTIES:
:CUSTOM_ID: appendix-08-lsp-agnostic-dot
...
* Appendix: 09_utils_claude.dot — utils-claude.el の HTTPクライアント
:PROPERTIES:
:CUSTOM_ID: appendix-09-utils-claude-dot
...
* Appendix: 10_orgtangle_hugo.dot — orgx の auto-tangle / auto-hugo
:PROPERTIES:
:CUSTOM_ID: appendix-10-orgtangle-hugo-dot
...
* Appendix: 11_leader_keymap.dot — ui-leader.el の M-SPC サブマップ
:PROPERTIES:
:CUSTOM_ID: appendix-11-leader-keymap-dot
...
* Appendix: 12_dev_ai_timing.dot — dev-ai.el の認証タイミング cond ロジック
:PROPERTIES:
:CUSTOM_ID: appendix-12-dev-ai-timing-dot
...
* Appendix: 13_personal_override_contract.dot — personal/ のロード順と defvar 上書きの契約
:PROPERTIES:
:CUSTOM_ID: appendix-13-personal-override-contract-dot
...
digraph personal_override_contract {
  ...
  s1 [label="init.el §9a — personal/user.el\nmy:modules-extraにsetqで追記"];
  s2 [label="init.el §9b — personal/ac1965.el\n(if (my:wsl-p) ...) でOS分岐しつつ\nmy:d:org*にsetqで先に値を設定"];
  s3 [label="init.el §10 — (require 'modules)\nmodules.elが全モジュールをロード開始"];
...
:EXPORT_OPTIONS: toc:nil num:nil author:nil creator:nil date:nil
:END:

本 Changelog は 2026-07 にリセットされ、以後は Fix ID 方式を廃止した和文エントリのみを記録する
（経緯は本節内「Changelog のリセットと Fix ID 方式の廃止」を参照）。

また、本リポジトリは 2026-09-10 に再作成されており、それ以前の変更はすべて初回コミット
（=feat: リポジトリ初期設定とドキュメント追加=）に一括で取り込まれている。そのため、本節の
エントリのうち初回コミットより前に相当する部分は、=git log= 上の個別コミットとは対応しない。
初回コミット以降に追加されたエントリのみが、実際のコミット単位の変更履歴と一致する。

** dirvish のカレント行ハイライトが表示されない不具合を修正

dirvish は =dired-noselect= の初回呼び出し時まで遅延 require される設計のため、
起動時に一度だけ実行される =my/nano-apply-faces=（=ui-nano-palette.el=）の時点では
=dirvish-hl-line= 等のフェイスがまだ =defface= されておらず、=ui-nano--safe-face=
の facep チェックにより黙ってスキップされていた。dirvish は既定でカーソル自体を
非表示にし、このフェイスのオーバーレイのみでカレント行を示す設計であるため、
パレットが未適用のまま dirvish を開くとカレント行が全く視認できなくなっていた。

=ui-nano-palette.el= に =(with-eval-after-load 'dirvish (my/nano-apply-faces))= を
追加し、dirvish が実際にロードされたタイミングでパレットを再適用するようにした。
...
** *scratch* での flymake 警告（Emacs 30.1 の trusted-content）
...
Emacs 30.1 が導入した =trusted-content= による、設計どおりの動作である。
=elisp-flymake-byte-compile= はバイトコンパイルを走らせるが、バイトコンパイルはマクロを
展開し =eval-when-compile= を評価する。すなわち *コードを実行する* 。信頼できない内容に
対してこれを自動実行するのは危険なため、Emacs は =trusted-content-p= が nil のバッファで
このバックエンドを無効化する。

=utils-lint= は =:hook ((prog-mode-hook . flymake-mode))= としていた。
=lisp-interaction-mode= （ =*scratch*= ）は =prog-mode= 派生なので flymake が有効になるが、
=trusted-content-p= は =buffer-file-truename= が nil のバッファを *決して* 信頼しない。
=*scratch*= はファイルを訪問していないため、 =trusted-content= に何を列挙しても信頼されない。

*対処*

=trusted-content= に =:all= を設定すれば警告は消えるが、docstring が「gaping security hole」と
明記しており採用しない。 =*scratch*= で flymake を有効化しなければ済む。

- =utils-lint.el= — =prog-mode-hook= に繋ぐ関数を =utils-lint--enable-flymake-maybe= に変更し、
  =buffer-file-name= が非 nil のバッファでのみ =flymake-mode= を有効化する。診断は
  =C-x C-e= / =C-j= での評価時に得られるため、 =*scratch*= で失うものは無い。
- =core-policy.el= — =trusted-content= に =lisp/= 、 =personal/= 、 =early-init.el= を登録した。
  これを宣言しないと、自分の設定ファイルを編集するたびに flymake のバイトコンパイル・
  バックエンドが無効化される。 =init.el= （ =user-init-file= ）は Emacs が無条件に信頼するため
  列挙不要である。ディレクトリ要素は末尾が =/= でなければならず、値は
  =abbreviate-file-name= 形式で与える必要がある。

** org-roam を起動時にロードしない構成へ

先の「Org の遅延ロード」で orgx 層の起動時ロード経路は塞いだが、既定構成では org-roam の
autosync が起動 5 秒後に org-roam → org を引き込むため、Org は結局ロードされていた。これを
やめ、org-journal / org-download / toc-org / org-cliplink は残したまま、org-roam だけを
遅延させた。

*変更点*

- =orgx-extensions= — =leaf org-roam= の =:config= にあった
  =(run-with-idle-timer 5 nil #'orgx--org-roam-autosync-enable)= を削除した。
  =org-roam-db-autosync-mode= を有効化するには org-roam のロードが必要で、org-roam は org を
  require する。このタイマーが「起動直後に Org を読む」経路そのものだった。
  代わりに対話コマンド =my/org-roam-autosync-enable= を新設した。 =org-roam-node-find= 等の
  エントリポイントは autoload されているため、初回呼び出し時に一度だけ DB が同期される。
  常時 autosync が必要なときだけこのコマンドを実行する。
- =personal/user.el= — 既定の =my:modules-extra= から =orgx-roam-ui= を外した。org-roam-ui は
  モジュールのロード時に依存を導入し、 =org-roam= → =org= を引き込むためである。グラフは
  =M-x org-roam-ui-open= （autoload 済み）で必要時に開ける。

*効果*

これで既定構成でも、Org（および org-roam）は次のいずれかまでロードされない。 =.org= を開く／
Org のコマンドを起動する／org-roam のコマンドを起動する／ =my/org-roam-autosync-enable= を
実行する。コード編集のみのセッションや =emacsclient -t= での短時間起動では、Org の初期化コストが
まるごと消える。

*元に戻すには*

グラフと常時 autosync を使う運用に戻す場合は、(1) =personal/user.el= の =my:modules-extra= に
=orgx-roam-ui= を加え、(2) 必要なら =my/org-roam-autosync-enable= を =emacs-startup-hook= や
アイドルタイマーから呼ぶ。

** core-editing: show-paren-not-in-comments-or-strings のクォート漏れ

起動時に次のエラーで =core-editing= のロードが失敗していた。

#+begin_example
[modules] Failed to load core-editing: Symbol's value as variable is void: on-mismatch
#+end_example

*原因*

=show-paren-not-in-comments-or-strings= （Emacs 31.1 の defcustom）に、シンボル値
='on-mismatch= を渡すべきところ、クォートが抜けて =on-mismatch= という *変数参照* に
なっていた。 =on-mismatch= という変数は存在しないため =void-variable= となり、 =leaf= の
=:config= が中断して以降の設定が適用されなかった。

削除安全性により =core-editing= の失敗は他モジュールに波及しないが、当該モジュールの
=:config= 後半（dirvish 設定など）が丸ごと未適用になっていた。

*対処*

='on-mismatch= とクォートした。ついでにコメントの記述も実際の値に合わせた。既定の
=on-mismatch= は「コメント／文字列内では、対応の取れない括弧だけを非強調にする」であり、
「常に非強調」は ='all= である（旧コメントは 'all の説明になっていた）。

Emacs master から抽出した型情報を用い、他の =setopt= に同種のクォート漏れが無いことも
確認した。値に裸のシンボルを渡している箇所は他に 7 件あるが、いずれも束縛済みの変数
（ =my:d:treesit= 、関数引数の =mode= 、 =let= 束縛の =new-list= 等）を意図的に渡すもので、
問題ない。

** 効かない Org 強調記法の検出（Makefile: check-emphasis）

「見出しレベル 1 段違いで tangle が壊れる」class の事故に続き、今度は「全角文字に接した
=verbatim= / *bold* が Org 上で効かない」という、tangle には影響しないが *ドキュメントとして
静かに壊れる* class の欠陥が見つかった。

*症状*

=org-emphasis-regexp-components= の既定値は、強調マーカーの直前・直後に許される文字の集合を
定めている（直前側: 半角空白・タブ・- – — ( " ' { ・行頭。直後側: 半角空白・タブ・
- – — . , : ! ? ; ' " ) } [ ・行末）。全角文字（、。（）「」等）はこの集合に含まれない。
したがって和文に接した =code= や *太字* はマーカーとして解釈されず、リテラルの記号として
そのまま表示される。

*経緯: Elisp 実装 → 実機で 2 件の不具合 → Python へ置き換え*

最初の実装は =scripts/check-emphasis.el= （ =--load= で読み込む Elisp スクリプト）だった。
本リポジトリの開発環境には Emacs 本体が無く、ロジックは Python への一時的な移植で
検証したものの、Elisp のバッチモード特有の挙動までは検証できなかった。実機
（ =make lint= ）で 2 件の不具合が報告された。

1. =:header-args:emacs-lisp:= に付けていた =:comments link= が、tangle 時にコード本体の
   前へ =;; [[file:README.org::...][...]]= という逆参照リンク行を挿入し、
   =-*- lexical-binding: t; -*-= を 1 行目から追い出していた。
   =scripts/reload.py= 等の Python スクリプトから実装をコピーした際、意識せず
   引き継いでしまったオプションである。
2. =emacs -Q --batch --load FILE.el -- ARGS= という起動方法では、 =--load= のハンドラが
   ファイルを見つけた時点で同期的に =load= を実行するため、読み込まれたファイルの
   トップレベルフォームが走る間、区切りの ="--"= 自体が =command-line-args-left= の
   先頭に残っている場合がある。 =(car command-line-args-left)= が ="--"= になり、
   =file-exists-p= が偽になって usage エラーで落ちていた。

いずれも「実行して確かめられない環境で書いた Elisp」に起因する。Python なら本環境で
実際に実行・検証できるため、 =scripts/check_emphasis.py= として書き直した。ロジックは
Elisp 版と同一で、 =scripts/reload.py= と同じく標準ライブラリのみに依存する。

*検出方法*

=/= verbatim と ~code~ を先に確定させて保護領域とし、 =*bold*= はそれと重ならない候補
だけを検査する。 =*scratch*= のように verbatim の内側にある =*= を誤って bold 候補と
見なさないよう、直前・直後が =/~ の場合は除外する。コードブロック本文・見出し行・
drawer/keyword/table 行は検査対象から除外する。

*Python 版で実際に検出した回帰*

書き換えた =check_emphasis.py= を =README.org= 自身に対して実行したところ、直近の
Changelog エントリ（この節を含む）に和文と =code= が直接隣接する箇所が 13 件見つかった。
Elisp 版では実行して確かめられなかったため気づけなかった回帰であり、Python 化した
直接の効果として解消した。

** Emacs 30/31 の廃止シンボルへの追随

Emacs master（32.0.50）の Lisp ツリーから =make-obsolete= / =make-obsolete-variable= /
=define-obsolete-*-alias= を全件抽出し（1,717 件）、tangle 対象の全コードと突き合わせた。
使用されていた廃止シンボルは 11 件で、うち 5 件が真の不整合、6 件は誤検知だった。

*修正した 5 件*

| シンボル | 廃止 | 対処 |
|------+------+------|
| =native-comp-deferred-compilation= | 29.1 | =native-comp-jit-compilation= に改名。 =boundp= でガード |
| =idle-update-delay= | 30.1 | 削除。後継の =which-func-update-delay= は which-func 専用であり、本設定は which-func を使わない |
| =org-catch-invisible-edits= | Org 9.6 | =org-fold-catch-invisible-edits= に改名 |
| =org-show-subtree= | Org 9.6 | =org-fold-show-subtree= に改名 |
| =focus-out-hook= | 27.1 | =after-focus-change-function= に統一（下記） |

*focus-out-hook の廃止に伴う、潜在バグの発見*

=focus-out-hook= は Emacs 27.1 で廃止されたが、 =handle-focus-out= が現在も実行するため
「動いてはいた」。しかし移行の過程で 2 つの欠陥が入り込んでいた。

1. *二重保存* — =core-editing= は =focus-out-hook= と =after-focus-change-function= の
   *両方* に =core-editing--save-on-focus-out= を登録していた。 =handle-focus-out= は
   =focus-out-hook= を実行したうえで =after-focus-change-function= も呼ぶため、フォーカスを
   失うたびに全バッファの保存処理が二重に走っていた。

2. *フォーカス取得時にも保存* — =after-focus-change-function= はフォーカスの取得時にも
   呼ばれる。 =frame-focus-state= による判定が無かったため、他のアプリから Emacs へ戻る
   たびにも保存が走っていた。

=core-editing--save-on-focus-out= に =(not (frame-focus-state))= の判定を加え、
=focus-out-hook= への登録を撤去した。 =core-gc= も =core-gc--on-focus-change= を新設し、
=after-focus-change-function= へ =add-function= で登録するよう改めた
（ =after-focus-change-function= は「フック」ではなく単一の関数を保持する変数であるため、
=add-hook= ではなく =add-function= が正しい）。

*誤検知だった 6 件*

=font-lock-comment-face= 等 6 つの =font-lock-*-face= は、Emacs 31.1 で *変数* が廃止された。
廃止メッセージ自体が「クォートしたシンボルを使え」と指示しており、フェイス名としての
=font-lock-comment-face= は現役である。 =ui-nano-palette= はフェイス名を渡しているため
推奨形であり、修正不要。

** tangle 先を継承できない src ブロックの検出（Makefile: check-tangle）

Org の =:header-args:= はサブツリーにのみ継承される。見出しレベルを 1 段間違えて
=**** foo Design Notes= を =*** foo Design Notes= と書くと、親（ =*** foo= ）の *兄弟* に
なってしまい、その配下の src ブロックが =:tangle= を受け取れない。

この事故の厄介な点は *静かに失敗する* ことである。 =org-babel-tangle= はエラーを出さず、
当該 =.el= だけが生成されない。古い =.el= がディスクに残っていればロードされ続けるため、
症状も出にくい。実際に 2 回踏んだ（ =init.el= 単独、その後 =early-init.el= / =init.el= /
=modules.el= の 3 件同時）。

=make check-tangle= を追加した。 =README.org= を Org のパーサで走査し、実効 =:tangle= が
nil または ="no"= の emacs-lisp ブロックを行番号付きで列挙して失敗する。 =make lint= の
先頭に組み込んだので、 =lint= を通せばこの class の事故は止まる。
...
** 起動プロファイリング用チェックポイントの追加（診断用、一時的）

起動ログの ~[modules] loaded:79 ... total:253.0ms~ と、末尾の
=Emacs ready in 1.11 seconds= の間に約 860ms の説明できない差があった。この差は
=[modules]= ループの外側、つまり straight.el / leaf のブートストラップ、
=exec-path-from-shell=、personal オーバーレイの読み込みなどのどこかに存在するが、
従来のログにはこの区間の内訳が一切出力されていなかった。

=core--boot-checkpoint= を追加し、 =before-init-time= からの経過ミリ秒を
early-init.el と init.el の主要な区切り（URL 状態ディレクトリ、straight
ブートストラップ、leaf ブートストラップ、exec-path-from-shell、gcmh、コア
組み込みポリシー、personal オーバーレイ、 =(require 'modules)= の前後）で記録する
ようにした。 =core--announce-startup= が起動完了時にこれを =*Messages*= へ
一覧表示する。

これは恒久的な機能ではなく、区間内訳を特定するための一時的な計装である。原因が
判明した時点で、本エントリが指すすべての =core--boot-checkpoint= 呼び出しと、
=core--boot-checkpoints= / =core--boot-checkpoint= / =core--report-boot-profile=
の定義そのものを、early-init.el と init.el の両方から削除してよい。

** straight.el ブートストラップの起動時コストを解消（起動時間を約半減）

起動プロファイル計測（前項）の結果、起動全体 1182.7ms のうち 619.9ms（約52%）が
straight.el のブートストラップ区間（=init:url-done= から =init:straight-done=
まで）に集中していることが判明した。これは =[modules]= ループ全体
（79モジュール、297.5ms）よりも大きい。

原因は =straight-check-for-modifications= の既定値
=(find-at-startup find-when-checking)= である。=find-at-startup= は起動のたびに
管理下の全パッケージへ =find= によるファイル変更チェックを実行する。管理下の
パッケージを Emacs の外から手動編集する運用ではないため、この全件スキャンは
実質的に無駄なコストだった。

=early-init.el= の straight.el ベース設定に =straight-check-for-modifications=
='(check-on-save find-when-checking)= を追加し、=find-at-startup= を外した。
以後、変更チェックはファイル保存時のみに限られる。手動で clone 先を直接編集した
場合は自動検出されなくなるが、その場合は =M-x straight-rebuild-package= または
=straight-pull-all= で明示的にリビルドすればよい。

次回起動時の起動プロファイルで =init:url-done= から =init:straight-done= までの
区間が大幅に縮んでいることを確認すること。

** [modules] ループ内の3パッケージを起動後アイドルタイマーへ遅延（pulsar / apheleia / project.el / editorconfig）

straight.el のブートストラップ修正（前項）後、起動プロファイル計測で
=[modules]= ループが起動全体の過半（282.9ms / 552.5ms）を占めるようになった。
うち上位の =ui-visual-aids=（18.7ms）、=dev-format=（13.8ms）、
=dev-web-core=（12.8ms）の3モジュールに共通する原因は同一だった: leaf の
=:custom= 、 =:config= 、 =:global-minor-mode= はいずれも暗黙に =:require t= を
伴うため、モジュールのロード時（=[modules]= ループの反復中）にパッケージ本体が
同期的に require されていた。

対象は pulsar（=ui-visual-aids.el=）、apheleia（=dev-format.el=）、
project.el と editorconfig（いずれも =dev-web-core.el=）の4パッケージである。
いずれも「保存時に整形する」「ナビゲーション後に光る」「ファイルを開いたときに
適用される」という性質上、起動完了の一瞬後に有効化されても実用上の差が無い。

各モジュールの該当 =leaf= 宣言を =:require nil= に変更し、実際の require と
有効化ロジックを =<module>--activate-*= という名前付き =defun= に切り出した
うえで、 =<module>--schedule-*= が =emacs-startup-hook= から
=(run-with-idle-timer 0 nil #'<module>--activate-*)= を仕込む形にした。
=org-roam= の autosync や =orgx-roam-ui= のアイドルタイマー起動と同じ、
本設定で既に使われているパターンである。パッケージの導入（straight による
取得・ビルド）自体は =:straight t= がここでも保証するため、初回起動時の
挙動に変化は無い。

=dirvish-override-dired-mode=（=core-editing.el=）は同様の遅延化を検討したが、
override モードの性質上、最初に dired を開く前に有効化されている必要があり、
単純なアイドルタイマーでは間に合わない可能性があるため、今回は見送った。
別途 advice ベースの実装を検討する。

** dev-web-core: project-prune-zombie-projects の型不一致を修正

前項の変更を実機に適用したところ、起動後に次の警告が出た。

#+begin_example
⛔ Warning (emacs): Value does not match project-prune-zombie-projects's type
‘alist’: t
#+end_example

=project-prune-zombie-projects= の =custom-type= は、Emacs のビルドによって
boolean と alist のどちらにもなり得る。旧来 boolean 前提で =(setopt
project-prune-zombie-projects t)= と直書きしていたが、この Emacs 31 ビルドでは
alist 型になっており、型検証エラーになっていた。

正しい alist の形（各要素が何を意味するか）を確認できていないため、値を
推測で埋めることはせず、 =dev-web-core--activate-project= の中で
=(get 'project-prune-zombie-projects 'custom-type)= を実行時に確認し、
boolean のビルドでのみ =t= を設定するよう変更した。alist 型のビルドでは
何もせず既定値に委ねる。正しい alist の形は =C-h v
project-prune-zombie-projects= で個別に確認し、必要になった時点で対応する。

** core-editing: dirvish を advice ベースで遅延ロード

=core-editing= の起動プロファイル計測値（29〜30ms）の主要因は dirvish だった。
leaf の =:global-minor-mode dirvish-override-dired-mode= は暗黙に =:require t=
を伴い、モジュールのロード時（=[modules]= ループの反復中）に dirvish 本体を
同期的に require していた。

pulsar / apheleia / project.el（前2項）とは異なり、dirvish は固定秒数の
アイドルタイマーでは遅延できない。 =dirvish-override-dired-mode= は
「dired を呼んだら代わりに dirvish を開く」という置き換えそのものであり、
ユーザーが最初に dired を開く *前* に有効化されていなければならないためである。

=dired-noselect= に自己解除型の =:around= advice
（=core-editing--ensure-dirvish=）を仕込んだ。 =dired= 、 =dired-jump= 、
=project-dired= 、 =find-file= のディレクトリ分岐など、dired を呼ぶほぼ全ての
経路は最終的に =dired-noselect= を通るため、ここ一箇所へのアドバイスで
束ねられる。

実装上の注意点が一つある。 =dirvish-override-dired-mode= 自身も
=dired-noselect= に advice を追加するため、ブートストラップ用の advice
（_ORIG-FN、アドバイス追加時点のスナップショット）をそのまま呼ぶと、
今まさに処理している最初の呼び出しだけ dirvish の advice を素通りしてしまい、
2 回目以降の呼び出しからしか dirvish の UI にならない。これを避けるため、
=core-editing--ensure-dirvish= は自分自身のアドバイスを除去したうえで
=core-editing--activate-dirvish=（dirvish の require・override モード有効化・
表示設定一式）を実行し、_ORIG-FN ではなくシンボル =dired-noselect= を改めて
=apply= で呼び直している。これにより、その時点の最新の advice チェーン
（dirvish 自身が追加したものを含む）を確実に通る。

=dirvish-attributes= 、 =dirvish-mode-line-format= 、 =dirvish-cache-dir= 、
ls スイッチの設定はいずれも =core-editing--activate-dirvish= の内部に
移動し、旧来 =leaf :custom= / =:config= に書いていた内容と機能的に同一である。

** 起動プロファイルに累積 GC 回数を追加（診断用、一時的）

最初の起動プロファイリング（前々項）で挙げた「起動時の 4 回の GC がどこで
発生しているか」を、これまで未検証のまま放置していた。=core--boot-checkpoint=
が記録する各チェックポイントの要素を =(LABEL . ELAPSED-MS)= から
=(LABEL ELAPSED-MS GCS-DONE)= に拡張し、その時点までの累積 =gcs-done= も
あわせて記録するようにした。=core--report-boot-profile= の出力にも
~gcs=N~ を追加した。

呼び出し側（各所の =(core--boot-checkpoint "label")=）の引数は変わらないため、
このファイル内の呼び出し箇所自体に変更は無い。次回の起動プロファイルで、
どの区間で GC 回数が増えているかを確認すること。

** early-init.el: GC 閾値の引き上げを最初のセクションへ移動（4回のGCを解消）

前項で追加した計測により、起動時の 4 回の GC がどこで起きているかが判明した。

#+begin_example
early-init:start    2.5 ms  gcs=0
early-init:end      39.0 ms  gcs=4
init:url-done       200.4 ms  gcs=4
  ...（以降 modules ループ完了まで一貫して gcs=4）
#+end_example

4 回とも =early-init.el= の実行中（2.5ms 〜 39.0ms の間）に発生しており、
それ以降の起動シーケンス（straight/leaf のブートストラップ、personal
オーバーレイ、 =[modules]= ループ）では 1 回も GC が発生していなかった。

原因は、 =gc-cons-threshold= を 512MB へ引き上げる処理が、従来
「8) 起動性能」として、ディレクトリ作成（旧 2〜3）、straight.el のベース設定
（旧 4）、Homebrew ツールチェーン検出（旧 5）、native-comp 設定（旧 6）よりも
*後* に置かれていたことである。これらの処理はすべて Emacs の既定の GC 閾値
（通常 800KB 程度）のまま実行されており、その間に GC が走っていた。閾値
引き上げ自体がボトルネックだったのではなく、置き場所が遅すぎたことが原因
である。

GC 閾値と =file-name-handler-alist= の初期化を =early-init.el= の最初の
セクション（ディレクトリ作成より前）へ移動した。これに伴い、旧「1) package.el
の無効化」以降のセクション番号をすべて 1 つずつ繰り下げ、新しい実行順序に
合わせて振り直した（新 1〜11、内容の入れ替えは無い）。

次回起動時の起動プロファイルで ~gcs=0~ のまま =[modules]= ループ完了まで
到達するか確認すること。ただし GC はメモリ確保量に依存するため、パッケージ
構成や開いているファイルの状況によっては 0 にならない場合もあり得る。

** 起動プロファイリング用チェックポイントの削除

一連の起動最適化（本 Changelog の直近 6 件）で診断対象がすべて解消したため
（起動全体 1.18 秒 → 0.49 秒、GC 4 回 → 0 回）、役目を終えた診断用計装を撤去した。

削除したもの:

- =early-init.el= の「0) 起動計測フック」セクション全体
  （=core--boot-checkpoints= 、 =core--boot-checkpoint= の定義、
  =(core--boot-checkpoint "early-init:start")= ）
- =early-init.el= 末尾の =(core--boot-checkpoint "early-init:end")=
- =init.el= 各所の =(core--boot-checkpoint "init:...")= 呼び出し 9 箇所
  （url-done、straight-done、leaf-done、exec-path-done、gcmh-done、
  core-policy-done、personal-done、modules-require-start、
  modules-require-done）
- =init.el= の =core--report-boot-profile= 、および
  =core--announce-startup= からのその呼び出し（元の
  「%.2f seconds with %d GCs」メッセージのみに戻した）

計測コードが指し示していた実際の問題（straight.el のブートストラップコスト、
pulsar/apheleia/project.el/editorconfig/dirvish の起動時同期ロード、GC 閾値
引き上げの位置）はいずれも本 Changelog の該当エントリで修正済みであり、
今回削除したのは計測のための足場のみである。恒久的な修正内容
（=straight-check-for-modifications= 、各モジュールの遅延ロード、
=early-init.el= のセクション順序）は残っている。

** daemon 化の事前調査で見つかった 2 件の daemon 対応バグを修正

=emacs --daemon= 運用への移行を検討するにあたり、フレーム／ウィンドウシステムに
依存する箇所を =ui-theme.el= （daemon 対応済みの模範例。 =after-make-frame-functions=
で毎フレーム適用＋ロード時に一度即時適用）と比較しながら洗い出したところ、
2 件の daemon 非対応バグが見つかった。いずれも「モジュールのロード時
（daemon 起動時、実フレームが無い状態）に一度だけ評価される」ことに起因する。

*1.* =ui-font.el= の =ligature=

=leaf ligature= の =:when (display-graphic-p)= は、モジュールのロード時に
一度だけ評価される。通常起動ではその時点で既にグラフィカルフレームがあるため
機能するが、 =--daemon= 起動時はロード自体がフレームの無い状態で行われるため、
=(display-graphic-p)= は常に =nil= と評価され、 =ligature= パッケージが
*永久に* ロードされなかった。

同ファイル内のフォント適用ロジック（ =ui-font--apply-to-frame= /
=after-make-frame-functions= ）と同じ「初回のグラフィカルフレームで一度だけ
有効化する」パターンに揃えた。 =leaf ligature= を =:require nil= にし、
=ui-font--activate-ligature= （require・リガチャ登録・ =global-ligature-mode=
有効化）と =ui-font--maybe-activate-ligature= （冪等ガード付きディスパッチ）に
分離し、 =(daemonp)= で =after-make-frame-functions= と =after-init-hook= を
出し分ける。

*2.* =early-init.el= の =core--fullscreen-once=

=window-setup-hook= は =--daemon= 起動時、実フレームが無い状態で走る
（デーモンはクライアント接続を待つだけで、起動時にフレームを作らない）。
そのまま =(set-frame-parameter nil ...)= すると存在しない／幽霊フレームに
適用され、かつ =core--did-fullscreen= フラグが立ってしまうため、後から
=emacsclient -c= で作られる実際のフレームには二度と適用されなくなる
「once ガードが本来の対象より先に消費される」バグだった。

=core--fullscreen-once= に =FRAME= 引数を追加し、 =(display-graphic-p frame)=
を確認してから初回に限り適用するよう変更した上で、 =(daemonp)= で
=after-make-frame-functions= （daemon）と =window-setup-hook= （通常起動）を
出し分けた。

*見送ったもの*

=ui-macos.el= の =system-taskbar-mode= （Dock 統合）は、daemon 起動時
（実フレーム無し）に呼んで正しく初期化されるか未確認。Dock 統合は通常
アクティブな GUI セッションを前提とすることが多く、疑わしい点ではあるが、
実機で daemon 起動して検証してから対応するかどうかを決める。今回は
推測で変更しなかった。

*確認できたこと*

=ui-theme.el= と =ui-font.el= のフォント適用ロジックは、既に
「デーモンに対して安全なフレーム処理」を明記した設計で書かれており、
=exec-path-from-shell= も =:when= 節に =(daemonp)= を含めて考慮済みだった。
今回の 2 件は、既に確立されていた daemon-safe パターンからの単純な逸脱で
あり、設計方針自体を daemon 対応にする作業ではない。

** daemon 実機テストで発覚した nano-modeline / nano-faces の色破壊バグを修正

前項の修正（ligature、core--fullscreen-once）を =emacs --daemon= +
=emacsclient -c= の実機で検証したところ、両方とも正しく動作した一方、
起動ログに次の警告と、後続の実害あるエラーが新たに見つかった。

#+begin_example
Warning: setting attribute ‘:background’ of face ‘nano-modeline-active’:
...
*原因（rougier/nano-modeline のソースで確認済み）*

=nano-modeline.el= は次のように、 =defface= の値を =backquote= で
*ロード時に一度だけ* 焼き込む実装になっている。
...
=--daemon= 起動時（実フレームが無い状態）で =(require 'nano-modeline)= が
走ると、 =(face-foreground 'default)= 等が =nil= を返し、その =nil= が
フェイスの色として *永久に* 焼き込まれる。 =defface= は一度評価されると
再評価されないため、後から =emacsclient -c= で本物のグラフィカルフレームが
繋がっても直らない。

=rougier/nano-emacs= の =nano-faces.el= （ =nano-face-faded= 等の警告）も、
同種の問題を抱えている。こちらは =defface= 自体は =nil= 初期値で安全だが、
実際の色は関数 =nano-faces= が =set-face-attribute= で設定する実装であり、
この関数もグラフィカルフレームの存在を前提にしている。

*修正*

- =ui-nano-modeline.el= の =ui-nano--setup= — =(require 'nano-modeline)= を
  行う前に =(display-graphic-p frame)= を確認するようガードした。
  =leaf nano-modeline= の =:init (add-hook 'after-init-hook ...)= を撤去し、
  =(daemonp)= で =after-make-frame-functions= （daemon）／
  =after-init-hook= （通常起動）を出し分ける、 =ligature= と同じパターンに
  揃えた。
- =ui-theme.el= の =ui-theme--init= — nano-faces の require・
  =(load-theme 'nano t)= ・ =(nano-faces)= 呼び出しを内包しており、同じ理由で
  daemon 起動時に色が壊れる。 =ui-theme--init-once-graphic= を新設し、
  =(daemonp)= で =after-make-frame-functions= （daemon、初回のグラフィカル
  フレームまで遅延）／ =emacs-startup-hook= （通常起動、従来どおり無条件
  呼び出しを維持）を出し分けた。通常起動でガードを追加しなかったのは、
  ターミナル（256色）でもテーマを適用する既存の挙動を変えないため。

*確認方法*

=emacs --daemon= の起動ログに =nano-modeline= / =nano-face-*= 系の
=nil value is invalid= 警告が出ないこと、および =emacsclient -c= で開いた
フレームで =Unable to load color= エラー無くモードラインが正しく配色される
ことを確認すること。

** completion-vertico: vertico-posframe が daemon で永久に効かないバグを修正

「posframe が効かない」という報告を受けて調査したところ、
=completion-vertico.el= に2つの daemon 対応バグが重なっていた。

*原因1: completion-vertico--init の一発除去ガード*

#+begin_src emacs-lisp
(defun completion-vertico--init ()
  (unless completion-vertico--initialized
    (setq completion-vertico--initialized t)   ; daemon起動時にここでtrueになる
    (vertico-mode 1)
    (marginalia-mode 1)
    (when (display-graphic-p)                   ; daemon起動時は常にnil
      (vertico-posframe-mode 1))
    (remove-hook 'window-setup-hook #'completion-vertico--init)))  ; 自身を除去
#+end_src

=--daemon= 起動時、 =window-setup-hook= は実フレームが無い状態で走る。
=(display-graphic-p)= が =nil= なので =vertico-posframe-mode= は有効化されず、
かつこの関数は「初期化済み」フラグを立てて自分自身をフックから除去して
しまうため、後から =emacsclient -c= で本物のフレームが繋がっても
二度と再試行されない。

*原因2: leaf vertico-posframe の :if*

=:if (display-graphic-p)= は =ligature= が抱えていたのと同じ問題で、
モジュールのロード時（daemon起動時、実フレーム無し）に一度だけ評価される
ため、 =vertico-posframe= パッケージ自体が daemon では永久にロードされ
なかった。

*修正*

=vertico-posframe= の有効化を =vertico-mode= / =marginalia-mode= の初期化
から切り離した。後者はミニバッファ補完でフレームに依存しないため、従来
どおり =window-setup-hook= で一度だけ初期化する。前者は
=completion-vertico--maybe-init-posframe= として独立させ、
=nano-modeline= ・ =ligature= と同じパターンで =(daemonp)= により
=after-make-frame-functions= （daemon）／ =window-setup-hook= （通常起動）を
出し分けた。 =leaf posframe= と =leaf vertico-posframe= はいずれも
=:require nil= にし、実際の require と =(setopt vertico-posframe-border-width 2)=
は =completion-vertico--activate-posframe= に集約した。

** daemon 対応バグの全件洗い出しと残り 3 件の修正

これまでに見つかった daemon 対応バグ（ligature、core--fullscreen-once、
nano-modeline、ui-theme、vertico-posframe）はいずれも同じ機序だった。
「モジュールのロード時、あるいは =after-init-hook= / =emacs-startup-hook=
/ =window-setup-hook= に無条件で登録された処理が、実フレームの無い
=--daemon= 起動時に一度だけ走ってしまう」というもの。この機序に該当する
箇所が他に残っていないか、README.org 全体を機械的に洗い出した。

*洗い出し方法*

1. =display-graphic-p= ・ =window-system= を参照する全箇所を =grep= で列挙し、
   関数内で FRAME 引数を伴って呼ばれているか（安全）、 =leaf= の =:if= /
   =:when= やモジュール直下でグローバルに評価されているか（危険）を
   1 件ずつ確認した。
2. =add-hook 'after-init-hook= / =emacs-startup-hook= / =window-setup-hook=
   の登録を全て列挙し、登録先の関数が色・フェイス・フレームパラメータを
   扱っていないか確認した。
3. =my:use-ui= の代替バンドルである =ui-doom-modeline.el= も確認した。
   =nano-modeline= と同様の懸念はあるが、現状 =my:use-ui= は ='nano= が
   既定であり本モジュールは autoload only でロードされないため、実害は
   未確認。 ='doom= に切り替えて daemon 運用する場合は改めて検証すること。

*見つかった残り 3 件と対応*

- =ui-utils.el= の =leaf treemacs= — =:if (display-graphic-p)= により
  daemon では永久にロードされない。treemacs はユーザーが =M-x treemacs=
  で明示的に呼ぶまで自動起動せず、ターミナルでも動作するパッケージなので、
  display-graphic-p での事前ゲート自体が不要と判断した。 =:require nil= +
  =with-eval-after-load 'treemacs= による通常の遅延ロードに変更し、
  display-graphic-p チェックそのものを撤去した。
- =ui-utils.el= の =leaf pbcopy= — =:if (memq window-system '(mac ns))= は
  グローバル変数 =window-system= をロード時に一度だけ読むため、daemon では
  常に =nil= と評価される。 =ui-utils--maybe-activate-pbcopy= を新設し、
  グローバル変数ではなく =(framep frame)= で実際のフレームの種別を
  個別に確認する形に変更、 =(daemonp)= で =after-make-frame-functions=
  ／ =after-init-hook= を出し分けた。
- =personal/ac1965.el= の =leaf my:mouse=（スクロール／マウス設定）—
  =:when (display-graphic-p)= が同じ理由で daemon では永久に評価されない。
  =ac1965--maybe-configure-mouse= を新設し、初回のグラフィカルフレームで
  一度だけ =setopt= するよう変更した。

*問題無しと判断したもの*

=orgx-visual.el= の =org-modern-todo-faces= 、 =completion-corfu.el= の
corfu/kind-icon/cape 初期化、 =zoom.el= のウィンドウリサイズは、いずれも
リテラルな色文字列を使うか、フレーム生成後のイベントに反応する設計であり、
=nano-modeline= のような「ロード時に一度だけ色を計算して焼き込む」機序を
持たないため対象外とした。 =dev-term-enable-p= / =dev-music-enable-p= の
=:if= は単純な boolean defcustom（既定 t）で、表示状態に依存しないため
問題無い。

** 【根本原因】desktop-restore-frames が daemon で「ゾンビフレーム」を作る問題を修正

前項までの修正（ligature、nano-modeline、ui-theme、vertico-posframe 等）を
実機の =--daemon= で再検証したところ、 =nano-modeline= / =nano-face-*= の
=nil value is invalid= 警告と =Unable to load color= エラーが *直っていな
かった*。個別パッケージ側の =after-make-frame-functions= ガードを増やす
アプローチそのものが、根本原因を取り違えていたことが分かった。

*原因*

ログの =Desktop: 1 frame, 0 buffers restored.= に注目する。
=desktop-save-mode= は起動時に自動的に =desktop-read= を呼び、フレーム
セットの復元を試みる。 =desktop-restore-frames= の既定値は =t= であり、
明示的に設定していなかった。

=--daemon= 起動時（実ディスプレイ接続が無い状態）でもこの復元が働き、
以前の GUI セッションで保存されたフレームパラメータ（ =display= /
=window-system= 等）を引き継いだフレームが作られる。このフレームは
*実際には表示に接続していない* にもかかわらず、パラメータ上は
グラフィカルに見えるため、 =(display-graphic-p FRAME)= が =true= を
返してしまう「ゾンビフレーム」になる。

これまでに =after-make-frame-functions= へ追加した daemon 対応ガード
（ =ui-nano--setup= 、 =ui-theme--init-once-graphic= 、
=completion-vertico--maybe-init-posframe= 等）は、このゾンビフレームに
対して発火し、ガードの =display-graphic-p= チェックを素通りしてしまう。
その結果、実ディスプレイの無い状態で =(require 'nano-modeline)= 等が
走り、以前と同じ壊れ方をする。個別パッケージ側をいくら直しても、
この根本原因を直さない限り再発する。

*修正*

=desktop.el= の設定に =desktop-restore-frames (not (daemonp))= を追加した。
daemon 運用時はフレームセットの復元自体を無効化する（実フレームは
=emacsclient= の接続によってのみ作られるべきであるため）。バッファの
復元（ =desktop-restore-eager= ）には影響しない。通常起動（daemon で
ない場合）は従来どおり =t= のままで、セッション間のフレーム復元は維持
される。

*教訓*

「ゾンビフレーム」というこのクラスの問題は、個別パッケージ側の
=display-graphic-p= ガードだけでは解決できない。ガードが信頼する
=display-graphic-p= 自体の前提（＝復元されたフレームは常に実ディスプレイに
接続している）が、desktop.el の frameset 復元によって崩れていたためである。
今後同種の「修正したのに直らない」daemon 関連の症状が出た場合、まず
=desktop-restore-frames= の状態を疑うこと。

** 【前項の訂正】desktop-restore-frames は原因ではなかった。真因は after-make-frame-functions の発火タイミング

前項の =desktop-restore-frames (not (daemonp))= を実機で再検証したところ、
=nano-modeline= / =nano-face-*= の警告は直っていなかった。ログを詳細に
追ったところ、前項の「ゾンビフレーム」説は誤りだったと判明した。

*誤りだった理由*

ログの =Desktop: N frame, M buffers restored.= は、desktop-read が
「フレームセットを復元した」ことを示すメッセージではなく、その時点での
*現在のフレーム数* を単に報告しているだけだった（daemon 起動時は内部フレーム
1 枚を含めて常に =1 frame= と出る）。 =desktop-restore-frames= の値に
関わらずこのメッセージは出るため、これを手がかりに frameset 復元の有無を
判断したこと自体が誤りだった。

*実際の原因*

ログの並び順を子細に見ると、 =[ui] nano-modeline initialized.= は
=Fonts applied:= や =When done with this frame, type C-x 5 0=
（＝ =emacsclient -c= の実接続時にのみ出るメッセージ）と同じタイミングで
出ている。つまり =ui-nano--setup= の daemon 分岐（
=after-make-frame-functions= ）は *正しく、本物のクライアントフレームで*
発火していた。それでもなお色が壊れていた。

原因は =after-make-frame-functions= 自体の発火タイミングにあった。この
フックは、フレームが生成される過程の *途中*（ウィンドウシステムがまだ
実際の色を確定させる前）で同期的に実行される。 =ui-font-apply= の
=set-face-attribute ... :family ... :height ...= のように値を明示的に
「設定」する処理はこのタイミングでも問題ないが、 =nano-modeline= の
=defface= が行う =(face-foreground 'default)= のように現在の色を
「問い合わせる」処理は、まだ色が確定していないために =nil= を返して
しまう。 =display-graphic-p= のガード自体は正しく機能していたが、
「グラフィカルなフレームである」ことと「色の問い合わせに応答できる状態に
なっている」ことは別問題だった。

*修正*

=ui-nano--setup= と =ui-theme--init-once-graphic= の daemon 分岐に、
=run-with-timer 0 nil= による 1 ティック分の遅延を追加した
（ =ui-nano--setup-after-frame-realized= 、
=ui-theme--init-after-frame-realized= ）。これにより、フレームが完全に
実体化してから実行されるようになる。直接の破損は未確認だが、
=completion-vertico--maybe-init-posframe= （posframe の子フレーム背景色も
同種の影響を受け得る）にも予防的に同じ遅延を適用した
（ =completion-vertico--init-posframe-after-frame-realized= ）。

=ligature= と =core--fullscreen-once= はいずれも値を明示的に「設定」する
だけの処理（色の問い合わせを伴わない）であり、実機で問題無く動作することが
既に確認済みのため、この修正は不要と判断した。

*教訓（更新）*

daemon 関連の色破壊は、フレームが「グラフィカルか」だけでなく「色の
問い合わせに応答できる状態か」までを区別する必要がある。 =after-make-frame-functions=
内で色を *問い合わせる* コードがあれば、 =run-with-timer 0 nil= で
1 ティック遅らせることを基本パターンとする。単に値を *設定* するだけの
コードであれば、この遅延は不要である。

** core-editing: global-hl-line-mode が有効化されていなかった不具合を修正

daemon 対応の検証中、「現在行の色がつかない」という報告を受けて調査した。
これは daemon とは無関係な、独立した不具合だった。

=core-editing.el= の Emacs 31 限定ブロックには =global-hl-line-sticky-flag=
（複数ウィンドウでの追従挙動を調整するだけの設定）はあったが、
=global-hl-line-mode= 自体を有効化する行が無かった。そのため、通常の
編集バッファ（.el / .org 等）で現在行がハイライトされていなかった。

=leaf hl-line :global-minor-mode global-hl-line-mode= を新設した。
=hl-line-mode= 自体は Emacs 31 より前から存在する機能のため、Emacs 31
限定ブロックの外に独立して置いた。

*追記: モードを有効化しただけでは直らなかった*

上記の修正を =M-x describe-face RET hl-line RET= で確認したところ、
モードは有効化されたが =hl-line= フェイス自体に背景色が割り当たって
いないことが判明した（色見本が空欄）。 =my/nano-apply-faces=
（ =ui-nano-palette.el= ）は =dirvish-hl-line= には =nano-color-subtle=
を設定していたが、通常の編集バッファで使われる素の =hl-line= フェイスは
設定対象に含めていなかった。 =region= フェイスの設定直後に
=(ui-nano--safe-face 'hl-line :background nano-color-subtle)= を追加し、
dirvish と同じ色に揃えた。

** ui-macos: system-taskbar-mode を daemon 対応に修正（保留していた件の対応）

daemon 対応バグの全件洗い出しの際に「確度: 中、要実機確認」として保留
していた =system-taskbar-mode= に対応した。

=(when (and (eq system-type 'darwin) (fboundp 'system-taskbar-mode))
  (system-taskbar-mode 1))= は、 =display-graphic-p= の確認すら無く
モジュールのロード時に無条件で呼ばれていた。通常起動ではその時点で
既にグラフィカルフレームが存在するため問題にならないが、 =--daemon=
起動時は実フレーム（＝Dock アイコンの紐付け先）が無い状態でこれが走る。

=ui-macos--maybe-enable-taskbar= を新設し、 =core--fullscreen-once= と
同じ =(daemonp)= + =after-make-frame-functions= のパターンで、初回の
グラフィカルフレームができてから一度だけ有効化するよう変更した。
=system-taskbar-mode= は値を「設定」するだけで色の「問い合わせ」を
伴わないため、 =nano-modeline= 等で必要だった =run-with-timer= による
追加の 1 ティック遅延は不要と判断した（ =core--fullscreen-once= /
=ligature= が実機で問題無く動作したのと同じ分類）。

** 【前項の撤回】system-taskbar-mode の daemon ガードは不要だった

前項の修正は推測ベースの予防措置だったが、 =system-taskbar.el= 本体の
ソースを実際に確認したところ、不要と判明したため撤回した。

=system-taskbar--set-back-end= のバックエンド判定は
=(boundp 'ns-version-string)= というビルド種別のチェックであり、実行時の
フレーム有無とは無関係。 ='ns= バックエンド（macOS/GNUstep）の
=system-taskbar--enable= は次の実装で、フレームの存在を一切確認しない。
...
=ui-macos--maybe-enable-taskbar= / =ui-macos--taskbar-enabled-p= を削除し、
元の単純な無条件呼び出し（ =(when (and (eq system-type 'darwin)
(fboundp 'system-taskbar-mode)) (system-taskbar-mode 1))= ）に戻した。

*教訓*

=display-graphic-p= を確認していないコードを見つけても、それだけで
daemon 対応バグと決めつけないこと。今回の一連の調査で見つかった
=ligature= / =nano-modeline= 等は、いずれも *ロード時に一度だけ色や
パッケージ状態を評価して焼き込む* という共通の実装上の欠陥を実際に
確認した上で修正していた。今回は実装を確認せず「フレームが必要そうだ」
という類推だけで先回りして直してしまった。 =fboundp= / =apropos-command=
だけでなく、可能なら該当パッケージのソースそのものを確認してから
判断すべきだった。実際の呼び出し元（ =system-taskbar-badge= 等）を
使う具体的な確認方法は、本 Changelog の一つ前の会話ターンを参照。

** design_spec.org を廃止し、図版と欠落知見を Modular Loader & Core Suite に統合

これまで別文書として並走させていた =design_spec.org= （起動シーケンス・GC 戦略・auth
解決フロー・daemon 対応など 18 節、および図版 =.dot= ソース 13 本）を廃止した。

内容を精査した結果、=design_spec.org= の説明文の大半は、既に =README.org= 側の
各モジュールの Commentary・:config コメント・「Design Notes」小見出しとして —
=design_spec.org= 自身よりも詳しい形で — 実装のすぐそばに存在していた（例:
=core-editing.el= の自動保存・dirvish 遅延ロード・=global-hl-line-mode= 修正、
=dev-ai.el= の :config コメントに書かれた認証タイミングの落とし穴 2 件、
=auth-keychain.el= の =:host= 意味づけの罠、=orgx-citar.el= の =leaf :custom=
pitfall 等）。したがって独立した「設計ノート」節を新設する統合は、既存記述の
大部分を焼き直すだけの重複になると判断し、行わなかった。

対応内容:

1. =design_spec.org= の Appendix にのみ存在していた図版 =.dot= ソース 13 本
   （=01_boot_flow.dot= 〜 =13_personal_override_contract.dot=）を、既存の
   =Appendix: emacs_config_layer_architecture.dot= 等と同じ形式で =* Changelog=
   の直前に追加した。各図の =[[file:svg/NN_*.svg]]= 参照は、新設の節を作らず、
   対応するモジュールの説明文の中に直接埋め込んだ（=Boot Flow= 、
   =modules.el Design Notes= 、=core-session.el= 、=core-gc.el= 、
   =core-editing.el= 、=auth-gpg.el Design Notes= 、
   =completion-capf-org-src.el= 、=dev-lsp-bridge.el= 、=utils-claude.el= 、
   =orgx-auto-tangle.el= 、=ui-leader.el Leader Key Infrastructure= 、
   =dev-ai.el= 、personal =Overview= ）。
2. 既存記述に無かった純粋な差分を 2 点だけ追加した。=core-gc.el= に
   early-init.el §1 の GC ガードを「3 層構成の第 1 層」として明示する 1 文、
   =core-session.el= に「=core-session-register-job= を呼んでいる箇所が
   現状どこにも無く、ジョブテーブルは常に空のまま」という既知の問題の注記。
3. Makefile の =DESIGN_SPEC= 変数と、=dot-tangle= / =mmd-tangle= ターゲットの
   2 ファイル対応ロジックを削除し、=$(ORG)=（=README.org=）のみを対象とする
   単純な形に戻した。=help= ターゲットの説明文、および
   =check_emphasis.py= の使用例・docstring から =design_spec.org= への言及も
   合わせて除去した。
4. =orgx-export--svg-to-pdf-graphics= 節の周辺コメントなど、過去に
   =design_spec.org= のビルドで実際に踏んだ不具合（Inkscape 未検出、
   =\includesvg= の書き換え順序）を記録した歴史的な注記はそのまま残した。
   =design_spec.org= の存在有無に関わらず有効な技術的知見のためである。

=design_spec.org= 自体、および対応する =svg/01_*.svg= 〜 =svg/13_*.svg= の生成物は
リポジトリから削除してよい。再生成が必要な場合は =make dot-svg= が本節の
Appendix から直接 tangle・レンダリングする。

** init.el: 起動完了メッセージが他の遅延初期化メッセージより先に出ていた不具合を修正

"Emacs ready in N.NN seconds with M GCs." が *Messages* の最後ではなく最初に
出力される、という報告があった。

原因は =core--announce-startup= のアイドルタイマー登録の位置にあった。
=init.el= §10 のトップレベルコードで =(run-with-idle-timer 0 nil
#'core--announce-startup)= を直接呼んでいたため、=init.el= の読み込み中
（=emacs-startup-hook= が発火する *前* ）にアイドルタイマーキューへ即座に
登録されていた。一方、=ui-font.el= / =ui-nano-modeline.el= /
=ui-visual-aids.el=（pulsar）などの遅延ロードモジュールは =emacs-startup-hook=
から自分自身のアイドルタイマーを登録する。遅延 0 秒のアイドルタイマーは
登録順に発火するため、先に登録された起動完了メッセージが、後から登録される
=Fonts applied:= や =[ui] nano-modeline initialized.= より *先に* 出力されて
いた。

対処として、タイマーの登録自体を =emacs-startup-hook= の末尾（depth 100）へ
移した。
...
他のモジュールが =emacs-startup-hook= 内で自分のアイドルタイマーを登録し
終えた *後に* このタイマーが登録されるようになり、同じアイドルタイマーキュー
の中でも最後に発火するようになった。

** 図版15枚（dot 13本 + 既存2本）と tangle-flow.mmd の配色を GitHub の light/dark 両モードで可視化できるよう修正

生成した =.dot= / =.mmd= 図版はいずれも背景を明示的に指定していなかった。Graphviz は
=bgcolor= 未指定時、SVG に背景の =<rect>= を出力しない（透過扱い）。この状態で GitHub
の dark モード（ページ背景が濃色）上に表示すると、ボックス自体は =fillcolor= を
持つため視認できるものの、エッジ線・エッジラベル・ノード外の文字（いずれも濃い
グレー =#5F5E5A= 系）がページの濃色背景に溶け込み、事実上見えなくなる。

対処として、全 15 本の =digraph= 定義（Appendix の =01_boot_flow.dot= 〜
=13_personal_override_contract.dot= 、および既存の
=emacs_config_layer_architecture.dot= 、
=emacs_keybindings_cheatsheet_concept.dot=）の =rankdir= 直後に
=bgcolor="white";= を追加した。これにより各図が自前の不透明な白キャンバスを
持つようになり、GitHub の表示テーマに関わらず、既存の配色（明色の =fillcolor=
に対する濃色の =fontcolor= 、白背景に対する =#5F5E5A= 系のエッジ線）がそのまま
コントラストを保つ。

=tangle-flow.mmd= についても同様の問題があった。=Makefile= の =mmd-svg= が
=mmdc= に =-b transparent= を渡しており、Mermaid のデフォルトテーマが使う黒に
近いエッジ・テキスト色が透過背景ごしに dark モードへ溶け込んでいた。=-b white=
に変更し、こちらも不透明な白背景を持つよう統一した。

いずれも「ページのテーマに図を追従させる」方式（=prefers-color-scheme= を使った
ライト／ダーク 2 枚出し分け等）ではなく、「図自体に固定の白キャンバスを持たせて
ページのテーマから独立させる」方式を採用した。図版の生成物を 2 倍に増やさずに
すみ、GitHub 側のテーマ判定ロジックに依存しない点が単純である。

** init.el: daemon モードでは起動完了メッセージを最初のクライアントフレーム接続時まで遅らせるよう修正

先日 =core--schedule-announce-startup= を =emacs-startup-hook= の末尾（depth 100）へ
回す修正を行ったが、daemon + =emacsclient -c= で運用したところ、依然として
=Emacs ready in N.NN seconds with M GCs.= が =Fonts applied:= や
=[ui] nano-modeline initialized.= より先に出る、という報告があった。

調査の結果、これはデグレーションではなく仕様どおりの挙動だった。
=When done with this frame, type C-x 5 0= は =emacsclient -c= の実接続時にのみ
出る Emacs 本体（server.el）のメッセージであり、この行が出力に含まれている
時点で、この 3 行（=Fonts applied:= 、=[ui] nano-modeline initialized.= 、
=When done with this frame...=）はすべて「daemon 初期化完了」とは別の、
「クライアントフレームが実際に接続した」という *後発の* イベントに属する
（=after-make-frame-functions= 経由。詳細は
[[#init-el][init.el]] と =ui-font.el= / =ui-nano-modeline.el= の daemon 分岐、
および本 Changelog の「【前項の訂正】desktop-restore-frames は原因ではなかった」
の節を参照）。先日の修正は =emacs-startup-hook= 内の競合を解消するものであり、
非 daemon 起動には正しく効くが、daemon 初期化完了とクライアントフレーム接続が
別々のタイミングで起きる daemon 運用には元々関与していなかった。

とはいえ「起動完了を告げるメッセージが、実際に人間がフレームを見られる前に
=*Messages*= へ埋もれてしまう」のは daemon 運用として不便であるため、
daemon モードでは表示のトリガー自体を =after-make-frame-functions= に切り替えた。
...
- =after-init-time= / =gcs-done= は daemon 初期化完了時点で既に確定した値なので、
  表示する「N.NN 秒」の中身（起動所要時間そのもの）は変わらない。変わるのは
  *表示するタイミング* だけである。
- =ui-font.el= / =ui-nano-modeline.el= と同じ =after-make-frame-functions= に
  乗るが depth 100 を指定し、それらより後に発火するようにした。
  =ui-nano--setup-after-frame-realized= はさらに =run-with-timer 0 nil= で
  1 ティック遅らせてから初期化メッセージを出す（色の問い合わせにはフレーム
  実体化後のもう 1 ティックが必要なため）ので、depth だけでは追い抜かれる
  可能性がある。=core--announce-startup-on-first-frame= 側でも同じだけ
  =run-with-timer 0 nil= で遅らせ、確実に最後になるようにしてある。
- daemon は複数のクライアントを受け付けられるため、2 つ目以降のフレーム接続で
  再度表示されないよう、初回発火時に =core--announce-startup-done= を立てて
  自分自身を =after-make-frame-functions= から取り除く（dirvish の自己解除型
  advice と同じパターン）。

非 daemon 起動（=emacs= を直接起動する場合）は先日の修正のまま変更していない。

** core-editing.el: paredit の git clone 失敗を、GitHub ミラーへのレシピ固定で回避

=leaf paredit :straight t= で実機ビルドしたところ、以下のエラーで失敗した。

#+begin_example
⛔ Warning (leaf): Error in `paredit' block at core-editing.el.
Error msg: Failed to run git clone --origin origin --no-checkout
https://paredit.org/paredit.git ... --no-single-branch, see buffer *straight-process*
#+end_example

=:straight t= は straight.el 既定のレシピ（MELPA recipes 由来）をそのまま使う。
paredit の既定レシピは =:repo= に =https://paredit.org/paredit.git= を指定して
おり、これは作者個人が運用する git サーバーである。GitHub のような大規模ホストと
比べて可用性が低く、到達不能・タイムアウトになりやすい。

対処として、レシピを GitHub 上のミラー（emacsmirror。ELPA/MELPA 掲載パッケージを
機械的にミラーしているリポジトリ群）へ明示的に固定した。
...
=~/.emacs.d/.cache/emacs/straight/repos/paredit/= に前回の失敗した clone の残骸が
残っている場合は、レシピを変更しただけでは straight が古いリポジトリ設定を使い
続けることがあるため、該当ディレクトリを削除してから =make reload= すること。

** init.el: no-littering.el 本体が一度もロードされていなかった不具合を修正

「no-littering で `~/.emacs.d/` 配下に収まるよう設定してあるはずのファイルが、
外に出ることがある」という報告を受けて全文検索したところ、`no-littering`
パッケージ本体を `require` している箇所が =README.org= のどこにも存在しない
ことが判明した。

`early-init.el` §8（現行番号）で `no-littering-etc-directory` /
`no-littering-var-directory` の 2 つのパス変数を `defvar` していたが、これは
「本家 `no-littering.el` がロード時にこれらを読んで自分のディレクトリを決める」
という前提のための準備でしかなく、`no-littering.el` 本体を実際にロードする
コードが存在しなければ何の意味も持たない。

代わりに、`core-history.el`（save-place / recentf / savehist）、
`core-editing.el`（tramp / backup / transient）、`core-treesit.el`
（tree-sitter）、`ui-window.el`（desktop）が、`no-littering-var-directory`
を単なるパス文字列の定数として使い、各自手作業でパスを組み立てていた。
これらの約 10 個の変数は正しく `var/` 配下に収まっていたが、本家
`no-littering.el` が自動的にリダイレクトするはずの残り 100 以上の変数
（`bookmark-default-file` 、`project-list-file` 、
`url-configuration-directory` 、`forge-database-file` 、
`treemacs-persist-file` 等）は一切リダイレクトされておらず、各パッケージ
自身の既定値のまま——`~/.emacs.d/` 直下や、パッケージによっては
`.emacs.d` の外（XDG 系ディレクトリ等）に書き出されていた。これが
「外に出ることがある」の実体である。

対処として、`init.el` に新しいステップ「4. no-littering のロード」を追加し、
straight/leaf のブートストラップ直後、他のどのパッケージよりも先に
`no-littering.el` 本体を `require` するようにした。
...
これに伴い、旧ステップ 4〜10（Org / macOS 環境変数 / 起動性能パラメータ /
コア組み込みポリシー / personal オーバーレイ / modules エントリポイント /
起動メトリクス）をすべて 1 つずつ繰り下げ、5〜11 に振り直した。本文中で
旧番号を参照していたコメント 5 箇所（`orgx-core.el` 、`personal/ac1965.el` 、
`personal/device-wsl.el` 、`utils-ime.el`）を新番号に更新した。あわせて、
Appendix の =04_gc_strategy.dot= と =13_personal_override_contract.dot= に
残っていた、2026-08 の early-init.el renumbering 以前の古いセクション番号
（=early-init.el §8= 、=init.el §8a/§8b/§9=）も、現行の番号
（=early-init.el §1= 、=init.el §9a/§9b/§10=）に合わせて修正した。

=~/.emacs.d= 以外に既に書き出されてしまったファイル（例:
=~/.emacs.d/bookmarks= 、パッケージによっては =~/.emacs.d= の外）は、
今回の修正だけでは移動されない。次回起動後、各パッケージの実際の変数値
（=C-h v bookmark-default-file= 等）を確認し、必要なら手動で
=var/= 配下へ移すこと。

** orgx-core.el: 強調マーカーの直前・直後に全角句読点があると効かない不具合を修正、74件の実例を全て解消

「READMEはorg記述の強調に日本語の「。」の後が考慮されていない」という指摘を受け、
=scripts/check_emphasis.py= の実際のロジックを =README.org= 全体に対して実行した
ところ、74 件の "ineffective marker" が検出された。ほとんどが =code=）
や *bold*、、 のように、強調マーカーの直後に全角の閉じ括弧・読点が続くパターン
だった。

=org-emphasis-regexp-components= の既定値（PRE/POST とも半角の空白・句読点のみ）に
全角文字が含まれていないことは Makefile のコメントで以前から把握されていたが、
対応は「検出するだけ」で止まっており、実際の文書側は直されていなかった。本文書は
和文が中心で、和文の直後に強調マーカーが続き、その直後にまた全角の閉じ括弧・読点が
続く書き方（例:「=README.org=（設定の本体）は」）が自然に大量発生する構造のため、
検出のみで放置する運用は現実的ではないと判断した。

対処は 3 段階。

1. =orgx-core.el= の =(leaf org :config)= に =org-emphasis-regexp-components=
   の =setopt= を追加し、和文でよく使う括弧・読点（「『（【、。・とその対）を
   PRE/POST 双方に加えた。setter は =org-set-emph-re= なので、=setopt= だけで
   =org-emphasis-re= / =org-verbatim-re= が正しく再生成される。
2. =scripts/check_emphasis.py= の =PRE_OK= / =POST_OK= を同じ文字集合に同期した
   （Elisp 側の値を実行時に import することはできないため、手作業での同期である
   旨をコメントに明記した）。これにより 74 件中 58 件が解消した。
3. 残り 16 件を実際に確認したところ、Japanese 句読点の問題ではなく、
   (a) 番号付き見出し（例: "*1. ...*"）の bold 範囲の末尾に、隙間なく =verbatim=
   がネストしていた（Org は隣接する異種強調の直接ネストを認識しない）、
   (b) ~gcs:N~ や ~[modules] loaded:79 ... total:253.0ms~ のように、verbatim
   の内容自体にリテラルの等号が含まれ、マーカーのペア対応がずれていた、
   という 2 種類の実質的なマークアップの誤りだった。前者は bold の範囲を番号
   部分だけに絞り、後者は外側マーカーを =~= に切り替えたうえで内容中の等号を
   コロンに置き換えて解消した。

修正後、=make check-emphasis= 相当のロジックを再実行し、0 件（=[check-emphasis]
ok=）まで確認した。

** orgx-core.el: org-emphasis-regexp-components の設定で void-variable エラーになる不具合を修正

前回の修正を実機に適用したところ、以下のエラーで起動時に失敗した。

#+begin_example
■  Warning (leaf): Error in `org' block at orgx-core.el.
Error msg: Symbol's value as variable is void: org-emphasis-regexp-components
#+end_example

原因は配置場所の誤りだった。=(leaf org :straight nil ... :config FORM...)= の
=:config= は、org.el のロードを待たない。=leaf= はこの本体を単純に、モジュールの
ロード時点でそのまま評価する。=org-todo-keywords= 等の既存の =setq= がこれまで
問題なく動いていたのは、たまたま =setq= が未束縛のシンボルにも新規束縛を作れる
ため（org.el が後からその変数を =defcustom= しても、既に束縛済みの値は
上書きされない）であって、=:config= 自体が =with-eval-after-load 'org= 相当に
遅延されているわけではなかった。

今回追加した =org-emphasis-regexp-components= の customization は =nth= で
*既存の値を読んで* 拡張する処理であり、org.el がまだロードされていない時点では
そもそも変数自体が存在しないため、読み取ろうとした瞬間に void-variable で
失敗する。同じ =:config= 内にあった「アーカイブ先」の設定（
=org-archive-location= ）が最初から =(with-eval-after-load 'org ...)= で
明示的に包まれていたのは、まさにこの理由による。今回の customization は
その隣に書いておきながら、このパターンに倣うのを見落としていた。

対処として、=org-emphasis-regexp-components= の =setopt= を =leaf= の =:config=
本体から外し、「アーカイブ先」と同じ =(with-eval-after-load 'org ...)= で
明示的に包んだ。=leaf org= の =:config= は org-todo-keywords /
org-refile-targets / org-capture-templates の 3 つの =setq= で終わり、
それより後の customization（今回のものとアーカイブ先）は 2 つとも独立した
トップレベルの =with-eval-after-load= フォームとして =(leaf org ...)= の
外に置く、という形に揃えた。

** check_emphasis.py: bold の中に verbatim/code が完全に入っている「ネスト」を新たに検出、8件の実例を修正

「org モードで強調されて見えなかった」という報告を受け、以前「1197行目は
問題ない」と回答したのが誤りだったと判明した。

見落としていたのは Org の強調構文が *ネストをサポートしない* という点である。
=org-emph-re= はマーカー種別（=*= =/= =_= == =~= =+=）ごとの正規表現ではなく、
全マーカーを 1 本の正規表現にまとめ、後方参照で開始マーカーと同じ文字の終了
マーカーを探す実装になっている。行内を走査する際、最初に見つかった外側の
マッチ（例: 行頭の =*= から行末の =*= まで）がその範囲を丸ごと 1 回のマッチで
消費するため、その内側にある =verbatim= や ~code~ は独立した対象として
再マッチされない。結果として、外側の bold 自体は正しく強調されるが、内側の
"=" や "~" は強調マーカーとして機能せず、literal 文字としてそのまま表示される。

これは以前修正した、="=" の直後に隣接して bold の閉じマーカーが続くケース
（PRE/POST の文字集合の問題）とは別種の不具合であり、間にスペースが入っていて
境界文字が完全に妥当でも、bold の中に完全な verbatim/code が入っている時点で
アウトになる。

対処:

1. 全文検索したところ、「なぜ =X= ...のか」を bold で囲んだ見出し的な書き方が
   8 箇所でこのパターンに該当していた（=make dot-svg= の説明、=url.el= の節、
   =:pre-setq= 、=emacs-startup-hook= 、=after-init-hook= 、=M-SPC= 、
   =ui-keymap--bind= 、=kill-buffer-hook= の各節）。いずれも外側の bold を
   外し、平文の見出し的な一文に変更した。
2. =scripts/check_emphasis.py= の =scan_line= を拡張し、bold 候補と
   verbatim・code の保護領域との関係を 3 通りに分類するようにした。
   (a) 保護領域が bold 候補を完全に包含する場合（=*scratch*= のように、
   =*= 自体が verbatim の内側の literal 文字）は従来どおり除外。
   (b) bold 候補が保護領域を完全に包含する場合（今回の実例）は新たに違反
   として報告する。(c) 部分的にしか重ならない場合（=if-let*= と
   =when-let*= のように、離れた 2 つの verbatim スパンの識別子内リテラル
   =*= がたまたま =*...*= の形に見えてしまうケース）は、実在する強調
   マーカーの組ではないと判断し除外する — 最初は (c) も違反として報告する
   実装にしたが、この偽陽性が 2 件出たため、包含関係が確定できる (b) のみを
   違反とする方針に改めた。

修正後、=make check-emphasis= 相当のロジックを再実行し、0 件まで確認した。

** early-init.el: daemon で 2 枚目以降のフレームがフルスクリーンにならない不具合を修正

daemon 運用時、 =emacsclient -c= で開いたフレームが最大化されていない
という報告を受けて調査した。

*原因*

=core--fullscreen-once= は「初回のグラフィカルフレームにのみフルスクリーンを
適用し、以降は何もしない」設計だった（ =core--did-fullscreen= フラグで
ガード）。これは元々、 =window-setup-hook= が --daemon 起動時に実フレームの
無い状態で走ってしまい、存在しないフレームに適用してフラグだけ立ってしまう
問題への対策として導入されたものだったが、対策自体が副作用として
「2 枚目以降の =emacsclient -c= フレームには二度と適用されない」という
新たな daemon 対応バグを生んでいた。 =ui-font-apply= や =ligature= が
「グラフィカルフレームが繋がるたびに毎回」実行される設計になっているのに対し、
=core--fullscreen-once= だけが単発ガード付きのままだったのが食い違いの原因。

*修正*

単発ガード付きの =core--fullscreen-once= （非 daemon 用、 =window-setup-hook=
から呼ぶ）と、ガード無しで FRAME をフルスクリーンにするだけの
=core--fullscreen-frame= （daemon 用、 =after-make-frame-functions= に直接繋ぐ）
に分離した。非 daemon 起動時の挙動（初回のみ）は変えず、daemon 起動時のみ、
新しいグラフィカルフレームが繋がるたびに毎回フルスクリーンを適用するように
なった。

** personal-llm.el: ローカル LLM ルーター（~/.local/bin/localllm）との統合を追加

Org-roam と dev（コーディング）バッファの双方から、ローカルの Ollama モデル
（qwen3:14b / qwen3-coder / LFM2.5-8B-A1B / qwen2.5vl）へ振り分けるルーター
スクリプト =~/.local/bin/localllm= を呼び出せるようにした。振り分けロジック
（=--task= 明示 → 画像判定 → coding/agent キーワード判定 → general
フォールバック）はすべてスクリプト側の責務であり、Emacs 側はプロンプトを
標準入力で渡して応答をストリーミング表示するだけの薄い統合に留めた。

=personal/personal-llm.el= を新設し、次の対話コマンドを公開した:

- =my/llm-run= — 任意のプロンプトをルーターへ送る（リージョンかミニバッファ
  入力、タスクは既定で自動判定）
- =my/llm-run-from-org-roam= — 現在の Org-roam ノート（リージョンか全体）を
  タスク未指定で送る。ノートは雑多な内容を含み得るため、ルーター側の
  ルールベース判定に委ねる方を選んだ
- =my/llm-run-from-coding-buffer= — ポイント位置の defun（かリージョン）を
  =--task coding= 固定で送る。自動判定を待たず確実に qwen3-coder へ
  振り分けるため

応答は org-mode の =*LLM Router Output*= バッファに、呼び出しごとの見出し
付きで追記表示する。utils-claude.el 経由の HTTP 通信とは異なりローカル
プロセスの起動のみで完結するため、本モジュールは utils 層にも org にも
依存しない（personal/user.el からの読み込みに =with-eval-after-load 'org=
のガードが不要な理由）。

*スクリプト名について*

当初はスクリプトを =~/.local/bin/llm= という名前にしていたが、ユーザーが
ターミナルから素の =llm= を実行したところ、意図せず Homebrew でインストール
済みの別ツール（datasette 版 =llm=、既定で OpenAI API を叩く）が PATH 上で
先に見つかり、"You have no credits remaining" という OpenAI の課金エラーが
出た。Emacs 側（本モジュール）は `expand-file-name' で絶対パスに解決してから
`make-process' に渡すためこの衝突の影響を受けないが、ユーザーが手元で
同じスクリプトを直接叩く運用も想定されるため、スクリプト名を
=~/.local/bin/localllm= に変更して衝突を回避した。Homebrew 版 =llm= は
そのまま残しており、削除や PATH の変更は行っていない。

** utils-ime.el: =utils-ime-target-modes= に =prog-mode= を追加し、プログラミング言語バッファでも自動 ASCII 化されるようにした

これまで =utils-ime-target-modes= は dired/dirvish・magit-status・vterm/eshell/
term/comint のみを対象とし、操作キー（ =RET= / =g= / =d= / =s= 等）が IME に
composed される誤爆防止に限定していた。org-mode で日本語散文とコードを
混在させて書く際、本文中の日英切替の手間自体は解決していなかった。

=utils-ime-target-modes= の既定値に =prog-mode= を追加した。 =derived-mode-p=
によるマッチのため、 =python-mode= ・ =emacs-lisp-mode= ・ =js-mode= など
=prog-mode= を継承するほぼ全てのプログラミング言語メジャーモードが
新規フックなしに自動的に対象へ加わる。

=org-edit-special=（ =C-c '= ）が開く Org Babel のソース編集バッファは、
その言語自身のメジャーモードを持つ独立バッファであるため、この一行追加
だけで既存の =buffer-list-update-hook= ベースの仕組みがそのまま検知する。
=org-mode= 自体は =prog-mode= の派生ではないため、本文（日本語散文）側の
挙動には影響しない。

コード中に日本語コメントを書く場合は、その区間だけ手動で IME に
切り替える必要がある点は変わらない。

** LICENSE / ChangeLog を GNU 系ディストリビューションの慣習にならって tangle 対象にした

これまで LICENSE.org はリポジトリに単体で置かれた文書で、README.org の tangle
パイプラインとは無関係だった。また Changelog 節（本節）はプレーンテキストの
ChangeLog として書き出す手段を持たなかった。

License 節（[[#license][License]]）に GPLv3 の要約文を =text= ソースブロック
（ =:tangle LICENSE= ）として直接埋め込み、LICENSE.org は廃止した。README.org
がすでに =early-init.el= / =init.el= / =lisp/= / =Makefile= 全ての単一のソース・
オブ・トゥルースである以上、ライセンス文だけを別の literate ファイルに分離する
理由はなく、=make tangle= の既定の（言語を絞らない） =org-babel-tangle-file=
呼び出しがこのブロックもそのまま拾って =LICENSE= を書き出す。独立した
=license-tangle= ターゲットは不要になったため削除した。

Changelog 節は見出し・表・強調記法を含む通常の Org 構造であり、内容をソース
ブロックへ複製すると二重管理になる。そのため tangle ではなく、
=changelog-tangle= ターゲットが =ox-ascii= で本節の subtree をそのまま
=ChangeLog= へエクスポートする（Changelog 見出しの =:EXPORT_OPTIONS:= で
目次・番号・author/creator/date を抑制）。こちらは =$(ORG)= 全体の tangle とは
別処理のため、引き続き独立したオプトインターゲットとした。

生成される =LICENSE= / =ChangeLog= はいずれも拡張子なし（GNU 系ディストリビューション
の慣習）で、README.org 由来の他の生成物と同様に =.gitignore= に追加し、常に
再生成できるものとして扱う。=distclean= はこの 2 ファイルも削除する。

** Makefile: distclean-deps が =$XDG_CACHE_HOME= を考慮しておらず実キャッシュを削除できていなかった不具合を修正

=distclean-deps= は =$(ROOT)/straight= =$(ROOT)/.cache= =$(ROOT)/eln-cache= を
削除していたが、early-init.el の =my:d:cache= は =$XDG_CACHE_HOME/emacs/=
（未設定時は =$HOME/.cache/emacs/=）に解決され、straight のパッケージ clone と
（実起動時の）eln-cache はそちらに置かれる。 =$(ROOT)/.cache= はそもそも
どこからも作られておらず、 =$(ROOT)/straight= も存在しないため、
=distclean-deps= は実質何も削除できていなかった。

=$(ROOT)/eln-cache= だけは削除の意味があった —  =~/.emacs.d= が本リポジトリへの
シンボリックリンクであるため、 =-Q= バッチ実行（=dot-tangle= / =mmd-tangle= /
=onepass-q= 等、early-init.el を経由しない）の既定 =user-emacs-directory= が
ここを指し、Emacs 標準の相対 =eln-cache/= がここに生成されるため。

あわせて、同じ根本原因（ =straight-base-dir= の値をそのまま「 =straight/=
ディレクトリそのもの」として扱っていた）で =LEAF_DIR= / =LEAFKW_DIR= も
実際には存在しないパス（ =straight/= の一段と、 =.el= サフィックスの両方が
欠落）を指しており、 =onepass-q= の leaf / leaf-keywords 注入が
「エラーは出ないが静かに何も起きない」形で失敗していた（ =ignore-errors= で
握りつぶされるため気付きにくい）。実ディスク（ =$XDG_CACHE_HOME/emacs/straight/
repos/leaf.el= 等）で確認のうえ両方を修正した。

修正後は =MY_D_CACHE_DIR=（ =my:d:cache= を直接問い合わせて解決）を新設し、
=distclean-deps= はこれを丸ごと削除する（ =straight/= ・ =eln-cache/= に加え、
同じキャッシュルートに同居する他パッケージのキャッシュ、たとえば dirvish の
サムネイルキャッシュも含めて一括削除される。これは元の意図「依存物・実行時
キャッシュを一掃する」により合致する）。 =$(ROOT)/eln-cache= の削除は
そのまま維持し、 =$(ROOT)/.cache= は対象から外した。 =echo-paths= にも
=MY_D_CACHE_DIR= を追加して次に同種の不一致が起きても気付けるようにした。

** personal/ 配下 4 ファイルのバイトコンパイル警告（16件）をすべて解消

=make= 実行時に =personal/ac1965.el= ・ =personal/device-darwin.el= ・
=personal/device-wsl.el= ・ =personal/personal-ai.el= から出ていた自由変数・
未定義関数・docstring 幅超過の警告を、コード自体の挙動は変えずに解消した。

- =ac1965.el= の =org-roam-ui-*= 系 6 変数（ =with-eval-after-load
  'org-roam-ui= 内で =setq= するのみで、パッケージ自体はトップレベルで
  require しないため defcustom がコンパイラに未知だった）に bare =defvar=
  を追加。
- 同ファイルの =dirvish-define-mode-line= 呼び出し（macro）を
  =eval-when-compile (require 'dirvish)= でコンパイル時に展開可能にし、
  第1引数 =ime= が自由変数として誤評価される警告を解消。
- 同ファイルの =nano-modeline-position=（関数を保持する
  nano-modeline 側の内部変数、 =funcall= で呼ぶ）に bare =defvar= を追加。
- 同ファイルの =org-capture-get= に =declare-function= を追加。
- =device-darwin.el= の =hydra-apple-music/body= 、 =device-wsl.el= の
  =hydra-wsl/body= （いずれも =defhydra= が実行時にのみ定義する関数）に
  =declare-function= を追加。
- =ac1965--nano-time-encoding= と =my/claude-analyze-require-graph= の
  docstring を 80 桁以内に収まるよう改行位置を調整（内容は変更していない）。

=make reload= 後に =personal/= のみを =-batch= でクリーンビルドし、
7 ファイルすべてが警告ゼロでコンパイルされることを確認済み。

** README.org 全体エクスポート時の "Unable to resolve link" を修正

=early-init.el= 節・ =init.el= 節の見出しに =:CUSTOM_ID:= が無いまま、本文中に
=[[#early-init-el][early-init.el]]= ・ =[[#init-el][init.el]]= という ID
参照リンクが張られており、README.org 全体をエクスポートすると
=user-error: Org export aborted.  Unable to resolve link: "early-init-el"=
で中断する不具合があった。両見出しに =:CUSTOM_ID: early-init-el= /
=:CUSTOM_ID: init-el= を追加し、全 =[[#...]]= リンクが対応する
=:CUSTOM_ID:= を持つことを確認した（ =-batch= での =org-export-to-file=
'html= 実行で再現・解消を確認済み）。

** personal/ac1965.el: dirvish 未インストール時に起動が失敗し、dirvish-ime-ml が void-function になる不具合を修正

=personal/ac1965.el= は init.el ステップ9で、dirvish を =:straight t= で
実際にインストールする core-editing.el のブロック（modules.el、
ステップ10）より前にロードされる。同ファイル内の
=(eval-when-compile (require 'dirvish))= は、バイトコンパイルされずに
生の =.el= として =load= されるとき、Emacs のロード時 eager
macro-expansion（=internal-macroexpand-for-load=）によって即座に
展開され、その際 =eval-when-compile= は単なる =progn= として本文を
その場で評価する。結果として dirvish がまだ load-path 上に存在しない
時点で本物の =(require 'dirvish)= が実行され、
=file-missing= で =personal/ac1965.el= 全体のロードが中断していた
（ =[load] failed: .../personal/ac1965.el (Cannot open load file...)=
としてログに現れる）。中断により、その直後にあるはずの
=(with-eval-after-load 'dirvish (dirvish-define-mode-line ime ...))=
の登録も一切実行されなくなり、後で実際に dirvish がロードされた
（初回 =dired-noselect= 呼び出し時）ときには
=dirvish-mode-line-format= の =ime= セグメント参照だけが残るため、
dirvish バッファに入るたびに再描画のたびに
=(void-function dirvish-ime-ml)= が発生していた。
=(require 'dirvish nil t)= として NOERROR を渡し、この時点での
require 失敗を無害な nil 戻り値にすることで、ファイルの残りが
最後までロードされ、実際に dirvish が読み込まれた時点で
=dirvish-define-mode-line= が正しく遅延展開されるようにした。
=emacs -Q --batch= で dirvish 未ロード状態を再現し、旧フォームが
同一のエラーで落ちること、新フォームがエラーなく通過することの
両方を確認済み。

** early-init.el: 起動時の "Package restclient-jq requires restclient version ..." 警告を抑止

straight.el の =straight-check-dependency-versions=（既定値 =warn=）は、
Package-Requires に書かれたバージョン文字列を =version<= で単純比較するだけで、
パッケージ単位で無視する仕組みを持たない。restclient-jq.el は
=(restclient "20200502.831")= を要求するが、restclient.el（emacsorphanage
フォーク）の =;; Version:= ヘッダーは静的な =1.0= のままであり、これは
MELPA が commit 日時から自動生成する日付版バージョンとは無関係の
プレースホルダーである。straight.el は git からソースをそのまま使うため
この静的ヘッダーをそのまま比較対象にしてしまい、実際には機能上の
問題がないにもかかわらず誤検知していた。個別パッケージだけを除外する
手段が straight.el 側にないため、=straight-check-dependency-versions= を
=nil=（旧来の既定動作）にして全体を無効化した。
