...
- Custom groups use the module name as the group symbol.

*Documentation*

- All public =defun= (=my/= or =<module>-= prefix without double dash)
  must carry a docstring documenting their contract.
- Private helpers (=<module>--= prefix) are exempt but encouraged.
- Each module must declare a =defgroup= when it owns =defcustom= variables.
  The =:group= in every =defcustom= must reference the module's own group,
...
#+begin_src emacs-lisp
  ...
  (leaf org
    ...
    ;; [Fix AA] org-agenda-files, org-todo-keywords, org-refile-targets, and
    ;; org-capture-templates are all defcustom; use setopt in :config.
...
#+begin_src emacs-lisp
  ...
  ;; my:d:org* are defvar path variables defined in orgx-core.el;
  ;; setq is correct here (not defcustom).
...
*** Fix AA: orgx-core.el — org defcustom vars switched to setopt in :config

*🟡 Minor — setq/setopt consistency (four variables)*

- *[Fix AA] orgx/orgx-core.el*: Changed =setq= to =setopt= for
  =org-agenda-files=, =org-todo-keywords=, =org-refile-targets=, and
  =org-capture-templates= inside the =leaf org :config= block.  All four are
  =defcustom= variables in =org.el= / =org-refile.el= / =org-capture.el=.
  The values involve runtime-evaluated expressions (=org-directory=,
  backquote templates) so they cannot be moved to =:custom= or =:pre-setq=;
  =setopt= inside =:config= is the correct pattern for this case.

  The =setq org-agenda-files= inside =orgx-core--rebuild-agenda-files= is
  intentionally left as =setq=: that call is an internal Phase 2 state update
  (replacing the Phase 1 fallback with the freshly scanned list), not an
  initial configuration of a =defcustom= value.
