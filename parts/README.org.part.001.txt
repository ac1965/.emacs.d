...
*** Overview
**** Purpose
Provide a **deterministic and explicit module loading architecture** for this
Emacs configuration.

This layer defines:
- a single entry point for module loading (=modules.el=)
- a stable directory-based responsibility split
- a predictable load order independent of file system side effects

The goal is to ensure that **startup behavior, dependency relationships, and
extension points are fully inspectable and reproducible**.

**** What it does
- Defines =modules.el= as the **only authoritative module loader**
- Loads modules explicitly by category and order:
  - =core/=        : fundamental runtime and infrastructure
  - =ui/=          : visual presentation and interaction layer
  - =completion/=  : completion framework and CAPF orchestration
  - =orgx/=        : Org-mode extensions beyond upstream defaults
  - =dev/=         : development and build-related tooling
  - =utils/=       : small, domain-specific utilities
- Prevents implicit loading via directory traversal or wildcard expansion
- Makes module boundaries visible and reviewable in a single file

**** Notes
- =modules.el= **does not perform configuration logic itself**
  - it only orchestrates *when* and *in what order* modules are loaded
- Each subdirectory represents a **semantic responsibility boundary**
  - cross-category dependencies are allowed but should be intentional
- Modules are written to be:
  - idempotent
  - side-effect conscious
  - safe to load under batch / byte-compile / native-comp contexts
- This structure is optimized for:
  - long-term maintenance
  - partial enable/disable during diagnostics
  - dependency visualization and auditing

**** Implementation
- =modules.el=
  - contains ordered =require= calls only
  - no feature definitions, advice, or configuration logic
- Each module file:
...
- Load order is:
  ...
  5. =dev=
  6. =utils=
- This ordering is intentional and must not be altered without
  documenting the dependency rationale
...
**** Purpose
Provide the **foundational runtime and infrastructure layer** required by all
other modules.

This category establishes guarantees about availability, defaults, and
execution context.

**** What it does
- Defines low-level behavior shared across the entire configuration
- Initializes essential subsystems (paths, environment, compatibility)
- Provides stable primitives relied upon by higher layers

**** Notes
- core modules must not depend on ui, completion, orgx, dev, or utils
- Side effects must be explicit and documented
- Failure here should be considered configuration-critical
